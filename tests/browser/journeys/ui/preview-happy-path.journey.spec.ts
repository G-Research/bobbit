...
test.describe("Preview panel retained full-stack smoke", () => {
	test("opens mounted preview and refreshes iframe cache-buster", async ({ page }) => {
		...
		// 1. PATCH preview=true â†’ WS broadcast flips state.isPreviewSession on
		//    the client and starts the SSE preview-events subscription.
		// 2. POST /api/preview/mount â†’ server emits preview-changed â†’ SSE
		...
		// forwards `entry` from the broadcast â€” no client-side pre-seed needed.
		await page.evaluate(() => {
			const s: any = (window as any).bobbitState ?? (window as any).__bobbitState ?? {};
		...
		// Refresh button bumps mtime â†’ iframe src changes.
