 ...
 * editors and don't need a project â€” this test asserts the happy path works
...
test.describe("Roles page â€” + New Role creates a session", () => {
	test("clicking + New Role opens a Role Assistant session @smoke", async ({ page }) => {
		await openApp(page);
		await navigateToHash(page, "#/roles");

		// "+ New Role" button â€” created via mini-lit Button with label "New Role".
		...
		// No connection-error modal â€” the bug surfaced as
