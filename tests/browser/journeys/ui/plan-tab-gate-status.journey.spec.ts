 ...
 * then patches the live `/descendants` response to inject the two fields â€”
...
import { apiFetch, createGoal, defaultProjectId, seedTeamLeadHeader } from "../../_helpers/e2e-setup.js";
import { openApp, navigateToHash } from "../../../support/harnesses/browser/legacy-ui/ui-helpers.js";

test.describe("Plan tab â€” per-node gate status + merge/conflict", () => {
	let parentId = "";
	let childId = "";
	...
	test.beforeEach(async ({ gateway }) => {
		...
		// spawn-child is ORCHESTRATION (cookie does NOT bypass) â€” authorize as
	...
	test("LIVE (non-archived) child node also surfaces gate-status from descendant enrichment", async ({ page, gateway }) => {
		...
		// state.goals, which never carries the enrichment fields â€” they must
	...
	test("running gate shows dot but no conflict pill, persists across reload", async ({ page }) => {
		...
		// Running, not conflicted â€” no conflict pill.
