/**
 * Plan tab + tree-cost â€” archived & completed children E2E.
 *
 * Verifies the user-facing contract for the archived-children fix:
 *
 *   1. Plan tab default view shows ALL direct children â€” live
 *      in-progress AND completed AND archived.
 *   2. Reload mid-test â†’ archived child still appears (catches the
 ...
 *   4. Tree-cost breakdown attributes spend to all descendants â€” the
 ...
 * â€” same pattern as `plan-tab-unarchive-dedupe.spec.ts` and friends â€”
...
test.describe("Plan tab + tree-cost â€” archived/completed children", () => {
	let parentId = "";
	let liveChildId = "";
	let completeChildId = "";
	let archivedChildId = "";
	...
	test("Plan tab shows live + completed + archived children; live-only toggle hides archived; cost breakdown attributes archived spend", async ({ page }) => {
		...
		// All three plan-nodes appear by default â€” including archived.
		...
		// Reload â€” archived child must still be visible after a fresh hydrate.
		await page.reload();
		...
		// Toggle live-only â€” archived child must disappear, live remains.
		...
		// (complete/shelved). The button is labelled "Live only" â€” it
		...
		// Toggle back â€” archived child reappears.
		await toggle.click();
		...
		// Tree-cost breakdown â€” archived child must be present with its
