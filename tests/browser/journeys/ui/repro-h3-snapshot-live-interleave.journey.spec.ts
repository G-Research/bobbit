/**
 * H3 reproduction â€” concurrent snapshot â†” live race during the RTT window.
 *
...
import { apiFetch, nonGitCwd } from "../../_helpers/e2e-setup.js";
import {
	openApp,
	navigateToHash,
	sendMessage,
...
test.describe("H3 â€” snapshot â†” live interleave race", () => {
	test.setTimeout(180_000);

	// QUARANTINED: H3-A is flaky on master (HEAD a9b87419 reproducible).
	// Refinement of PR #520 (5f8207eb "Fix snapshot â†” live-event race") is
	// needed â€” separate goal. Do NOT undo this fixme until that goal lands.
	...
	test("(C) WS drop+reconnect mid-stream loses no rows", async ({ page }) => {
		...
		for (let i = 0; i < ITER; i++) {
			...
			// instantly â€” that produced false-positive `waitForFunction` returns
	...
	test("(D) two-tab visibility+resync convergence on identical transcripts", async ({ page, context }) => {
		...
		for (let i = 0; i < ITER; i++) {
			...
			// Total user rows (includes all iters) â€” should equal i+1.
