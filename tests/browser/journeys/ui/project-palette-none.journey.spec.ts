 ...
 *
 * Bug:
 *   In Settings â†’ <project> â†’ Appearance, clicking the "None (use global)" card
 ...
 * "Fix project None palette" â€” a client-side state-merge bug in
...
test.describe('Per-project palette "None (use global)" card', () => {
	...
	test("clicking None clears the per-project palette override", async ({ page }) => {
		...
		// Appearance tab content should load â€” wait for the "Color Palette" heading
		...
		// --- Step 3: reload â€” "None" should still be Active and DOM still global.
		await page.reload();
		await expect(
