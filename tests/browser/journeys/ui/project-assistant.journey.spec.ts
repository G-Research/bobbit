 ...
 * promote â†’ config write â†’ terminate-session chain, so DOM-only polling can
...
test.describe("Project assistant UX (consolidated)", () => {
	...
	test("happy path â€” create provisional, accept proposal, project promoted with config", async ({ page }) => {
		await openApp(page);
		...
		// Verify form fields are present â€” Project Name + Root Path live in the
	...
	test("panel updates live across two propose_project calls", async ({ page }) => {
		...
		// in the same turn â€” first with components only, then with components +
		...
		// Components view is the default â€” both components should render.
		...
		// Switch to Workflows tab â€” workflow cards from the SECOND proposal must
