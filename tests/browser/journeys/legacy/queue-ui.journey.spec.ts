...
test.describe("Queue UI E2E", () => {
	...
	test("PI-10: steer pill dispatches queued row mid-turn without abort @smoke", async ({ page, rec }) => {
		...
		await rec.capture("Follow-up queued â€” pill + Steer button visible");

		// PI-10 step 2: Click Steer â†’ streaming promotion immediately
		...
		await rec.capture("Steer clicked â€” queued row dispatched");
		...
		// AC Â§5: queue row drops once the steer is delivered.
		await expect(page.locator(".queue-pill")).toHaveCount(0, { timeout: 15_000 });
	});

	test("PI-10b: batch steer â€” two pills promoted, both delivered without abort", async ({ page, rec }) => {
		...
		await rec.capture("Two messages queued â€” both pills visible");
		...
		// AC Â§5: queue rows drop once the steers are delivered.
		await expect(page.locator(".queue-pill")).toHaveCount(0, { timeout: 15_000 });
	});
	...
	test("story 22: draft text persists across page reload", async ({ page, rec }) => {
		...
		// Type draft text (don't send) â€” use fill which fires input event
	...
	test("story 9: edit pill â€” remove, modify, re-queue at end", async ({ page, rec }) => {
		...
		try {
			...
			// Re-queue modified version â€” should appear AFTER "keep me"
