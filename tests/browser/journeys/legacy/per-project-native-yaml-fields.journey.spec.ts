 ...
 *   - qa_env         (currently no dedicated UI â€” exercised via the same
...
import { apiFetch, readE2EToken, base } from "../../_helpers/e2e-setup.js";
...
/**
 * Assert the on-disk YAML uses native YAML for a migrated field â€”
...
test.describe("Per-project native-YAML field editing", () => {
	test("sandbox_tokens: toggle Add token + enabled in Settings persists across reload and writes native YAML", async ({ page }) => {
		...
		try {
			...
			// initialize once host tokens have loaded â€” the editor briefly
			...
			// Locate the row by walking up from the key input â€” the [value=]
			...
			// Toggle disabled, then re-enable â€” exercises both transitions.
