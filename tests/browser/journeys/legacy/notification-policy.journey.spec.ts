 ...
 *
 *   1. Team member idle â†’ silent (no unread dot, no favicon badge).
 *   2. Goal complete + team lead idle â†’ notify (unread dot appears).
 *
 ...
 *     scenario under test. The server fields don't matter â€” the predicate
 ...
 *  E2Es is unreliable on Playwright â€” we call `__bobbitRenderApp` directly
...
test.describe("Notification policy â€” sidebar unread dot", () => {
	...
	test("standalone idle session shows the unread dot; team-member patch silences it", async ({ page }) => {
		...
		//    now and leaves lastReadAt undefined â†’ unread.
		...
		// 2. Patch the session in client state to look like a team member â€”
		...
		// goal) and the goal doesn't exist server-side â€” so the row vanishes.
		...
		// 3. Revert the patch â€” the session looks standalone again and the dot
	...
	test("team-lead idle with goal complete shows the unread dot; in-progress goal hides it", async ({ page }) => {
		...
		// Flip the goal to in-progress â€” the team lead is no longer at the
	...
	test("dot state on reload matches server-side lastReadAt", async ({ page }) => {
		...
		// Reload â€” fresh navigation. The dot must NOT appear, because server
