...
test.use({ basePath: GATEWAY_PATH });
...
test.describe("Journey: authoritative explicit prefixed gateway", () => {
	test("keeps REST and viewer traffic on the stored prefix and never sends Bearer localhost", async ({ page, gateway }) => {
		...
		expect(gateway.uiBaseURL).toBe(gateway.baseURL);

		// Explicit saved gateways remain supported when the browser and gateway
		// share an admitted origin. Arbitrary cross-origin browser API/WebSocket
		// access is intentionally denied by the outer request-admission policy.
		const explicitBase = gateway.baseURL;
		...
		try {
			...
			await page.goto(`${gateway.uiBaseURL}/`, { waitUntil: "domcontentloaded" });
			...
			const createStaffResponse = await gatewayApiFetch(gateway, "/api/staff", {
				...
				body: JSON.stringify({
					...
					systemPrompt: "Exercise explicit mounted gateway inbox actions.",
