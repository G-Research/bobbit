...
async function installFixture(page: Page): Promise<void> {
	...
	await startTailPhaseTracker(page, TRACKER, [MARKER]);
}

async function emitMarkerEvent(page: Page, type: "message_update" | "message_end", id: string): Promise<void> {
	...
	}, { marker: MARKER, eventType: type, eventId: id });
}

async function addProjection(page: Page, phase: "streaming" | "settled", id: string): Promise<void> {
	await page.evaluate(({ marker, projectionPhase, eventId }) => {
	...
	}, { marker: MARKER, projectionPhase: phase, eventId: id });
