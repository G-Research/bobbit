...
} from "../../support/harnesses/shared/unit-file-budget-reporter.js";
...
describe("UnitFileBudgetReporter", () => {
	test("allows every tier-1 project at the exact 25 second boundary", () => {
		...
		for (const project of ["v2-core", "v2-dom", "v2-integration", "v2-isolated"]) {
			elapse(moduleFor(`/repo/tests/unit/core/${project}.unit.test.ts`, project), UNIT_FILE_WALL_BUDGET_MS);
		}
	...
	test("requires the exact proof-mode opt-in value", () => {
		...
		elapse(moduleFor("/repo/tests/unit/core/slow.test.ts", "v2-core"), 25_001);
	...
	test("does not create exemptions for additional unit projects", () => {
		const { reporter, elapse } = timedReporter();
		elapse(moduleFor("/repo/tests/unit/core/future.test.ts", "v2-future-unit"), 25_001);
	...
	test("ignores the conditional e2e project", () => {
		const { reporter, elapse } = timedReporter();
		elapse(moduleFor("/repo/tests/unit/core/team-manager.unit.test.ts", "v2-e2e-vitest"), 60_000);
	...
	test("resets completed timing aggregates at the start of each run", () => {
		const { reporter, elapse } = timedReporter();
		elapse(moduleFor("/repo/tests/unit/core/slow.test.ts", "v2-core"), 26_000);
		reporter.onTestRunStart();
