...
// Source: tests/goal-metadata-second-review-fixes.test.ts
...
describe("assembleSystemPrompt — role-only backward compatibility (F2)", () => {
	...
	beforeEach(() => {
		bobbitDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "sp-role-only-")));
		stateDir = path.join(bobbitDir, "state");
		fs.mkdirSync(path.join(stateDir, "session-prompts"), { recursive: true });
		// initPromptDirs takes an explicit dir; the BOBBIT_DIR override is applied
		// per-test via withEnv so it is restored (never leaks into the fork).
		withEnv({ BOBBIT_DIR: bobbitDir }, () => initPromptDirs(stateDir));
	...
	it("renders a role-only session inside a `# Goal` section when no order given", () => withEnv({ BOBBIT_DIR: bobbitDir }, () => {
	...
	it("keeps Role as a standalone section (no `# Goal` header) when an order is supplied", () => withEnv({ BOBBIT_DIR: bobbitDir }, () => {
	...
	it("goalSpec + role with no order keeps the merged historical shape", () => withEnv({ BOBBIT_DIR: bobbitDir }, () => {
