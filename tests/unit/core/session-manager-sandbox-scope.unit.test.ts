...
describe("session-manager sandbox scope regressions", () => {
	...
	it("force-abort respawn honors a false sandbox wiring result", () => {
		...
		// Recovery now preserves the original filesystem realm by failing closed
		// instead of silently downgrading the durable sandbox session to the host.
		assert.match(SESSION_MANAGER, /if \(!sandboxApplied\) \{\s*throw new Error\(`Cannot recover sandboxed session \$\{id\}: sandbox realm is unavailable`\);\s*\}/);
		assert.doesNotMatch(SESSION_MANAGER, /if \(!sandboxApplied\) \{[\s\S]*?session\.sandboxed = false;[\s\S]*?update\(id, \{ sandboxed: false \}\);/);
	});

	it("direct agents receive scoped gateway credentials from session-manager", () => {
		// Sandboxed agents retain the per-project scoped-token boundary; direct
		// host agents intentionally receive the host-readable admin credential.
		...
		assert.match(SESSION_MANAGER, /private scopedGatewayEnvForDirectAgent\([\s\S]*?const adminToken = readToken\(\);[\s\S]*?env\.BOBBIT_TOKEN = adminToken;/);
		assert.match(SESSION_MANAGER, /else \{\s*this\.applyScopedGatewayCredentials\(bridgeOptions, id, session\.projectId/);
	});
});
