...
// Source: tests/propose-project-validation.test.ts
...
// when this test runs inside a live Bobbit session (env-inherited). Every
// execute() runs inside withEnv({ BOBBIT_SESSION_ID: undefined }) so the value
// is cleared for the call and restored afterwards.
const NO_SESSION = { BOBBIT_SESSION_ID: undefined };
...
describe("propose_project — happy path with no qa_ keys", () => {
	it("accepts a minimal proposal", () => withEnv(NO_SESSION, async () => {
...
describe("propose_project — pi-coding-agent calling convention", () => {
	...
	it("does not crash when invoked with the real pi-coding-agent calling convention", () => withEnv(NO_SESSION, async () => {
		const tool = getProposeProject();
		const result = await tool.execute("tooluse_abc123", { name: "p", root_path: "/tmp/p" });
		assert.ok(result);
	}));

	it("does not crash when params contain a legacy qa_* key under the two-arg convention", () => withEnv(NO_SESSION, async () => {
		const tool = getProposeProject();
