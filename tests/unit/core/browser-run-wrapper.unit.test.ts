...
describe("browser-v2 coordinator wrapper", () => {
  ...
  it("places positional test-file filters before Playwright's project option", () => {
    ...
    expect(playwrightCommandArgs(forwarded)).toEqual([
      ...
      ...forwarded,
      "--project", "browser-mcp", "browser-isolated", "browser-canonical",
    ]);
  });
  ...
  it("preserves mixed caller filters and flags in order before the configured projects", () => {
    ...
    expect(args.slice(4, -4)).toEqual(forwarded);
    expect(args.slice(-4)).toEqual(["--project", "browser-mcp", "browser-isolated", "browser-canonical"]);
  });
