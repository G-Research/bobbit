...
// Source: tests/staff-session-staffid-persistence.test.ts
// Bucket: v2-core | Method: codemod | Classification: needs-withEnv
// Review: migrated SessionStore persistence to injected memfs; no process.env mutation remains.
...
const stateDir = path.join("/state", "staff-staffid-test");
...
describe("staff session staffId backfill migration", () => {
	...
	function makeBackfillRunner() {
		const runnerMemfs = createSessionStoreMemFs();
		const sessionStore = new SessionStore(path.join("/state", "pcm-backfill"), runnerMemfs);
		const staffRecords = new Map<string, any>();
		const staffStore = {
			put(staff: any) { staffRecords.set(staff.id, staff); },
			getAll() { return Array.from(staffRecords.values()); },
		};
