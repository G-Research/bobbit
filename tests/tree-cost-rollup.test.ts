...
describe("computeTreeCost", () => {
	...
	it("cache hit: 2 calls within same generation use cached result", () => {
		const tracker = freshTracker();
		_resetTreeCostCacheForTesting(tracker);
		tracker.recordUsage("s1", { cost: 0.05, inputTokens: 100 });
		const goals: G[] = [{ id: "R", title: "R", createdAt: 1 }];

		let calls = 0;
		const sidsFn = (_gid: string) => { calls++; return ["s1"]; };

		const r1 = computeTreeCost("R", goals, tracker, sidsFn);
		assert.equal(calls, 1, "first call invokes sids fn");
		const r2 = computeTreeCost("R", goals, tracker, sidsFn);
		assert.equal(calls, 1, "second call within same generation should NOT invoke sids fn (cache hit)");
		// Returns the same object (referential equality is acceptable for cached result).
