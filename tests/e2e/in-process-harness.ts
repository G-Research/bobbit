...
export const test = base.extend<{}, { enableWorktreePool: boolean; gateway: GatewayInfo }>({
	...
	gateway: [async ({ enableWorktreePool }, use, workerInfo) => {
		...
		// Canonicalize: see gateway-harness.ts for the full rationale
		// (/var/folders → /private/var/folders on macOS; mixing symlink and
		// canonical forms breaks POST /api/projects symlink_root checks AND
		// the preview-mount path-guard containment check, producing 400 instead
		// of 404 for missing files).
		try {
