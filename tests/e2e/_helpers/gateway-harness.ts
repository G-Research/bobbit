...
// the git repo — otherwise isGitRepo() returns true for the project rootPath and
// sessions auto-create worktrees (slow, conflicts with git state).
...
export const test = base.extend<{ failureContext: void; restoreDefaultProject: void }, {
	...
	gateway: [async ({ enableMcp, enableWorktreePool, enableDevHarnessRestart, splitHeadquartersServerRoot, sameRootProjectAtStartup, basePath, separateUiOrigin, browserRenderLease }, use, workerInfo) => {
		...
		const {
			...
			const { defaultBgProcessSpawn } = await importBuiltServerModule<typeof import("../../../src/server/agent/bg-process-manager.js")>("../../../dist/server/agent/bg-process-manager.js");
