...
import { expect, test, type GatewayInfo } from "../_helpers/gateway-harness.js";
import {
	agentEndPredicate,
	apiFetch,
	createGoal,
...
test.describe.serial("canonical host notification E2E", () => {
	...
	test("reconciles the inbox-commit/outbox-ack crash window after a real restart without duplicates", async ({ gateway }) => {
		...
		try {
			...
			if (accepted === null) throw new Error("accepted delivery poll completed without a matching delivery and inbox entry");
			...
			if (reconciled === null) throw new Error("restart reconciliation poll completed without an accepted delivery");
