import { expect, test } from "../_helpers/in-process-harness.js";
import { apiFetch, createGoal, defaultProjectId, deleteGoal } from "../_helpers/e2e-setup.js";

const GATE_ID = "active-snapshot-gate";
// The Windows tracked-process supervisor contributes one diagnostic stderr
// line to the live stream. Omit one otherwise-unobserved fixture label there so
// the semantic 60-line selection boundaries remain identical across platforms.
const LIVE_OUTPUT_CMD = process.platform === "win32"
	? `node -e "for (let i=1;i<=60;i++) if(i!==45) console.log('active-live-line-'+i); setTimeout(()=>process.exit(0),30000)"`
	: `node -e "for (let i=1;i<=60;i++) console.log('active-live-line-'+i); setTimeout(()=>process.exit(0),30000)"`;
