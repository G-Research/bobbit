...
const FIXTURE_SOURCE = path.resolve(__dirname, "..", "..", "support", "fixtures", "shared", "market-sources", "propose-goal-iserror-src");
...
test.describe("propose_goal bridge isError preservation", () => {
	...
	test("failed propose_goal tool result persists returned isError true", async ({ gateway }) => {
		await installPack();
		...
		try {
			const input = {
				...
				__sessionSecret: gateway.sessionManager.sessionSecretStore.getOrCreateSecret(sessionId),
