...
import { apiFetch, createSession, deleteSession, deleteGoal, nonGitCwd } from "../_helpers/e2e-setup.js";
...
async function notifyPackFilesystemMutation(): Promise<void> {
	const current = await apiFetch("/api/marketplace/pack-order?scope=server");
	if (current.status !== 200) throw new Error(`pack order read failed: ${current.status}`);
	const order = (await current.json()).order as string[];
	const response = await apiFetch("/api/marketplace/pack-order", {
		method: "PUT",
		body: JSON.stringify({ scope: "server", order }),
	});
	if (response.status !== 200) throw new Error(`pack mutation notification failed: ${response.status}`);
}
...
let sessionSecretFor: (sessionId: string) => string;
...
async function callBeforePrompt(sessionId: string, prompt: string): Promise<{ status: number; content: string; tail: string }> {
	const resp = await apiFetch(`/api/sessions/${sessionId}/provider-hooks/before-prompt`, {
		method: "POST",
		headers: { "X-Bobbit-Session-Secret": sessionSecretFor(sessionId) },
...
test.describe.serial("provider hook endpoints resolve the effective goal (teamGoalId)", () => {
	...
	test.beforeAll(async ({ gateway }) => {
		...
		sessionSecretFor = (sessionId) => gateway.sessionManager.sessionSecretStore.getOrCreateSecret(sessionId);
		await notifyPackFilesystemMutation();
	...
	test.afterAll(async () => {
		...
		await notifyPackFilesystemMutation().catch(() => {});
	});
