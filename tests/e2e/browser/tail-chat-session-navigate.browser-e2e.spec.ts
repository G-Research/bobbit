...
import { createSession, waitForSessionStatus, waitForHealth } from "../../../tests/support/harnesses/browser/e2e-setup.js";
import {
	TAIL_PX,
...
test.describe("tail-chat: session navigate lands on latest message", () => {
	...
	test("A → B → A: replayed transcripts are bottom-pinned", async ({ page, rec }) => {
		test.slow(); // 3× timeout for scroll-pinning precision under concurrent load
