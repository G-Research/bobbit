...
test.describe("base_ref API validation", () => {
	test("PUT round-trip — set origin/<branch>, GET returns it, empty clears it", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-rt-")));
		gitInit(root);
	...
	test("rejects commit SHA shape with the exact error string", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-sha-")));
		gitInit(root);
	...
	test("rejects tag with the exact error string", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-tag-")));
		gitInit(root);
	...
	test("rejects invalid branch grammar with the exact error string", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-grammar-")));
		gitInit(root);
	...
	test("rejects non-origin remote prefix with the exact error string", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-nonorigin-")));
		gitInit(root);
	...
	test("rejects local ref when sandbox = docker with the exact error string", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-sandbox-")));
		gitInit(root);
	...
	test("accepts local branch ref in a non-sandbox project", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-local-")));
	...
	test("multi-repo: missing ref in subset of components returns 400 with structured details[]", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-multi-")));
	...
	test("validation only fires when base_ref is in the PUT body", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-skip-")));
		gitInit(root);
	...
	test("whitespace-only value is treated as unset (200)", async () => {
		const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bobbit-baseref-ws-")));
		gitInit(root);
