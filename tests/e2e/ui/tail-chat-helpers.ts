 ...
 * (`_pinIfSticking` + RO `delta>0` + `_imageLoadHandler`) to be the single
 * contract. Without this, Chromium's default `overflow-anchor: auto`
 * transparently pins the viewport even when the JS path is broken —
 ...
 */
export async function disableScrollAnchoring(page: import("@playwright/test").Page): Promise<void> {
