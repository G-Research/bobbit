...
// landed well ahead of the Marketplace UI redesign this spec pins (config-home
// state badge, inline Configure form, guided wizard). Per the W2.E restoration
// review (commit 1311e5ce), that Marketplace-side UI was never carried into
// marketplace-page.ts's lineage after the "embedded dashboard" redesign landed
// on top of it — confirmed an abandoned-mid-flight feature, not a merge-drop,
// via `git stash` bisection (identical failure before/after). The three
// fs.existsSync checks below only prove the pack-contribution half is ready;
// they say nothing about the Marketplace UI half, so this file also needs a
// static source check for a stable marker unique to that redesign — without
// it STACK_READY goes true prematurely and every test here fails hard instead
// of skipping as this file's own header promises ("green-by-skip until the
// parallel coder branches land").
