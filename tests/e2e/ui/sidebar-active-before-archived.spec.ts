...
async function createLiveChild(parentId: string, title: string, planId: string): Promise<string> {
	...
	const resp = await apiFetch(`/api/goals/${parentId}/spawn-child`, {
		method: "POST",
		body: JSON.stringify({ planId, title, spec: "Child goal created for active-before-archived sidebar ordering test; validates live/archived ordering.", autoStartTeam: false }),
	});
	expect(resp.status).toBe(201);
	const data = await resp.json();
	return data.id;
