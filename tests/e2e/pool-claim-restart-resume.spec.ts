...
test.describe.serial("pool claim restart-resume", () => {
	test("session/<id8> branch is byte-stable across simulated restart; no git branch -m runs post-restart", async ({ gateway }) => {
		// mkdtemp gives each concurrent test invocation an owned root. Canonicalize
		// it immediately so macOS's /var → /private/var spelling cannot leak into
		...
		try {
			...
			// Delete gateway state first so its session/worktree handles have closed.
			...
			expect(cleanup.removed, `owned fixture root was not removed: ${fixtureRoot}`).toBe(true);
		}
	});
});
