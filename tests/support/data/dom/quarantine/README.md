...
**Current placement guidance:** keep these `.txt` files as inactive historical ports. To restore an assertion, fold it into the corresponding `tests/browser/journeys/**/*.journey.spec.ts` journey (sidebar navigation, search page, settings, or goal workflow). The browser path and semantic suffix provide automatic Playwright ownership; do not give a support file a runnable test suffix.
