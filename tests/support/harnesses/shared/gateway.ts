...
async function seedDefaultWorkflows(baseURL: string, token: string, projectId: string): Promise<void> {
	...
	if (!resp.ok) {
		...
		throw new Error(`[tests/gateway] default workflow seed failed: ${resp.status} ${resp.statusText} body=${body}`);
	}
}
...
async function registerDefaultProject(baseURL: string, token: string, rootPath: string): Promise<string> {
	...
	if (!resp.ok) throw new Error(`[tests/gateway] default project register failed: ${resp.status} ${resp.statusText} body=${text}`);
	const project = JSON.parse(text) as { id?: string };
	if (!project.id) throw new Error(`[tests/gateway] default project register returned no id: ${text}`);
	return project.id;
}
...
function prepareBuiltinPacksDir(bobbitDir: string): string {
	...
	for (const name of FIRST_PARTY_PACKS) {
		...
		if (!existsSync(src)) throw new Error(`[tests/gateway] first-party pack not found: ${src}`);
...
async function boot(): Promise<BootedGateway> {
	...
	console.log(`[tests/gateway] fork ${process.pid}: server runtime=${serverRuntimeMode()}`);
	...
	if (useFakeCommandStep) console.log(`[tests/gateway] fork ${process.pid}: injecting FAKE verification command-step runner (no shell spawns)`);
	...
	const fixture: BootedGateway = {
		...
		async apiJson<T = any>(path: string, init?: RequestInit): Promise<T> {
			...
			if (!resp.ok) throw new Error(`[tests/gateway] ${init?.method ?? "GET"} ${path} -> ${resp.status} ${resp.statusText}: ${text}`);
		...
		async connectWs(suffix, opts) {
			...
			return await new Promise<WebSocket>((resolvePromise, reject) => {
				...
				const onMessage = (raw: WebSocket.RawData) => {
					...
					else if (msg.type === "auth_failed") { cleanup(); ws.close(); reject(new Error("[tests/gateway] ws auth_failed")); }
				};
