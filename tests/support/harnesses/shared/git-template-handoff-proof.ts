...
const STATE_KEY = Symbol.for("bobbit.tests.git-template-handoff-proof-state");
...
function canonicalTestPath(path: string): string | undefined {
	...
	const markerIndex = normalized.lastIndexOf("/tests/");
	const relativePath = markerIndex >= 0
		? normalized.slice(markerIndex + 1)
		: normalized.startsWith("tests/") ? normalized : undefined;
