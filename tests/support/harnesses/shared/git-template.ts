...
const STATE_KEY = Symbol.for("bobbit.tests.git-template-state");
...
function invalidTemplate(message: string): Error {
	return new Error(`[tests/git-template] cannot adopt template: ${message}`);
}
...
function validateTemplateShape(repository: string): void {
	try {
		...
		if (error instanceof Error && error.message.startsWith("[tests/git-template]")) throw error;
...
function assertSafeDestination(source: string, destination: string): void {
	...
	if (target === source || relative(source, target).split(/[\\/]/)[0] !== "..") {
		throw new Error(`[tests/git-template] destination must be outside the immutable template: ${target}`);
	}
...
export function readGitTemplateBootstrapAudit(descriptor: GitTemplateDescriptor): GitTemplateBootstrapAudit {
	...
	if (!isOwnedRunChild(getRunRoot(), source)) {
		throw new Error("[tests/git-template] bootstrap audit source must be an owned descendant of the run root");
	}
	...
	if (!Number.isSafeInteger(audit.ownerPid) || (audit.ownerPid ?? 0) <= 0
		...
		throw new Error("[tests/git-template] coordinator bootstrap audit is missing or invalid");
	}
	...
	for (let attempt = 1; attempt <= 3; attempt++) {
		try {
			...
			if (commitState === "invalid") {
				throw new Error("[tests/git-template] initial commit reported failure after creating an unexpected repository state");
			}
		}
	}
	throw failure;
...
export async function prepareGitTemplate(options?: PrepareGitTemplateOptions): Promise<GitTemplateDescriptor> {
	...
	if (explicitCoordinatorCreate && !isRunRootOwner()) {
		throw new Error("[tests/git-template] explicit template creation is coordinator-only");
	}
	...
	shared.promise = (async () => {
		...
		if (explicitCoordinatorCreate && existsSync(container)) {
			throw new Error(`[tests/git-template] coordinator template container already exists: ${container}`);
		}
