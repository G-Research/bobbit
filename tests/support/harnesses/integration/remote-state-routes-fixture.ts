...
function apiFetch(path: string, opts: RequestInit = {}): Promise<Response> {
	...
	if (forcedRemoteRead && !forceRequestBurstOpen) {
		// Calls started synchronously together are one user burst and must coalesce.
		// Once the caller yields, the next explicit read is a distinct force even
		// when a loaded lane completes both requests inside the production 250 ms.
