...
import {
	...
	prepareGitTemplate,
} from "../shared/git-template.js";
import { registerGitTemplateHandoffWorker } from "../shared/git-template-handoff-proof.js";
...
const STATE_KEY = Symbol.for("bobbit.tests.tier1-spawn-guard-state");
...
function blocked(api: GuardedApi, firstArgument: unknown): never {
	...
	throw new Error(
		`[tests/tier1-spawn-guard] blocked child_process.${api} executable=${JSON.stringify(executable)}. ` +
