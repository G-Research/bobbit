...
test.describe("PreviewOpenRenderer", () => {
	...
	test("v3 marker: missing artifact id keeps requested historical tab active with restoreError", async ({ page }) => {
		await gotoAndWait(page);
		...
		expect(tabs[0].state.contentHash).toBe(oldHash);
		expect(tabs[1].state.contentHash).toBe(HASH);
		expect(tabs[1].state.restoreError.message).toBe("Preview artifact unavailable");
		expect(tabs[1].state.restoreError.retryable).toBe(false);
		expect(previewState.panelWorkspaceActiveBySession[SESSION_ID]).toBe("preview:entry:inline.html:v:2");
		expect(previewState.previewPanelMountedTabId).toBe("");
	});
