/**
 * Gateway integration — per-turn provider hooks (Extension Platform G1.4).
 *
 ...
 *
 * The fixture pack is installed into the fork-owned Headquarters config directory: the
 ...
 * fork-scoped in-process gateway).
 *
...
const fixturePackDir = path.resolve(__dirname, "..", "..", "support", "fixtures", "shared", "packs", "provider-demo");
const PACK_NAME = "provider-demo";
let sessionSecretStore: { getOrCreateSecret(id: string): string };
let bridgeEnvironment: { gatewayUrl: string; token: string };
...
async function callBeforePrompt(sessionId: string, prompt: string): Promise<BeforePromptResult> {
	const resp = await apiFetch(`/api/sessions/${sessionId}/provider-hooks/before-prompt`, {
		method: "POST",
		headers: { "X-Bobbit-Session-Secret": sessionSecretStore.getOrCreateSecret(sessionId) },
...
async function registerGeneratedBridgeHandlers(sessionId: string, tempDir: string): Promise<Map<string, (event: any) => Promise<any> | any>> {
	...
	const overrides = {
		BOBBIT_GATEWAY_URL: bridgeEnvironment.gatewayUrl,
		BOBBIT_TOKEN: bridgeEnvironment.token,
		BOBBIT_SESSION_SECRET: sessionSecretStore.getOrCreateSecret(sessionId),
	};
...
test.describe("provider per-turn hooks", () => {
	...
	async function newSession(label: string): Promise<{ id: string; cwd: string }> {
		const requestedCwd = freshCwd(label);
		const id = await createSession({ cwd: requestedCwd });
		sessions.push(id);
	...
	test.beforeAll(async ({ gateway }) => {
		...
		sessionSecretStore = gateway.sessionManager.sessionSecretStore;
		bridgeEnvironment = { gatewayUrl: gateway.baseURL, token: gateway.token };
	...
	test("beforePrompt returns dynamic-context message content, then afterTurn fires; both appear in the context trace", async () => {
		...
		expect(demoBlock!.providerId, "the route exposes normalized host-interceptor ownership").toBe("host-hooks");
