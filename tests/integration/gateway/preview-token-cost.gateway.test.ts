...
test("50 × 100 KB mount calls → snapshot blocks sum ≤ 12 500 B; each ≤ 250 B", async () => {
	...
	const blocks: Array<string | undefined> = Array(50);
	const blockByteSizes: Array<number | undefined> = Array(50);

	await Promise.all(
		Array.from({ length: SESSION_POOL_SIZE }, async (_, sessionIndex) => {
			const sessionId = sessionIds[sessionIndex]!;
			for (let i = sessionIndex; i < 50; i += SESSION_POOL_SIZE) {
	...
	const populatedBlocks = blocks.filter((block): block is string => block !== undefined);
	const populatedByteSizes = blockByteSizes.filter(
		(byteSize): byteSize is number => byteSize !== undefined,
	);
	expect(populatedBlocks).toHaveLength(50);
	expect(populatedByteSizes).toHaveLength(50);

	const total = populatedByteSizes.reduce((sum, byteSize) => sum + byteSize, 0);
	...
	expect(new Set(populatedBlocks).size).toBe(50);
});
