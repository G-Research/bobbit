...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description: "Merge a child's branch locally into the parent without pushing. Clean merge auto-archives the child; conflicts return structured 409 data. Pass force=true to bypass the ready-to-merge gate check (appropriate when the child's workflow has no such gate, or when the merge has been manually approved as safe).",
