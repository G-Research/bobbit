...
1. **LSP first (exact, cheapest).** Use the `LSP` tool: `workspaceSymbol` to find a symbol, then `goToDefinition` / `findReferences` / `incomingCalls` / `outgoingCalls` to trace. TS LSP is available (`typescript-language-server` installed).
   ...
   **No LSP tool in subagent sessions** — it's interactive-session-only. If you have the `code_*` tools (`code_definition`/`code_references`/`code_hover`/`code_symbols` — default-allow on every role since wave 1 of `docs/design/lsp-product-tools.md`), prefer those: same tsserver backend, but a warm per-worktree instance instead of a fresh spawn per call, so later calls in the same worktree are fast. Only fall back to `node scripts/lsp-cli.mjs <cmd> ...` in a genuinely Bash-only session (no other tools): `workspace <file> <query>`, `symbols <file>`, `refs <file> <line> <col>`, `def <file> <line> <col>`, `hover <file> <line> <col>` (line/col are 1-based). Workspace root = the git toplevel of `<file>`, so a query against your own worktree loads that worktree's tsconfig, not the primary checkout's. First call can take ~30s while the TS project loads (it polls internally up to `--timeout`, default 60s). `workspace` warms the anchor file with `documentSymbol` before polling workspace symbols and returns repo-relative file suffixes for worktree-safe comparison. Most of `tests/**` isn't covered by any `tsconfig.json`, so project-wide `refs`/`def` won't resolve there — the same gap applies to `code_*`.
