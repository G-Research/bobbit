...
| Review | `review:<encoded-reviewId>` | stable `reviewId`, display `title` | One closable primary tab represents the whole review. Its ordered files are review-pane navigation, never workspace tabs. |
...
Review state belongs to the session that opened it. A live background-session open persists the complete group and opens or focuses that session's primary tab without changing the selected session or the foreground review model. A background `review_close` likewise affects only its owner session. Switching to the owner later hydrates the group and its selected file only if that exact primary tab is still authoritative. Duplicate display titles remain separate because workspace identity is `reviewId`, not title.

Closing a primary tab closes that whole review after the aggregate unsent-comment confirmation. A successful Approve or Reject decision likewise cleans up only the selected review, including all its persisted files and annotations. Other review groups and their primary tabs remain intact. Historical tool-result replay may restore neither a closed nor a submitted review; only a new explicit live open may recreate its primary tab. See [Review Pane Sign-Off — Review hierarchy and identity](review-pane-signoff.md#review-hierarchy-and-identity) for file navigation, decision, and cleanup details.
