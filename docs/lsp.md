...
> **Tool-selection rule — canonical location.** The mandatory **`## Tool selection — LSP before text search`** section is a protected core rule injected by `assembleSystemPrompt()` in `src/server/agent/system-prompt.ts`. It is read from the base system prompt first, before role prompts, AGENTS.md, or goal specs. By default the base prompt is `defaults/system-prompt.md`; if a project supplies `.bobbit/config/system-prompt.md`, that file is used instead. Either way, **Bobbit automatically appends the canonical LSP section when the selected base prompt omits it** — project overrides may replace general wording but cannot suppress this protected rule. The rule applies to all session types: normal sessions, worktree/goal sessions, team-leads, assistant/goal/project/tool sessions, delegate sessions, and sessions rebuilt on respawn or role-switch. Because the base prompt is loaded before — and separately from — the role template, a **project-level role override** in `.bobbit/config/roles/*` cannot suppress it either (role overrides fully shadow the default role YAML — no field-level merge — see [Config cascade](internals.md#config-cascade)). The earlier per-role runtime injection via `session-setup.ts` and the `buildLspSymbolLookupHint` step in `assembleSystemPrompt()` have both been removed to prevent duplicate competing sections. **Role YAMLs (default and project-level overrides) must not define their own `## Tool selection — LSP before text search` section** — the protected injection ensures exactly one copy exists.
...
| `ENOENT … stat '/workspace-wt/…'` in an LSP error, or `routeSelfCheck: "failed:diagnostics:initialize_failed"` | Bridge-attached-without-container bug: a `DockerSandboxLspBridge` was attached even though no sandbox container is running, so `tsserver` was spawned on the host but initialised with container-side paths it cannot stat. Fixed by the commit that introduced `containerIdForWorktree` gating in `typescript.ts`. | Should not recur; if it does, file a bug and use `lsp_disabled: true` as a temporary workaround. |
