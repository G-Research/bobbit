...
> **Tool-selection rule — canonical location.** The mandatory **`## Tool selection — LSP before text search`** section lives in `defaults/system-prompt.md` — the shipped default base system prompt. `assembleSystemPrompt()` in `src/server/agent/system-prompt.ts` reads the base system prompt first, before role prompts, AGENTS.md, or goal specs. By default this is `defaults/system-prompt.md`; if a project supplies `.bobbit/config/system-prompt.md`, that file is used instead and must preserve the LSP rule (or the project takes responsibility for the omission). Apart from that global override, the rule applies to all session types: normal sessions, worktree/goal sessions, team-leads, assistant/goal/project/tool sessions, delegate sessions, and sessions rebuilt on respawn or role-switch. Because the base prompt is loaded before — and separately from — the role template, a **project-level role override** in `.bobbit/config/roles/*` cannot suppress it even though role overrides fully shadow the default role YAML (no field-level merge — see [Config cascade](internals.md#config-cascade)). The earlier per-role runtime injection via `session-setup.ts` and the `buildLspSymbolLookupHint` step in `assembleSystemPrompt()` have both been removed to prevent duplicate competing sections. **Role YAMLs (default and project-level overrides) must not define their own `## Tool selection — LSP before text search` section** — the canonical one in the base system prompt is sufficient.
