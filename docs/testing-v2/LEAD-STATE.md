...
## OPEN USER DECISIONS
1. **Concurrency bar** — UNRESOLVED. Measured: single 251s green; 5-way ~800–960s FAIL; 3-way ~570–697s 0/9 FAIL. Root cause = per-test GATEWAY BOOTS (each v2-integration test boots a full gateway; ~15–20 concurrent boots saturate 24 cores; the ~6 heaviest integration tests starve — DETERMINISTIC, not flaky). Options posted (relocate-heavy-to-daily / ledger-queue reframe / drop-to-2 / gateway-cost-reduction). **User asked for a FEASIBILITY STUDY into reducing the gateway cost FIRST** (NOT a 4-way run, NOT auto-adopting "make it a prerequisite"). Study in progress (see roster). Decide the bar AFTER the study.
