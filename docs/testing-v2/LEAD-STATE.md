...
- **Cluster B: DENOMINATOR CLOSED** (N=M=24, 18 holes, 0 un-mutated, authoritative 18/18, null-integrity PASS) — merged to goal 1860c57c, coder-51bf dismissed. Only **Cluster A** still active (~35/45, ~10 TODO; coder-b93a b7246fe1). CONSOLIDATION TODO (lead): when Cluster A closes, renumber BR-id collisions folding clusterA.json+clusterB.json into canonical browser-mutants.json, reconcile the per-cluster tally arithmetic into a unified burn-down, run the single canonical `browser-chaos.mjs --all` (0 real holes = the D8 gate).
