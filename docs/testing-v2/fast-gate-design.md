...
The hard outcome is two consecutive green solo runs at or below 300 seconds, followed by three simultaneous green runs on the Windows 12-core acceptance machine. The stretch target is 180 seconds. `retry: 3` remains the workflow safety net, not stability evidence: cross-OS qualification separately runs with `--retry=0`, and every retried result is diagnostic debt. Passing by deleting coverage, weakening assertions, or relocating any file other than the two named below is not allowed.
...
- `retry: 3` remains in `vitest.config.ts`; no project may override it downward. Retry-free qualification uses the CLI override, not a weaker committed workflow policy.
