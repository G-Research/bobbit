...
`vitest.config.ts` applies a fixed suite-wide cap of three workers and `retry: 3`. The workflow retry budget is developer-productivity protection for isolated transients, not evidence of stability; every retried result is diagnostic debt to root-cause. Cross-OS qualification runs the same inventory with `--retry=0` and accepts only first-attempt passes. `VITEST_MAX_WORKERS=1` or `2` may lower the cap for diagnosis; it cannot raise the cap above three.
