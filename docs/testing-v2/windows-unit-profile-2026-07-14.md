...
| Three-worker one-init probe | Three worker-local initializations and 30 bootstrap Git commands | 1.879 / 1.865 / 1.899 s | 1.881 s profile wall | Every round used three forks and one coordinator initialization: 10 Git commands total, **66.7% fewer** bootstrap commands. |
...
The incidental slice occupied three forks in every round and launched no fixture Git beyond the ten coordinator-bootstrap commands. Its four fixtures therefore no longer amplify repository creation. The one-init probe additionally pins one shared path and digest, distinct worker identities, private writable copies, an immutable shared source, unchanged ten-command audit before and after adoption, and no worker cleanup authority.
...
| Tier-1, three guarded forks | Guarded setup registers each fork before tests; `git-template-handoff-probe-{a,b,c}.test.ts` activates coordinator end-of-run certification without a scheduler barrier. | Coordinator-only ten-command bootstrap; common path/digest; three distinct process/pool/worker identities; immutable source; private writable copies; worker cleanup denial. |
