...
| Tier-1, three guarded forks | `git-template-handoff-probe-{a,b,c}.test.ts` — `adopter {a,b,c} shares the one-init template and receives a writable private copy` | Coordinator-only ten-command bootstrap; common path/digest; three distinct adopters; immutable source; private writable copies; worker cleanup denial. |
