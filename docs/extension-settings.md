...
derived from the pack id, `provider`, `hook`, `runtime`, or `sandboxRequirement` kind, and
contribution id; clients never
...
| `PATCH` | `/api/projects/:projectId/extension-settings/:packId/:kind/:id` | Changes one server-resolved `provider`, `hook`, `runtime`, or `sandboxRequirement` target. Body is `{ expectedRevision, enabled?, values? }`. `values` maps declared keys to a valid primitive, a valid multi-enum string array, or `null` to clear. Returns `{ revision, target }`, with the target redacted. |
