...
The `LSP` tool itself is interactive-session-only; subagents run the same queries via `scripts/lsp-cli.mjs` (`workspace`/`symbols`/`refs`/`def`/`hover`) — see [`.claude/skills/orient/SKILL.md`](../.claude/skills/orient/SKILL.md). The `workspace <file> <query>` command warms the anchor file before polling workspace symbols and emits repo-relative file suffixes, which avoids treating primary/worktree absolute path prefixes as different symbols. Wave 1 of productizing this as an agent-facing tool group (per [docs/design/lsp-product-tools.md](design/lsp-product-tools.md), F6) has shipped: `code_definition`/`code_references`/`code_hover`/`code_symbols` (`defaults/tools/code/`) are default-allow tools backed by a gateway-owned, per-worktree `TsServerSupervisor` (`src/server/lsp/`) — TS-only, no diagnostics yet. `scripts/lsp-cli.mjs` is unchanged as the Bash-only subagent fallback and now shares its JSON-RPC client (`src/server/lsp/client.ts`) with the supervisor.
