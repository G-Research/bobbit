...
The `LSP` tool itself is interactive-session-only; subagents run the same queries via `scripts/lsp-cli.mjs` (`symbols`/`refs`/`def`/`hover`) — see [`.claude/skills/orient/SKILL.md`](../.claude/skills/orient/SKILL.md).
