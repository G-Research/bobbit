...
npm test               # Unit, browser, then E2E; manual remains opt-in
```

Use `npm run test:unit` for the normal edit loop. Run the browser and E2E lanes when their behavior is relevant, then run every phase required by the workflow before merge. Retry-free qualification uses:
...
The boot sweeper (`worktree-sweeper.ts`) scans these against persisted state for diagnostics, but never repairs or cleans a discovered worktree based on its branch or path shape. Pool initialization re-adopts only entries authorized by the exact v1 records Bobbit wrote to `state/worktree-pools.json`, after every Git path/branch and live-session exclusion check succeeds; a pool-shaped leftover without that proof stays diagnostic-only. Graceful shutdown stops pools, flushes the record, and retains unclaimed entries for reuse. Explicit project deletion still drains its pool locally. See [internals.md — Session worktrees](internals.md#session-worktrees) and [Preserve user worktrees](design/preserve-user-worktrees.md).
