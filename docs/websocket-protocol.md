...
| `gate_verification_step_complete` | `goalId`, `gateId`, `stepIndex`, `status`, `outputTruncated?`, `originalOutputBytes?`, `previewOutputBytes?`, `cancellation?` | A verification step finished or was cancelled. `cancellation` carries the durable cause for an interrupted step. Large completion output is truncated to a bounded WS preview; full output remains available through gate inspection/retained diagnostics. |
...
| `gate_verification_complete` | `goalId`, `gateId`, `signalId`, `status`, `cancellation?` | All verification steps finished. A cancellation has `status: "cancelled"` and its durable cause; it is not a failure. |
| `gate_status_changed` | `goalId`, `gateId`, `status`, `verificationStatus?`, `cancellation?` | Gate status changed. `verificationStatus` and `cancellation` are optional and not guaranteed on this invalidation event. For a cancellation cause, consume the matching `gate_verification_complete` event or refresh the authoritative gate summary. |
