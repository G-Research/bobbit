...
| `project_import_decision_requests_updated` | `projectId`, `ts` | Exact payload `{ type: "project_import_decision_requests_updated", projectId: string, ts: number }`. Metadata-only invalidation after a project's durable import-decision projection changes; it carries no import context, request, question, answer, option, resolution, or secret. Clients re-fetch the project import-decision REST projection. |
