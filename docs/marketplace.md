...
### The pack-scoped contributions (panels, entrypoints, routes, channels, settings sections)

Panels, entrypoints, routes, channels, and settings sections are **pack-scoped** — they are not anchored to a carrier tool, which is what lets a pack ship them with no `tools/` dir at all:
...
- **Settings sections** — one `settings/<id>.yaml` per section (`{ id, title?, scope: "system", tab, order?, entry, preferenceKeys? }`), **auto-discovered** like panels. Renders inline in an existing Settings tab — no session, no tab lifecycle, no `instanceMode`. The Host API is a narrow `SettingsHostApi` (`preferences.{get,set}` + `requestRender()` only), key-allowlisted per the section's declared `preferenceKeys` and re-checked LIVE server-side on every write regardless of the declaration — keys gated behind the Claude-Code-runtime operator-confirmation flow or the agent-directory workflow are hard-rejected unconditionally. v1 supports `scope: "system"` only. See [docs/design/pack-settings-contribution.md](design/pack-settings-contribution.md).
...
<scope>/.bobbit/config/market-packs/<pack>/
  ...
  settings/<id>.yaml            # auto-discovered pack Settings-page sections
...
| `GET /api/ext/packs/:packId/settings-sections/:sectionId?projectId=` | Serve a **pack-scoped Settings-section's** pre-built ESM module bytes, mirroring the panel-serving route ([docs/design/pack-settings-contribution.md](design/pack-settings-contribution.md)). |
| `POST /api/ext/packs/:packId/settings-sections/:sectionId/surface-token` | Mint a session-less pack-bound surface token for a settings section's `SettingsHostApi`. Binds only `{packId, sectionId}` — carries no preference-key claims; the allowlist is re-resolved LIVE from the registry at write time. |
| `GET /api/ext/contributions?projectId=` | Project-scoped pack-contribution metadata for the client registries: one row per installed+active pack `{ packId, packName, panels, settingsSections, entrypoints, routeNames, channelNames }` (empty arrays allowed — the always-emit contract keeps reconcile deterministic). **Activation-filtered** (disabled entrypoints omitted; panels/routes/channels/settingsSections always present). |
