...
- **Tools** — ship code that runs **directly in the Bobbit server process on the host**, deterministically, with no LLM and no sandbox in the loop. This is the highest, most immediate risk. Three distinct code surfaces ship in a tool pack:
  ...
  - **Session write (`host.session.postMessage`) drives the agent, so it is the highest-risk Host-API surface.** Because pack UI runs in the main realm and *can* monkey-patch globals like `window.fetch`, its SEND deliberately does **not** ride a `fetch`: it goes over the app's already-authenticated **session WebSocket**, which pack code has no handle to and cannot send on, so there is no capturable session secret on any request to replay. The server authorizes it (the pack's tool ∈ the session's `allowedTools`, server-derived packId), targets only the WS connection's own session (cross-session posting is structurally impossible), and **audits every post**; the client adds a `navigator.userActivation` "no post on mount" check as defense-in-depth. **FULL realm isolation of pack UI logic (a separate iframe/worker realm) is out of scope** for now — the source-level trust decision plus the server-side authorization/audit are the durable boundary; UI-thread realm isolation is a documented future hardening.
