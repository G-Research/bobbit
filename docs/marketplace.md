...
The Phase-2 keys `panels:`, `entrypoints:`, `routes:`, and `stores:` are **live** — each is parsed into a typed contribution, surfaced on `/api/tools`, and acted on (panels are served + lazily mounted, routes dispatched, stores backed, entrypoints registered). Path-bearing values (`renderer`, `actions.module`, `routes.module`, `panels[].entry`) are rejected at parse time if they contain `..` segments or are absolute (traversal guard); a malformed contributions block degrades gracefully (the tool still loads, with a `console.warn`) and is never fatal. The two real-conflict rejections happen at pack-level registry build: a duplicate `routes:` name within a pack, or a duplicate `entrypoints:` `routeId` across packs. (An older Phase-1 server simply ignores these keys, so a forward-authored pack still installs cleanly.)
