...
Persistent tool and group grants follow the session's role scope as well as its tool scope. A project session writes through its `ProjectContext.roleStore`: a project-local winning role is updated in place, while a builtin, server, or global-user winner is copied on write as a project-local override so the source role remains unchanged. Marketplace and other immutable read-only winners are never written, so their grants remain session-only. A genuinely projectless session retains server role persistence. After a grant, effective role and allowlist resolution must reuse the session's `projectId` and `cwd`; otherwise the persisted policy and regenerated callable surface could diverge.

Preserve the session-allowlist distinction through every lifecycle transformation: `undefined` means unrestricted by an explicit session allowlist and remains unrestricted or role-derived, while `[]` means intentionally no tools. An explicit persisted `allowedTools: []` remains no-tools across restart and replacement unless actual granted tool names are merged into it; it must never be treated as missing and widened to the role-derived surface. Pi extension loading is separate from permission: even when an extension must load for runtime compatibility, the generated guard must enforce the empty or policy-restricted callable surface. Policy resolution details are in [Tool access policies](#tool-access-policies).
...
The ownership roots are every current live persisted row with `teamGoalId === goal.id`, plus valid current lead/agent references from the in-memory or persisted team entry. Exact `teamGoalId` ownership wins regardless of ancestry. Team references may supplement an unstamped row, but a different non-empty `teamGoalId` is a conservative conflict and the foreign row is not archived. `goalId` by itself never selects a standalone session. From valid roots, reconciliation follows only the established recursive child relationships: `delegateOf`, or a non-empty `childKind` with matching `parentSessionId`. Full- and bare-lifecycle children created from a trusted TeamStore-derived owner persist that ownership as `teamGoalId` in their initial row, preventing optional team state from remaining their only durable authority.

The runtime boundary is ordered as follows:

1. The goal archive is durably acknowledged. Concurrent strict archive callers for the same goal share that publication outcome, so a rejected write cannot appear successful to another caller and bypass reconciliation. A per-goal admission queue then excludes starts, role spawns, and team-owned child publication that arrive after terminal intent; previously admitted work completes before reconciliation's final closure scan.
2. Current ownership references are captured, then runtime team subscriptions, timers, reverse indices, and active in-memory membership are detached.
3. If authoritative ownership exists on a legacy goal whose `team` marker is false or absent, reconciliation strictly publishes sticky `team = true` before changing session or TeamStore evidence. This preserves local and remote branches and worktrees on all later archive replays. If publication fails, verification is cancelled and selected runtimes are quiesced, but no persisted session is archived and no TeamStore row is removed; the exact selected ids are suppressed while current `teamGoalId` rows and team references remain retry authority.
4. After marker acknowledgement, verification work is cancelled best-effort. Each current live or store-only selected row is stopped if possible and soft-archived through the canonical session cascade. Only an explicit successful durable return from that archive operation counts as acknowledgement; a mutable in-memory `archived` projection never does.
5. Failed ids enter a bounded per-goal in-process retry set, including exact-`teamGoalId` rows that have no TeamStore entry, and become the exact dispatch-suppression set. The persisted team entry is removed only after every selected id has an acknowledged archive.
6. A failed team-entry removal leaves its on-disk row as passive retry evidence. Runtime deactivation is not rolled back, and event resubscription defensively drops archived-goal entries.

The archive call is replay-safe even when the goal is already archived. Same-process replays retain failed exact ids even if a rejected persistence operation changed the mutable store projection; after restart, disk-live `teamGoalId` rows and retained team references reconstruct the retry. This preserves the existing merge contract: a completed child merge remains successful after an ordinary process-stop or persistence failure, while durable goal intent makes incomplete cleanup reconstructable and retryable.
...
Repairs are persisted before `restoreOneSession()` can dispatch an agent process. Legacy ownership-marker promotion is also acknowledged before session archival begins. If marker or session publication still fails, the exact selected rows remain suppressed for the boot; an archived goal wins over stale `wasStreaming`, queued prompts, or team metadata. Marker failure leaves persisted session and TeamStore evidence intact so the next replay publishes `team = true` first. Unrelated sessions, including a `goalId`-only standalone control, stay on the unchanged eager restore path. A retained archived-goal team entry cannot receive event subscriptions.

The repair is idempotent. A partial first pass is retried from the bounded in-process failed-id set or reconstructed after restart from still-live `teamGoalId` rows and retained team entries. After successful marker/session publications and team removal, a clean second boot discovers no candidate, emits no repair summary, and performs no reconciliation write. See [Startup performance audit](startup-performance-audit.md) for the restart fixture evidence.

##### Evidence preservation

Reconciliation uses the session archive path with evidence preservation enabled. It archives rows rather than deleting or purging them, and it does not remove local/sandbox worktrees or eagerly delete remote branches. Sticky promotion of a legacy goal's `team` marker preserves those artifacts even after session and TeamStore ownership rows disappear. Transcripts, proposal drafts, cost records, queued-prompt and model metadata, branch/worktree recovery coordinates, and archived sidebar hierarchy remain readable under their existing retention rules. The ownership guard is independent of transcript-forensics recovery: one repairs current runtime lifecycle; the other diagnoses missing metadata.
...
Regression coverage lives in `tests/unit/core/team-manager-ghost-workers.unit.test.ts`, alongside the existing reviewer-resume coverage in `tests/unit/core/team-manager-reviewer-resume.unit.test.ts`.
...
Key files: `src/server/agent/team-manager.ts`, `src/server/agent/team-store.ts`. Regression test: `tests/unit/core/team-manager-reviewer-resume.unit.test.ts`.
...
Key files: `src/server/agent/session-manager.ts` (`waitForStreaming`), `src/server/agent/verification-harness.ts`. Tests: `tests/unit/core/verification-reminder-race.unit.test.ts` (unit), `tests/e2e/api/gate-verification-resume.api-e2e.spec.ts` (API E2E that drives a full restart cycle).
...
The full restart and cleanup contract is in [Exact process ownership for command verification](verification-restart.md). Focused regression coverage includes `tests/unit/core/spawn-tree-process-cleanup.unit.test.ts`, `tests/unit/core/verification-harness-timeout.unit.test.ts`, and `tests/e2e/node/spawn-tree-shutdown-survival.node-e2e.test.ts`.
...
Regression coverage is split by contract: `tests/unit/core/play-finish-sound.unit.test.ts` pins precedence, fallback, raw parsing, and concurrent cache/write behavior; the `tests/dom/project-audio-*.dom.test.ts` suites pin Settings behavior and foreground/background source routing; `tests/dom/bell-toggle.dom.test.ts` pins the global-only Bell; `tests/integration/gateway/project-config-api.gateway.test.ts` pins persistence and removal across store reloads; and `tests/browser/journeys/project-settings.journey.spec.ts` covers the end-to-end user flow, reload, source-session audio, and unaffected badges.
...
Fork and Continue-Archived both clone transcript history and hand that clone to the agent with `switch_session`. A fork request with a durable Pi `entryId` materializes only the source's active ancestors strictly before that prompt; the selected and later entries are not copied. Any path values copied from the source runtime must be treated as provenance, not as the fork/continue runtime. Their handlers therefore pass old cwd candidates as `preExistingAgentSessionOldCwds` so `session-setup.ts` can rebase only top-level runtime cwd metadata before `switch_session`. User and assistant message content is not inspected or rewritten, so ordinary mentions of old paths remain byte-identical. Transcript filesystem realm is also provenance: a canonical sandbox transcript remains container-side for whole and cut forks, while a legacy host-absolute sandbox transcript stays on the compatibility host path. Forking does not silently translate an untrusted transcript path between realms.

#### Independent staff forks

The fork handler treats a source as staff-backed only when its persisted `staffId` resolves in the same project and the staff record's `currentSessionId` equals the source session. It allocates destination session and staff IDs before launch, then strictly stages the complete destination snapshot in `staff.json` with an internal `forkPublication: { version: 1, sessionId }` marker. Ordinary store, manager, REST, orphan, search, and ownership reads hide marked rows. The route launches with the new ID in both `session.staffId` and `BOBBIT_STAFF_ID`, flushes that project’s session store, and makes the staff row visible only when the marker, `currentSessionId`, session ID, `session.staffId`, and project all form the exact non-archived destination join. The strict commit derives owned-worktree coordinates from that session and atomically replaces the row without the marker before indexing or lifecycle notification. Name and cwd are never association keys, so colliding `Fork: <source>` names remain harmless; an unmatched or stale source `staffId` follows the unchanged plain-session fork path. Restore and process replacement rebuild authorization from the persisted destination session field.

The new staff row is a snapshot rather than a second reference to the source object. It copies description, prompt, role, accessory, memory, context policy, state, sandbox preference, project/configured cwd, and trigger values. Triggers are deep-cloned with fresh IDs while retaining snapshot cursor values, which prevents both replaying old trigger work and sharing future edits or firings. Inbox storage is deliberately not cloned: the new staff ID starts with an empty queue and gives inbox tools an independent authorization target. The source row, permanent-session pointer, triggers, inbox, session, and resource ownership never change.

A history fork with `newWorktree:false`, and any staff fork with that setting, is marked `borrowsWorktree` and carries no teardown coordinates. The destination stays writable in the source's exact cwd, while the persisted flattened `borrowedWorktreeOwnerSessionId` makes restore and teardown independent of staff names or lookup. Staff borrower creation and owner teardown serialize through a realm-neutral FIFO; owner deletion/archive fails before mutation with `409 SHARED_WORKTREE_IN_USE` on host or `409 SHARED_SANDBOX_WORKTREE_IN_USE` in a sandbox while a borrower remains. Deleting the borrower first releases only its logical record, after which owner cleanup can run once. Fresh mode records disjoint destination branch/worktree coordinates and uses the established owned-worktree lifecycle.

Staff publication holds the owner lifecycle boundary through hidden staff staging, durable session flush, and strict marker removal. Route compensation cleans the destination session and its transcript, sidecars, owned worktree, and other copied state before deleting only the still-marked destination candidate and its empty inbox/search residue; it never mutates the source or a borrowed worktree. Notifications are failure-isolated after commit so a client disconnect cannot roll back a valid pair.

At boot, `reconcile-staff-fork-publications` runs before session restoration. A marked candidate with its exact durable, non-archived destination session commits through the same join validation; one with no such session is removed as unpublished. A malformed or mismatched join fails closed rather than binding by name or path. Thus interruption before the session commit leaves only hidden destination staging to discard, while interruption after it completes the intended publication. Project deletion still terminates borrowers before owners, then refuses project-context removal with typed `409 PROJECT_SESSIONS_STILL_ACTIVE` if a concurrent launch or replacement leaves survivors. See [Fork session endpoint](rest-api.md#fork-session-endpoint) for cursor validation, clone filtering, response semantics, and retry behavior.
...
Forked staff use the session-fork worktree choice rather than provisioning a second blank staff session. `newWorktree:true` gives the destination record the worktree coordinates already persisted on its fork session. `newWorktree:false` records the destination session as a borrower and deliberately omits ownership coordinates from the destination staff record. This keeps logical identity, inbox, triggers, and cleanup authority independent even when source and destination share one cwd.
...
**Staff are not merged into Sessions.** Created staff agents live exclusively in the Staff sub-section. The staff-creation **assistant session** (`assistantType: "staff"`) is a transient normal session and shows up in the project's Sessions list while open — only the persisted staff record that results from accepting `propose_staff` moves into Staff. A permanent staff-session fork stages its destination record while hidden and publishes it only with the exact destination session join, so source and destination render as separate Staff rows and the destination never remains in ordinary Sessions. Sidebar classification uses the ID-based `state.staffList[*].currentSessionId` exclusion set for permanent staff-agent sessions; fork-name collisions do not affect it. Do not filter regular sessions by `assistantType === "staff"`: that value belongs to the staff-creation assistant, which must stay in regular Sessions.
...
**Lifecycle push refresh.** Staff REST lifecycle routes broadcast `staff_changed` to all authenticated clients so changes made outside the current UI flow still update open sidebars. `POST /api/staff` and a successful permanent staff-session fork send `reason: "created"` with the new `staffId`, `projectId`, and permanent `sessionId`; the fork also publishes the normal `sessionForked` lifecycle fact. `PUT /api/staff/:id` sends `reason: "updated"`; `PATCH /api/staff/:id` sends `reason: "reassigned"` with the target `projectId`, `previousProjectId`, and the old `sessionId`; `DELETE /api/staff/:id` sends `reason: "deleted"` with the removed session id when present. Bobbit staff tools go through these REST routes, so they produce the same invalidation event. See [websocket-protocol.md — Server → Client](websocket-protocol.md#server--client) for the wire shape.

The client treats `staff_changed` as a combined staff-and-session invalidation. The push handler reloads `GET /api/staff` and `GET /api/staff/orphaned` before `refreshSessions()`, so `getSidebarData()` sees the latest `currentSessionId` set before classifying session rows. A cold session deep-link or reload likewise awaits authoritative `GET /api/staff` hydration before connecting to the session and rendering its sidebar; the renderer’s fire-and-forget fallback is not used as the classification boundary. This prevents a permanent session from appearing under Sessions, or an expanded Staff section from being toggled closed, before its owner row arrives. The same refresh updates orphan banners after reassignment or deletion.
...
**Regression coverage.** `tests/integration/gateway/staff.gateway.test.ts` verifies create/update/reassign/delete `staff_changed` metadata and companion `session_created` / `session_removed` events. `tests/dom/sidebar-staff-rendering.dom.test.ts` verifies push-triggered invalidation order, permanent staff-session exclusion, staff-assistant inclusion, and staff cache-key invalidation. `tests/browser/journeys/staff-lifecycle-no-refresh.journey.spec.ts` keeps the app open while API-created/API-deleted staff update desktop and mobile sidebars without a page reload.
...
Goal drafts have a stronger contract than syntax-valid proposal files. The goal seed route first stamps the authenticated default project when omitted and performs only permitted host-derived enrichment, such as an eligible team lead's implicit parent. It then runs the canonical side-effect-free goal-candidate validator **before serialization or any write**. The parsed-form pre-commit callback repeats that contract for seed, validated frontmatter edit, worktree-mode change, and restore. Ordinary creation, both proposal acceptance modes, direct child creation, and verification-harness child creation consume the same validator. The shared contract normalizes an omitted, `null`, or empty goal spec to the valid empty string, but rejects a blank title, any other non-string or oversized spec, and malformed proposal frontmatter. Non-goal proposal seed behavior is unchanged.
...
- `tests/unit/core/proposal-files.unit.test.ts` - unit: write/read/edit/parse/delete round-trip, atomic-rollback, path-traversal rejection.
- `tests/unit/core/proposal-registry.unit.test.ts` - unit: per-type `mergeFields` and validators.
- `tests/unit/core/proposal-helpers.unit.test.ts` - unit: unified draft + dismissal.
- `tests/integration/gateway/proposal-edit-api.gateway.test.ts` - gateway integration: edit-before-propose, restart survival, malformed-edit rollback (SHA-256 byte-equal pre/post).
- `tests/browser/journeys/ui/proposal-edit-flow.journey.spec.ts` - browser journey: project propose → edit → accept happy path.
- `tests/browser/fixtures/proposal-review-fixture.fixture.spec.ts` - parametrised proposal UX parity: dismissal stickiness, "Open proposal" reopen, first-emit auto-select, streaming shallow-merge, restart survival.
- `tests/browser/journeys/ui/proposal-open-all-types.journey.spec.ts` - browser journey proving every supported proposal type opens, rehydrates, dismisses, and exposes its tab from a normal session.
...
Locked by `tests/dom/spurious-idle-unread.dom.test.ts`.
...
| `tests/unit/core/session-restore-last-activity.unit.test.ts` | Restore quarantine, rejected and raced dispatches, monotonic timestamps, and mark-read durability |
| `tests/unit/core/message-author-dispatch.unit.test.ts` | Exact echo attribution, bounded digest fences, overflow, and terminal settlement |
| `tests/unit/core/session-manager-direct-prompt-lifecycle.unit.test.ts` | Direct/queued/steered acceptance, cancellation, and recovery ownership |
| `tests/browser/journeys/session-activity-restart.journey.spec.ts` | Navigation mark-read, graceful restart durability, exact timestamps/indicators, and genuine new activity |
...
Regression coverage lives in `tests/e2e/api/mcp-tool-permission.api-e2e.spec.ts`: the refresh scenario pins `never` → `ask` registration plus the permission-card broadcast, and the role-deny scenario pins role-level `mcp__mock: never` precedence over group `allow`.
...
Unknown YAML names still warn once per process and scope with `reason=unknown-yaml-tool`, preserving typo visibility without repeating on every session spawn. `tests/unit/core/tool-activation-mcp-warn.unit.test.ts` pins both behaviours.
...
| `tests/unit/core/rpc-bridge-spawn-args.unit.test.ts` | Asserts separate `--provider` / `--model` / `--thinking` injection, first-slash parsing, and extended thinking levels |
| `tests/unit/core/review-model-override.unit.test.ts` | Covers the `skipSetModel` read-back contract |
...
| `tests/unit/core/openrouter-key-bridge-repro.unit.test.ts` | OpenRouter direct/restored spawn env, redaction, idle transition, retry recovery. |
| `tests/unit/core/spawn-env.unit.test.ts` | Bridge env merge invariants and session-secret precedence. |
| `tests/dom/remote-agent-outbox.dom.test.ts` | Client-side provider-auth state and secret redaction. |
...
| `tests/unit/core/preview-*.unit.test.ts`, `tests/integration/gateway/preview-*.gateway.test.ts`, and the preview fixtures/journeys under `tests/browser/` | Unit, gateway integration, and browser coverage |
...
- `tests/unit/core/event-buffer.unit.test.ts` covers count/byte head eviction, retained-byte accounting, monotonic allocation, oversized events, `pushFrame()` holes, and the `canResumeFrom()`/`since()` boundary.
- `tests/dom/remote-agent-seq-dedup.dom.test.ts`, `tests/dom/remote-agent-seq-overflow.dom.test.ts`, and `tests/dom/remote-agent-sequence-hole.dom.test.ts` cover duplicate suppression, ordering, resume, and bounded snapshot fallback.
- `tests/dom/remote-agent-outbox.dom.test.ts` covers IndexedDB admission, FIFO resend, server-ownership transfer, monotonic projections, stale-tab Retry/Dismiss races, and bounded local failure.
...
- `tests/browser/fixtures/verification-dedup.fixture.spec.ts` - Playwright file:// fixture that dispatches the same `gate-verification-event` 6× and asserts a single rendered occurrence in both `<verification-output-modal>` and `<gate-verification-live>`. This pins the multi-layer guarantee end-to-end on the listener side.
...
| `tests/unit/core/bg-process-manager.unit.test.ts` | Abort before exit, abort after exit, and abort after timeout |
| `tests/integration/gateway/bg-wait-steer-abort.gateway.test.ts` | Long-running background process plus dispatched steer; asserts fast wait interruption and process continuity |
...
User-facing behavior and lifecycle for the segmented transcript navigator, searchable history, and bottom controls lives in [chat-scroll-controls.md](chat-scroll-controls.md). This section covers the scroll-lock mechanism they share.
...
| `_animation` | Shared spring rAF state for explicit transcript jumps. Jump-to-bottom re-reads its moving tail target; prompt and history jumps use a fixed measured target. Cancelled synchronously by every user-intent listener so a wheel-up during the spring releases immediately. |
...
6. **Springs are reserved for explicit navigation.** `_scrollToBottomNow({ animate: true })` runs the shared rAF spring while re-reading the tail each tick so transcript growth moves the goalpost. Prompt, history, and unanswered-question jumps use the same spring with a fixed target measured from the live row after any targeted deferred resolution. Automatic follow-tail re-pins use the synchronous `animate: false` path.
7. **Navigation visibility is geometry-derived.** Each recompute classifies rendered user prompts as above, intersecting, or below the viewport. A prompt above enables the top **Previous prompt** segment; a prompt below selects the split **Next prompt | Bottom** control; otherwise the single bottom control uses the half-viewport distance threshold. An unresolved ask or open history dialog can keep the segmented shell visible without enabling **Previous prompt**. Scroll-intent flags control follow-tail but do not control these geometry classifications.
...
When modifying proposal-panel scroll behaviour: route through `reconcileFollowTail` rather than touching `scrollTop` or `setSelectionRange` directly; do not introduce timer-based intent heuristics; do not widen the 5px tail. See `src/app/follow-tail.ts` and the panel render functions in `src/app/render.ts`. Behavioural twin test: `tests/browser/fixtures/follow-tail.fixture.spec.ts`.
...
When extending transcript handling: every new transcript mutation goes through a new action in the reducer - do **not** push directly into `state.messages` from `RemoteAgent`. Compute `_order` from `seq` (live), `SNAPSHOT_ORDER_FLOOR + i` (snapshot), `highestSeq + 0.5` (synthetic), or the legacy optimistic sentinel. Reliable user occurrences reconcile by `deliveryIntentId`; text fallback is reserved for legacy optimistic rows and compaction markers. Pinned by `tests/unit/core/message-reducer.unit.test.ts` (proposal burst, `ask_user_choices` envelope routing, and the stranded-optimistic suite R1–R4 plus the corrected pending case (5)), the wiring spec `tests/dom/remote-agent-settle.dom.test.ts` (asserts both the `error` and `agent_end` handlers settle the row out of the sentinel while keeping it visible — mechanism-agnostic, so it pins observable behaviour not an action name), and the ST-DEDUP-02 / ST-DEDUP-03 / ST-DEDUP-04 stories in `tests/browser/journeys/ui/stories-streaming.journey.spec.ts`. The settle tests are RED before the production fix (the reducer returns `undefined` for the unknown action / the handlers never re-stamp, so the row stays at the sentinel) and GREEN after — suitable as the reverted-fix proof for the PR description. Full design: [`docs/design/unified-message-ordering-reducer.md`](design/unified-message-ordering-reducer.md).
...
Regression tests: `tests/unit/core/dual-render-noid-message.unit.test.ts` (id=undefined/null/numeric/empty-string cases), `tests/unit/core/message-reducer.unit.test.ts`, `tests/browser/journeys/ui/bg-wait-no-dup.journey.spec.ts`.
...
Regression coverage lives in `tests/dom/command-history-edits.dom.test.ts`; it covers recalled-entry edits, untouched neighbors, live-draft round trips, reset after send, and storage immutability.
...
`tests/dom/message-editor-arrows.dom.test.ts` documents the history state machine and row arithmetic, but happy-dom has no layout engine and its row model cannot reproduce trailing-newline collapse, font-dependent wrapping, or caret affinity. The authoritative coverage is `tests/browser/fixtures/message-editor-arrows-real.fixture.spec.ts`, which bundles the real component and drives Chromium with trusted key presses through the production send path.
...
| `tests/dom/queue-dispatch.dom.test.ts` | Unit coverage: happy-path unstick, cap parking, success resets counter, explicit Retry bypasses cap, steer path, queue drain, auto-retry timer cancellation |
| `tests/integration/gateway/stuck-session-recovery.gateway.test.ts` | Gateway integration: mock-agent error turn → new prompt dispatches without UI Retry |
