...
- **Resolution chain** — workflows are resolved INSIDE `GoalManager.createGoal` (`src/server/agent/goal-manager.ts`) at goal-creation time so the snapshotted `goal.workflow` is the frozen output. 5-tier cascade: (1) goal's own `inlineWorkflow`; (2) walk up `parentGoalId` chain via `_findAncestorInlineWorkflow` — first ancestor with an `inlineWorkflow` wins, child mirrors the inherited override onto its own record so deeper descendants walk depth-1 (avoids O(depth) climbs and survives ancestor archival); (3) caller's pre-resolved `resolvedWorkflow` (project config cascade); (4) `workflowId` lookup in the workflow store; (5) fallback to `"general"`. Roles use the parallel `role-resolution.ts` module which still exists separately. The historical `workflow-resolution.ts` module was deleted at `ca6b2ac5` after the cascade was inlined here — zero production callers, all workflow lookups go through `goalStore.get(...).workflow` or `cascade.resolveWorkflows(projectId)`. Tests: `tests/goal-manager-nesting.test.ts` "createGoal — inline-workflow ancestor-walk inheritance" suite.
