...
- **`_settleWindowActive` / `_settleWindowDeadline` / `_lastSettleScrollHeight` / `_settleQuietTickCount` (3 s post-navigate settle window).** A timer-bounded loop that re-pinned on every RO tick for up to 3 s after session navigate. Exited on "two consecutive quiet ticks" — which fired *before* lazy markdown highlights and image decodes, dropping case (3). Replaced by the capture-phase `load` listener plus the always-on RO `delta > 0` re-pin: termination is now "the content stopped growing", not "the deadline expired".
...
**Behavioural tests.** `tests/agent-interface-scroll.spec.ts` (the canonical `delta === 0` vibration regression), `tests/agent-interface-scroll-hardening.spec.ts` (sub-pixel echo absorbing, ring-buffer multi-write race, geometry-doesn't-flip-flag), `tests/scroll-anchor-shrink.spec.ts` (shrink/grow while scrolled up), `tests/collapse-scroll-bugs.spec.ts` (post-collapse clamp), `tests/mobile-scroll-keyboard.spec.ts`, `tests/e2e/ui/jump-to-bottom.spec.ts` (button visibility threshold + click), and `tests/e2e/ui/tail-chat-*.spec.ts` (the five reliability scenarios: tool-card insert, tool-card expand, rapid stream, session-navigate, user-scroll-up release).
