...
Staff agents are project-scoped permanent sessions: each staff record carries a `projectId`, lives in that project's `staff.json`, and owns a long-lived worktree on a `staff-<name>-<id>` branch. Because they ARE sessions (just with a `staffId`), they render inline in their project's regular **Sessions** list — same active/unread/last-activity treatment as any other session, with a `Bot` accessory and a hover-action pencil that opens `#/staff/<id>`. The collapsed (icon-only) sidebar buckets them under the same project group as that project's goals and ungrouped sessions; there is no global staff tail list.

The earlier dedicated **Staff** subsection per project is gone — it made staff feel like a separate top-level concept when they are really just sessions that happen to outlive a single conversation. The `renderStaffSidebarSection` helper remains in the codebase so the `#/staff` management page (canonical place to edit staff config, reachable from the project header's **Manage staff** button) keeps working, but it is no longer rendered in the per-project sidebar bucket.

The project header carries two staff entry points alongside **New goal** / **New session**: a **Manage staff** (`List` icon) shortcut to `#/staff`, and a **New staff** (`Plus` icon) button. The latter calls `createStaffAssistantSession({ projectId, cwd })` so the creation assistant always lands in the right project context — no second project picker, no system-project fallback, no `propose_staff(cwd)` re-link dance after the fact.
...
**Collapse state is per-project**: The Sessions section collapse toggle is stored per-project, not globally. Collapsing Sessions in Project A does not affect Project B. State is persisted as a collapsed-project-ID set in localStorage (`bobbit-collapsed-ungrouped`). Default state is expanded for all projects. Access via `isUngroupedExpanded(projectId)` / `setUngroupedExpanded(projectId, value)` in `state.ts`. (A legacy `bobbit-collapsed-staff` key and `isStaffExpanded` / `setStaffSectionExpanded` accessors remain for the `#/staff` management page; they no longer affect the per-project sidebar bucket.)

**Per-project Archived subsections**: Each project group ends with its own collapsible Archived subsection (rendered by `renderProjectArchivedSection` in `src/app/render-helpers.ts`, shared between desktop `renderSidebar` (`src/app/sidebar.ts`) and mobile `renderMobileLanding` (`src/app/render.ts`) so both breakpoints render identically). Desktop and mobile both use the `bucketArchivedByProject` helper in `render-helpers.ts`. The global Archived block that used to sit at the bottom of the sidebar is gone.

Within each project group, active and archived goals are separated by a muted divider via the canonical `bucketActiveArchived` helper (`src/app/render-helpers.ts`). When multiple team-leads are visible in the same expanded subtree, each team-lead's divider carries the lead's name ("ARCHIVED · Bugs Bunny", "ARCHIVED · Otis") so stacked dividers are visually unambiguous. See [docs/design/archive-rendering-unification.md](design/archive-rendering-unification.md).
...
- **Forest exclusion (claim → exclude)** — `forestInput` in `src/app/sidebar.ts::renderProjectContent` AND the mobile `forestGoalsMobile` filter in `src/app/render.ts::renderMobileLanding` both exclude goals whose ids are in the deterministic `computeSpawnedClaim` set (`src/app/sidebar-spawned-children.ts`). Path A claims, Path B excludes — no heuristic. The helper mirrors `renderTeamGroup`'s lookup: for each parent goal P, find its team-lead session(s) S in `state.gatewaySessions` (ANY status — matches `goalSessions.find(role==="team-lead")`), plus archived team-leads of P from `state.archivedSessions` when `state.showArchived` is on; for each (P, S) tuple, run `selectSpawnedChildren(goals, P.id, S.id, showArchived, S.id)` and union the result ids. Status-agnostic by design — terminated-but-still-listed team-leads still claim their children, mirroring how `renderTeamGroup` actually renders them. The previous `teamLeadIdsAttributable` heuristic (live filter `status !== "terminated"` + ad-hoc `g.spawnedBySessionId` check) leaked terminated leads, unstamped children (parent-lead fallback), and the entire mobile path. See `tests/sidebar-no-double-render.test.ts` and AGENTS.md "Goal rendered twice in sidebar (live, mid-flight)".

**Boot-time backfill for legacy records.** `GoalManager.backfillSpawnedBySessionId(teamStore, sessionStore?)` (in `src/server/agent/goal-manager.ts`) walks every sub-goal (live or archived) missing the field and stamps it. Lookup precedence: (1) `teamStore.get(parentGoalId)?.teamLeadSessionId`; (2) `SessionStore` fallback — sessions with `role === "team-lead" && (teamGoalId === parentGoalId || goalId === parentGoalId)`; if exactly one such session exists (live OR archived), stamp it. Multi-team-lead parents stay ambiguous and are skipped. Idempotent — only writes when the field is absent. Per-goal try/catch keeps boot resilient (endless-restart guard — a single bad row can't crash-loop the harness). Wired in `src/server/server.ts` immediately after `restoreTeams`, alongside `backfillCompleteState`. Each stamp logs `[goal-manager] Backfilled spawnedBySessionId=<sid> for legacy sub-goal <gid>`.

**Header source for new spawns.** `defaults/tools/children/extension.ts` sets `X-Bobbit-Spawning-Session: <sessionId>` on every children-tools API call. `POST /api/goals/:id/spawn-child` reads the header (falls back to `body.spawnedBySessionId`) — header wins so a forwarded request can't lie about the spawning agent — and stamps it in the same atomic `updateGoal` that already stamps `spawnedFromPlanId` immediately after `createGoal` (no extra await between createGoal and the stamp).
