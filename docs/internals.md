...
`AgentInterface` (`src/ui/components/AgentInterface.ts`) keeps the message viewport pinned to the bottom only while `_stickToBottom` is true. The flag is governed by **four rules** — no timers, no debounces, no settle windows:

1. **Single intent flag, single mutation discipline.** `_stickToBottom` flips **FALSE** only via observed user gestures — the `wheel`, `touchstart`, and `keydown` (PageUp/Down, Arrow Up/Down, Home, End) listeners on the scroll container. It flips **TRUE** only at programmatic restore points: `_pinIfSticking()`, `_handleJumpToBottomClick`, `setupSessionSubscription`, `sendMessage`. Geometry NEVER mutates the flag. `_handleScroll`'s only responsibilities are (a) consuming programmatic-scroll echoes and (b) recomputing jump-button visibility from geometry.
2. **Single re-pin path — `_pinIfSticking()`.** When `_stickToBottom` is true and the viewport is not already within 1 px of the bottom, the helper pushes a `(scrollTop, scrollHeight)` echo entry into the ring buffer and writes `scrollTop = scrollHeight` (the browser clamps). It is the only function that mutates `scrollTop` for stick-to-bottom purposes. Call sites: ResizeObserver callback when `delta > 0`, `_updateAndPin()` (used by every transcript-mutating session event — `message_update`, `tool_execution_update`, `state_update`, compaction/turn/agent events), `setupSessionSubscription` post-`updateComplete`, `sendMessage`, jump-to-bottom click, and the capture-phase `load` listener that catches `<img>`/`<iframe>` decode reflows.
3. **Programmatic-scroll echo ring buffer.** Multiple programmatic writes can be in flight before any browser-emitted echo scroll event fires. A single-pair latch dropped older echoes on the second write, causing them to fall through. The new model uses a small ring buffer (`_programmaticEchoes`, capacity 4); `_handleScroll` consumes the **oldest** matching entry within `< 1 px` tolerance (HiDPI device-pixel rounding) and splices it. Anything else falls through to geometry, which only updates jump-button visibility — it never mutates `_stickToBottom`.
4. **Auto-scroll only on positive delta.** The `ResizeObserver` callback computes `delta = newScrollHeight - _lastScrollHeight`. `delta < 0` (shrink) runs the post-collapse clamp from `tests/collapse-scroll-bugs.spec.ts` and returns. `delta === 0` is a no-op — the original vibration regression came from clamping `scrollTop = scrollHeight` on every zero-delta RO fire. Only `delta > 0` calls `_pinIfSticking()`.

**Session-navigate flow.** `setupSessionSubscription` resets `_stickToBottom = true`, clears the echo ring, awaits `this.updateComplete`, calls `_pinIfSticking()`, and registers a one-shot capture-phase `load` listener on the scroll container. The listener catches lazy `<img>`/`<iframe>` decode reflows that grow `.max-w-5xl` after the initial pin. Subsequent async growth (markdown, syntax highlighting, hydrated tool-content) is caught by the existing ResizeObserver — every `delta > 0` tick re-pins. There is no time-bounded settle window: termination is event-driven, not deadline-driven.

**Jump-to-bottom button.** `_showJumpToBottom` is recomputed in `_handleScroll` purely from geometry (`scrollHeight - scrollTop - clientHeight > clientHeight * 0.5`). The geometry recompute does NOT mutate `_stickToBottom`. Click handler: `_stickToBottom = true`, hide the button, call `_pinIfSticking()`. The ring-buffer latch absorbs the resulting echoes; there is no post-click suppression timer.

**Deleted (do NOT re-introduce):** `_lastProgrammaticScrollTop`/`_lastProgrammaticScrollHeight` (single-pair latch — replaced by `_programmaticEchoes` ring), `_wasAtBottomAtLastUserScroll` (carry-over for `state_update` — unnecessary now that geometry doesn't flip the flag), `_settleWindowActive`/`_settleWindowDeadline`/`_lastSettleScrollHeight`/`_settleQuietTickCount` (3 s session-load settle window — replaced by event-driven RO + image-load delegate), `_suppressJumpUntilTs` (600 ms jump-button suppression — unnecessary; the ring-buffer latch absorbs the click echoes). Ten-percent-of-clientHeight stick-grace band on geometry: gone (no geometric flips at all). Triple-rAF chain in `setupSessionSubscription`: gone. The `requestAnimationFrame` re-assert inside `_scrollToBottom`: gone.

When modifying scroll behaviour: do not introduce timers (the only acceptable timing primitive is `requestAnimationFrame` and Lit's `updateComplete`); do not introduce parallel flags (the button reuses `_stickToBottom`); do not let geometry mutate `_stickToBottom`. If a new code path scrolls programmatically, route through `_pinIfSticking()` (or `_pushEcho` + write) so its echo is consumed by the ring buffer. Behavioural tests: `tests/agent-interface-scroll.spec.ts` (canonical vibration regression — `delta === 0` is a no-op), `tests/agent-interface-scroll-hardening.spec.ts` (echo absorbing, geometry-doesn't-flip-flag, multi-write race), `tests/scroll-anchor-shrink.spec.ts` (shrink/grow while scrolled-up), `tests/collapse-scroll-bugs.spec.ts` (post-collapse clamp), `tests/mobile-scroll-keyboard.spec.ts`, `tests/e2e/ui/jump-to-bottom.spec.ts` (button visibility + click), and `tests/e2e/ui/tail-chat-*.spec.ts` (the five tail-chat reliability scenarios).
