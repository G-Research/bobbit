...
4. **Archived-session cleanup**: If an archived session still has a host worktree, Settings → Maintenance → Archived Session Worktrees can remove that worktree and safe branch without purging the archive. The scan keeps archived metadata as provenance, skips rows protected by live sessions/goals/teams/staff, evaluates multi-repo `repoWorktrees` independently, and treats branch-only leftovers as already cleaned. See [maintenance.md](maintenance.md#archived-session-worktrees).
5. **Orphan detection**: Orphaned `session/*` worktrees (from ungraceful shutdowns where cleanup didn't run) are exposed in Settings → Maintenance and through the REST API (`GET /api/maintenance/orphaned-worktrees`, `POST /api/maintenance/cleanup-worktrees`). The same shared-path guard keeps live referenced paths out of the orphan list; unreferenced worktrees remain eligible for cleanup.
6. **Restore**: After a restart, existing session worktrees are reused - the server reconnects to the worktree on disk without recreating it.
