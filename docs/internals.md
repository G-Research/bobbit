...
**Prompt-level safety injection.** Because project-level role overrides (`.bobbit/config/roles/*.yaml`) fully shadow their default counterparts with no field-level merge, a custom role YAML that omits a global rule would silently suppress it. To prevent this, `session-setup.ts` injects a small set of global, role-specific rules directly into the composed prompt _after_ the role template is resolved — independently of whichever YAML won the cascade. The current injected rule is the **LSP tool-selection rule** (`## Tool selection — symbol queries`), applied to `coder`, `reviewer`, `code-reviewer`, `security-reviewer`, `architect`, and `spec-auditor` roles. The injection is deduplicated: if the resolved template already contains the section header, it is skipped. See [docs/lsp.md](lsp.md) for the rule text and rationale.
