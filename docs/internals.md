...
#### Cold-reviewer resume and durable terminal recovery

A revived reviewer can be cold while its model and extensions initialise, so restart recovery waits for readiness before sending its reminder with a longer prompt allowance. The reviewer/QA session is non-interactive, preventing the ordinary session boot nudge from racing the harness reminder.

Recovery records provenance structurally, never by parsing reviewer summaries or command output. A conclusively no-verdict recovered row is marked `restartInterrupted`; `shouldSuppressExplicitRestartInterrupt` leaves an aggregate pending only when every non-passed row has that marker. `_hasUnmarkedTerminalResumeFailure` protects the opposite case: an actual failed sibling remains a product failure even if another row was interrupted.

The active record is the recovery transaction. `reviewerCleanupPending` retains authority over the exact persisted reviewer session IDs until termination and unregistering settle. A determined pass/fail aggregate is stored as a terminal intent before reviewer, command-tree, Docker payload/transport, and sign-off cleanup; it publishes only afterwards. Work that is genuinely unfinished cleans its retained ownership and then resumes. Missing goal, gate, workflow, or signal context remains durable active work for a later retry rather than becoming a fabricated outcome.

After strict signal and current-gate writes, recovery retires its active record durably before emitting status or transcript events. It checks the current generation again after the writes, so stale recovery can preserve its historical audit without publishing an event for a replacement signal. See [Verification cancellation lifecycle](verification-cancellation.md) for the outcome contract and [Debugging: gate marked `failed` after gateway restart with a "Resume Error" step](debugging.md#gate-marked-failed-after-gateway-restart-with-a-resume-error-step) for diagnostics.
...
After lifecycle admission and before cache reuse, the handler synchronously calls `fenceStaleVerificationsForGates(goalId, [gateId], cause)`. The fence persists cancellation before detached exact cleanup begins. If it cannot persist, the route returns retryable `503 VERIFICATION_CANCELLATION_FENCE_FAILED` and admits neither a cached nor a replacement signal. On success the order is fence → cache decision → `beginVerification` → `recordSignal` → `gate_signal_received` → `gate_verification_started` → fire-and-forget `verifyGateSignal`; old exact cleanup may overlap the new generation.

Fresh and cached signal responses, persisted `GateSignalStep` rows, and historical `gate_signal` chat cards preserve `phase`, explicit `status`, and `skipped` so terminal skipped rows stay grouped and rendered correctly. Full design and the symbol-level map are in [docs/gate-signal-step-enumeration.md](gate-signal-step-enumeration.md); symptom→fix lookup in [debugging.md — Empty `verification.steps[]` after `gate_signal`](debugging.md#empty-verificationsteps-after-gate_signal). Pinned by `tests/gate-signal-step-enumeration.test.ts`, `tests/e2e/gate-signal-progress.spec.ts`, `tests/e2e/gate-signal-renderer.spec.ts`, and `tests/e2e/ui/verification-progress-indicator.spec.ts`.
