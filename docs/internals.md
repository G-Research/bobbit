...
**Directory creation**: When the typed Add Project path does not exist, the dialog offers **Create directory**. It calls `POST /api/create-directory`, which creates only the final path segment, returns the resolved path on success, and reports structured error codes for invalid paths, missing parents, file conflicts, permission failures, already-existing directories, and unexpected create failures. After success or recoverable `already_exists`, the dialog refreshes detection and preflight before continuing through the normal assistant/scaffolding flow.
