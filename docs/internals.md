...
**Pool branch namespace.** Pool entries pre-create worktrees under the `pool/_pool-<id>` branch prefix (was `session/_pool-*` pre-Phase 3). The namespace keeps pool entries out of the user's session branch list, but it is not ownership proof. Restart adoption requires Bobbit's exact v1 durable record in `state/worktree-pools.json`, current Git path/branch agreement for every member, and live-session exclusion; unrecorded pool-shaped branches remain diagnostic-only.
...
**Boot sweeper.** `worktree-sweeper.ts` scans `.git/worktrees/*` against persisted session/goal/staff records for diagnostics. Boot never repairs or removes a worktree discovered from Git metadata, branch naming, or root placement. Pool initialization may re-adopt only entries already authorized by its exact durable record and independently revalidated against current Git and live-session records; it never derives authority from discovery or shape.
...
**Graceful pool shutdown.** After new work is fenced and boot initialization settles, the gateway stops every live per-project pool with a 15-second bound, then flushes `state/worktree-pools.json`. Ready entries are retained and may be re-adopted after restart; successful claims have already left both the pool and its record. Tracked claim-failure cleanup participates in the stop barrier. Explicit project removal remains destructive: `removeWorktreePool()` drains the stopped pool locally, revokes its record, and never performs remote Git cleanup. Failures are logged without allowing another shutdown caller to start competing teardown.
...
**Worktree pool** (host-side, `worktree-pool.ts`): The pool pre-creates local-only worktrees so session and goal startup can claim them quickly. Pools are per project, including coordinated multi-repository sets whose members share one `pool/_pool-<id>` branch. Ready entries are recorded in server-global `state/worktree-pools.json`; claims remove the record before branch/directory mutation transfers ownership to the session or goal. At startup, an entry is reused only when the strict v1 project/repository record, current component identities, expected container-relative member paths, every repository's exact `git worktree list` path/branch, and persisted/runtime live-session exclusions all agree. Invalid records are revoked without touching their paths; branch/path shape alone is never authority. Graceful shutdown stops pools and flushes the record without draining. `DELETE /api/projects/:id` explicitly drains the project pool and forgets its record. The pool status API (`GET /api/worktree-pool`) retains its per-project and aggregate shapes.
