...
The fork handler treats a source as staff-backed only when its persisted `staffId` resolves in the same project and the staff record's `currentSessionId` equals the source session. It allocates destination session and staff IDs before launch, persists the destination ID as `session.staffId`, injects the same value as `BOBBIT_STAFF_ID`, then strictly publishes a new staff row whose `currentSessionId` points back to that session. Restore and process replacement rebuild the environment from the persisted session field. This two-way ID join is the durable authority; name and cwd are never association keys. Legacy backfill likewise prefers exact `currentSessionId` ownership and accepts title/path fallback only when exactly one record matches, so colliding `Fork: <source>` names remain harmless. An unmatched or stale source `staffId` follows the unchanged plain-session fork path.
...
Staff publication holds the owner lifecycle boundary through durable session flush and strict staff-store write. Transcript, sidecar, launch, session-persistence, or staff-persistence failure compensates only destination session/staff/transcript/sidecar/worktree artifacts; borrowed resources and every source artifact remain untouched. Notifications are failure-isolated after strict publication so a client disconnect cannot turn a valid persisted staff/session pair into an orphan. Project deletion still terminates borrowers before owners, then refuses project-context removal with typed `409 PROJECT_SESSIONS_STILL_ACTIVE` if a concurrent launch or replacement leaves survivors. See [Fork session endpoint](rest-api.md#fork-session-endpoint) for cursor validation, clone filtering, response semantics, and retry behavior.
