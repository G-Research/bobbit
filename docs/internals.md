...
Set `BOBBIT_TOOLS_MD=index` to drop the `(params)` list from every bullet (rendering `- name — summary` for all tools, not just the ones with no `params:` field) and add one pointer line under the `# Tools` heading noting that full schemas ship with the tool definitions. Default is unset = `"full"` = today's byte-identical output. Measured on this repo's own default builtin tool set (69 tools / 15 groups): full = 7,257 chars (~1,815 tokens) → index = 5,324 chars (~1,331 tokens), a 26.6% reduction. Group headers and detail-docs pointers are unchanged in both modes — they are not duplication (the pointer path exists nowhere else). The persisted `<sessionId>-prompt.json` breakdown records the active mode on the "Tools" section's `toolsMdMode` field so an A/B can compare token deltas per session, and the System Prompt inspector shows an "index" badge when active.
