...
**Per-component editors**: The project Settings tab renders one card per component with editable `commands`, **Command Environment**, and `config` key-value tables. Command Environment is a native plaintext `env` map positioned between Commands and Config; it uses the card's explicit dirty/Save model, not autosave, and affects the next command invocation without restart. It is not an opaque `config` entry or a secret store. All three tables persist through the same `PUT /api/projects/:id/config` `components` payload. There are no longer top-level `qa_*` fields on the Settings page - QA settings live exclusively under the relevant component's `config:` map (see [Multi-repo & components](#multi-repo--components)). See [Project command environments](project-command-environments.md).
...
|---|---|---|
| `view-tab-components` | `projectComponentsView` | One card per component (`component-card-${name}`): `repo`, `relative_path`, `worktree_setup_command`, commands, declared Command Environment values, plus a per-component `Config` key-value table (`component-config-${name}`) listing entries from `components[*].config` (e.g. `qa_start_command`, `qa_max_duration_minutes`). Data-only components (no `commands` map) are flagged. |
...
| `view-tab-diff` | `projectDiffView` | When a previous proposal exists in the same session, shows a unified diff of the changed project YAML. The `components` section includes declared `commands`, `env`, and `config` changes. |
...
components:                   # the only collection in project.yaml
  - name: myapp               # default component is named after the project
    ...
    env:                      # literal plaintext map for selected workflow command steps
      E2E_V2_PW_WORKERS: "4"
...
- `env` is a sibling native `{ name: string }` map for literal, non-secret workflow command settings. It is resolved only when the workflow command selects the component and is distinct from `config`; a command step may override it. It must not contain credentials. See [Project command environments](project-command-environments.md) for the exact validation and runtime boundary.
...
| `env` | no | Native literal plaintext `{key: string}` map for selected workflow command steps. Validated separately from `config`; never use for secrets. |
