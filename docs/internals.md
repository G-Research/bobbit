...
Key resolver: `SessionManager.getImageModelForSession(sessionId)` - returns `{ provider, id }` for the session if both fields are set and still registered in the provider-aware image registry. Otherwise it falls back through `pickDefaultImageModelPref(preferencesStore)`: saved `default.imageModel` only if authenticated, then the best authenticated image model, then `undefined`. This prevents disabled or unauthenticated direct-cloud image providers from being selected silently. In direct-cloud mode with no authenticated image provider, `POST /api/image-generation/generate` returns `409 { code: "cloud_auth_required", status }`; provider-side failures still surface as `500 { error: "<provider message>" }`.
...
3. Server endpoint (`src/server/server.ts::handleApiRoute` for `POST /api/image-generation/generate`):
   ...
   - Resolves the session-selected image model or authenticated system default with `getImageModelForSession(sessionId)` / `pickDefaultImageModelPref()`.
   - Canonicalises both the request `model` and the selected/default model through the same helper before comparing - prevents `OpenAI/GPT-Image-2` from being treated as a different model than `openai/gpt-image-2`.
   - **Override resolution.** A request `model` is honoured when (a) it canonicalises equal to the selected/default model, (b) there is no `sessionId`, **or** (c) `imageModelMentionedInText` finds the request model id in the user's most-recent prompt text. Otherwise the request `model` is ignored and the selected/default model is used. This last-prompt check is why an explicit override sometimes "works" and sometimes doesn't - it must be named in the user's text, not just sent in the API body.
   - Unknown or unauthenticated image-model prefs fail closed through the resolver or direct-cloud auth gate. There is no separate `400 { error: "unknown image model" }` response on this endpoint.
...
4. The provider helper makes the upstream HTTP call and returns `{ images, format }`. Any error thrown from a helper is caught by the endpoint and surfaced as `500 { error: err?.message || "Image generation failed" }`, except the no-authenticated-provider case in direct-cloud mode, which is translated to `409 cloud_auth_required`. Helpers throw arbitrary `new Error(...)` strings (missing credentials after an override, upstream HTTP failures, the Codex `n=1` clamp, the 25 MB remote-image cap, etc.) - there is no required prefix format, and the API surface never emits `502` or `503`.
