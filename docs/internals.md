...
- **`gates.json` / `tasks.json` / `team-state.json` / `inbox/<staffId>.json` / `project.yaml`** (`gate-store.ts`, `task-store.ts`, `team-store.ts`, `inbox-store.ts`, `project-config-store.ts`): the shared helper `src/server/agent/atomic-json.ts` — `atomicWriteJsonSync()` for the JSON stores, or the format-agnostic core `atomicWriteFileSync()` directly for YAML (tmp-write → fsync → rename → best-effort directory fsync), with 3-deep `.bak.N` rotation. On-disk shape/serialization is unchanged; only the write/recovery mechanics are hardened. Keep it this way: new durable stores must route through these helpers, never re-implement the tmp/rename/rotation dance per format.

Load-fallback order for the shared-helper stores (`loadJsonWithBackupFallback()` / the generic `loadWithBackupFallback()`): primary file → newest parseable `.bak.N` (logs a `Loaded from backup` warn) → start empty. Deliberate deletion (e.g. staff removal wiping `inbox/<staffId>.json`) must go through `removeJsonWithBackups()`, which purges the `.bak.N` files with the primary — otherwise the deleted state would resurrect from backup on the next load. Pinned by `tests/atomic-json.test.ts` and `tests/durable-store-atomic-write.test.ts` (session/bg stores: `tests/session-store-atomic-write.test.ts`).

**project.yaml is stricter (CON-02)**: because it is the highest-value *user-authored* file (workflows, components, pack activation/order), `ProjectConfigStore` never "starts empty" over a corrupt-but-present file. If neither the primary nor any `.bak.N` parses, the store enters a `loadFailed` state in which `save()` throws (`refusing to save`) instead of serializing empty/stale in-memory state over the corrupt file; the state clears as soon as a re-read finds a parseable primary (it re-reads per `getWithDefaults()` call, so a hand-fix needs no restart). An empty file is a deliberate reset (fresh, no backup fallback); a missing file with `.bak.N` history recovers from backup. Pinned by `tests/project-config-store-atomic-write.test.ts`. Symptom→fix lookup: [debugging.md — Corrupt or truncated durable store on restart](debugging.md#corrupt-or-truncated-durable-store-on-restart-gatesjson--tasksjson--team-statejson--inbox--projectyaml).
...
The verification system is split into two modules, plus a small policy store:
...
- **`verification-logic.ts`** - pure functions extracted for unit testability: `substituteVars` (template variable resolution), `matchExpectFailure` (expect:failure gate evaluation), `groupStepsByPhase`/`getSortedPhases` (phased execution ordering), `partitionOptionalSteps` (optional step filtering), `buildStepCache`/`canSkipAllSteps`/`resolveGateCacheMode` (step-cache reuse for re-signals — SHA-exact by default, content-keyed under policy/env opt-in; see below), `isParallelReviewsEnabled`/`computeEarlyReviewPhases`/`isRetryAfterCommandFailure` (VER-07 parallel reviews; see below), `mergeVerificationPolicyRaw`/`resolveVerificationPolicy` (VerificationPolicy V0 parsing/defaulting), `isTransientReviewError`/`isTransientQaError` (transient failure detection). These are tested in `tests/verification-logic.test.ts` and `tests/verification-policy.test.ts` without requiring a running server.
- **`verification-policy-store.ts`** - YAML-backed override store for `verification-policy.yaml`. `ConfigCascade.resolveVerificationPolicy(projectId)` merges builtin `defaults/verification-policy.yaml` -> server/Headquarters override -> project override, then runs one final validation/defaulting pass.

#### Parallel reviews (VER-07) & gate step cache

Two verification-throughput levers are resolved at signal time in `verifyGateSignal`. Env vars remain highest precedence; when unset, the cascade-resolved `VerificationPolicy` governs.

- **Parallel reviews are default-on.** `BOBBIT_PARALLEL_REVIEWS`, when present, is the highest-precedence override (`"0"` disables; any other value enables). When absent, `VerificationPolicy.parallelReviewsDefault` applies, and the builtin policy default is `true` - byte-identical to the previous default-on behavior. `computeEarlyReviewPhases` conservatively identifies the first contiguous block of review-only phases following the leading command phases (anything not matching the clean "commands, then reviews" shape stays serial) and the harness starts those reviewer sessions concurrently with the command track; their verdicts are only *kept* once the preceding command phases pass. A **retry-aware guard** (`isRetryAfterCommandFailure`) suppresses the early start when the same gate's most recent completed signal failed a non-review step. With the flag/policy off or the guard firing, phase ordering is byte-identical to the pre-VER-07 serial path.
- **Gate step cache modes.** `BOBBIT_GATE_CACHE`, when present, is the highest-precedence override; otherwise `VerificationPolicy.gateCacheDefault` applies, and the builtin policy default is `"sha"`. In `"sha"` mode, `buildStepCache`/`canSkipAllSteps` reuse a passed step result on re-signal only for a byte-identical commit SHA. `"content"` widens reuse: a step declaring `cacheInputGlobs` (a `VerifyStep` field) can be reused across *different* SHAs when none of its declared globs changed between the two commits; a step without `cacheInputGlobs` stays SHA-exact in either mode, and any env/policy value outside the allowed set resolves to `"sha"` — misconfiguration can only be more conservative, never silently widen reuse. Seeded default workflows ship `cacheInputGlobs` on their Build/Type check/Unit tests command steps. Full step-cache semantics and safety analysis: [docs/goals-workflows-tasks.md](goals-workflows-tasks.md).
...
**Goal-nesting prompt stanzas are a declarative table, not code branches.** The team-lead system prompt's nested-goals awareness copy (`## Goal nesting context (...)` and the `subgoal` vs `team_spawn` vs `task_create` decision table — see [docs/nested-goals.md](nested-goals.md)) is assembled by `buildNestingContextSection()` in `src/server/agent/goal-nesting-stanzas.ts` (re-exported from `system-prompt.ts` for callers/tests). Each stanza is a row — an `applies(ctx)` predicate plus a template string using the same `{{PLACEHOLDER}}` / `{if:NAME}…{endif:NAME}` syntax as role `promptTemplate`s (`role-prompt.ts`, `prompt-conditionals.ts`) — so a new stanza variant is a new table row, not a new `if`/`else` branch. `system-prompt.ts` itself only does conditional *wiring* (whether to call the function, not what it returns). Byte-parity of the assembled output across the full `(team, parent-presence, subGoalsEnabled, fallback-field)` input matrix is pinned by `tests/goal-nesting-stanzas-snapshot.test.ts` against `tests/fixtures/goal-nesting-stanzas.snapshot.json` — this is a product prompt surface, so an intentional copy change must regenerate that fixture explicitly rather than have the test edited to pass.
...
**Ralph-loop framing.** The `implementation` gate's `verify` list is the agent's loop body: failures circle back to the implementing agent, which fixes and re-signals until verification passes. The `description` field on `implementation` gates produced by the canonical helpers carries `RALPH_LOOP_DESCRIPTION` so the gate cards in both the proposal panel and the goal dashboard remind reviewers it's a loop, not a checkpoint. The `general`, `feature`, and per-component templates in the authoring guide include gap-analysis steps at design-time (in `design-doc`) AND post-implementation (`implementation` phase 2) to bracket the loop - design-time catches missing requirements before iteration burn, post-impl catches drift between design and code. `quick-fix` and `solo-fast` skip both; `solo-fast` additionally drops `e2e` and collapses the code-review/bug-hunt fan-out into a single consolidated `reviewer` review - see §6.5 of the authoring guide. Full authoring rules and worked examples live in [`defaults/workflow-authoring-guide.md`](../defaults/workflow-authoring-guide.md) §3.1 / §6.
...
**CLF-W1c (F5-read verify, corrects a prior claim):** until this pass, `team-manager.ts`'s `startTeam` and `spawnRole` did NOT actually rely on the fallback above for model/thinkingLevel — they separately read `storedRole.model`/`storedRole.thinkingLevel` (via `resolveRole()` in `resolve-role.ts`, which checks the goal's ephemeral `inlineRoles` snapshot first, then `this.config.roleStore` — a flat server+builtin store with **no project-cascade awareness**) and forwarded them unconditionally as explicit `opts.initialModel`/`opts.initialThinkingLevel`. Because an explicit `plan.initialModel`/`initialThinkingLevel` wins outright in `_resolveBridgeOptions` (see the "Resolution helpers" list above), this silently short-circuited session-setup's project-cascade-aware resolution whenever the role came from the flat `roleStore` layer — a project-level override of a team-lead/worker role's `model`/`thinkingLevel` never reached team spawns, even though `projectId` itself was correctly threaded (`createSession` derives it from `goal.projectId`). Builtin-tier values (the common case, since no project override existed) still worked by coincidence.
>
> Fixed by forwarding `initialModel`/`initialThinkingLevel` ONLY when the goal carries a matching ephemeral `inlineRoles` entry (`goal.inlineRoles?.[roleName]`) — the one case that is genuinely invisible to `configCascade` and therefore must be threaded explicitly. For every other case, only `roleName` is passed, letting session-setup's cascade-aware resolver (`ctx.resolveInitialModel`/`resolveInitialThinkingLevel`) own it — the same pattern `staff-manager.ts` already used. Verified end-to-end: `tests/team-manager.test.ts` ("should not shortcut role model/thinkingLevel…" red-before/green-after, plus "honours a goal-scoped inline role override…" proving the ephemeral-inline path still works), `tests/session-manager-role-tier-resolution.test.ts` (the shared `resolveInitialModel`/`resolveInitialThinkingLevel` resolvers ARE project-cascade-aware), and `tests/verification-harness-role-tier-resolution.test.ts` (the reviewer/QA/legacy-sub-session `resolveRoleForGoal` helper already correctly layers inline-role > cascade > flat-roleStore and needed no fix, only a pin).
...
The same decision-seam framework also carries **observe-only classifiers** for model-tier, gate-risk, and swarm-topology choices (`model-tier-classifier.ts`, `gate-risk-classifier.ts`, `swarm-topology-classifier.ts`). Model-tier and gate-risk are unconditionally registered production classifiers; swarm-topology is now W4.3's real classifier at `(goal-create, swarm-topology)` and selects `best-of-n` only when the caller already requested fan-out and a deterministic verifier exists. These record would-have-chosen labels for evidence-gathering and have no enforce mode — they never alter live model selection, workflow gating, or team topology. Evidence summaries are generated by `scripts/clf-evidence-report.mjs`, and the UI renders the new `decisionKind` labels/filters in the lazy-loaded transparency panel (`src/ui/lazy/transparency-panel.ts` -> `src/ui/components/TransparencyPanel.ts`; the message editor has the same lazy-loader split at `src/ui/lazy/message-editor.ts`).
...
Set `agents_md_budget: "<tokens>"` in `project.yaml` to cap the cascade for that project, or set `BOBBIT_AGENTSMD_BUDGET=<tokens>` (e.g. `BOBBIT_AGENTSMD_BUDGET=6000`) as the process-wide highest-precedence override. Resolution is env → project config → server/Headquarters config → built-in default. The built-in default is empty/OFF = today's uncapped, byte-identical behavior. When set:
...
**Key file:** `src/server/agent/system-prompt.ts` (`resolveAgentsMdBudgetTokens`, `createAgentsMdBudget`, `readAllAgentFiles`, `resolveMarkdownRefs`). Config key: `agents_md_budget`; env override: `BOBBIT_AGENTSMD_BUDGET`.
...
**`# Tools` markdown mode (`BOBBIT_TOOLS_MD`, F22).** The `(params)` name list in each bullet (e.g. `bash(command, timeout?)`) is authored in a completely separate YAML `params:`/`summary:` field, independent from the actual JSON tool schema the model receives alongside every request (`pi.registerTool({name, description, parameters})` in `defaults/tools/<group>/extension.ts`, or pi-coding-agent's own builtins for read/write/edit/grep/find/ls) — the two are hand-maintained in parallel and never derived from one another. The wire schema always carries full types/`required`/per-field descriptions; the markdown's `(params)` list is names only, restating information already sent, at a real (measured) token cost. See `docs/design/tools-md-dedup.md` for the measurement.

Set `tools_md: "index"` in `project.yaml`, or set `BOBBIT_TOOLS_MD=index` as the process-wide highest-precedence override, to drop the `(params)` list from every bullet (rendering `- name — summary` for all tools, not just the ones with no `params:` field) and add one pointer line under the `# Tools` heading noting that full schemas ship with the tool definitions. Resolution is env → project config → server/Headquarters config → built-in default. Default is `"full"` = today's byte-identical output. Measured on this repo's own default builtin tool set (69 tools / 15 groups): full = 7,257 chars (~1,815 tokens) → index = 5,324 chars (~1,331 tokens), a 26.6% reduction. Group headers and detail-docs pointers are unchanged in both modes — they are not duplication (the pointer path exists nowhere else). The persisted `<sessionId>-prompt.json` breakdown records the active mode on the "Tools" section's `toolsMdMode` field so an A/B can compare token deltas per session, and the System Prompt inspector shows an "index" badge when active. Config key: `tools_md`; env override: `BOBBIT_TOOLS_MD`.

**Key file:** `src/server/agent/tool-manager.ts` (`resolveToolsMdMode`, `getToolDocsForPrompt`).
...
| `tools/<group>/*.yaml` | `ToolManager` | Built-in tool definitions + extensions, including the `code_*` LSP product tools in `defaults/tools/code/` backed by `src/server/lsp/` |
| `tool-group-policies.yaml` | `ToolGroupPolicyStore` | Built-in group grant policies |
| `verification-policy.yaml` | `BuiltinConfigProvider`, `VerificationPolicyStore` | Builtin VerificationPolicy defaults (`gateCacheDefault`, `parallelReviewsDefault`, future gate/rubric policy fields) |
...
| `verification-policy.yaml` | `VerificationPolicyStore` | Server/project VerificationPolicy overrides merged by `ConfigCascade.resolveVerificationPolicy()` |
...
| `provider-bridge/` | `provider-bridge-extension.ts` | Content-addressed generated per-turn provider-hook bridge (beforePrompt/beforeCompact). Mounted read-only into Docker sandboxes — without the mount, sandboxed sessions with a hooks-declaring lifecycle provider die at spawn. |
