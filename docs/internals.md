...
**Per-project Archived subsections**: Each project group ends with its own collapsible Archived subsection (rendered by `renderProjectArchivedSection` in `src/app/render-helpers.ts`, shared between desktop `renderSidebar` (`src/app/sidebar.ts`) and mobile `renderMobileLanding` (`src/app/render.ts`) so both breakpoints behave identically), containing the archived goals and standalone archived sessions whose `projectId` matches that project. The global Archived block that used to sit at the bottom of the sidebar is gone.
