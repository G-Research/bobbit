...
| Resubmit toasts a 404 from `DELETE /api/sessions/:id` | The archive-guard was bypassed. The `isSessionArchived` helper in `src/app/render.ts` should short-circuit that DELETE for archived parents. | `isSessionArchived` call sites in `render.ts` — pinned by `tests/browser/fixtures/search-preview-archive.fixture.spec.ts`. |
