...
- **Turn timing and effort**: The footer stats area and stats popover show the last assistant turn duration when timing is available. Claude Code uses reported `duration_ms` / `duration_api_ms` values when the CLI provides them and a best-effort inferred elapsed duration otherwise. Bobbit thinking/effort maps to Claude CLI `--effort`; idle Claude Code sessions restart/resume to change effort, while active turns reject the change clearly.
