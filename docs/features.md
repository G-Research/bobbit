...
The composer accepts any file by default. Documents become immutable, session-owned send-time snapshots: the model receives escaped metadata, an opaque pointer, and at most a bounded text excerpt, while arbitrary binary data stays out of prompt text and remains readable through the bounded `session_attachment` tool. The original composer text and safe attachment tiles remain the user-visible form across queueing, retry, recovery, and reload. See [Browser-uploaded attachments](uploaded-attachments.md) for classification, limits, lifecycle, and security boundaries.
...
Readable text is snapshotted into `<file-reference>` blocks; images use `images[]`; other binaries are snapshotted from the workspace and admitted through the same immutable document path as browser uploads. Their literal `@path` and chip remain visible, while model-only context carries safe metadata and an opaque pointer—not binary base64—and policy-enabled bounded `session_attachment` reads provide the exact send-time bytes. Snapshots, punctuation boundaries, and UTF-16 chip ranges persist through reload via the shared sidecar. Whole-send admission is atomic at 8 MiB of authenticated UTF-8 prompt text, 8,192 non-code candidates, and 4,096 distinct targets; these bounds are separate from delivery limits and browser-upload attachment quotas. Backed by `GET /api/file-mentions`. See [at-mention-file-references.md](at-mention-file-references.md) for complete behavior, limits, path safety, cancellation, and ordering guarantees.
...
Every accepted prompt or steer—whether submitted in the composer or generated by the server for tasks, auto-nudges, verification, or system work—has a stable occurrence ID and remains visible in the delivery outbox or as a settled Pi transcript row.
...
- Only a settled Pi user row transfers the visible carrier into chat; Pi/socket acknowledgement and recovery projections remain in the outbox.
- Definite failures can Retry with the same occurrence ID. Ambiguous delivery remains visible and is never automatically replayed, including after reload or `agent_settled`.
