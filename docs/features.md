...
Bobbit can source models from built-in providers, local custom providers, or named model gateways. An enterprise AIGW retains well-known-first discovery so each upstream keeps its native Responses, Bedrock Converse, or chat-completions route; generic OpenAI-compatible gateways use raw IDs and ordinary completions routing. See [Bring your own models](bring-your-own-models.md) for setup and [AI Gateway routing](ai-gateway-routing.md) for security, migration, and refresh behavior.
