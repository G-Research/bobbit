...
Models become usable either through the configured AI Gateway, via an **account OAuth login** (Settings → Account), or with a **provider API key** (Settings → Models → Provider API Keys). OAuth credentials are provider-partitioned in `auth.json` and are propagated into agent sandboxes through provider-specific sanitized paths. The Claude Agent SDK Docker sandbox is an exception to generic credential forwarding: it requires explicit `ANTHROPIC_OAUTH_TOKEN` policy opt-in and passes only a current access token to its `docker exec` process, with no generic sandbox-credential or API-key fallback. See [Claude Agent SDK sessions](claude-agent-sdk-sessions.md#sandbox-subscription-handoff). Text-session model fallback is opt-in via `allowSessionModelFallback`; see [Controlled session model fallback](session-model-fallback.md).
