...
Claude Agent SDK sessions are the exception for interactive live requests: the initialized Query's session-local capability map is authoritative, so an unavailable level is rejected rather than clamped. At SDK startup, Bobbit normalizes the role/default preference against that map and falls back conservatively to `off` when the Query cannot prove a capability. See [Live model and thinking controls](claude-agent-sdk-sessions.md#live-model-and-thinking-controls).
...
- Pi live `set_model` validates the exact session-selectable row, clamps the requested level, applies the model, and verifies the complete tuple before persistence. For an interactive SDK request, the Query capability map must explicitly allow the level.
- Pi live `set_thinking_level` resolves exact metadata for the currently bound tuple and refuses the change when thinking metadata is unavailable. An interactive SDK request rejects an unavailable level rather than clamping it.
- Session, delegate, review, QA, restore, role-replacement, and force-abort spawns pass through the final spawn tuple validator. It clamps the effective thinking level against the exact effective provider/model after all raw arguments are assembled. After SDK Query initialization, startup normalizes the candidate against its live metadata; unavailable metadata proves only `off`.
...
The client updates optimistically, then replaces both model and thinking fields with the server's authoritative state. Standalone thinking changes use `set_thinking_level`. Pi requests are clamped against exact metadata; interactive SDK requests require a level from the live Query capability map. On failure, the client requests state again so an optimistic value cannot remain displayed.
...
- [Claude Agent SDK sessions](claude-agent-sdk-sessions.md#live-model-and-thinking-controls)
