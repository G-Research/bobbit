...
### Upstream persistence race and installed-runtime status

The race predates Bobbit's Pi `0.80.6` upgrade. Upstream Pi commit [`ff5148e7`](https://github.com/badlogic/pi-mono/commit/ff5148e7cc7dc330fcc61b2619de43feb21022c0) introduced asynchronous message-event forwarding in Pi `0.52.10`, released on 2026-02-12. `AgentSession._handleAgentEvent` began awaiting extension message handlers before `sessionManager.appendMessage`, while event listener invocations remained unserialized. A later tool-result event could therefore append before—or survive an interruption without—the assistant event that introduced its call ID. Bobbit first adopted an affected Pi line with `@mariozechner/pi-coding-agent@0.57.1` on 2026-03-13; the later move to Earendil `0.80.6` did not introduce the defect.

Upstream fixed the race on 2026-03-02 in [`dfc779faab24478fd4f6c608d78efe760a51160a`](https://github.com/badlogic/pi-mono/commit/dfc779faab24478fd4f6c608d78efe760a51160a), tracked by [`badlogic/pi-mono#1717`](https://github.com/badlogic/pi-mono/issues/1717), by serializing session event handling. That fix is absent from the currently installed `@earendil-works/pi-coding-agent@0.80.6`. Pi owns these conversation writes, so Bobbit cannot atomically order its internal appends. Force-abort, agent exit, or gateway restart can expose an incomplete turn, but Pi's handler-before-append race is the root cause—not Anthropic retry behavior, Bobbit's `agent_end.willRetry` handling, or the `0.80.6` upgrade.

Bobbit's boundary sanitizer remains necessary both for history written by any affected runtime and while the current Earendil `0.80.6` runtime lacks the upstream fix. A future Pi upgrade must verify that the serialization fix is actually present; even then, existing malformed history still requires repair.
