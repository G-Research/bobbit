...
Startup runs `migrateLegacyHeadquartersDirectory()` (`src/server/agent/state-migration.ts`) before `ProjectRegistry` loads. It moves legacy server/HQ state into the Headquarters directory and repairs installs that were promoted by the earlier design. It is preserve-first — it copies/merges and never deletes legacy files during automatic migration. The default-layout steady state is guarded by `.headquarters-dir-migrated`; the current policy is version 3 with explicit `running` and `complete` status, resolved path topology, and content-qualified recovery evidence. Missing, empty, malformed, older-version, `running`, completion-fenced, or evidence-mismatched checkpoints run a full pass. Legacy timestamp markers and version 2 checkpoints receive a full upgrade pass before version 3 completion becomes authoritative.
...
- **Per-project stores** (including legacy migration inputs `goals.json`, `sessions.json`, `staff.json`, `tasks.json`, gates, costs, and search indexes) are attributed by rule before project contexts open: records with a current normal `projectId` route to that project; records restorable from a `.pre-headquarters-id-migration` backup as a same-root normal project are restored to `<server-run-dir>/.bobbit/state` under the old id; missing-`projectId` records are treated as ambiguous when a same-root normal project exists and are preserved in the legacy normal-project state (not surfaced under Headquarters); only records with no same-root normal evidence are stamped `projectId: "headquarters"`.
...
Diagnostics are written to `<headquarters-state-dir>/headquarters-migration-diagnostics.json` (source paths, copied/merged/quarantined files, ambiguous records, restored normal ids, previous-override hints, failures, and — see below — tombstoned records skipped during recovery). User work is never silently moved between Headquarters and a normal project without a deterministic rule or a visible diagnostic.

### Deletion tombstones: why deleted staff/sessions/goals stay deleted

The migration's per-store recovery has a **backup-only recovery loop**: for any record key present in a `.pre-headquarters-id-migration` backup but absent from the live store, it assumes the record was legitimately dropped and restores it (this is "Finding C" — a promotion that stripped a record should not permanently lose it). That heuristic is correct for records that vanished *by accident*, but it cannot, on its own, tell the difference between "dropped and needs recovery" and "**deleted on purpose after the migration**".

**The resurrection bug.** Because `migrateLegacyHeadquartersDirectory()` runs on every boot and (in the same-root / `BOBBIT_DIR`-override case) re-ran its repair each time, deleting a staff agent, session, or goal only stuck until the next restart. On boot the frozen backup still contained the deleted record, the live store did not, so the recovery loop re-materialised it and `staff.json` reverted byte-for-byte to its pre-delete size. This is a **data-integrity and safety** bug, not a cosmetic one: a resurrected staff record re-registers its triggers, so a deleted automation (e.g. a `git push on master` trigger that spawns sessions) silently reactivates.

**The fix — durable deletion tombstones.** Hard-deletes now leave a durable marker that survives restarts and tells the migration "this key was removed on purpose; do not bring it back".

- **Tombstone store** — `src/server/agent/deletion-tombstones.ts` maintains one JSON file per state dir, `<stateDir>/.deletion-tombstones.json`, shaped `{ "<fileName>": ["<recordKey>", ...] }`. Keys are the record `id` — the same key derivation the migration uses (`recordKeyForFile`) — so tombstones line up with recovery. The file lives beside its owning project stores (the Headquarters state dir for Headquarters, or the normal project's state dir), satisfying the durability requirement without becoming part of either SQLite transaction. All reads/writes are best-effort and wrapped in `try/catch`: a tombstone write failure must never throw into a delete caller (deletion still succeeds; the marker guard and backup retirement below are the backstop).
- **Recording at delete time** — the hard-delete `remove()` on `StaffStore`, `SessionStore`, and `GoalStore` records a tombstone in its own state dir (`staff.json` / `sessions.json` / `goals.json`). These namespace names remain stable even though production goals now live in `goals.sqlite`, because Headquarters repair and store-owned SQLite recovery both consume the legacy namespace. Recording in the store, rather than only in `StaffManager.deleteStaff` / the session / goal delete endpoints, guarantees every permanent-delete caller is covered. Goal **`archive()` deliberately does not tombstone** — archiving keeps the record (sets `archived`), so a subsequent boot must still see it. The migration itself never calls `store.remove` (it uses `writeStoreRecords` directly), so relocation cannot self-tombstone.
- **Consulted during recovery** — `routeLegacyProjectStoreFile` reads the tombstone set (union of the target and legacy state dirs) and **skips any backup-only key that is tombstoned**, recording it under `diagnostics.tombstonedSkipped`. `GoalStore` applies the same rule to recovery-only goals during SQLite import, and `TaskStore` skips recovery-only tasks whose `goalId` is tombstoned. Un-tombstoned backup-only keys still recover, so Finding C is preserved: a record that was legitimately dropped (never deleted, no tombstone) is still restorable.

**Which stores are tombstoned, and why not all.** Tombstones are recorded only for the **array** stores with user-facing hard deletion and an `id` key: `staff.json`, `sessions.json`, and the legacy `goals.json` namespace. `TaskStore` does not record independent task tombstones; during SQLite `.pre-migration` recovery it consults goal tombstones and skips recovery-only tasks whose `goalId` was deleted. Team state follows the same goal lifecycle during Headquarters backup retirement. Gates use composite identity and keep their independent recovery authority. Object-shaped stores (session costs, session colours, and background processes) are excluded for the same structural reason. If a future store gains user-facing hard deletion and can be replayed from an older backup, its recovery design must include equivalent deletion intent.

**Marker guard and steady-state evidence.** For the default Headquarters layout, version 3 records bounded evidence that can make migration newly useful without recursively walking runtime trees on every boot:

- resolved source/target path topology and top-level legacy state/config entries, including real paths so a new source or retargeted junction/symlink invalidates completion;
- SHA-256 content digests for project registries, deletion tombstones, and `.pre-headquarters-id-migration` authorities, so same-size/same-mtime changes still invalidate;
- a recovery digest that qualifies same-root backup keys against their expected live stores and tombstones.

The recovery digest reopens migration when an untombstoned backup can repair a live store that became missing, empty, or corrupt, even if the backup bytes did not change. Unreadable evidence fails closed into a full pass. Normal-project runtime and config changes outside these recovery boundaries do not invalidate the checkpoint.

A matching, unfenced `complete` checkpoint skips recursive legacy copying, project-store rerouting, config quarantine traversal, backup retirement, and diagnostics rewriting. An unchanged intentionally preserved ambiguous backup does not force repeated work. Deleting the checkpoint explicitly requests a full recovery pass.

**Completion durability.** Before migration changes recovery evidence, version 3 atomically publishes `running`. Completion is authoritative only after a sibling `.headquarters-dir-migrated.completion-pending` fence is durably published, the `complete` record is atomically renamed and acknowledged, and the fence removal is acknowledged. A visible `complete` record remains incomplete while the fence exists. If fence removal or its final directory sync fails — including `EIO` after the fence is already absent — the publisher recreates the fence before reporting the original error, making the next boot retry. If compensation fails too, an aggregate error retains both failures because checkpoint authority is indeterminate; stop the gateway, repair storage, remove the checkpoint and any fence, and restart to force migration.

The `BOBBIT_DIR`/override same-root case (the "B1" branch) retains its marker-plus-backup-retirement qualification because its source and target topology differs. The always-run security/correctness steps — server-secret relocation, `projects.json` repair, and Headquarters execution-store sanitisation — are **never** gated by either fast path. Secret relocation remains fail-closed: inability to remove a reachable legacy secret aborts migration even on an otherwise clean checkpointed boot.

**Spent-backup retirement.** Once repair is confirmed complete (marker present at boot start), `retireSpentHeadquartersBackups` renames each spent `.pre-headquarters-id-migration` per-store backup to `.pre-headquarters-id-migration-recovered` (mirroring the `.pre-migration` → `.pre-migration-recovered` precedent in `recoverPreMigrationData`) so it can never resurrect anything again. A **per-store safety gate** protects unrecovered data: a backup is retired only when **every** key it holds is either present in a live store the recovery could route into (the Headquarters store or a same-root normal project's store) **or** tombstoned. If any backup key is neither — genuinely-unrecovered, never-deleted data — the backup is preserved so it stays recoverable. Retirement is gated on `alreadyMigrated`, so it never runs on the first migration (before recovery has happened).
