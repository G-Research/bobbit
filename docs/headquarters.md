...
**Marker guard — the repair is a no-op on the second boot.** Tombstones are the primary correctness fix, but two defence-in-depth layers stop the repair from re-running forever. The completion marker `.headquarters-dir-migrated` is read at boot start (before it is re-written at the end of the run). In the `BOBBIT_DIR`/override same-root case (the "B1" branch), once the marker is present the per-store re-routing loop is **skipped entirely** on subsequent boots — any legitimately-droppable backup-only record was already merged into the live store on the first completed migration, so re-running the loop could only re-surface records. The always-run, preserve-first steps — server-secret relocation, `projects.json` repair, and store sanitisation — are **not** gated on the marker; they are idempotent and must keep running to avoid regressing the same-root privilege/secrets handling.
