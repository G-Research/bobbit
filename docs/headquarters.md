...
- **Tombstone store** — `src/server/agent/deletion-tombstones.ts` maintains one JSON file per state dir, `<stateDir>/.deletion-tombstones.json`, shaped `{ "<fileName>": ["<recordKey>", ...] }`. Keys are the record `id` — the same key derivation the migration uses (`recordKeyForFile`) — so tombstones line up with the recovery loop. The file lives in the **headquarters state dir**, satisfying the durability requirement while staying out of any normal-project-reachable secret path. All reads/writes are best-effort and wrapped in `try/catch`: a tombstone write failure must never throw into a delete caller (deletion still succeeds; the marker guard and backup retirement below are the backstop).
- **Recording at delete time** — the hard-delete `remove()` on `StaffStore`, `SessionStore`, and `GoalStore` records a tombstone in its own state dir (`staff.json` / `sessions.json` / `goals.json`). Recording in the store, rather than only in `StaffManager.deleteStaff` / the session / goal delete endpoints, guarantees every permanent-delete caller is covered. Goal **`archive()` deliberately does not tombstone** — archiving keeps the record (sets `archived`), so a subsequent boot must still see it. The migration itself never calls `store.remove` (it uses `writeStoreRecords` directly), so relocation cannot self-tombstone.
- **Consulted during recovery** — `routeLegacyProjectStoreFile` reads the tombstone set (union of the target and legacy state dirs) and **skips any backup-only key that is tombstoned**, recording it under `diagnostics.tombstonedSkipped`. Un-tombstoned backup-only keys still recover, so Finding C is preserved: a record that was legitimately dropped (never deleted, no tombstone) is still restorable.

**Which stores are tombstoned, and why not all.** Only the **array** stores that can be resurrected through the backup-only recovery loop with an `id` key: `staff.json`, `sessions.json`, `goals.json`. Team-state, gates, and tasks are deliberately excluded — they are goal-scoped / composite-keyed and lifecycle-managed alongside the goal they belong to (cleared with the goal, not user-facing hard-deletes that get resurrected via this path). The object-shaped stores (session-costs / session-colors / bg-processes) are excluded for the same structural reason. If a future array store gains a user-facing hard-delete **and** routes through the same recovery loop, it should tombstone on `remove()` too.
