...
Startup runs `migrateLegacyHeadquartersDirectory()` (`src/server/agent/state-migration.ts`) before `ProjectRegistry` loads. It moves legacy server/HQ state into the Headquarters directory and repairs installs that were promoted by the earlier design. It is preserve-first — it copies/merges and never deletes legacy files during automatic migration. The default-layout steady state is guarded by a versioned `.headquarters-dir-migrated` checkpoint containing the resolved path topology and shallow signatures of project-registry, tombstone, and migration-backup evidence. Legacy timestamp-only markers receive one final full pass and are upgraded.
...
**Marker guard and steady-state evidence.** For the default Headquarters layout, a matching versioned `.headquarters-dir-migrated` checkpoint skips recursive legacy server-state comparison, project-store rerouting, config quarantine traversal, backup retirement, and diagnostics rewriting. Normal-project runtime state/config changes do not invalidate it. A changed path topology, project registry, deletion tombstone file, or top-level `.pre-headquarters-id-migration` artifact forces a full pass and publishes a new checkpoint; an unchanged intentionally-preserved ambiguous backup does not force repeated work. Deleting the marker explicitly requests a full recovery pass.

The `BOBBIT_DIR`/override same-root case (the "B1" branch) retains its existing marker-plus-backup-retirement policy because its source and target topology differs. The always-run security/correctness steps — server-secret relocation, `projects.json` repair, and Headquarters execution-store sanitisation — are **never** gated by either fast path.
