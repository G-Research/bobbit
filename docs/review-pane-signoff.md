...
Every open first persists the complete group and selected file synchronously for its owning session. Its ordered effect stops if a newer exact-key intent has superseded it, then creates or focuses the exact primary tab in that owner's server-backed workspace. Opens never clear or otherwise rewrite replay tombstones: the authoritative primary proves that an explicit open committed, while a tombstone suppresses only passive replay when that primary is absent. This removes the cross-file clear-then-restore transaction, so a failed workspace mutation leaves exact and sibling tombstones unchanged and the same payload remains retryable.
