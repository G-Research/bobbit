...
`Continue in new session` uses `canContinueArchivedSession()` for client-side visibility. It is shown only for archived/read-only sources that are not goal sessions, delegates, child/delegate rows, team sessions, team leads, or non-interactive sessions; whose `projectId` still resolves to a registered project; and whose runtime has a resumable source. Pi sources require an available transcript (`agentSessionFile` when supplied, and `transcriptAvailable` not `false`); SDK sources require their persisted SDK identity and are preflighted by the server. Ineligible sessions hide Continue rather than rendering a disabled item, while the other read-only actions remain available.

Running Continue opens the confirm-only `ContinueSessionChooser`, then posts an empty JSON body to `POST /api/sessions/:id/continue`. On success the client refreshes sessions and connects to the returned session id as an existing session with message refetch enabled. The action intentionally does **not** call the live `/api/sessions/:id/fork` endpoint: Pi Continue clones the archived `.jsonl` transcript into a fresh session slot and rehydrates through `switch_session`; SDK Continue creates a fresh Bobbit wrapper around the same opaque SDK UUID after official source preflight. See [REST API — Continue-Archived endpoint](rest-api.md#continue-archived-endpoint) for `404 SDK_SESSION_UNAVAILABLE` and `422 RUNTIME_CONTINUE_UNSUPPORTED`, [Internals — Continue-Archived sessions](internals.md#continue-archived-sessions), and [Claude Agent SDK persistence and resume](design/claude-agent-sdk-persistence-resume-g6.md).
