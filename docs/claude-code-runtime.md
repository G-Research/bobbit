...
- **Usage and timing display** — token usage is normalized when Claude Code reports usage, and local-runtime cost/usage fields are treated as best-effort rather than authoritative API billing data. Claude Code turn timing uses reported `duration_ms` / `duration_api_ms` values when present, with a best-effort elapsed duration inferred by Bobbit otherwise. The chat footer and stats popover show the last-turn duration when available.
...
| `tool_result` | Emits matching tool-result/end events. For `ask_user_choices`, the Claude CLI may emit the known `Answer questions?` placeholder as an error-marked result after posting the widget. Bobbit recognizes only that exact placeholder on a matching ask call, converts it to the normal posted-widget result, and leaves genuine tool errors untouched. |
| `result` | Ends the assistant turn and emits `agent_end`; errors surface as visible assistant errors. Bobbit preserves reported `duration_ms` / `duration_api_ms` timing and infers a best-effort total duration when the CLI omits one. |
