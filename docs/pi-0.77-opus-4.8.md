...
- **Bedrock request-header patch.** Bobbit still patches Pi's Bedrock provider so Bedrock traffic can carry Bobbit's request headers. The compatibility test verifies that the installed Bedrock stream implementation still has the expected patch anchors, or is already patched, before asserting the Bobbit hook is present. This catches upstream Pi provider rewrites early — as it did across the Pi 0.80 upgrade, when the implementation moved from `dist/providers/amazon-bedrock.js` to `dist/api/bedrock-converse-stream.js` (see `docs/design/pi-fork-edit-safety.md`'s Bedrock row for the current path).
