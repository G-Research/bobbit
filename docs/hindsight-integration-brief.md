...
- Directives are first-class endpoints (`GET/POST /directives`, `GET/PATCH/DELETE /directives/{id}`), not bank-config fields.
- Mental-model creation is asynchronous: `POST /mental-models` returns `{ mental_model_id?, operation_id }`; content is read later from `GET /mental-models/{id}`.
...
- `market-packs/hindsight/src/provider.ts`
  ...
  - Flow:
    1. Require `cfg.mentalModelEnabled`, active data plane, and real `projectId`.
    2. `GET /mental-models/{id}`.
    3. If 404/null, `POST /mental-models` with stable id and return no model block this turn unless a later GET has content.
    4. If content exists, inject block:
    ...
    6. If no content/error/unconfigured, fall back to current `doRecall(ctx, cfg, ctx.prompt)`.
  - `sessionSetup` should inject mental model if available and skip raw recall for that hook to avoid duplicate start-of-session memory. `beforePrompt` should keep targeted recall.
...
- `tests/hindsight-provider.test.ts`:
  - `sessionSetup` injects mental model block above raw recall and skips raw recall when content exists.
...
Preferred implementation: route/tool first, provider hook later only if the runtime exposes a reliable goal/PR completion lifecycle signal.
...
- If no clean completion hook exists, agents/team lead call `hindsight_retain_outcome` explicitly at goal/PR completion.
- If Hindsight is unhealthy, route returns a nonfatal error; future enhancement can enqueue outcome digests like auto-retain. Prefer queueing in implementation if route has store access.
...
- `tests/hindsight-provider.test.ts` or `tests/hindsight-tool-execute.test.ts`: outcome helper builds stable id/tags/entities.
...
Goal: apply Bobbit-specific reflect/recall behavior idempotently: prefer recent durable facts, cite source facts when included, answer for a coding agent, avoid transient turn noise.

Verified API:

- Use `/directives`, not bank config.
- Create request requires `name` and `content`; supports `priority`, `is_active`, `tags`.
...
- `market-packs/hindsight/src/shared.ts`
  - Config keys:
    ...
    - `directives: [{ name, content, priority, tags }]` default list.
  - Store signature key `DIRECTIVES_APPLIED_PREFIX` per namespace/bank.
  - `applyDirectives(store, client, cfg)` that lists existing directives, matches by stable name, creates or PATCHes changed content/priority/tags, and caches signature.
- `market-packs/hindsight/src/provider.ts`
  - Call `applyDirectives` next to `applyBankMission` before retain/mental-model refresh; best-effort and non-blocking for turns.
- `market-packs/hindsight/src/routes.ts`
  - `status` can expose directive applied diagnostics if useful, but no UI work is required.
...
- Unit: directives applied once per signature, re-applied on config change, failure non-fatal.
- Reflect stub-level test can assert directives list exists; live reflect honor is not deterministic and should not be asserted beyond request/endpoint behavior.
...
Implementation should touch only these production pack files plus generated lib outputs and tests:
...
- Session start with mental model available: one `GET /mental-models/{id}` and one injected project model block capped at ~1000-1200 tokens; no broad raw sessionSetup recall for that hook. Refresh is cadence-bounded and async via operation IDs.
...
- Retain calls: remains 1 per 5 routine turns; outcome digest adds one replace call per completed goal/PR. This should reduce future search/recall calls by making completed work discoverable in one outcome memory.
...
- Outcome digest: implement advanced retain fields, dedicated route/tool, replacement tests.
...
- Observation scopes/entities/query timestamp/directives: add config, client mappings, provider/route wiring, tests.
