...
- A new successful request emits affected `gate_status_changed` events and `gate_reset` with the same `reopen` object. An actual reopen additionally emits `goal_state_changed` to UI principals (`admin` or `localhost`); sandbox-token sockets do not receive this unrelated goal-list invalidation. A retry that resumes a retained recovery intent suppresses duplicate events and notification.
