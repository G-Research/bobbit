...
| `POST` | `/api/sessions/:id/fork` | Fork a live session. Body `{ newWorktree?: boolean, entryId?: string }`: omit `entryId` to clone the whole source JSONL, or supply a durable Pi prompt cursor to clone the active branch strictly before that prompt. SDK sessions have no compatible Pi transcript and return the existing missing-transcript `404`; create a fresh SDK session instead. See [Fork session endpoint](#fork-session-endpoint). |
