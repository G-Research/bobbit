...
There is no `?scope=server` parameter on workflow endpoints — it was removed when the system-scope workflow layer was eliminated. Use `projectId=headquarters` for the visible server workspace workflow scope. A command verify step may carry native `env?: Record<string, string>` overrides; writes reject it on all other step types. Workflow responses return those declared values only. See [Project command environments](project-command-environments.md) for validation and runtime semantics.
