...
| `GET` | `/api/sessions/:id/transcript` | Paginated, regex-filterable transcript reader and agent `read_session` backing route. Direct REST keeps the legacy projection; a request bound to a real caller session gets the bounded agent projection, result handles, normalized page coordinates, and agent guards. See [Bounded session diagnostics](read-session.md). |
...
`GET /api/sessions/:id/transcript` has two compatibility boundaries over the same transcript source:

- **Direct REST/UI:** a normal authenticated request, or one with an absent/unknown `x-bobbit-session-id`, receives the legacy projection. Omitting both result flags includes tool results. Existing compact/verbose blocks, author objects, pagination, and errors are unchanged.
- **Agent-bound:** after authentication, a caller header that resolves to a real live or persisted session selects the canonical bounded projection used by `read_session`. The current wrapper supplies this header and explicitly sends `include_tool_results=0` by default. The header selects context policy; it grants no additional authority.

Query parameters:

| Parameter | Direct REST/UI | Agent-bound `read_session` |
|---|---|---|
...
| `include_tool_results` / `includeToolResults` | Omitted means legacy inclusion; `false`/`0` redacts, `true`/`1` includes. If both appear, snake case has precedence. | True requests bounded excerpts, never unbounded bodies. Each alias is treated as heavy by the spawned-agent guard; current `read_session` exposes snake case. |
| `result_handle` | No direct legacy projection contract. | Selects one canonical result for a targeted bounded excerpt. |
...
Agent projection also has bounded work admission. A request reserves shared capacity before transcript I/O, then charges the complete raw JSONL before parsing and bounds cumulative indexing, result count, canonicalization, hashing, copying, and retained text. Exhaustion returns `400 { error: "TRANSCRIPT_WORK_LIMIT_EXCEEDED", detail }`; the agent tool preserves the same discriminator in its errored result. `detail` is explanatory rather than stable. These projection reservations do not change direct REST/UI behavior.

Marketplace Pi contributions resolve before ordinary tool activation so discovery can populate the scoped catalogue used by policy resolution. Their runtime arguments follow ordinary Bobbit tool arguments, but the immutable result boundary is prepended ahead of both. A discovered `read_session` registration, or a runtime-enabled extension whose failed/skipped discovery leaves registrations unknown, requires the fail-closed result boundary and pre-handler heavy guard. Unknown registrations are not exposed as speculative catalogue tools; if they register `read_session` at runtime, heavy calls are still rejected before the handler fetches.

The immutable agent boundary canonicalizes both success and error results after the resolved extension and downstream Pi result listeners. The final Pi return, AgentSession `toolResult` state/event value, and persisted JSONL row each serialize within the unchanged 50 KiB production ceiling; provider-only wrapper fields and duplicate result data cannot reappear later in the lifecycle. Budget-shortened successful pages and result slices return typed continuations. The internal fitter's 1,536-byte minimum only proves that a compact targeted slice can retain canonical result identity, size, handle, and truthful excerpt metadata under adversarial escaping; it is not a query parameter or a lower production ceiling. Direct REST responses remain outside this Pi lifecycle policy and retain their legacy transport and rendering contract.

See [Bounded session diagnostics](read-session.md) for canonical Pi/Anthropic records, result metrics/handles, continuation examples, dictionaries, work-limit recovery, and the compact-first workflow.
