...
| `POST` | `/api/goals` | Create a goal (`{ title, cwd, spec, team?, worktree?, reattemptOf?, parentGoalId?, divergencePolicy?, maxConcurrentChildren?, inlineRoles?, workflowId?, workflow?, subgoalsAllowed?, maxNestingDepth? }`). `subgoalsAllowed: false` disables subgoal spawning for this goal tree (cannot loosen a system-level OFF). `maxNestingDepth` tightens the per-goal depth ceiling; the server clamps to `min(systemMax, clamp(value, 1, 10))` — values above the system ceiling are silently capped. Both fields are optional; omitting them inherits system prefs. The goal-creation form and proposal modal in the UI only forward these fields when the user explicitly changes them from the default. See [docs/nested-goals.md — Nesting depth limit & per-goal toggle](nested-goals.md#nesting-depth-limit--per-goal-toggle). Workflow resolution runs four layers: cascade lookup → project-store fallthrough (handles transient stale-cascade) → lazy auto-seed of built-in `general`/`feature`/`bug-fix`/`parent` workflows when the project store is empty → friendly `400 { code: "WORKFLOW_NOT_FOUND", workflowId, available: [...] }` when an explicit `workflowId` is unknown in a non-empty store. When `workflowId` is omitted, the first workflow in the project's store is used. See [goals-workflows-tasks.md — Default workflow resolution](goals-workflows-tasks.md#default-workflow-resolution) for the full convention. |
| `GET` | `/api/goals/:id` | Get a goal |
| `PUT` | `/api/goals/:id` | Update a goal. Accepted body fields: `title`, `cwd`, `state`, `spec`, `team`, `repoPath`, `branch`, `reattemptOf`. Server-derived fields (`rootGoalId`, `mergeTarget`, `parentGoalId`, `spawnedFromPlanId`, etc.) cannot be set via this endpoint. Divergence policy and concurrency are set via `PATCH /api/goals/:id/policy`. Broadcasts `goal_spec_changed` when `spec` changes. |
| `DELETE` | `/api/goals/:id` | Archive a goal (soft-delete). **Cascade contract**: requires explicit `?cascade=true\|false` — omitted returns 422 `{code:"CASCADE_REQUIRED"}`. With `?cascade=false`, returns 409 `{code:"HAS_DESCENDANTS", count}` if the goal has any non-archived descendants. With `?cascade=true`, archives descendants bottom-up (leaves first) then the root via `cascadeSubtree` — see [docs/design/hierarchical-cascade.md](design/hierarchical-cascade.md). Optional `?mergedManually=true` reconciles the target's `state` to `"complete"` before archiving (target-only, NOT cascaded descendants); used by the `goal_archive_child` tool when the team-lead manually merged a child whose `ready-to-merge` failed. |
