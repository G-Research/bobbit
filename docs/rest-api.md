...
| `GET` | `/api/sessions/:id/transcript` | Paginated, regex-filterable transcript reader. Backs the `read_session` tool. Query params: `offset` (negative = from end), `limit` (default 20, clamped 1..200), `pattern`, `case_sensitive`, `context` (±5 max), `verbose`. Requires the route's normal bearer/session authentication, then resolves live, dormant, and archived transcripts across gateway-accessible projects. Errors: `session_not_found` (404), `transcript_unavailable` (404), `invalid_regex` / `invalid_params` (400). Pure parser lives in `src/server/agent/transcript-reader.ts`. |
