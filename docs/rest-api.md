...
The marker has a hard 250-byte cap so tool output stays bounded. The writer preserves its ordered payload variants and selects the first that fits. When the complete route and optional metadata do not fit, it may store the compact form `url: "/preview/<sid>/"` with an explicit single-filename `entry` and a compact `path`. It can also use `aid` for `artifactId`, or omit optional `contentHash` and artifact metadata as later fallbacks. `contentHash` and artifact identifiers are used only after validation; a supplied malformed value is not silently treated as missing.

The compact URL is an encoding for the marker, not a relaxation of preview routing. When reopening an existing transcript, the reader accepts that directory form only with a safe `entry`, reconstructs `/preview/<sid>/<entry>`, then passes the reconstructed route through the same strict preview-route validation as a full URL. A compact URL without an entry, an unsafe entry, traversal-shaped data, or an invalid artifact route remains malformed. This read compatibility is necessary because historical markers cannot be rewritten; recognized same-origin/base-path forms are reduced to the validated internal route rather than used as public navigation URLs.
