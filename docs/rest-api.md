...
| `GET` | `/api/sessions/:id/prompt-prefix-attribution?limit=N` | Hash-only comparison of Bobbit-owned system prompt, tools, dynamic context, and skills fingerprints. Returns `{ entries }` oldest→newest; `limit` keeps the most recent N (clamped to 1000). Each row includes component SHA-256/byte metadata, comparison and boundary semantics, model/compaction metadata, and provider-cache telemetry, never prompt or block content. See [Prompt-prefix attribution](prompt-prefix-attribution.md). |
