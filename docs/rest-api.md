...
A filtered sidecar copy failure fails the request instead of returning a destination with stale references. Transcript cloning, session launch/persistence, and strict staff-record persistence form one destination publication boundary. Failure before that boundary removes the destination transcript, session, staff row, proposals, caches, copied sidecars, and any destination-owned worktree. Compensation never mutates the source or a borrowed worktree. Once strict staff publication succeeds, notification or response-write failures are isolated rather than rolling back a valid staff/session pair.
