...
Client call sites use the shared helpers `errorFromResponse(res, fallback)` and `errorDetails(err)` from `src/app/error-helpers.ts` to parse this body, attach `code`/`stack` to the thrown `Error`, and forward both to `showConnectionError(title, message, { code, stack })`, which renders via the `<error-details>` component (`src/ui/components/ErrorDetails.ts`). The set of modal call sites that must forward `{ code, stack }` is pinned by `tests/unit/core/error-modal-call-sites.unit.test.ts`; the helper contract is pinned by `tests/unit/core/error-helpers.unit.test.ts`.
...
| `POST` | `/api/sessions/:id/fork` | Fork a live session. Body `{ newWorktree?: boolean, entryId?: string }`: omit `entryId` to clone the whole source JSONL, or supply a durable Pi prompt cursor to clone the active branch strictly before that prompt. Forking a permanent staff session also creates an independently persisted staff identity. See [Fork session endpoint](#fork-session-endpoint). |
...
| `DELETE` | `/api/sessions/:id` | Terminate a session. A worktree owner with a live borrower returns typed `409 SHARED_WORKTREE_IN_USE` on host or `409 SHARED_SANDBOX_WORKTREE_IN_USE` in a sandbox without changing the owner. |
...
`POST /api/sessions/:id/fork` creates a writable live session that rehydrates through `switch_session` from a server-built clone of the source Pi transcript. The route supports both ordinary sessions and permanent staff sessions. Whole-session and prompt-history forks share source eligibility, transcript handling, worktree provisioning, cleanup, and response behavior; staff sources additionally publish a new independent staff identity.
...
- `newWorktree: false` reuses the source session's exact live `cwd`, including a nested directory within its worktree, and therefore shares the same filesystem state and branch. A history fork, and any staff fork using this mode, is persisted as a writable borrowed-worktree session without worktree/repository/branch teardown coordinates. It does not register, claim, reset, clean, stash, adopt, repair, or remove that tree. Staff forks persist the flattened final owner in `borrowedWorktreeOwnerSessionId` in both host and sandbox realms; sandbox history forks retain the same flattened provenance. Terminating, archiving, recovering, or rolling back a borrower cannot tear down or recreate the shared tree.
- `newWorktree: true` uses the established session-level Fork lifecycle. A Git-backed project gets a fresh owned `session/...` branch/worktree and the response waits for setup to finish; a non-Git project uses the established project-root behavior. Sandbox provisioning follows the same fresh-fork path rather than a history-specific lifecycle. A staff fork owns only the destination coordinates recorded on its new session and staff record; source and destination cleanup remain disjoint.

Staff reuse-fork creation in either filesystem realm, plus sandbox history-fork creation, serializes with owner termination/archive through one FIFO keyed by the flattened final owner. Launch revalidates the source session, exact staff ownership when applicable, cwd, and owner inside the FIFO, so a fork cannot attach after owner teardown. Owner teardown checks for live borrowers before any lifecycle mutation and returns `409` with `SHARED_WORKTREE_IN_USE` for host ownership or the backward-compatible `SHARED_SANDBOX_WORKTREE_IN_USE` for sandbox ownership. Callers should terminate the borrower first and retry. Rejection leaves both staff records, sessions, inboxes, and the shared worktree unchanged.
...
Both fork modes preserve the source context that still applies: project; goal, task, and reattempt-goal association; assistant type; role/accessory configuration; sandbox realm; allowed tools; selected/effective model and thinking level; and generated `Fork: <source title>` naming. A valid fork may transition an associated todo goal through the existing shared behavior, but cursor validation completes first. Ordinary session forks otherwise retain their existing behavior.

A source is treated as staff-backed only when its persisted `staffId` resolves in the same project and that staff record's `currentSessionId` is exactly the source session. This ID join prevents colliding names or shared cwd values from becoming ownership authority. A stale or unmatched `staffId` follows the ordinary-session fork path.

A staff-backed fork creates a new persisted staff record and gives the destination session that record's new `staffId`; it never reuses or reassigns the source ID. The record and session are both named `Fork: <source title>`, and the destination record owns the destination session through `currentSessionId`. The staff configuration is a point-in-time snapshot of description, system prompt, role, accessory, memory, context policy, state, sandbox preference, project/configured cwd, and triggers. Each trigger is deep-cloned with a fresh ID; its definition and runtime cursor values are copied as independent values so the fork neither replays old work nor shares later edits/firings with the source.

The destination inbox starts empty. Inbox data is not part of transcript or staff-configuration cloning, and the new ID gives it separate storage and authorization. The same destination ID is persisted on the session and supplied as `BOBBIT_STAFF_ID`; restore and agent replacement rebuild the environment from that persisted field, so inbox tools continue to target the fork after refresh or gateway restart.

Staff publication is ID-bound across the project’s staff and session stores. Before launch, the route strictly writes the complete destination staff snapshot with an internal `forkPublication: { version: 1, sessionId }` marker. `GET /api/staff`, `GET /api/staff/:id`, and other ordinary staff reads exclude marked candidates. After the destination session is durably flushed, the route strictly removes the marker only if the marker/session ID, staff `currentSessionId`, `session.staffId`, and project identify the same non-archived destination. Only then is the row listed, indexed, notified, and eligible for the `201` response; names and paths do not participate in this join.

Gateway boot reconciles marked candidates before restoring sessions. An exact durable destination session completes publication through the same validation; an absent or archived destination removes the unpublished candidate and its destination-only inbox/search residue. Malformed or mismatched joins fail closed instead of guessing an owner. This closes interruptions on either side of the session-store commit without exposing a pending staff row.
...
A filtered sidecar copy failure fails the request instead of returning a destination with stale references. Transcript cloning, hidden staff staging, session launch/persistence, and strict marker removal form one destination publication boundary. Before commit, compensation first removes the destination session, transcript, proposals, caches, copied sidecars, and any destination-owned worktree, then deletes only the still-marked destination staff candidate. It never mutates the source, its inbox, or a borrowed worktree. Once marker removal succeeds, notification or response-write failures are isolated rather than rolling back a valid staff/session pair.

For a sandbox fork, canonical container transcripts stay in container coordinates through source reads, destination publication, cwd rebasing, sanitization, final-path rename, persistence, and `switch_session`. Container copies perform a bounded read in the exact registered, freshly attested source runtime and an atomic write in the destination owner's separately attested runtime. Fixed runtime code validates the container paths, streams content instead of putting it in command arguments, and creates and renames its own sibling staging entry. The gateway never resolves those container paths as host transcript paths or uses the shared project-control container.

Failure cleanup validates canonical container session paths and deletes generated transcripts only through the exact session runtime. Archived or store-only access may establish a short-lived isolated runtime; unavailable authority fails closed. Bobbit can still purge host-owned destination records and sidecars while leaving an unreachable transcript orphan for trusted maintenance rather than risk host filesystem mutation.

#### Source immutability, single-flight, and navigation

The source process, session record, transcript bytes, sidecars, cwd/branch, prompt queue, running project state, connected clients, and session-list entry remain unchanged. For a staff source, its staff record, `currentSessionId`, triggers, inbox, identity, and lifecycle ownership are unchanged as well. The server reserves each exact `(sourceId, entryId, newWorktree)` history request while it runs; an identical concurrent request receives `HISTORY_FORK_IN_PROGRESS`, while different boundaries remain independent. The reservation is released on success and failure.

The browser also disables prompt actions while session creation is pending. On a successful `201`, the initiating client refreshes the session list, connects to the returned id as an existing session, refetches cloned history when ready, and navigates there. Staff forks additionally emit the normal `staff_changed` creation event, so all clients refresh staff before classifying sessions: the permanent destination appears immediately under the owning project's **Staff** section and is excluded from ordinary **Sessions**. On a cold session route, including reload and deep-link entry, the client awaits authoritative `GET /api/staff` hydration before connecting and first rendering the sidebar; it does not rely on the Staff renderer’s asynchronous fallback. The ID-based `currentSessionId` join therefore controls placement without a transient ordinary-session row or premature Staff-section toggle. Other clients are not navigated away from the source. A request, launch, or connection failure stays on the source route and uses the existing visible connection-error surface.
...
`goalId` is omitted when not applicable. The title is `Fork: <source title>` and is marked generated so first-prompt auto-titling does not replace it. Staff forks keep this response shape for compatibility; the new staff identity is available through the normal staff list/event, joined by `currentSessionId === response.id`.
...
An invalid `newWorktree` value returns `400 { "error": "Invalid newWorktree flag" }` without a code. A reuse request whose required final owner cannot be resolved returns `422` with code `HISTORY_FORK_SOURCE_UNAVAILABLE`; the message names history forking when `entryId` is present and ordinary forking otherwise. If the source cwd, owner, or exact staff/session join changes before serialized launch, the route returns the same status and code with `error: "The source session is no longer available for history forking"`. Existing route failures remain unchanged: missing persisted source or transcript is `404`; an unregistered project or missing goal is `410`; archived, terminated/non-live, delegate, child, read-only, non-interactive, team, or cross-realm sources are `422`; clone, filtered-sidecar, worktree-setup, session persistence, strict staff persistence, and launch failures are `500` with a descriptive `error`.
...
Staff agents are project-scoped permanent sessions: every record carries a `projectId`, lives in that project's `staff.json`, and renders in a dedicated collapsible **Staff** sub-section under the owning project in the sidebar (see [internals.md — Staff agents in the sidebar](internals.md#staff-agents-in-the-sidebar)). The staff-creation **assistant session** (`assistantType: "staff"`) is a normal session and appears in that project's Sessions list while open — it is not a staff agent until `propose_staff` is accepted. Forking a permanent staff session creates a new independent staff record through the [session fork endpoint](#fork-session-endpoint), rather than creating an unowned ordinary session or sharing the source staff identity.
...
`contextPolicy` is persisted as `"compact"`, `"preserve"`, or `"clear"`.
Compact is the default for new records and for missing, legacy, unknown, or
malformed values loaded from `staff.json`. Clear awaits the existing durable
session-context clear before each inbox wake; it preserves the staff and Bobbit
session identity/configuration while excluding prior conversation from model
context. See [Staff Inbox Queue — Context policy](staff-inbox.md#contextpolicy).
...
| `DELETE` | `/api/staff/:id` | Delete a staff agent and terminate its session. If that session owns a worktree still used by a fork borrower, returns typed `409 SHARED_WORKTREE_IN_USE` on host or `409 SHARED_SANDBOX_WORKTREE_IN_USE` in a sandbox before changing either staff/session. |
