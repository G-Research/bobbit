...
`workflowId` may be derived from the goal's workflow snapshot. The summary omits `spec`, workflow details, paths, configuration, provider data, and branch/merge internals. This exhaustive allowlist also keeps future unclassified detail fields out of summaries. The detailed `get_goal` projection — used when `view` is omitted, set to `"full"`, or has another non-summary value — retains `spec` without truncation and includes the persisted generic `metadata` object when present. `list_goals` rows and `get_goal(view: "summary")` omit `metadata`; keeping arbitrary metadata behind a single-goal detail read prevents list and summary responses from growing unexpectedly.
