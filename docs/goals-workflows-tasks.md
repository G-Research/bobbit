...
Settings → Workflows exposes the same schema as inline workflow YAML. Authors can edit gate `id`, `name`, `dependsOn`, `content`, `injectDownstream`, `optional`, `manual`, and `metadata`; verification step `type`, command/run source, command-only `env` overrides, prompt, role, component, timeout, phase, description, optional toggle, and `optionalLabel`; and `human-signoff`-specific `label` and `prompt` fields. Command steps show environment overrides in Advanced fields; changing to a non-command type removes them. See [Project command environments](project-command-environments.md) for its inheritance display, validation, and plaintext warning.
