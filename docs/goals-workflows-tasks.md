...
3. **No `workflowId` supplied** — the server selects the first visible workflow in current store order. If the live store is empty, it builds project-derived defaults in memory and selects the first generated workflow instead. The UI mirrors the visible-store case by seeding the workflow `<select>` to the first available id once `fetchWorkflows` resolves.
4. **Generated fallback selected** — creation freezes that validated workflow onto the goal, then persists the generated set only after the goal or current-session promotion commits and only if the store remains empty. A failed candidate or promotion therefore cannot mutate workflow configuration.

These defaults are final goal-creation and user-side acceptance safety nets. Proposal seed validation follows the same precedence for bespoke workflows: a structurally valid `inlineWorkflow` satisfies the workflow requirement and becomes the optional-step validation source. Without a valid `inlineWorkflow`, `propose_goal` must still name an explicit project workflow ID when project workflows are resolvable and non-empty; an omitted selection in an empty store uses the first generated default (see [Canonical goal-candidate validation](#canonical-goal-candidate-validation)).
