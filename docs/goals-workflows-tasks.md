...
The persisted pinned-checkout attestation is v1 for one repository and v2 for a multi-repository layout. A v2 attestation records the aggregate digest plus ordered per-repository commit/digest identities, so post-acquisition step reuse cannot treat a change in one repository as equivalent to a pass from another. The route-level whole-gate fast path has only a preliminary branch-container witness and cannot establish a current v2 repository manifest; multi-repository signals therefore proceed to fresh pinned verification. A whole-gate cache materialization copies the prior attestation after its preliminary witness check and creates no checkout lease because no process executes. Legacy or incomplete attestations are cache misses.
