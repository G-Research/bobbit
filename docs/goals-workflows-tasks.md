...
On a real transition, `goal_state_changed` reaches UI principals (`admin` or `localhost`) so sidebar, dashboard, widget, and other-tab goal caches refresh from the goal-list API. Sandbox-token sockets do not receive this unrelated goal-list invalidation; unchanged goal-scoped delivery continues to serve own-goal runtime updates. The client coalesces bursts through its existing session-list push debounce, preserving prompt browser freshness while avoiding redundant refreshes. The status widget treats its lifecycle field as a reversible mirror, not a one-way completed latch. It watches the authoritative app goal cache while open, so completion performed by another agent or tab appears without remounting; on reset it applies the response and affected pending gates immediately, then refreshes goal, gate, and active-verification state. Its own goal-scoped viewer subscription performs the same authoritative refresh on external `goal_state_changed`, including reopen when no active chat socket is mounted.
