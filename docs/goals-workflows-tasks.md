...
Nested children always use the per-root scheduler. A child that is otherwise eligible may begin its own setup when scheduled, but its team starts only after that child's readiness has been verified. A dependency- or capacity-blocked child may retry to repair its worktree, but remains teamless and `blocked` until the scheduler releases it; repairing setup neither completes dependencies nor treats the child as normally paused. Every path that actually acquires and then releases a child-capacity permit drives the scheduler again, including a verification-created child rejected by final pre-creation validation. This prevents a failed candidate from stranding an eligible queued sibling, while the validation failure itself creates no child state. Operator-paused lifecycle semantics are also preserved: setup recovery does not itself resume a paused goal.
...
- [`tests/unit/core/parallel-goal-setup-repro.unit.test.ts`](../tests/unit/core/parallel-goal-setup-repro.unit.test.ts) covers same-goal single-flight and atomic active-error clearing.
- [`tests/unit/core/setup-ready-team-start-guard.unit.test.ts`](../tests/unit/core/setup-ready-team-start-guard.unit.test.ts) covers the Team Manager readiness boundary.
- [`tests/integration/gateway/shared-git-config-coordination-repro.gateway.test.ts`](../tests/integration/gateway/shared-git-config-coordination-repro.gateway.test.ts) deterministically exercises parallel root/child shared-config coordination and postcondition reconciliation.
- [`tests/integration/gateway/api-goals-child-autostart-ready.gateway.test.ts`](../tests/integration/gateway/api-goals-child-autostart-ready.gateway.test.ts) covers child scheduler and blocked-retry behavior.
- [`tests/browser/journeys/goal-setup-recovery.journey.spec.ts`](../tests/browser/journeys/goal-setup-recovery.journey.spec.ts) covers the live UI and reload after recovery.
...
4. Replace the agent bridge in place with canonical `team-lead` prompt, goal/team tools, and `BOBBIT_GOAL_ID`, then resume the same transcript.
5. Finalize the team reservation and transition the ready goal to `in-progress` before admitting any promotion kickoff.
6. Admit one system-authored kickoff to the existing session's durable prompt path and transcript, with first-user-message title generation suppressed. Bobbit substitutes the accepted goal title exactly:
...
This ordering ensures the lead cannot act until its canonical runtime, team ownership, and actionable goal state agree. The kickoff is specific to current-session promotion; ordinary new-worktree `startTeam()` creation and kickoff behavior are unchanged.

The Bobbit session ID, transcript file, title, connected clients, queued work, model/thinking tuple, `cwd`, branch, single- or multi-repository worktrees, sandbox container, and checkout contents stay attached to the same session. Promotion performs no Git checkout, reset, commit, copy, move, rename, or worktree creation, so staged, unstaged, and untracked work is unchanged. The existing transcript remains a prefix of the promoted transcript, followed by the single kickoff and subsequent lead activity. `worktreeOwnerSessionId` is a provenance and recovery link only: the source session remains the checkout and sandbox lifecycle owner.
...
Before the replacement runtime becomes canonical, compensation may remove only the attempt-created empty lead reservation, gates, and adopted goal. It does not touch the source runtime, transcript, sandbox, checkout, or kickoff state, and it sends no kickoff. Once runtime replacement commits, later finalization failures retain the exact goal/session/team graph; retry finalizes and returns that same goal instead of creating another.

Kickoff admission uses durable per-goal idempotency. Concurrent acceptance, an exact retry, and restart recovery all converge on the same accepted kickoff occurrence, so none can enqueue or dispatch a second copy. Keeping this state durable also closes the crash boundary between finalization and prompt dispatch without moving the kickoff ahead of the promotion commit.

On gateway restart, adopted-goal reconciliation runs before ordinary orphan-team recovery. It verifies same-project identity and exact coordinates, repairs an unambiguous missing lead reservation, attachment fields, or workflow gates, and then restores the original session runtime and team subscriptions. For a committed, fully finalized promotion, recovery admits the kickoff only when its durable per-goal state shows that no occurrence was accepted previously. It never calls ordinary team start, creates a session, provisions a worktree, or transfers a sandbox. Ambiguous records fail closed. If the promoted transcript, worktree, or recorded sandbox realm cannot be restored safely, Bobbit preserves the source as dormant rather than archiving it, repairing the adopted checkout, replacing its container, or silently downgrading it to the host.
...
Workflows are **project-scoped only** — there is no system-scope or builtin layer and no config cascade. New normal projects do **not** receive an eager default seed; the project assistant designs workflows from the discovered components and commands, and a project may legitimately persist with zero workflows. At a creation boundary with no selected or stored workflow, Bobbit can validate against project-derived defaults in memory and persist them only after the goal commits. Headquarters can own workflows too: `projectId=headquarters` reads the Headquarters `project.yaml::workflows` block from the aliased server config store. Every step references project-specific `(component, command)` pairs that have no meaning outside the owning project, so cascading would be ceremony around an empty upper layer. See [internals.md — Workflows are project-scoped only](internals.md#workflows-are-project-scoped-only), [No default workflow scaffold](internals.md#no-default-workflow-scaffold), and [Headquarters project](headquarters.md).
...
Goals without workflows still work fine — workflows are optional **at the data-model layer**. Creation boundaries nevertheless freeze a workflow snapshot for new goals. That snapshot may come from an inline definition, an explicit or first visible project workflow, or a project-derived in-memory default when the live store is empty. The goal form intentionally exposes stored and inline authoring choices rather than synthesizing the deferred server default; see [Default workflow resolution](#default-workflow-resolution) and [Goal creation in a zero-workflow project](#goal-creation-in-a-zero-workflow-project).
...
Goal creation never assumes a workflow named `"general"` exists. Canonical server resolution uses this order:
...
2. **Explicit `workflowId` supplied** — the exact id is used. If it does not resolve for the project, canonical validation returns the structured unknown-workflow error; it never replaces an invalid explicit choice with a default.
3. **No selection, visible workflows exist** — ordinary creation selects the first visible workflow in current store order. The UI mirrors this stored-workflow tier by seeding its `<select>` to the first available id once `fetchWorkflows` resolves. `propose_goal` seeds against a non-empty store still require an explicit id so the persisted draft records the agent's choice.
4. **No selection and the live store is empty** — canonical validation builds project-derived defaults in memory, selects the first generated workflow, and freezes it onto the candidate. Ordinary creation persists the generated set only after the goal snapshot exists; current-session promotion waits until its commit succeeds. Both paths write only if the store remains empty, so invalid candidates, failed promotion, and concurrent workflow edits cannot leak or overwrite configuration.

This deferred default is part of the creation contract, not eager project setup. Proposal seed and acceptance validation use the same generated definitions and optional-step semantics when the store is empty. See [Canonical goal-candidate validation](#canonical-goal-candidate-validation) and [Internals — No eager workflow scaffold at registration](internals.md#no-default-workflow-scaffold).

No source file outside seed data, tests, and documentation may use the literal string `"general"` as a workflow default. This is enforced by the pinning test [`tests/unit/core/no-general-workflow-default.unit.test.ts`](../tests/unit/core/no-general-workflow-default.unit.test.ts), which scans `src/server/agent/` and `src/app/` for the string and rejects new occurrences (the role named `"general"` is explicitly allowlisted; it is unrelated to workflows). The pin exists because `"general"` was historically a magic default hardcoded in five places — UI dropdown initial state, accept handler fallback, `GoalManager` lookup, the goal-assistant prompt, and the re-attempt context builder — but workflows are now project-scoped with no system-level builtins, so there is no guarantee any given project has a workflow with that id. Hardcoding the string produced confusing `Workflow not found: general` errors on projects whose assistant had generated a bespoke workflow set with different names. The fix routes everything through "first workflow in store" instead, with the pinning test preventing reintroduction. See [Workflows](#workflows) for why workflows are project-scoped.

#### Goal creation in a zero-workflow project

Projects can legitimately exist with zero persisted workflows, including immediately after registration. That state does not by itself make the canonical creation API fail: an omitted selection reaches the generated in-memory tier in [Default workflow resolution](#default-workflow-resolution). The interactive form keeps a more explicit authoring experience:

- **Goal preview panel** (the +New Goal picker and the goal-assistant accept panel) renders an **empty-workflows banner** with a brief explanation and an **Open Project Assistant** button. The button calls `createProjectAssistantSession(project.rootPath, …)` from `src/app/dialogs.ts`, passing the linked project's id and existing name. The Accept button is disabled while the banner is visible, with a tooltip explaining why. A proposal with a valid `inlineWorkflow` is the exception: the inline snapshot supplies the workflow, so the banner stays hidden and Accept remains enabled. This is UI guidance toward deliberate authoring, not the server's missing-workflow recovery contract.
...
- **Goal assistant LLM** receives guidance from `SessionManager._buildWorkflowList()` (via the exported `buildWorkflowListText()` helper) when the project has no registered workflows. It recommends authoring a registered workflow through the project assistant and documents a valid inline workflow as the proposal-time escape hatch. Those recommendations help users inspect and tailor the resulting gates; they do not replace the canonical empty-store default used by creation-equivalent APIs.

Project-assistant workflow authoring is therefore recommended, not required solely because the persisted store is empty.

The per-project workflow availability is cached client-side (`_workflowCacheByProject` in `src/app/render.ts`) so switching back to a project that has workflows does not refetch unnecessarily. Browser journey coverage lives in [`tests/browser/journeys/ui/goal-empty-workflows-banner.journey.spec.ts`](../tests/browser/journeys/ui/goal-empty-workflows-banner.journey.spec.ts).
...
Browser coverage lives in [`tests/browser/journeys/ui/goal-metadata.journey.spec.ts`](../tests/browser/journeys/ui/goal-metadata.journey.spec.ts), [`tests/browser/fixtures/goal-tabs-wiring.fixture.spec.ts`](../tests/browser/fixtures/goal-tabs-wiring.fixture.spec.ts), [`tests/browser/journeys/ui/goal-role-tabs-wiring.journey.spec.ts`](../tests/browser/journeys/ui/goal-role-tabs-wiring.journey.spec.ts), and [`tests/browser/journeys/ui/goal-proposal-workflow-tab.journey.spec.ts`](../tests/browser/journeys/ui/goal-proposal-workflow-tab.journey.spec.ts). They pin the default Goal tab, required tab visibility, Metadata-tab-only editor placement, Sub-goals flag behavior, metadata edit persistence, inline Workflow/Roles customization wiring and persistence, bespoke inline workflow labels and submission, active-tab/draft isolation across contexts, historical revision isolation, and accepted-goal metadata persistence.
...
On failure (any rejection from `createGoal`), the assistant session, chat history, draft, `gatewaySessions` entry, and form state are left intact. The standard `showConnectionError("Failed to create goal", …)` toast surfaces the error message — the user can edit the proposal (e.g. replace an invalid explicit workflow id or fix the title) and click Accept again, or ask the assistant to revise. This avoids the prior failure mode where a structured creation error would dismiss the assistant and force the user to start over with no context. Re-attempt sessions (with `reattemptGoalId`) go through the same accept handler and benefit from the same guarantee. Browser journey coverage lives in [`tests/browser/journeys/ui/goal-accept-failure-keeps-assistant.journey.spec.ts`](../tests/browser/journeys/ui/goal-accept-failure-keeps-assistant.journey.spec.ts).

#### Canonical goal-candidate validation

Every path that can create a goal delegates its creation-shaped input to the same read-only candidate validator. This includes ordinary `POST /api/goals`, `propose_goal` seed, edits and restores that change validated goal frontmatter, proposal acceptance, direct child creation, and verification-harness child creation. Central ownership matters because a proposal is a promise that its current fields are creatable; validating only workflow fields at seed time previously allowed invalid paths, parent relationships, roles, policies, or metadata to survive until Accept.

The validator composes the existing project, execution-directory, workflow, inline-role, nesting, and policy validators. It checks the whole candidate together: bounded title/spec and structured fields; visible project scope; workflow and optional-step selection; parent authorization, project ownership, pause state, and nesting allowance; inline roles; metadata; sub-goal settings; and root-only divergence/concurrency policy. An omitted, `null`, or empty goal spec normalizes to the valid empty string and remains empty across proposal serialization, rehydration, acceptance, and direct creation. A blank title, malformed proposal frontmatter, other non-string spec, or spec beyond its bound remains invalid. The validator returns one normalized snapshot or a structured `{status, code, message, details?}` error and performs no persistence, provisioning, secret resolution, session start, or store mutation.

##### Proposal and creation boundaries

A goal proposal is validated before its temporary file can be promoted to the live draft. Seed, validated edit, worktree-mode update, and restore use the same pre-commit callback, so a failure leaves the previous draft byte-for-byte unchanged. It also creates no revision or history snapshot, opens no workspace tab, emits no proposal update or success marker, and acquires no promotion reservation. A failed `propose_goal` tool call keeps its input and structured failure in the transcript so the renderer can show an actionable failed proposal without presenting it as a persisted revision.

Accept re-runs canonical validation from current project, workflow, parent, ownership, nesting, and limit state. The ordinary create path first validates before environment preparation, performs asynchronous repository/worktree support preflight, then rebuilds and validates the candidate immediately before the synchronous coordinate/config-bound create. Child creators follow the same order and re-fetch mutable parent, sibling, workflow, role, and nesting state after preflight. Raw caller-supplied workflow and role values are carried through that refresh rather than normalized away, so malformed input reaches the canonical validator and cannot become trusted inheritance accidentally.

A final validation or stale-preflight failure creates no goal, task, gate, team, worktree, default-workflow write, or other partial goal state. The proposal and its revision remain available for correction, and optimistic-concurrency behavior is unchanged. Generated workflow defaults are validated in memory. Ordinary creation persists them only after the goal snapshot exists; current-session promotion waits until the goal/session promotion commit succeeds. Both paths recheck that the workflow store is still empty. This ordering prevents a failed acceptance from leaking convenience configuration.

##### Execution-directory authority

`projectId` selects scope; `cwd` never does. Omitted `cwd` defaults to the selected project's root. User-authored proposal and ordinary creation input may name only that root or a contained path. Containment resolves symlinks and junctions through the longest existing prefix, permits a non-existent suffix below a valid root, normalizes separators, and uses filesystem-aware path identity so Windows case aliases compare consistently without granting a cross-project path. A symlink escape, another project's worktree, or a stale/forged ownership record fails with `422 CWD_OUTSIDE_PROJECT`.

A Bobbit-owned worktree outside the project root is accepted only from an authenticated server-owned source:

- **Current-session promotion** derives the project, worktree/sandbox coordinates, and `cwd` from the proposal-owning session's current live and durable records. Acceptance re-resolves those records and requires the candidate path to match the derived coordinate.
- **Child creation** derives its execution coordinate from the authenticated parent goal or verification context and binds it to that parent.

Callers cannot obtain either allowance by placing a session, goal, or ownership id in proposal fields. The durable proposal stores the user's worktree-mode choice, not reusable path authority. This prevents transient ambient session ownership from becoming a privilege that survives until a later acceptance. See [Promote the current session in place](#promote-the-current-session-in-place) for the broader adoption lifecycle.
...
For registered workflows, explicit ids win. Ordinary creation with no explicit selection chooses the first currently generated or visible workflow in store order; no layer assumes an id named `general`. Proposal seeds still require an explicit registered workflow when the project already exposes choices, unless they carry a valid inline snapshot. Optional-step validation uses the chosen workflow's canonical optional step names. During current-session acceptance, an explicitly supplied `enabledOptionalSteps` value — including an empty array — replaces serialized draft `options`; omission inherits the draft selection. This distinction lets a user deliberately clear stale options without making absence destructive.

Structured workflow failures retain their established guidance: `MISSING_WORKFLOW` and `UNKNOWN_WORKFLOW` can list available workflows, while `UNKNOWN_OPTIONAL_STEP` can list valid optional step names. Other candidate failures use the same status/code/message family at proposal and creation boundaries. The proposal renderer consumes that structured result and lets the agent or user correct and resubmit the candidate. The route-level response shapes are documented in [REST API — Proposal drafts](rest-api.md#proposal-drafts), and draft persistence is described in [Internals — Editable proposals](internals.md#editable-proposals).
...
- `defaults/workflows/*.yaml` no longer exists. There is no shipped-builtins location for workflows; `BuiltinConfigProvider.getWorkflows()` returns `[]`. The project assistant can author project-specific workflows from the MD guide, while empty-store goal creation derives its deferred defaults through the canonical validator. See [No eager workflow scaffold at registration](internals.md#no-default-workflow-scaffold).
...
- [`tests/unit/core/gate-store-logic.unit.test.ts`](../tests/unit/core/gate-store-logic.unit.test.ts) — gate-store reset and dependency behavior.
- [`tests/integration/gateway/gate-reset-invalidation.gateway.test.ts`](../tests/integration/gateway/gate-reset-invalidation.gateway.test.ts), [`gate-reset-state-policy.gateway.test.ts`](../tests/integration/gateway/gate-reset-state-policy.gateway.test.ts), [`gate-reset-runtime.gateway.test.ts`](../tests/integration/gateway/gate-reset-runtime.gateway.test.ts), and [`gate-reset-lifecycle.gateway.test.ts`](../tests/integration/gateway/gate-reset-lifecycle.gateway.test.ts) — DAG invalidation, idempotency, preserved history/content, active verification cancellation, sandbox denial, team lead notification, and completed-goal lifecycle.
- [`tests/browser/journeys/ui/goal-status-widget.journey.spec.ts`](../tests/browser/journeys/ui/goal-status-widget.journey.spec.ts) — passed-gate View/Reset controls, dashboard focus route reload/back behavior, confirmation guard, widget/sidebar/dashboard refresh, and team lead message visibility.
...
- [`tests/unit/core/gate-dependency-enforcement.unit.test.ts`](../tests/unit/core/gate-dependency-enforcement.unit.test.ts) — a bypassed upstream gate unblocks dependents like a passed one.
- [`tests/integration/gateway/gate-bypass-api.gateway.test.ts`](../tests/integration/gateway/gate-bypass-api.gateway.test.ts) — endpoint happy path, audit-signal persistence, broadcast, guard/validation errors, sandbox denial, reset-to-pending, and completion gating.
- [`tests/browser/journeys/ui/gate-bypass.journey.spec.ts`](../tests/browser/journeys/ui/gate-bypass.journey.spec.ts) — Bypass button visibility, why/who form submission, bypassed row state, red `(N/N)!` badge, and persistence across reload.
