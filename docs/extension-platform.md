...
3. **Activate deliberately.** Enable the pack/contribution in the installed activation catalogue, then enable its project runtime target and supply required settings. These are separate eligibility gates: a setting cannot revive an uninstalled or activation-disabled contribution, and none of these controls launches a managed-service process.
4. **Grant only the exact capability required.** In Market's existing **Review grants** control, review and confirm either a named `(pack, hook, capability)` tuple or the Pack row's `(pack, pack principal, capability)` tuple. The pack-only values are `service.manage`, `memory.read`, `memory.write`, `memory.reflect`, `memory.invalidate`, and `memory.read.all`; hook capabilities remain separately declared and exact. Activation is never a grant. A grant does not add a raw Host API or let a pack write project configuration. See [Extension capability grants](extension-capability-grants.md).
...
|---|---|---|
| Install/activation | Makes a declared contribution eligible for resolution. | Grants a hook authority, launches a managed service, or approves a change. |
| Project settings | Supplies typed, project-local configuration and project/target enablement. | Installs a pack, launches a managed service, reveals a secret, or bypasses activation/grants. |
| Exact grant | Lets core use one active hook for one declared capability, or one active pack principal for one of the six closed platform-owned non-hook capabilities. | Launches a managed service, adds a Host API method, permits arbitrary code, or authorizes another principal. |
...
| EP-11 | Durable extension advisories and typed deferrable/consent-required decision requests. | [Extension decision requests](extension-decision-requests.md) |
...
- **Services use a gateway-owned worktree runtime.** The gateway constructs the lifecycle manager and coordinator, derives the exact worktree instance, rechecks settings and deny-wins `service.manage`, and injects closure-bound `host.services.call()` into server hosts. Packs receive no service path, process, socket, or transport. No built-in consumer has registered a closed operation adapter and compatible launcher, so declarations fail closed; listing, activation, settings, and grants never launch a service. See [Managed service-extension contract](service-extension-runtime.md).
