...
| Exact grant | Lets core use one active hook for one declared capability, or one active pack principal for one of the seven closed platform-owned non-hook capabilities. | Launches a managed service, adds a Host API method, permits arbitrary code, or authorizes another principal. |
