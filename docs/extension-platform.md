...
4. **Grant only the exact capability required.** In the hook's Market grant control, review and confirm the named `(pack, hook, capability)` tuple. Activation is never a grant. Common capabilities are `decide`, `mutate`, `filter:tool-result`, `prompt:system-static`, and `prompt:system-author`; a grant does not add a raw Host API or let a pack write project configuration. See [Extension capability grants](extension-capability-grants.md).
...
| Exact grant | Lets core use one active hook for one declared capability. | Adds a Host API method, permits arbitrary code, or authorizes another hook. |
...
| EP-6 | Per-project exact capability grants, revoke behavior, and grant audit. | [Extension capability grants](extension-capability-grants.md) |
...
- **Services are declarative today.** The managed-service declaration/registry/lifecycle contract is implemented, but no gateway consumer starts these services yet. A runtime entry currently starts nothing. See [Managed service-extension contract](service-extension-runtime.md).
