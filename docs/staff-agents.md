...
- **Editing.** The staff edit page (`#/staff/<id>`) can change name, description, system prompt, triggers, role, colour, accessory, and pinned context. The current runtime cwd and sandbox state are shown read-only. Prompt-related settings and triggers live on separate sub-tabs. For the trigger-type reference (including the push-based `goal_created` / `goal_archived` types and their required-prompt rule) see [staff-triggers.md](staff-triggers.md).
...
Rather than ship a half-correct migration that strands work, sandbox mode is intentionally fixed for the staff's lifetime. The same reasoning applies to sessions. `PUT /api/staff/:id` therefore rejects a `sandboxed` field with `400` naming that field; it never reports a successful edit while quietly preserving the old mode.
...
- **REST.** `POST /api/staff` accepts `sandboxed?: boolean`, `worktree?: boolean`, `accessory?: string`, and `roleId?: string`. `GET /api/staff` and `GET /api/staff/:id` return the stored `sandboxed` value and normalised persisted `accessory`. `PUT /api/staff/:id` accepts `accessory` (mirrored to the current staff session when present) and `roleId` (string to set, `null` to clear; unknown role → 404, non-string non-null → 400); it rejects `sandboxed` with `400` because sandbox mode cannot change after creation.
...
- **Tests.** `tests/integration/gateway/staff-cwd-parity.gateway.test.ts`, `tests/integration/gateway/staff-patch-reassign.gateway.test.ts`, and `tests/browser/fixtures/proposal-review-fixture.fixture.spec.ts` pin the project/cwd/worktree invariants. `tests/integration/gateway/staff.gateway.test.ts` and `tests/browser/journeys/ui/staff-sandbox-indicator.journey.spec.ts` pin sandbox persistence and immutability.
