...
Panel poll loop:
  └─ host.callRoute("status", {childSessionId, jobId})
       status route (worker):
         ...
         submitted   ← store.get(submittedKey(jobId))
         if submitted → {phase:"submitted", yaml, baseSha, headSha}; host.agents.dismiss(child)
         elif agentStatus==="terminated" → {phase:"error", …}; (no submission)
...
**Group 1 — OrchestrationCore + server wiring** (`src/server/agent/orchestration-core.ts`, `src/server/server.ts`)
...
5. **Session-secret verification plumbing.** `verifyCallerSession` adds a header + a `sessionSecretStore` lookup to `routes.ts`. It is routing-grade (not a security boundary), so a missing secret should degrade to the existing `sandboxScope` check rather than hard-fail in non-sandbox contexts. *Mitigation:* treat the secret as a strengthening check; keep the `sandboxScope` gate as the floor.
