...
- `src/server/pr-walkthrough/walkthrough-agent-manager.ts`
  ...
  - Historical `WALKTHROUGH_ALLOWED_TOOLS = ["readonly_bash","read_pr_walkthrough_bundle","submit_pr_walkthrough_yaml"]` (`:198-202`); the current pack role grants the full PR Walkthrough reviewer group.
...
- `src/server/pr-walkthrough/walkthrough-readonly-policy.ts` — the `readonly_bash` allowlist. Pinned by `tests/unit/core/pr-walkthrough-readonly-policy.unit.test.ts`.
...
- The agent tools `market-packs/pr-walkthrough/tools/pr-walkthrough/*.yaml` + `extension.ts` — normal role/tool-policy-resolved tools owned by the pack, with the env-gate + proof header removed (§ Decision C).
...
- **`pr-reviewer` role** (new, `market-packs/pr-walkthrough/roles/pr-reviewer.yaml`): `promptTemplate` = ported `buildRolePrompt` + `REQUIRED_YAML_SCHEMA_PROMPT`; `accessory: review`; `toolPolicies` grant the PR Walkthrough reviewer tools.
- **Agent tools** (`market-packs/pr-walkthrough/tools/pr-walkthrough/extension.ts`): env-gate + proof header removed; bundle and submission tools send only `sessionId` (+ session secret), and the server resolves `jobId` from the binding.
...
**Problem.** `host.agents.spawn({role:"pr-reviewer", readOnly:true})` must grant the child the PR Walkthrough reviewer tools from the `pr-reviewer` role — NOT the owner's tools — and the child must be a **visible** session. Today `readOnly ⇒ lifecycle:"bare"` → `createDelegateSession`, which ignores `role` and derives tools from the owner (`orchestration-core.ts:255-285,330`).
...
2. **Role-sourced child tools (FAIL CLOSED).** Add an optional dep and use it in `childAllowedTools`. A role-carrying spawn is granted the ROLE's tools and **never** falls back to the owner's tools — if the role's grants cannot be resolved, the spawn throws:
   ...
   For `pr-reviewer`, `resolveRoleAllowedTools("pr-reviewer")` returns the PR Walkthrough reviewer tools (the role's `toolPolicies` grant that group, and the "PR Walkthrough" group is default-deny — see Decision C); none are spawn verbs or in `READ_ONLY_DENY_TOOLS`, so the child gets the scoped reviewer toolset. **No regression / no isolation hole:** the owner-derived fallback path remains ONLY for role-LESS delegate/team spawns (unchanged) — it is reached exclusively when no `role` is set. A role spawn whose resolver is unavailable or returns empty does **not** silently inherit the owner's broader tools; it throws `ROLE_TOOLS_UNRESOLVED`. Tests exercising role spawns MUST wire `resolveRoleAllowedTools`; the `run` route surfaces `ROLE_TOOLS_UNRESOLVED` as a **retryable** launch error (Decision E).
...
- New file `market-packs/pr-walkthrough/roles/pr-reviewer.yaml`:
  ...
  accessory: review
  toolPolicies:
    # Group-level grant for the "PR Walkthrough" group (the reviewer tools' group).
...
|---|---|
| Run mints a NEW read-only reviewer session; user's agent NOT prompted/modified | **New** API E2E `tests/e2e/api/pr-walkthrough-host-agents.api-e2e.spec.ts`: call `run` route; assert a new session with `childKind:"host-agents"`, `role:"pr-reviewer"`, `readOnly:true`, `accessory:"review"`; assert the owner transcript has **no** injected user message (the anti-`postMessage` assertion). |
| Reviewer runs toolchain + submits; panel renders SAME cards (byte-stable) | **Ported/kept** `tests/unit/core/pr-walkthrough-card-synthesis.unit.test.ts`, `tests/unit/core/pr-walkthrough-diff-parser.unit.test.ts`, `tests/unit/core/pr-walkthrough-yaml-schema.unit.test.ts` (pure synthesis, unchanged). **Kept** `tests/integration/gateway/pr-walkthrough-api.gateway.test.ts` `bundle`/`publish` parity. **New** browser E2E (below) asserts the rendered cards. |
| Read-only enforced (write/commit/disallowed blocked) | **Kept** `tests/unit/core/pr-walkthrough-readonly-policy.unit.test.ts` (the policy is unchanged). **New** assertion in the spawn test that the reviewer's `allowedTools` are the PR Walkthrough reviewer tools (no `write`/`edit`/`bash`). |
...
| Scope: pack drives only its own reviewer child | **Kept** `tests/unit/core/host-agents-scope.unit.test.ts` (source-filtered owner scope) — extend with a `pr-reviewer` child to confirm a sibling delegate/team child is invisible. |
| `npm run check` + unit + e2e green; full browser E2E | **New/ported** `tests/e2e/browser/pr-walkthrough-pack.browser-e2e.spec.ts` (replace the postMessage flow): Run → reviewer child appears in sidebar → submits → panel publishes the same cards → reviewer cleaned up; read-only denial; idempotent re-run. Use the e2e mock agent (canned, non-flaky → e2e phase, never test:manual), as `tests/e2e/api/host-agents.api-e2e.spec.ts` does. |
...
3. **Group default-deny blast radius.** Setting `PR Walkthrough: deny` as a default group policy could hide the tools from any existing flow that relied on env-gated registration. *Mitigation:* the only consumer is the reviewer (granted via role); add the unit test that `general` lacks submit and that `pr-reviewer` has the reviewer tools.
