...
| Read-only enforced (write/commit/disallowed blocked) | **Kept** `tests/pr-walkthrough-readonly-policy.test.ts` (the policy is unchanged). **New** assertion in the spawn test that the reviewer's `allowedTools` are the PR Walkthrough reviewer tools (no `write`/`edit`/`bash`). |
