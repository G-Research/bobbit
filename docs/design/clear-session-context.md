...
`contextClearBoundaries` is recovery-critical and is published in the same session-record generation as `agentSessionFile`. The latest boundary's activated path/materialization marker is updated atomically with the active pointer when an intentionally empty generation is recreated on boot, and is marked materialized once when Pi flushes its first assistant message. It is not count-truncated: dropping an old record would silently destroy a durable transcript affordance. Disk purge of the Bobbit session is the retention boundary.
...
### 7.1 Restart and respawn

Pi intentionally leaves a cleared generation's JSONL absent until its first assistant message. `restoreSessions()` must distinguish this valid state from transcript loss:

1. If the active path exists, restore normally.
2. If it is missing, accept it as intentionally empty only when the latest normalized boundary has the same `activatedAgentSessionFile` and `activatedTranscriptMaterialized === false`.
3. Recreate a fresh Pi runtime without `switch_session`, reapply/verify the persisted model and thinking tuple, require no restored transcript messages, then atomically update both `agentSessionFile` and that latest boundary's `activatedAgentSessionFile` to the fresh reported path. Keep the marker false. Preserve `wasStreaming`, queue, and in-flight occurrence evidence: if accepted successor work had started before the crash but Pi had not yet flushed an assistant, the existing reliable-turn boot recovery—not an invented transcript row—owns its exact redrive/parking decision.
4. Once Pi flushes the first assistant message, the normal metadata/lifecycle path verifies the file exists and atomically flips the latest matching marker true. A later missing file with marker true is transcript loss and follows existing fail-closed recovery; it is never silently treated as another clear.

This narrow branch also supports two clears with no intervening turn. The second boundary records its previous segment as unmaterialized/empty, so expansion returns zero rows without inventing a file. No synthetic message is ever used merely to force Pi persistence.

After materialization, agent respawn uses the active pointer exactly as today. Historical paths are never passed to `switch_session` except during explicit rollback. Repeated restart/respawn cannot restore an earlier clear segment into model context.
...
`restoreSessions()` must add every validated `previousAgentSessionFile` and current `agentSessionFile` to the orphan-transcript tracked set. Convert container-coordinate files to the comparable host bind-mount identity before comparing with the host scan; raw container strings do not match `scanOrphanedTranscriptsAsync()` host paths.
...
| Core: repeated clear | A/B/C | two stable ordered boundary pairs; A reads only A, B only B; no boundary row in Pi; no duplicate cards on repeated snapshot/event/reload |
...
| Integration: non-leak | next provider request | prior user, assistant, tool input/result, compaction summary, `/clear`, and synthetic boundary absent; current assembled system prompt and tools present |
...
7. Repeated clear, lazy empty-generation restart, materialized respawn, host/sandbox reads, orphan tracking, and purge preserve exact segment ownership.
