...
The `enum` is the typo-proof menu. `args` accepts either a free-form object
or a JSON-encoded object string because some OpenAI-Completions models encode
the nested object as a string. The generated extension parses a top-level
string once and requires the resulting value to be a plain object before
dispatch. Per-operation arg validation remains the underlying MCP server's
job (re-emitting every op's full schema would defeat the entire context-bloat
fix). The explicit open-object branch also avoids the unconstrained
`Type.Any()` schema that Bedrock rejects.
...
[meta extension on the agent side]
  ...
  normalizes args to a plain object or returns invalid_args locally
       │
 ...
 *      - normalize params.args from either a plain object or JSON-encoded object string;
 *        reject every other shape locally as invalid_args
 *      - POST { tool: `mcp__${serverName}__${params.operation}`, args: normalizedArgs }
