...
- `tests/unit/core/missing-live-messages-repro.unit.test.ts` — real `SessionManager` red→green: concurrent dormant `addClient()` revives join one restore and every attached client receives the post-revive assistant frame; a second case asserts stale-generation recover/drain writers no-op without bumping `statusVersion` or dispatching.
- `tests/browser/journeys/ui/dormant-revive-live-reply.journey.spec.ts` — spawned-gateway browser E2E: attach a client, let the session hibernate, send a prompt, assert the reply renders live (no reload) and the status returns to a correct idle.
