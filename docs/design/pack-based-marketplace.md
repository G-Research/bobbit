...
- Authored pack `listName` remains the activation identity for authored MCP packs. Gateway installs derive a stable contribution id from source id (or source URL fallback), install/provider/list/public-server/sub-namespace identity for `DisabledRefs.mcp` and `DisabledRefs.mcpOperations`, so activation never leaks across sources with the same list name and never drifts when gateway fingerprints or runtime keys change.
