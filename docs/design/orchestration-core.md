...
| 4 | `market-packs/pr-walkthrough/` (pack) | The launch privilege was **deleted** — re-expressed (interim) as a client `host.session.postMessage` gesture driving the user's own agent. | Capability downgrade; no new principal. *(Now superseded: the pack spawns a real, isolated read-only `pr-reviewer` child via the shipped `host.agents` capability — see [pr-walkthrough-launch-ux.md](pr-walkthrough-launch-ux.md).)* |
