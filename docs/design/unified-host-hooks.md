...
Update `docs/extension-host-authoring.md`, `docs/lifecycle-hub.md`, staff trigger/inbox documentation, Host API contract/version reference, WebSocket reference, REST/project snapshot refresh guidance, and marketplace fixture documentation. Place every new test in its canonical semantic cell with the matching suffix; add no tests outside the canonical `tests/` root.
...
- Contributions/activation: `tests/unit/core/pack-contributions.unit.test.ts`, `pack-providers-loader.test.ts`.
...
- Domain durability: `goal-store-sqlite.test.ts`, `goal-task-store-lifecycle.test.ts`, `task-state-machine.test.ts`, `task-generation.test.ts`, `gate-store-logic.test.ts`, `gate-store-sqlite.test.ts`, `project-config-store-durability*.test.ts`, and `tests/integration/gateway/history-fork-*.gateway.test.ts`.
- Staff compatibility: `staff-trigger-engine.test.ts`, `goal-trigger-dispatcher.test.ts`, `tests/integration/gateway/staff-goal-triggers.gateway.test.ts`, `inbox-manager.test.ts`, and `inbox-store.test.ts`.
...
Acceptance is complete only when every catalogue entry has exactly one tested authoritative publisher, public payload privacy tests pass, cross-project attempts are silent, existing provider/panel/staff compatibility tests remain green, and all new files use canonical paths and semantic suffixes.
