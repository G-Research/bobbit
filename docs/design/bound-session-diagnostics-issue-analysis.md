...
type WrapperDiagnostics =
  | {
      omitted: true;
      actualBytes: number;
      measurementOmitted?: never;
      measurementReason?: never;
    }
  | {
      omitted: true;
      actualBytes?: never;
...
The `extension_return_unrecognized` variant has exact base values `total: 0`, `returned: 0`, `offsetStart: -1`, `offsetEnd: -1`, and `messages: []`; `matchCount`, `pageStart`, `pageCount`, `nextOffset`, `authors`, and `correlations` are absent because they cannot be recovered. Its `continuationRequest` is the bounded retry allowlist defined below. `wrapperDiagnostics` is required and must be exactly one of two mutually exclusive branches, with no additional keys:

- `{ omitted: true, actualBytes }` reports a non-negative safe integer equal to the discarded actual wrapper's exact UTF-8 JSON byte length. It is legal only when a side-effect-free, JSON-exact snapshot proves that measurement.
- `{ omitted: true, measurementOmitted: true, measurementReason: "unsafe_unmeasurable" }` reports that an exact measurement could not be obtained safely. It contains no `actualBytes` estimate.

A missing diagnostic branch, both branches at once, or `{ omitted: true }` alone is invalid. No other envelope variant may carry `wrapperDiagnostics` or retry fields.
...
An unrecognized **successful** legacy shape is also fail-bounded, not passed through and not converted to a transport error. The boundary emits the legal `extension_return_unrecognized` variant above with exact empty/unknown base values, a fixed-schema `continuationRequest`, and one required `WrapperDiagnostics` branch. The exact branch is preferred for ordinary JSON data. The boundary must instead use `unsafe_unmeasurable` when proving the byte count would require unsafe observation or exceed bounded snapshot work. It detects proxies without entering their traps, inspects property descriptors without reading accessors, and never invokes getters, setters, custom `toJSON`, or other extension-controlled code merely to produce diagnostics. Safety takes priority over a size statistic.

A fabricated value is forbidden: do not substitute `0`, `Number.MAX_SAFE_INTEGER`, a work ceiling, a cap-plus-one marker, or the size of a sanitized replacement for the discarded wrapper. `actualBytes` is an audited fact that callers may compare, so a sentinel would silently turn “unknown” into false data. The explicit unmeasurable branch preserves the reason without weakening the final-output ceiling; the small canonical fallback itself is still measured and bounded normally.

`RetrySameRequest` has the exact required fields `kind: "retry"` and `retrySameRequest: true`; its optional allowlist is a well-formed `session_id` truncated at a scalar boundary to 64 UTF-16 units with `sessionIdTruncated` (ill-formed IDs are omitted); safe-integer `offset`/`limit`; booleans `case_sensitive`, `verbose`, and `include_tool_results`; integer `context` clamped to 0–5; and `patternOmitted: true` when a pattern was present. It never echoes pattern text, result handles, unknown parameters, or arbitrary strings. Invalid/out-of-range scalars are omitted rather than stringified. Consequently even a multi-megabyte input pattern cannot grow this prebuilt fallback beyond its separately asserted 1-KiB ceiling. It does not invent `nextOffset` when the unknown wrapper did not expose one and never includes an excerpt of the unknown return. This compatibility partial tells the caller to retry the already-known request after the override is updated while maintaining the hard ceiling. Existing consumers that only need omission/retry semantics may continue to read `omitted` and `continuationRequest`; consumers that report wrapper size must discriminate on `actualBytes` versus `measurementReason`. The branch is agent-bound and additive to the compatibility partial, so direct REST and browser payloads remain unchanged. Recognized upstream success that merely exhausts the byte budget uses the legal `transport_budget` variant and retains the exact `nextOffset`/result cursor described below.
...
11. One valid complete envelope, both valid `transport_budget` continuation variants, both valid `extension_return_unrecognized` wrapper-diagnostic branches, and each invalid cross-variant/missing-required-field case listed in §6.4.2.
...
Unrecognized-wrapper tests split by measurability. An ordinary plain-data wrapper requires exact `actualBytes` equal to `Buffer.byteLength(JSON.stringify(wrapper), "utf8")`. Throwing accessors, custom serializers, proxies, and wrappers beyond bounded snapshot work require the exact `unsafe_unmeasurable` branch, with zero getter, serializer, or proxy-trap invocations and no fabricated `actualBytes`. Each case also uses a multi-megabyte input `pattern`, requires the exact retry continuation, stays under 1 KiB, and proves no pattern/wrapper sentinel is echoed. Run downstream lifecycle mutation fixtures through the same matrix so a received upstream compatibility partial preserves exactly one legal diagnostics branch. Negative schema fixtures remove each required variant field, combine both diagnostics branches, add unknown diagnostics fields, and mix diagnostics/continuation kinds to prove validation fails. These unit tests complement, but do not replace, the real stale-winner lifecycle proof below.
...
5. In each winner, configure the resolved stale execute path to return unknown wrappers for a valid call carrying a multi-megabyte `pattern`. A measurable plain-data wrapper must preserve exact `wrapperDiagnostics: { omitted: true, actualBytes }`. Hostile accessor/proxy/custom-serializer and over-work-limit wrappers must preserve `{ omitted: true, measurementOmitted: true, measurementReason: "unsafe_unmeasurable" }` without invoking extension-controlled code. Assert the actual emitted and persisted values validate as `extension_return_unrecognized`, are each under 1 KiB (therefore also under 50 KiB), contain exactly one diagnostics branch plus the exact retry continuation and `patternOmitted: true`, and contain neither the pattern nor unknown-wrapper sentinels.
