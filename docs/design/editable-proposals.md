...
- `tests/e2e/ui/proposal-panel-subsection-diff.spec.ts` — diff view. (Removed with the dead `ProjectProposalPanel.ts` module — superseded by the form-based editors in `src/app/proposal-panels.ts`.)
