...
`WorktreePool` construction performs no Git or filesystem I/O. Coalesced `initialize()` asynchronously resolves the Git top-level, validates exact v1 durable pool records against every repository's current Git worktree identity and live-session guards, adopts only complete matches, then fills the shortfall. It never scans for shape-based candidates. Failed repository-path resolution remains retryable, and `stop()` prevents new work while repeatedly joining every tracked fill, freshen, claim, and late claim-failure cleanup operation.

On graceful gateway shutdown, the retained boot promise settles before the pool snapshot. The gateway starts a bounded `stop()` for every live pool, flushes the durable ownership record, and retains ready entries for restart reuse. A failure or timeout is isolated. Explicit project removal remains the awaited destructive `drain()` path; drain and tracked claim-failure cleanup force `skipRemotePush: true`.

A crash, forced termination, failed record write, or invalid record may leave an unowned entry. A later boot reports it diagnostically and neither repairs, removes, nor adopts it by branch/path/root shape. See [Preserve user worktrees](preserve-user-worktrees.md) for the authoritative ownership and failure contract.
...
| Post-listen boot work and pools | This is one retained promise rather than an interval. The diagnostic sweep precedes sequential pool initialization; only exact durable records may be re-adopted. | Gateway shutdown awaits boot, starts bounded `stop()` on every live pool, flushes durable ownership, and retains ready entries. Explicit project deletion drains. |
...
- Worktree suites cover deferred event-loop progress, shared ceilings, pure pool construction, same-instance fill/claim, stop/drain barriers, diagnostic boot sweep ordering, exact durable pool-record adoption, archived-record revalidation, shared/container protections, graceful retention, explicit local-only deletion drain, and targeted symlink-safe deletion.
