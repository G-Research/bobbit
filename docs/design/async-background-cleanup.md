...
`WorktreePool` construction performs no Git or filesystem I/O. Coalesced `initialize()` asynchronously resolves the Git top-level, derives the worktree root, reclaims eligible pool entries, and starts fill. Claims use the existing cold-create fallback while initialization is pending. `stop()` prevents new work and repeatedly joins every in-flight fill, freshen, claim, and late failure-cleanup operation; `drain()` stops first and removes entries with bounded, per-item-isolated cleanup.
