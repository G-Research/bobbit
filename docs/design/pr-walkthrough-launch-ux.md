...
On click, the menu action calls `runLauncherEntrypoint(launcher.key, onResult, { sessionId })` inside the user gesture. Spawn launchers emit pending feedback (e.g. `Starting <label>…`, derived from the launcher's own label) via the `bobbit-launcher-feedback` custom event, which renders in a dedicated persistent surface (`data-testid="launcher-feedback"`, not the transient header toast). The `pending` state persists until the launch resolves; `{ ok:false }` results replace it with a dismissible `error` (which does not spawn, switch sessions, or leave the menu wedged open), and success emits `resolved` to clear it. `NO_PR` surfaces as a user-readable message such as `No pull request found for this branch.` See [docs/extension-host-authoring.md § launcher feedback](../extension-host-authoring.md) for the full three-kind contract.
...
| **G-E** (tests) | `tests/e2e/browser/pr-walkthrough-pack.browser-e2e.spec.ts`; `tests/e2e/api/pr-walkthrough-host-agents.api-e2e.spec.ts`; `tests/unit/core/pr-walkthrough-role-tools-policy.unit.test.ts`; new unit for the launcher dispatch | §8. |
