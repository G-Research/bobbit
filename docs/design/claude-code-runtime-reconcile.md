...
**Addendum (verified against the real PR #841 diff, implementation pass):** `gh pr view`/`gh pr diff 841 --repo G-Research/bobbit` were reachable in a later session. The disposition table below held up well — most files are functionally identical to #841 or already stricter (see per-file notes retained as-is). Three genuine, narrowly-scoped gaps were found by diffing the real PR content against `aj-current` and were fixed:

1. **Runtime dispatch seam bypass on the live `set_model` path (highest-severity finding).** `assertRuntimeSwitchAllowed()`/`RuntimeSwitchError` (`session-runtime.ts`) were defined and unit-tested (`tests/session-runtime-selection.test.ts`) but never actually called from the WebSocket `set_model` handler — `ws/runtime-model-selection.ts::applyRuntimeSessionModelSelection()` went straight to `applyModelString()` regardless of runtime. A `set_model` crossing the Pi/Claude Code boundary was not rejected with `RUNTIME_SWITCH_REQUIRES_NEW_SESSION`; it fell through to a generic RPC attempt/failure. The UI model picker already blocks this client-side (`tests/e2e/ui/claude-code-ui.spec.ts`), but the server did not enforce it independently — exactly the "Runtime dispatch seam" risk this doc calls out. **Fixed**: the guard is now called at the top of `applyRuntimeSessionModelSelection()`, and `ws/handler.ts`'s `set_model` catch block preserves the specific `RUNTIME_SWITCH_REQUIRES_NEW_SESSION` code via `isRuntimeSwitchError()`. Pinned by 3 new unit tests in `tests/controlled-model-fallback.test.ts` and a new E2E in `tests/e2e/claude-code-runtime-api.spec.ts`.
2. **`sendFallbackModelState()`/`buildArchivedStateData()` (`ws/handler.ts`) never carried `runtime`/`claudeCodeSessionId`/`claudeCodeModelAlias`.** A client attaching to a Claude Code session whose live `getState()` is unavailable (fresh gateway restart, lazy bridge not yet attached) or an archived Claude Code session (never gets a live `getState()` push at all) had no way to learn it was looking at a Claude Code session from these two `state` pushes. **Fixed**: both now include the fields when `persisted.runtime === "claude-code"`. Pinned by 3 new unit tests in `tests/ws-claude-snapshot-hydration.test.ts`.
3. **`src/app/remote-agent.ts` (missing from this doc's original file table entirely — a blind spot from not having the real diff) never hydrated `runtime`/`claudeCodeSessionId`/`claudeCodeModelAlias` from `state` frames, and never rolled back an optimistically-set `_state.model` after a rejected `set_model`.** `setModel()` sets `_state.model` before the server confirms; the `case "error"` handler never reversed it on `SET_MODEL_FAILED` or `RUNTIME_SWITCH_REQUIRES_NEW_SESSION`, so a rejected switch left the footer showing a model the agent never actually bound to. **Fixed**: added a `_pendingModelRollback` field, hydration of the three runtime fields in `case "state"`, and rollback in `case "error"`. Pinned by 4 new source-level tests in `tests/remote-agent-claude-code-runtime-hydration.test.ts` (same pattern as `tests/snapshot-clears-streaming-message.test.ts` — a live harness would need a full WS+DOM rig).

**Explicitly deferred (found, not fixed — lower priority given AJ's framing above):**

- `session-manager.ts::tryAutoSelectModel()` unconditionally no-ops for Claude Code sessions rather than applying a `role.<role>.model`/`default.sessionModel` preference that targets `claude-code/*` (PR #841 had `tryAutoSelectClaudeCodeModel()` for this). Real gap, but fixing it touches 4 call sites (fresh spawn, restore, role reassignment, force-abort-respawn) and needs restart-based alias switching wired into post-spawn verification — larger blast radius than the three fixes above.
- `AgentInterface.ts` never implements the "last turn duration" footer/popover display or the model-popover's dedicated Claude Code section that `docs/claude-code-runtime.md`/`docs/design/claude-code-capability-notice.md` describe. Cosmetic UI polish; per AGENTS.md every user-facing feature needs a new browser E2E, which is a larger addition than warranted here.
- Minor: no `title` tooltip on the footer model button for Claude Code sessions (only an `sr-only` label). Trivial but skipped to keep this change tightly scoped.
