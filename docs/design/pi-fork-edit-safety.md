...
Scope: whether Bobbit should maintain a fork of `@earendil-works/pi-*` (pinned at `0.79.6` when
this spike was written) for edit-safety and related root-cause fixes, vs. continue working
around pi from Bobbit-core, vs. upstream.

**Update:** the recommendation held — Bobbit upgraded `0.79.6` → `0.80.3` in-place (no fork) via
the UPGRADE-AFTER-import-surface-rework eval verdict; see the pi-0.80.3 upgrade PR. All three
workarounds below still apply post-upgrade, ported to the moved vendor paths (root API →
`/compat` and `/providers/all`, per-API `stream()`/`streamSimple()` → `/api/<name>.lazy`
factories); the Bedrock patch row documents the moved anchor path.
...
| **Bedrock request-header patch** | pi's Bedrock provider builds its own `BedrockRuntimeClient` internally; there is no supported hook to inject custom request headers into that client (AI Gateway needs Bobbit-specific headers on Bedrock traffic). | `src/server/agent/pi-ai-bedrock-headers-patch.ts` — **live-patches the installed vendor file** by string-anchored source rewriting (`patchPiAiBedrockFile()`): finds the import anchor and the `new BedrockRuntimeClient(config)` anchor, splices in a generated middleware-injection helper, rewrites the file on disk, and is idempotent via a marker string. As of the pi 0.80.3 upgrade, `piAiBedrockCandidates()` tries `dist/api/bedrock-converse-stream.js` (the new home of the implementation) first, falling back to the pre-0.80 `dist/providers/amazon-bedrock.js` path (now a thin provider-factory wrapper with neither anchor, kept only so an older installed pi-ai still gets patched) — a target only warns if every one of its candidates fails to patch, so the now-permanently-skipped old path never produces a false "unsupported file shape" warning on 0.80+. Explicitly anticipates upstream anchor drift (per-target warn-on-total-failure branch). Regression-pinned by `tests/pi-ai-bedrock-headers-patch.test.ts` and documented in `docs/pi-0.77-opus-4.8.md` ("Bedrock patch and RPC lifecycle coverage"). | `node_modules/@earendil-works/pi-ai/dist/api/bedrock-converse-stream.js` (pi ≥0.80; anchors: `import { transformMessages } from "./transform-messages.js";` and `const client = new BedrockRuntimeClient(config);`) or `node_modules/@earendil-works/pi-ai/dist/providers/amazon-bedrock.js` (pi <0.80, same anchors). | **Direct vendor source-patch**, applied at process start (`ensurePiAiBedrockHeadersPatch()`), not an extension. This is the highest-risk workaround in the inventory: it edits pi's own installed code rather than routing around it. |
