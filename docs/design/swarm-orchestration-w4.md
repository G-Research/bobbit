...
| 4.3 | Topology-choice heuristic classifier, observe-mode (§3.3 step 2) | S | 4.2 | codex drafts against the tool-approve-heuristic.ts/thinking-router-classifier.ts precedent; **judgment lane reviews the rule table itself** (exactly the kind of review that caught CLF-W2.5's File-System-group-tightening gap) |
