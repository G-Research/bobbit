...
| `entrypoints:` | **IMPLEMENTED (P2)** | Non-chat launchers (composer slash-commands and session-menu items) **and** `kind:"route"` deep-link routes. Parsed by `parseEntrypoints`; launcher clicks dispatch through `runLauncherEntrypoint`: panel targets open a panel, route targets navigate, and spawn targets call a pack route, open the returned child session's panel, and surface `NO_PR`/failure feedback visibly without switching views. Routes populate the client pack-route registry. |
...
| `PrWalkthroughPanel.ts` full-surface viewer | **`panels:`** → mounted by the host in the reviewer child session with `host.ui.openPanel({ panelId: "pr-walkthrough.panel", sessionId: childSessionId, params })` |
| Launch a fresh walkthrough reviewer | **`entrypoints:`** composer-slash and session-menu launchers use `target: { action: "spawn", route: "run", panelId: "pr-walkthrough.panel" }`. The launcher dispatch calls the pack `run` route, mints a fresh read-only reviewer child, opens the walkthrough panel there, and switches to that child. The owner session never mounts a walkthrough panel; `NO_PR`/route failures surface as launcher feedback and spawn nothing. |
| Deep-link to a walkthrough (`#/...`) | **`entrypoints:`** `kind:"route"` registers `routeId:"pr-walkthrough"` → `pr-walkthrough.panel` for reload/restoration inside the reviewer child session. Structured `host.ui.navigate({ route, params })` still maps deep links to `#/ext/pr-walkthrough?...`; it is not the first-party launch path. |
