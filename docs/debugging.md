...
- **Image and workspace checks**: rebuild `bobbit-agent` with Agent SDK `0.3.222`, bundled Claude `2.1.222`, Pi `0.84.1`, runtime schema `2`, the image-owned dependency, and executable `bobbit-claude-agent-sdk` wrapper under `bobbit-sdk`. Bobbit also requires node-owned workspace roots, validates selected-workspace access, and translates the scoped callback to a container-reachable address. A host binary/dependency tree or rewritten callback is not a substitute.
...
- **Credentialed Docker dogfood repeat**: the completed agent-run record is linked from [Pending credentialed Docker dogfood](claude-agent-sdk-sessions.md#pending-credentialed-docker-dogfood); it is G11 evidence, not a parent release decision. For a repeat, first use a fresh owner-only temporary `BOBBIT_AGENT_DIR`, authenticate through a separate loopback Bobbit gateway, and export it to Playwright as `MANUAL_CLAUDE_AGENT_SDK_AUTH_DIR` before directory reset or auth-sensitive imports. Never copy/paste auth data, mix this subscription OAuth with enterprise Anthropic OAuth in one normal instance, or delete the temporary directory before evidence review. Then run `BOBBIT_RUN_CLAUDE_AGENT_SDK_SANDBOX_SMOKE=1 MANUAL_CLAUDE_AGENT_SDK_AUTH_DIR="$MANUAL_CLAUDE_AGENT_SDK_AUTH_DIR" npm run test:manual -- --grep "Docker sandbox lifecycle"`; the default model is the unprefixed `haiku` alias. An idle session may be created before SDK auth/provider construction; the first accepted input is the canonical startup boundary.
