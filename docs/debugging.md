...
- **Symptom**: many leaked walkthrough child node processes (sessions with `childKind: "pr-walkthrough"`, ~100 MB each) accumulate on the host, and they come back after every gateway restart even though the walkthroughs are long finished.
- **Cause**: walkthrough children are first-class **persisted** sessions, but finished walkthroughs were never terminated. `restoreSessions()` respawns every persisted session on boot, so each restart resurrected all of them at once — the count only grew.
- **Where the fix lives**: (1) `WalkthroughAgentManager.terminateChild()` (`src/server/pr-walkthrough/walkthrough-agent-manager.ts`) tears the child down on terminal job states (`ready`, mapping/runtime `error`, non-recoverable launch dispatch error) — but **not** on `validation_failed`, which the agent retries in the same session. (2) `SessionManager.terminateSession` cascades to `childKind === "pr-walkthrough"` children (keyed by `parentSessionId`, not just `delegateOf`), covering both terminate and archive; its worktree-cleanup guard excludes prw children so it never deletes the parent's shared `/workspace-wt/<name>`. (3) `SessionManager.restoreOneSession` reaps terminal/orphaned children on boot via the pure helper `shouldReapWalkthroughChildOnBoot()` (`src/server/pr-walkthrough/walkthrough-reap.ts`) instead of respawning them; genuinely in-flight walkthroughs with a live parent are kept so their rotated submit proof still works. Full design: [docs/design/pr-walkthrough-agent-session.md — Child session lifecycle and teardown](design/pr-walkthrough-agent-session.md#child-session-lifecycle-and-teardown).
- **Pinning test**: `tests/pr-walkthrough-agent-manager.test.ts` (terminal-state termination, `validation_failed` exemption, parent cascade, sandbox-worktree preservation, reap-helper matrix).
