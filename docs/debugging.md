...
- **Pinning test**: `tests/unit/core/vite-bundled-dev.unit.test.ts`.
...
- **Symptom**: `tests/unit/core/anthropic-oauth-credential-store.unit.test.ts` passes on a retry or exceeds its per-file budget, usually in the tests that hold a Bobbit or Pi lock while fake timers are active.
...
- **Diagnostic**: inspect the two lock-contention cases in `tests/unit/core/anthropic-oauth-credential-store.unit.test.ts`. A newly added `runAllTimersAsync()` in either release path is a regression; `tests/unit/core/unit-flake-source-contract-repro.unit.test.ts` pins that prohibition.
- **Retry-free checks**:

  ```bash
  npm run test:unit -- --retry=0 tests/unit/core/anthropic-oauth-credential-store.unit.test.ts
  npm run test:unit -- --retry=0 tests/unit/core/unit-flake-source-contract-repro.unit.test.ts
  ```

## Tier-1 sandbox-status test invokes Docker or leaks between gateways

- **Symptom**: `tests/integration/gateway/sandbox.gateway.test.ts` intermittently consumes its file budget, most visibly on Windows; status requests may wait for a real Docker invocation despite the integration gateway's command fence.
...
- **Diagnostic**: trace the `commandRunner` argument from gateway creation through `handleApiRoute()` to the sandbox-status call. The route must not reference `serverCommandRunner`; `tests/unit/core/unit-flake-source-contract-repro.unit.test.ts` statically guards both requirements.
- **Retry-free checks**:

  ```bash
  npm run test:unit -- --retry=0 tests/integration/gateway/sandbox.gateway.test.ts
  npm run test:unit -- --retry=0 tests/unit/core/unit-flake-source-contract-repro.unit.test.ts
  ```
...
- **Confirm a kill**: use the retained verification output and active record, not a historical PID, to establish cleanup state. A timeout/cancel terminal result is published only after the owned tree is settled; Docker additionally requires payload cleanup before host `docker exec` transport cleanup.
- **Pinning tests**: `tests/unit/core/verification-harness-timeout.unit.test.ts` (unit), `tests/e2e/api/verification-timeout.api-e2e.spec.ts` (E2E), `tests/unit/core/verification-command-restart-lifecycle.unit.test.ts` (restart cleanup).
...
- **Pinning tests**: `tests/integration/gateway/gate-inspect-slicing.gateway.test.ts`, `tests/unit/core/gate-verification-snapshot.unit.test.ts`, `tests/unit/core/gate-diagnostics.unit.test.ts`, `tests/unit/core/gate-diagnostics-cleanup.unit.test.ts`, `tests/integration/gateway/gate-diagnostics-cleanup.gateway.test.ts`.
...
- **Pinning tests**: `tests/unit/core/verification-command-restart-lifecycle.unit.test.ts`, `tests/unit/core/verification-command-restart-regression.unit.test.ts`, `tests/unit/core/verification-harness-restart.unit.test.ts`.
...
- **Pinning tests**: `tests/integration/gateway/human-signoff.gateway.test.ts` (REST end-to-end), `tests/browser/journeys/ui/goal-status-widget.journey.spec.ts` (browser).
...
- **Pinning tests**: `tests/unit/core/verification-push-guard.unit.test.ts` and `tests/unit/core/goal-push-safety-regression.unit.test.ts`.
...
- **Pinning test**: `tests/unit/core/system-prompt-merged-branch.unit.test.ts` asserts the system-prompt `## Pull requests` section keeps the concrete detection commands and fresh-branch recovery wording, so it can't rot back into a vague one-liner.
...
- **Symptom**: pills wrap onto a second row in desktop / landscape mode.
  ...
  - Pinned by `tests/browser/fixtures/pill-overflow-promotion.fixture.spec.ts → 'wide mode: strip stays on a single row even with many pills'`.
...
- **Symptom**: pills wrap onto 3+ rows in mobile / portrait mode.
  ...
  - Pinned by `tests/browser/fixtures/pill-overflow-promotion.fixture.spec.ts → 'narrow mode: strip never wraps to more than 2 rows'` (asserts `[data-pill-strip].offsetHeight ≤ 2 × 22px + gap`).
...
- **Symptom**: hidden pills inside the "more" popover refuse to promote back into the strip when visible pills are dismissed (the original B-A1 regression).
  ...
  - Pinned by `tests/browser/fixtures/pill-overflow-promotion.fixture.spec.ts → 'narrow mode: hidden pills promote back into the strip after visible pills are dismissed'` (and the wide-mode equivalent in the same file).
...
- **Symptom**: the "X more" pill label wraps to two lines (e.g. "4" / "more") on a narrow viewport.
  ...
  - Pinned by `tests/browser/fixtures/pill-overflow-promotion.fixture.spec.ts → "'X more' pill label stays on a single line"`.
...
- **Where the logic lives**: `MessageEditor._isCursorOnVisualTopRow` / `_isCursorOnVisualBottomRow` in `src/ui/components/MessageEditor.ts`, both using `_measureCaretRowGeometry` with `_charRowTop` / `_charRectTop` as the rect primitive and `_firstWrapBoundary` for soft-wrap detection. The history state machine is not implicated; diagnose the geometry predicate first.
...
- **Verify**: `npx playwright test --config playwright-v2.config.ts --project=browser-v2 tests/browser/fixtures/message-editor-arrows-real.fixture.spec.ts --reporter=line`. `tests/dom/message-editor-arrows.dom.test.ts` is an arithmetic model only: happy-dom has no layout engine and cannot expose browser geometry bugs.
...
- **Pinning test**: `tests/dom/streaming-blob-state.dom.test.ts` — drives `isStreaming` false→true within the exit window and asserts the blob ends up `active`, not `idle`. Must fail on pre-fix master.
...
- **Pinning tests**: `tests/unit/core/missing-live-messages-repro.unit.test.ts` (real `SessionManager`: concurrent dormant `addClient` revives join one restore and every attached client gets the post-revive frame; stale-generation writers no-op without bumping status or dispatching), `tests/browser/journeys/ui/dormant-revive-live-reply.journey.spec.ts` (browser: attach → hibernate → send → reply renders live with no reload).
...
- **Focused coverage**: `tests/unit/core/event-buffer.unit.test.ts` pins count/byte eviction, retained-byte accounting, oversized events, and `pushFrame()` holes. `tests/dom/remote-agent-seq-dedup.dom.test.ts`, `tests/dom/remote-agent-seq-overflow.dom.test.ts`, and `tests/dom/remote-agent-sequence-hole.dom.test.ts` cover client ordering and fallback; `tests/dom/remote-agent-outbox.dom.test.ts` covers FIFO preservation across reconnect send races.
...
- **Pinning coverage**: `tests/unit/core/ws-overflow-guard.unit.test.ts` covers deferred terminate decisions, payload diagnostics, and resume byte-budget fallback. `tests/unit/core/truncate-large-content.unit.test.ts` covers bounded session event/history payloads, including `verification_result` reports. `tests/unit/core/gate-verification-snapshot.unit.test.ts` covers bounded verification WS output.
...
- **Repro test**: `tests/browser/fixtures/verification-dedup.fixture.spec.ts` is a Playwright file:// fixture that dispatches the same event 6× and asserts a single rendered occurrence on each component.
...
- **Reference and coverage**: [Pi runtime compatibility — Orphan tool-result persistence and recovery](pi-runtime-compatibility.md#orphan-tool-result-persistence-and-recovery); `tests/unit/core/transcript-orphan-tool-results.unit.test.ts`, `tests/unit/core/orphan-tool-result-recovery.unit.test.ts`, `tests/e2e/vitest/orphan-tool-result-rehydration-boundaries.vitest-e2e.test.ts`, and `tests/e2e/browser/orphan-tool-result-recovery.browser-e2e.spec.ts`.
...
- Regression coverage: `tests/unit/core/gate-status-summary.unit.test.ts`, `tests/integration/gateway/gate-status-summary.gateway.test.ts`, `tests/e2e/browser/gate-status-cross-surface.browser-e2e.spec.ts`, and the sign-off/reset cases in `tests/browser/journeys/ui/goal-status-widget.journey.spec.ts`.
...
Coverage lives in `tests/unit/core/rpc-bridge-spawn-args.unit.test.ts` and the focused model-selection, restore, and spawn-boundary canaries. See [docs/internals.md — Spawn-time model pinning](internals.md#spawn-time-model-pinning).
...
- E2E coverage: `tests/integration/gateway/archived-footer-model.gateway.test.ts` (uses `window.__bobbitState` and `data-testid="footer-model-id"`).
...
- Browser fixture coverage: `tests/browser/fixtures/search-preview-archive.fixture.spec.ts` (Path A + Path B + reload persistence + no-draft fallback), `tests/integration/gateway/continue-archived-assistant.gateway.test.ts` (server-side clone happy paths + cross-realm rejection regression).
...
- **Pinning test**: `tests/unit/core/staff-session-staffid-persistence.unit.test.ts` covers spawn-path forwarding, `SessionStore` round-trip, backfill idempotency, and source-level guards for the read/write field.
...
- **Coverage**: `tests/unit/core/reliable-intent-{queue,attempt}.unit.test.ts`, `reliable-compaction-release.unit.test.ts`, `tests/integration/gateway/reliable-intent-recovery.gateway.test.ts`, and `tests/browser/journeys/reliable-agent-turns.journey.spec.ts`.
...
- **Regression test**: `tests/browser/journeys/ui/preparing-ux.journey.spec.ts` (browser journey). It artificially extends the preparing window so the banner is observable, asserts visibility + editor disabled, then asserts both clear once the session goes idle.
...
- Coverage: `tests/unit/core/reliable-compaction-release.unit.test.ts`, `tests/unit/core/pi-installed-contract.unit.test.ts`, `tests/unit/core/assistant-stream-session-broadcast.unit.test.ts`, and the compaction cases in `tests/browser/journeys/reliable-agent-turns.journey.spec.ts`.

## Goal creation fails with `Workflow not found: general`

**Symptom:** Clicking Accept on a goal proposal returns a structured unknown-workflow error for `general` or another stale explicit id.

- `"general"` is no longer a built-in default. Workflows are project-scoped; a project may have any set of ids, or none at all. If a stale code path is still sending `workflowId="general"`, the pinning test [`tests/unit/core/no-general-workflow-default.unit.test.ts`](../tests/unit/core/no-general-workflow-default.unit.test.ts) should have caught it — re-run `npm run test:unit` and look for the failing scan of `src/server/agent/` or `src/app/`.
- Canonical resolution order is: inline workflow snapshot → explicit workflow id → first visible workflow in current store order → generated project-derived in-memory default, with the last tier available only when the live store is empty. An invalid explicit id is rejected rather than replaced by a generated default. The selected definition is frozen onto the goal; generated defaults are persisted only after successful ordinary creation or current-session promotion and only while the store remains empty.
- Zero persisted workflows alone does not produce a public missing-workflows failure. The project assistant remains the recommended way to author tailored, inspectable workflows, but it is not required recovery for API callers that omit the selection: canonical validation can derive the deferred default. The goal form still presents its empty-workflows banner and inline-workflow escape hatch as authoring UX; diagnose that client state in the next entry rather than treating it as the server creation contract. A direct low-level `GoalManager` call that bypasses canonical validation may still use an internal no-workflows assertion, but that message is not the public API behavior.
- Full convention: [docs/goals-workflows-tasks.md — Default workflow resolution](goals-workflows-tasks.md#default-workflow-resolution) and [Canonical goal-candidate validation](goals-workflows-tasks.md#canonical-goal-candidate-validation).
...
- Regression test: [`tests/browser/journeys/ui/goal-accept-failure-keeps-assistant.journey.spec.ts`](../tests/browser/journeys/ui/goal-accept-failure-keeps-assistant.journey.spec.ts) — stubs a 400 from `POST /api/goals` and asserts the assistant panel, chat, and `gateway.sessionId` survive.
...
- Regression test: [`tests/browser/journeys/ui/goal-empty-workflows-banner.journey.spec.ts`](../tests/browser/journeys/ui/goal-empty-workflows-banner.journey.spec.ts).
...
**Regression test:** `tests/browser/journeys/ui/goal-reattempt-project-binding.journey.spec.ts` (browser journey) — opens a re-attempt assistant against a project-bound goal, emits `propose_goal`, clicks Create, asserts the new goal is created with the inherited `projectId` and no toast fires.
...
- **Pinning test**: `tests/unit/core/sidebar-loading-flash.unit.test.ts` — proves a second poll tick with an empty `gatewaySessions` list does not re-enter initial-load once `sessionsGeneration >= 0`, while genuine first load and post-error retry still show the spinner.
...
- **Tests**: `tests/dom/pwa-lifecycle.dom.test.ts` pins the pure decision and exact fake-rAF/timer budgets, including competing generations and stalled long-suspend reload. `tests/browser/fixtures/pwa-lifecycle.fixture.spec.ts` pins persisted restore, cooldown across a real reload, event targets, standalone gating, quick resume, and boot-watchdog clearing. Retain manual quick and greater-than-30-minute suspend/freeze/kill relaunch verification on a real installed iOS PWA.
...
- **Tests and architecture**: `tests/browser/fixtures/canvas-eye-getanimations.fixture.spec.ts` pins boundary pixels, wrap, negative delay, wakeup counts, visibility, replacement, cleanup, archived/static zero work, and live CSS body motion. See [Bobbit Iconography & Animation System](bobbit-sprites.md#canvas-eye-scheduling-contract).
...
- **Repro coverage**: `tests/e2e/browser/tail-chat-real-stream.browser-e2e.spec.ts` and `tail-chat-session-navigate.browser-e2e.spec.ts` exercise real-fidelity streaming and navigation. `tests/browser/fixtures/chat-scroll.fixture.spec.ts`, `jump-to-last-prompt.fixture.spec.ts`, and the agent-interface scroll fixtures pin jump-button visibility, near-bottom relock, reflow, and image/geometry behavior. All tests are outcome-only (`getBoundingClientRect()` + computed style) — never assert on private fields. The original sensitivity matrix remains in [the tail-chat redesign record](design/tail-chat-redesign.md#outcome-of-the-use-stick-to-bottom-port).
...
- **Repro test**: `tests/unit/core/message-reducer.unit.test.ts` — pure unit tests of the reducer. Scenarios 4, 5, 10, 12 exercise the snapshot-merge invariant directly.
...
- **Repro test**: `tests/browser/journeys/ui/new-tab-no-duplicate-messages.journey.spec.ts` (canonical regression — opens the same session in multiple browser contexts and asserts message count is identical and stable).
...
- **Repro test**: `tests/unit/core/message-reducer.unit.test.ts` — scenario 8 ("proposal-tool burst": two consecutive `propose_*` assistant turns + matching toolResult, both widgets present in correct order, no overwrite) and scenario 9 (`ask_user_choices` envelope routes to the correct toolUseId). Browser-level: `ST-DEDUP-02` / `ST-DEDUP-03` / `ST-DEDUP-04` in `tests/browser/journeys/ui/stories-streaming.journey.spec.ts`.
...
- **Repro tests**: unit `tests/unit/core/proposal-update-policy.unit.test.ts` (pins the pure policy — the out-of-order server-rev race can't be reproduced deterministically in a browser); browser journeys `tests/browser/journeys/ui/goal-proposal-revision-autoupdate.journey.spec.ts` (Mode A: 2nd `propose_goal` + `edit_proposal type=goal` auto-update with no click) and `tests/browser/journeys/ui/goal-proposal-offscreen-return.journey.spec.ts` (Mode B: off-screen proposal visible on fast-path switch-back, slow-path reconnect, and reload). Mock-agent triggers live in `tests/e2e/_helpers/mock-agent-core.mjs`.
...
- **Dropdown opens but `git-status-dropdown-open` fires N times / untracked refetches stack?** `session-manager.ts` used to attach an anonymous listener on every `connectToSession`, with no matching `removeEventListener`. Cached session switch-backs piled them up (one extra refetch per past connect, plus a memory leak). The listener is now stored on the agent interface as `__gitStatusDropdownOpenHandler` and `removeEventListener`'d before the new one is wired. Pinned by `tests/unit/core/session-manager-git-dropdown-listener.unit.test.ts` (static source assertion — fails if anyone reverts to anonymous closures).
- **Reproducing locally?** The Lit instance only runs `disconnectedCallback` when its host is actually removed from the DOM. The plain-JS replica fixture used by `tests/dom/git-status-interactions.dom.test.ts` does not exercise this — use `tests/dom/git-status-widget-wedge.dom.test.ts` instead. It mounts the real widget via the `git-status-widget-states` bundle, opens the dropdown, starts the close animation, and either disconnects+reconnects the host, externally removes the portal, or dispatches `animationcancel`, then asserts the next click reopens. All three scenarios must reopen the dropdown and clear the `git-dropdown-closing` class.
...
- **Pinning tests**: `tests/unit/core/pr-walkthrough-bundle-hunk-header.unit.test.ts` (server unit — reconstructed hunks always carry a string `header`). NOTE: the historical browser case lived in the now-deleted `tests/e2e/ui/pr-walkthrough-panel.spec.ts`; the current pack viewer is exercised by `tests/e2e/browser/pr-walkthrough-pack.browser-e2e.spec.ts`, and the header-coercion guards now live in the shared synthesis module `src/shared/pr-walkthrough/yaml-to-cards.ts` + the pack panel (`market-packs/pr-walkthrough/lib/panel.js`).
...
- **Pinning tests**: `tests/unit/core/orchestration-core.unit.test.ts` / `tests/unit/core/host-agents-scope.unit.test.ts` (generic owner-gone reap, cascade-reap, terminated-child-reapable) and `tests/e2e/api/pr-walkthrough-host-agents.api-e2e.spec.ts` (submit never dismisses the reviewer; the reviewer stays live + selectable and survives a simulated restart; user-terminate dismisses it).
...
- **Deleted remote branch reappears during recovery?** This is a no-publication boundary violation. Repair, restore, sandbox recovery, and recreate-from-persisted-branch paths must preserve the local branch without recreating its remote counterpart. Reproduce with a local bare origin and inspect the lifecycle command log for a non-delete `git push`; `tests/e2e/vitest/git-lifecycle-no-publication-real-git.vitest-e2e.test.ts` pins creation, pool reuse, multi-repo, recovery, configured `base_ref`, and child-merge cases.
...
- **Pinning tests**: `tests/unit/core/openrouter-key-bridge-repro.unit.test.ts`, `tests/unit/core/spawn-env.unit.test.ts`, `tests/dom/remote-agent-outbox.dom.test.ts`.
...
- **Diagnostic order**:
  ...
  3. Browser journey: `tests/browser/journeys/project-onboarding-post-archive.journey.spec.ts` pins the archive → Continue → assistant flow.
...
- **Pinning tests**: `tests/unit/core/gate-signal-step-enumeration.unit.test.ts` (unit — immediate gate-store read after signal); `tests/integration/gateway/gate-signal-progress.gateway.test.ts` (gateway integration — POST response vs summary vs inspect vs active endpoints all match within one scheduler tick); `tests/browser/journeys/ui/verification-progress-indicator.journey.spec.ts` (browser journey — chips render immediately and survive reload from persisted state alone).
...
- **Pinning tests**: `tests/unit/core/verification-harness-restart.unit.test.ts`, `tests/unit/core/verification-command-restart-lifecycle.unit.test.ts`, `tests/integration/gateway/verification-restart-resignal.gateway.test.ts`.
...
- **Pinning tests**: `tests/unit/core/verification-resume-restart-prompt.unit.test.ts` (resume-prompt timeout leaves the gate `pending`, never `failed`); `tests/unit/core/verification-resume-restart-recovery.unit.test.ts` (cold reviewer waits for readiness then passes; rerun-from-scratch fallback is reachable); `tests/unit/core/verification-logic.unit.test.ts` (`isRestartInterruptError` classification).
...
- **Pinning test**: `tests/unit/core/cold-restart-reprompt.unit.test.ts` (readiness wait + generous timeout on the mid-turn re-prompt; no escaped unhandled rejection from the nudge drain; single re-prompt for a session that is both mid-turn and a lead with work).
...
- **Test-only knob**: a deterministic preparing-window extension hook exists in `src/server/agent/session-setup.ts` for the regression journey (`tests/browser/journeys/ui/preparing-ux.journey.spec.ts`). It is intentionally undocumented in user-facing material; do not surface it as a tuning option.
...
1. The legacy `setupCommand` parameter and the `setupWorktreeDeps` helper have been removed from `src/server/skills/git.ts`. If a recent change reintroduced either, `tests/unit/core/worktree-setup-fallback.unit.test.ts` will fail — run `npm run test:unit -- worktree-setup-fallback`.
...
- Regression guard: `tests/unit/core/tool-guard-extension.unit.test.ts` transpiles and dynamically imports the generated source across all four policy-input variants (allow-only, ask-only, never-only, mixed). Any parse-level quoting slip fails that spec.
...
- **Reference**: [AI Gateway routing — Adopting a pre-v0.17.0 publication](ai-gateway-routing.md#adopting-a-pre-v0170-publication) and [AI Gateway internals — Publication ownership and legacy block adoption](internals.md#publication-ownership-and-legacy-block-adoption). Pinned by `tests/unit/core/aigw-legacy-block-adoption.unit.test.ts`.
...
Key files: `src/server/agent/team-manager.ts` (`registerReviewerSession`, `resubscribeTeamEvents`, `notifyTeamLead`), `src/server/agent/team-store.ts` (`PersistedTeamEntry.agents[].kind`). Regression test: `tests/unit/core/team-manager-reviewer-resume.unit.test.ts`. See [docs/internals.md — Reviewer kind & restart resume](internals.md#reviewer-kind--restart-resume).
...
3. Confirm all four reminder sites await it. The regression-guard test (`tests/unit/core/verification-reminder-race.unit.test.ts`) mocks a session that flips from idle to streaming after 50ms and asserts `_tryResumeFromSession` does not terminate within the first second.

Key files: `src/server/agent/session-manager.ts` (`waitForStreaming`), `src/server/agent/verification-harness.ts` (the four reminder sites). Tests: `tests/unit/core/verification-reminder-race.unit.test.ts`, API E2E `tests/e2e/api/gate-verification-resume.api-e2e.spec.ts`. See [docs/internals.md — Reminder race after restart-resume](internals.md#reminder-race-after-restart-resume).
...
Key files: `src/server/agent/verification-harness.ts` (`runLlmReviewViaSession`, bounded retry loop in `verifyGateSignal`), `src/server/agent/session-manager.ts` (`createSession` clobber guard). Tests: `tests/unit/core/verification-harness-review-reliability.unit.test.ts`, `tests/unit/core/session-id-clobber-guard.unit.test.ts`. Full detail: [docs/llm-review-recovery.md — Reviewer session lifecycle](llm-review-recovery.md#session-lifecycle-and-transcript-preservation).
...
Fix: prompts now carry a `PromptSource` (declared in `src/server/agent/session-manager.ts`, persisted as `SessionInfo.lastPromptSource`). Only `"user"` / `"system"` sources reset `idleNudgeCount` / `noWorkersNudgeCount` on the next `agent_start`; `"auto-nudge"` / `"task-notification"` / `"verification"` preserve them so `scheduleWorkersNudge` / `scheduleNoWorkersNudge` keep stepping up. See [docs/design/notification-policy.md — Team-lead idle-nudge backoff](design/notification-policy.md#9-team-lead-idle-nudge-backoff) for the full mechanism. Pinning test: `tests/unit/core/team-manager-idle-nudge-backoff.unit.test.ts`.
...
Wait interruption is neither Pi receipt nor settlement; correlate the occurrence with its Pi row and sidecar before treating it as delivered. Tests: `tests/unit/core/bg-process-manager.unit.test.ts`, `tests/integration/gateway/bg-wait-steer-abort.gateway.test.ts`.
...
- **Diagnostic order**:
  1. Run `npm run test:unit` and look at `tests/unit/core/tool-activation-contract.unit.test.ts`. If it fails, the flag contract has regressed at unit speed — fix `computeToolActivationArgs()` before anything else.
  2. If the unit pin is green but real agents still substitute, run `npm run test:manual` against `tests/manual/agent-tool-use.manual.spec.ts`. The seven scenarios cover pi builtins, a Bobbit extension (`web_fetch`), and an MCP meta-tool (`mcp_describe`); a failing scenario tells you exactly which tool category is being stripped.
...
- **Pinning tests**: `tests/unit/core/anthropic-oauth-adapter.unit.test.ts` covers Pi's Anthropic contract, cancellation, contention, and redaction. `tests/unit/core/oauth-external-callbacks.unit.test.ts` covers Codex interaction mapping, credential persistence, cancellation, and log redaction.
...
- **Pinning test**: `tests/unit/core/oauth-external-callbacks.unit.test.ts` asserts exact-id and heuristic browser preference, single-option selection, and explicit rejection of unsupported prompts.
...
- **Pinning test**: `tests/unit/core/pi-ai-browser-boundary.unit.test.ts`. See [Pi runtime compatibility](pi-runtime-compatibility.md#browser-safe-pi-ai-boundary).
...
- **Pinning test**: `tests/unit/core/clean-build-warnings-regression.unit.test.ts` covers quiet minimal/temp fallback paths and the once-only origin-backed diagnostic.
...
Pinned by `tests/unit/core/error-modal-call-sites.unit.test.ts` (enumerates every modal call site that must forward `{ code, stack }`) and `tests/unit/core/error-helpers.unit.test.ts` (helper contract). Add new modal call sites to the former; do not add a new `showConnectionError(...)` without forwarding `errorDetails(err)`.
...
- **Quick checks**: `cat .bobbit/state/gateway-url` and the browser auto-open target should never use `0.0.0.0` or `[::]` as the URL host; an IPv6 wildcard bind must become `[::1]` in connect targets. If either literal wildcard remains, a startup path bypassed `buildStartupUrls` or `loopbackForBind`. Coverage: `tests/unit/core/cli-loopback-for-bind.unit.test.ts` and `tests/unit/core/base-path-cli.unit.test.ts`.
...
- **Nudger gating**: `InboxNudger.tickOne` skips when (a) `staff.state !== "active"`, (b) `staff.currentSessionId` is unset, (c) `session.status !== "idle"`, (d) `nudgePending` is already set for that staff, or (e) the pending list is empty. The tick is 15 s, so worst-case latency on the idle→nudge edge is about 15 s plus the selected context-policy operation.
- **Clear policy intentionally retries without a digest**: `contextPolicy === "clear"` awaits `SessionManager.clearContext` and then requires the same active staff/session binding, an idle session, and an empty prompt queue. A clear failure or incompatible operation clears `nudgePending`, leaves inbox rows pending, and sends no digest into the old or contested context. Check `[inbox-nudger] applyPolicyThenNudge failed` logs, then let the normal idle tick retry after the competing lifecycle/prompt work settles. A successful attempt retains the same staff/session identity and emits the normal durable **Context Cleared** boundary.
...
- **Fix shipped in the staff-inbox release**: `PUT /api/staff/:id` forwards `contextPolicy` (`"compact"` \| `"preserve"` \| `"clear"`) through to `StaffStore.update`. Pre-fix builds dropped it on the allow-list.
- **If seen again**: confirm the request body contains the selected known token, the `server.ts` staff `PUT` handler forwards it, and the response plus `GET /api/staff/:id` return the same value. The API intentionally ignores an unknown token and preserves the previous policy; separately, missing/unknown/malformed values loaded from `staff.json` normalize to Compact. The radio binding is `editContextPolicy` in the staff page.
...
- **Pinning tests**: `tests/unit/core/synthesize-attachment-text.unit.test.ts`, `tests/unit/core/image-only-prompt-dispatch.unit.test.ts`, `tests/unit/core/image-only-prompt-unstick-recovery.unit.test.ts`, `tests/unit/core/transcript-sanitizer.unit.test.ts`.
