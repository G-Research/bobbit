...
- State in `GateStore` (`.bobbit/state/gates.sqlite`). Legacy `gates.json` and recovery `gates.json.pre-migration` are fully validated, imported transactionally, and renamed without deletion to collision-safe `.sqlite-retired` / `.pre-migration-recovered` backups. A source still present after a startup rename error is retried from durable retirement intent; do not edit, delete, or copy it over SQLite. Stop the gateway and inspect a complete state-directory copy read-only. See [Gate store SQLite persistence — Operational inspection and recovery precautions](design/gate-store-sqlite-persistence.md#operational-inspection-and-recovery-precautions).
