...
- **Dropdown opens but untracked files never appear?** Default / poll / pill refreshes must stay summary-only (`git status --porcelain=v1 -uno`) for speed; dropdown open is the explicit full-status path. `GitStatusWidget._toggle` fires `git-status-dropdown-open` (bubbles, composed), and both session chrome (`session-manager.ts`) and dashboard (`goal-dashboard.ts`) must handle it by refetching `?untracked=1` (`-uall`). Check the response carries `untrackedIncluded: true`. Summary vs untracked responses are separate cache keys, so both can coexist without server cache cross-contamination.
