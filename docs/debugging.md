...
- **Cause and upstream status**: Pi `0.80.6` persists agent events asynchronously. `AgentSession._handleAgentEvent` awaits extension message handlers before appending a message, but listener invocations are not serialized, so a later tool-result event can reach disk before—or survive an interruption without—the assistant event that introduced its call ID. Force-abort, process exit, or gateway restart can expose the incomplete turn, but Bobbit does not write Pi conversation rows and cannot make Pi's internal appends atomic. Upstream Pi issue `#1717` tracks the race; upstream commit `dfc779f` serializes event handling, but that fix is not included in the installed `0.80.6` packages. Bobbit therefore keeps the boundary repair even after adopting a fixed Pi release, for already-persisted transcripts and interrupted writes.
