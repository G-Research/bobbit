...
- **Pinning tests**: `tests/verification-harness-timeout.test.ts` (unit), `tests/e2e/verification-timeout.spec.ts` (E2E), `tests/verification-command-restart-lifecycle.test.ts` (restart cleanup).
...
- **Pinning tests**: `tests/verification-command-restart-lifecycle.test.ts`, `tests/verification-command-restart-regression.test.ts`, `tests/verification-harness-restart.test.ts`.
...
## Corrupt or truncated durable store on restart (gates.json / tasks.json / team-state.json / inbox / project.yaml)

- **Symptoms**: after a crash/kill or power loss, a restart logs `[gate-store] Loaded from backup gates.json.bak.1 — primary missing/corrupt` (or the `[team-store]` / `[task-store]` / `[inbox-store]` / `[project-config-store]` equivalent); or — the bad case — gate/task/team/inbox state appears empty after a restart with a `Failed to load` error in the log.
- **What recovery does now**: these stores write via `src/server/agent/atomic-json.ts` (tmp-write → fsync → rename → directory fsync) with a 3-deep `.bak.N` rotation, and `load()` falls back primary → newest parseable `.bak.N` → empty. A torn write can therefore cost at most the last save, never the whole store. The `Loaded from backup` warn is informational — state was recovered; at most the final pre-crash mutation is missing.
- **How to inspect**: look at `<stateDir>/gates.json` (or `tasks.json`, `team-state.json`, `inbox/<staffId>.json`, `<configDir>/project.yaml`) and its `.bak.1`..`.bak.3` siblings. `jq . <file>` on each tells you which generations parse (`yq`/`yaml` for project.yaml); `.bak.1` is the newest. A leftover `<file>.tmp` is a write that never completed and is ignored by the loader.
- **Pre-fix failure mode (context, CON-01)**: these four stores used a plain truncating `writeFileSync` with no backup. A kill mid-write zeroed the file, load silently started empty, and the next save overwrote the corrupt file — permanent loss of verification/team/task/inbox state. If a store is empty after a restart on a pre-fix state dir, there is nothing to recover from disk.
- **Resurrection guard**: deliberate deletion paths must purge backups too (`removeJsonWithBackups()`), otherwise deleted state (e.g. a removed staff's inbox) would come back from `.bak.N` on the next load. If deleted state reappears after a restart, check for stale `.bak.N` files next to the deleted primary.
- **project.yaml (CON-02) — corrupt-file save guard**: `.bobbit/config/project.yaml` (`project-config-store.ts`) additionally refuses to save while corrupt. Pre-fix, a corrupt project.yaml silently loaded as `{}` and the next setter (any config PUT, workflow edit, pack toggle) serialized that empty state OVER the corrupt file — deterministic total loss of workflows/components/pack config (repro: write `:\n:` into project.yaml, restart, call any setter → file became `{}`). Now: load falls back to the newest parseable `.bak.N` (recovered state is live, saves resume; the bad primary is left on disk and only rotated into `.bak.1` by the next successful save); if NO backup parses, the store enters a `loadFailed` state — every setter throws `refusing to save` (surfaced as HTTP 500 on config PUTs) and the corrupt file is never touched. **Fix**: repair the YAML by hand or `cp project.yaml.bak.N project.yaml`, then retry (the store re-reads per request; no restart needed). Two intentional asymmetries: an **empty** (0-byte/whitespace) project.yaml is treated as a deliberate reset — fresh state, no backup fallback; a **missing** project.yaml with `.bak.N` siblings present is treated as accidental deletion — recovered from backup, NOT started fresh.
- **Pinning tests**: `tests/atomic-json.test.ts`, `tests/durable-store-atomic-write.test.ts`, `tests/project-config-store-atomic-write.test.ts`. Design/context: [internals.md — Durable-store write discipline](internals.md#durable-store-write-discipline-atomic-writes--bak-recovery).
...
## Pressing Escape to dismiss a dialog aborts the streaming agent

- **Symptom**: a modal (confirm, error, rename, add-project, …) is open while a session streams; the user presses Escape to close the modal, the modal closes — and the streaming turn is also silently aborted.
- **Root cause**: `AgentInterface._handleGlobalEscape` (`src/ui/components/AgentInterface.ts`) is a **capture-phase** `document` keydown listener that calls `session.abort()` on Escape while streaming, unless it finds an open overlay in the DOM. Its bail-out selector matches `[role="dialog"]` / `[aria-modal="true"]` plus a short allow-list of named overlay custom elements. Mini-lit's `Dialog` component renders **no ARIA modality of its own**, so any modal container that isn't explicitly tagged is invisible to the guard — capture runs before the dialog's own bubble-phase Escape handler, so one keypress both aborts the agent and closes the dialog.
- **Fix**: all `src/app/dialogs.ts` helpers create their body-appended container via the shared `createDialogContainer()` helper, which sets `role="dialog"` + `aria-modal="true"` (also fixing the screen-reader modality gap). Pinned by `tests/ui-fixtures/agent-interface-dialog-escape.spec.ts` (streaming session + real `confirmAction()` dialog: Escape closes the dialog without calling `session.abort()`, plus a control asserting the guard is not overbroad).
- **Invariant for new dialog authors**: any new modal container appended to the DOM MUST go through `createDialogContainer()` (dialogs.ts) or itself carry `role="dialog"`/`aria-modal="true"` (or be added to the named-element allow-list in `_handleGlobalEscape` if it's a custom element) — otherwise Escape reaches the global abort handler and kills the in-flight turn.
- **Diagnosis**: reproduce with a streaming session, open the suspect modal, and run `document.querySelector('[role="dialog"], [aria-modal="true"]')` in the console — if it returns `null` while the modal is visible, the container is untagged and `_handleGlobalEscape` will not bail.
- **Known remaining case (tracked separately)**: `MessageEditor` has its own *local* Escape-to-abort handler (`src/ui/components/MessageEditor.ts`, `Escape && isStreaming → onAbort()`) that fires whenever the composer textarea has focus. It is NOT guarded by dialog state — if a modal opens without moving focus out of the composer, Escape still aborts via that path. Distinct mechanism, not covered by this fix.
...
- **Sessions archived instead of recovered?** The 3-tier worktree recovery failed: worktree doesn't exist on the volume, `git worktree repair` didn't help, and `createWorktree` from the persisted branch also failed. Check: (1) the session has a persisted `branch` value in `sessions.json`, (2) the local branch/ref still exists in the sandbox clone or was intentionally published to the remote, (3) the named volume `bobbit-worktrees-<projectId>` survived the container death (`docker volume ls`).
...
- **Pinning tests**: `tests/verification-harness-restart.test.ts`, `tests/verification-command-restart-lifecycle.test.ts`, `tests/e2e/verification-restart-resignal.spec.ts`.
...
- **Pinning tests**: `tests/verification-resume-restart-prompt.test.ts` (resume-prompt timeout leaves the gate `pending`, never `failed`); `tests/verification-resume-restart-recovery.test.ts` (cold reviewer waits for readiness then passes; rerun-from-scratch fallback is reachable); `tests/verification-logic.test.ts` (`isRestartInterruptError` classification).
