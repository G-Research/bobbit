...
## Corrupt or truncated durable store on restart (gates.json / tasks.json / team-state.json / inbox)

- **Symptoms**: after a crash/kill or power loss, a restart logs `[gate-store] Loaded from backup gates.json.bak.1 — primary missing/corrupt` (or the `[team-store]` / `[task-store]` / `[inbox-store]` equivalent); or — the bad case — gate/task/team/inbox state appears empty after a restart with a `Failed to load` error in the log.
...
- **How to inspect**: look at `<stateDir>/gates.json` (or `tasks.json`, `team-state.json`, `inbox/<staffId>.json`) and its `.bak.1`..`.bak.3` siblings. `jq . <file>` on each tells you which generations parse; `.bak.1` is the newest. A leftover `<file>.tmp` is a write that never completed and is ignored by the loader.
...
- **Pinning tests**: `tests/atomic-json.test.ts`, `tests/durable-store-atomic-write.test.ts`. Design/context: [internals.md — Durable-store write discipline](internals.md#durable-store-write-discipline-atomic-writes--bak-recovery).
