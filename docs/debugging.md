...
- **Many REST requests but uncertain external volume?** Inspect `[remote-state]` telemetry. Count `started` by `source` and one-way `record` digest; `fresh`, `joined`, `coalesced`, `budget`, and `backoff` do not represent additional external calls. Explicit actions and deferred lifecycle/verification paths are outside the automatic budget.
