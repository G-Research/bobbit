...
Trace the attempt transaction in the shared session-activity attributor. Begin must be side-effect free. Only a positive RPC acknowledgement or an exact, unambiguous echo of the current attempt may commit; rejection must cancel that exact attempt without releasing restore quarantine. An exact echo can precede a negative acknowledgement and still win. Unrelated lifecycle, assistant, or tool frames do not prove prompt acceptance. Replacement must invalidate old-bridge tokens before replay begins.

For same-text retries, inspect author-correlation state without logging prompt bodies. Cancelled attempts and restored settled-keyless occurrences share a digest-only ambiguity fence capped at 256 records and 64 KiB. Once it overflows, the flag remains sticky and raw/keyless pre-ack correlation fails closed. `message_update` is progress, not settlement; `message_end` is the terminal user echo that may settle the occurrence and retire queue/steer recovery ownership. A positive acknowledgement can accept the current attempt after an ambiguous update, but the update remains nonterminal; a negative acknowledgement must leave the attempt recoverable.
