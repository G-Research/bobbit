...
- **Server-side fallback**: `sendCanonicalSessionState()` in `handler.ts` calls live `getState()` only on the current readable lifecycle generation. Dormant, terminated, fenced, stale-generation, failed, or raced live reads return persisted model state; an explicit read during preparing/starting returns `{ preparing: true }`, while initial attachment may separately seed persisted model info. If replacement wins during an RPC, its result is discarded for the persisted fallback. `sendFallbackModelState()` reads persisted `modelProvider`/`modelId` and calls `resolveModelStateMeta()` (registry cache → pi-ai catalog → `inferMeta`) to attach the correct `contextWindow` / `reasoning` / `thinkingLevelMap`. A stopped placeholder bridge must never surface `Agent process not running` as `COMMAND_ERROR` during revival. Deriving the frame from `inferMeta` alone was the cause of the 200k-instead-of-1M symptom and missing thinking selector; reconnect/`get_state` must preserve the full map — see [Per-model thinking-level capabilities](thinking-levels.md#the-thinkinglevelmap-has-to-reach-the-client-to-be-useful).
...
- **Prompt or steer duplicated across Stop/restart?** Production delivery keeps the exact FIFO row through the SessionStore publication barrier, frames its stable prompt ID/body digest, and settles only from Pi's validated post-persistence ACK. A resend of the same ID/digest is a no-op; a body collision fails closed. Inspect `messageQueue[*].deliveryState` / `deliveryPromptId`, `acceptedPromptDispatches`, and the prompt-author sidecar. `inFlightSteerTexts` is the legacy-bridge fallback, not the production owner. See [Stable prompt-delivery protocol](prompt-queue.md#stable-prompt-delivery-protocol).
- **Steered messages lost after abort on a legacy bridge?** Look in this order: (1) was the steer dispatched at all — check `_dispatchSteer()` recorded `inFlightSteerTexts` and persisted it with the queue row removal; (2) did `_reconcileAfterAbort` run — it runs for user Stop, a cancellation-shaped terminal that arrives while still `streaming`, force-abort, and restore after `switch_session` replays durable echoes; (3) did the normal `drainQueue()` boundary pick up the re-enqueued steered batch — it should pop them via `dequeueAllSteered()` and dispatch via `prompt` (idle path), not `steer`. A cancellation drain is neither an auto-retry nor a manual-retry condition; see [Cancellation-shaped terminal recovery](prompt-queue.md#cancellation-shaped-terminal-recovery).
- **Direct live-steer (WS `{type:"steer"}`) lost when user clicks Stop on a legacy bridge?** PI-25b fallback. `SessionManager.deliverLiveSteer()` enqueues the row in `promptQueue` with `isSteered=true` and forwards to the single `_dispatchSteer` site. Without v1 downstream delivery, `_dispatchSteer()` records the batch in the shadow ledger, removes the row, persists both together, then awaits `rpcClient.steer(batchText)`. Cleanup paths: happy-path — `_consumeSteerEcho` splices the entry on the `message_end(role:user)` echo; abort/restore — `_reconcileAfterAbort` drains the ledger and re-enqueues at front. RPC-layer failure rolls the row back to the front of `PromptQueue` only if the ledger entry is still present, so a late rejection after abort/restart reconciliation cannot duplicate a recovered row. See PI-25b / PI-25c (`tests/e2e/abort-status-e2e.spec.ts`) and the gateway-restart and reconnect tests (`tests/e2e/steer-gateway-restart.spec.ts`, `tests/e2e/steer-reconnect.spec.ts`).
- **Legacy steered message briefly disappears from chat and reappears after the next prompt?** Dispatch→echo continuity race. The legacy `_dispatchSteer()` path removes the queue row and broadcasts the empty queue while the SDK echo is still pending; the SDK only echoes the text back as `message_end(role:user)` after a roundtrip, and the agent only flushes that echo to `.jsonl` then. A snapshot taken in that window (visibility resync, WS reconnect resume-fallback, second tab) sees no queue pill and would miss the user row without help. Fixed by `spliceInFlightSteers()` (`src/server/agent/splice-inflight-message.ts`), which appends a synthetic user-role row for every `session.inFlightSteerTexts` ledger entry not already present in the snapshot. Wired at `ws/handler.ts::get_messages`, `session-manager.ts::refreshAfterCompaction`, and the post-respawn `broadcast({type:"messages"})` site. Synthetic rows carry id prefix `inflight-steer:` and are reconciled into the real echo via the H3 reducer machinery (multiset dedup + `_order > snapshotMaxOrder` survivor guard + prior-snapshot artifact drop). If the symptom returns, check the splice helper is being called at all three sites and that the ledger isn't being cleared prematurely (the entry should live from `_dispatchSteer` record until `_consumeSteerEcho` or `_reconcileAfterAbort` removes it). See [docs/design/snapshot-live-race-fix.md §9](design/snapshot-live-race-fix.md#9-steer-continuity-splice-companion-fix).
- **Follow-up reports `Agent is already processing` at a turn boundary?** Pi can emit `agent_end` before `finishRun()` clears its busy guard. Bobbit must retain the same durable FIFO row, return queued/retrying state, and redrain after bounded positive delays without `COMMAND_ERROR`. Check `recoverDrainAttempts`/`pendingPromptDrainTimer`; an immediate recursive or tick-zero retry is a regression. See [Dispatch failure](prompt-queue.md#dispatch-failure).
- **Steered messages arriving one-at-a-time instead of batched?** `drainQueue()` batches consecutive front-priority steered rows under one stable delivery ID. If they arrive separately, check that the rows are contiguous and share the expected `deliveryPromptId` during a recovered batch.
...
- **First classify the idle state**: a cancellation-shaped terminal drains through the normal queue boundary and clears `lastTurnErrored`; `auto_retry_pending` means a retry timer is armed and queued work waits for it; a genuine error with queued work but no timer emits `manual_retry_required`, so use **Retry** after fixing the cause. Only the last case belongs to this diagnostic; see [Parked work is never silently idle](prompt-queue.md#parked-work-is-never-silently-idle).
...
- Canonical project state is partitioned under `.bobbit/state/gate-records/v2/`; `gates.json` is only the v1 migration source and is retired after validated cutover. See [Gate store persistence](gate-store-persistence.md).
- Check dependencies via `GET /api/goals/:id/gates`
- Inspect project size/migration/reclaim state with `bobbit_read(operation="maintenance_inspect", probe="gate_store", projectId="...")`; never delete migration or payload files manually.
...
- Skipped steps carry `skipped: true` on `GateSignalStep`, persisted in the affected gate history shard — this lets the UI show the correct dash icon after reload (without it, skipped steps would appear as passed or failed based on the `passed` field alone)
...
- `llm-review` steps produce a `text/markdown` primary artifact; QA may produce HTML.
- Artifact creation caps still apply, but persisted bodies are content-addressed under `gate-records/v2/payloads/` and hot history JSON retains metadata plus a managed reference.
- Fetch a primary body with `gate_inspect(section="artifact", artifact="primary", mode=...)`; diagnostic files use the IDs/relative paths returned by explicit verification inspection.
- If an artifact is missing, check that the step completed, then inspect the managed-ref hash/byte contract and the project-scoped gate-store maintenance report. Cross-project refs are rejected.
- See [Gate store persistence](gate-store-persistence.md) and [Retained gate diagnostics](gate-diagnostics.md).
