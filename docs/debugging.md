...
## Tool validation errors not auto-retried

- **Symptom**: a tool call (`ask_user_choices`, `propose_*`, or any structured-input tool) returns a validation error and the user sees the errored `tool_use` + `tool_result` pair in the transcript even when the agent later self-corrects — sometimes producing two visible cards for one user-facing question.
- **Expected behaviour**: `ToolRetryHarness` (`src/server/agent/tool-retry-harness.ts`) observes `tool_execution_end` events on each session's `rpcClient` and, on schema/validation errors, dispatches a single targeted nudge via `rpcClient.prompt(...)` so the model re-emits a corrected call. Cap: 2 retries per `tool_use_id`. Retries are recorded in `PersistedSession.toolAutoRetries` and surfaced on `GET /api/sessions/:id`.
- **Wiring check**: harness is constructed in `subscribeToEvents()` in `session-setup.ts` for new sessions, and re-bound in `SessionManager` at `restoreSession`, `_respawnAgentInPlace`, and the in-memory `recoverSandboxSessions` branch. If retries stop firing after a Docker container recreate or sandbox respawn, verify the new `rpcClient` got a fresh harness instance bound (`session.toolRetryHarness?.stop()` then re-construct).
- **Classifier**: `"schema"` covers `isError===true` matching `detectJsonValidationError()` plus `isError===false` bodies whose text begins with `ask_user_choices: questions[` or `propose_*:` (or a `{ "error": "..." }` JSON envelope around those). Anything else is `"domain"` and stays visible by design.
- **Verification harness coordination**: `verification-harness.ts` adds the `tool_use_id` it's about to retry to `session._verificationOwnedToolUses`. The generic harness skips any id in that set so the two paths can't double-retry. The legacy standalone-RpcBridge path inside `verification-harness.ts` (LLM-review review session) is intentionally not tagged — the generic harness is never bound to that bridge.
- **Distinguish from the LLM stream watchdog**: the watchdog (`docs/llm-stream-watchdog.md`) fires on **stream inactivity** (no frames). The tool-retry harness fires on **specific event content** (`tool_execution_end` with a schema-class error). They cannot fire on the same event.
- See [docs/design/tool-retry-harness.md](design/tool-retry-harness.md) for the full design.

## Session wedged after errored turn

- **LLM stream silently stalls (UI shows Stop spinner forever)** — `[stream-watchdog]` log lines; tune via `BOBBIT_LLM_STREAM_TIMEOUT_MS` / `BOBBIT_LLM_STREAM_MAX_RETRIES`. See [llm-stream-watchdog.md](llm-stream-watchdog.md).
