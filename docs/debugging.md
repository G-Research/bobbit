...
- **LLM stream silently stalls (UI shows Stop spinner forever)** — `[stream-watchdog]` log lines; tune via `BOBBIT_LLM_STREAM_TIMEOUT_MS` (default 30000ms, 0 disables) and `BOBBIT_LLM_STREAM_MAX_RETRIES` (default 2). See `src/server/agent/stream-watchdog.ts`.
