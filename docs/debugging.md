...
Fix: cost entries are stamped with `goalId` at record time (`SessionCost.goalId`, set write-once in `CostTracker.recordUsage`). `getGoalCost(goalId)` scans entries by stamped goalId instead of consulting `sessionStore`, decoupling cost from session lifetime. A boot-time backfill (`src/server/agent/cost-backfill.ts`, wired in `src/server/server.ts`) first runs the authoritative live-session/sidecar pass after session restore, then runs a post-listen transcript pass in boot background tasks. The sidecar pass tries: (1) live persisted session record (`getPersistedSession`), (2) sidecar scan — walks the agent sessions root for `*.bobbit.json` sidecars and indexes `bobbitSessionId → teamGoalId`. The transcript pass scans production `<isoTs>_<sessionId>.jsonl` files (bounded to 50 lines / 64 KiB / 30s per project) and stamps only confidence-gated, unambiguous known goal ids. Entries that survive all passes contribute to the `unattributableLegacy` bucket on the `/tree-cost` response (an informational residual row with optional `firstSeenAt`, NOT added to `totalCostUsd`) rather than silently reading `$0.0000`.
...
Pinned by `tests/cost-tracker-goal-stamp.test.ts`, `tests/cost-tracker-backfill.test.ts`, `tests/cost-backfill.test.ts`, `tests/cost-backfill-transcript-pass.test.ts`, `tests/tree-cost-purge-survival.test.ts`, and `tests/e2e/cost-backfill-on-boot.spec.ts`. Full design in [docs/nested-goals.md — Cost rollup](nested-goals.md#cost-rollup) and [docs/cost-backfill.md](cost-backfill.md).
