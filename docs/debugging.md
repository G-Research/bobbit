...
## Leaked `pr-walkthrough` child processes that reappear after restart

- **Symptom**: leaked walkthrough reviewer child processes (~100 MB each) accumulate on the host and come back after every gateway restart even though the walkthroughs are long finished.
- **Cause**: reviewer children are first-class **persisted** sessions, but finished walkthroughs were never terminated. `restoreSessions()` respawns every persisted session on boot, so each restart resurrected all of them at once — the count only grew.
- **Where the fix lives (current, generic)**: the PR-walkthrough reviewer is now a `childKind: "host-agents"` child minted via `host.agents.spawn`, and cleanup is the **generic** `OrchestrationCore` lifecycle — there is no PR-walkthrough-specific launcher/reap any more (the legacy `WalkthroughAgentManager`, `walkthrough-reap.ts`, and the submit-proof secret were deleted). (1) The `submit-yaml` server route dismisses the reviewer terminal-synchronously and stamps a generic persisted `childTerminal` marker via `SessionManager.markChildTerminal` (also stamped by `dismiss`). (2) `OrchestrationCore.shouldReapChildOnBoot` reaps any child whose `childTerminal` is set, or whose parent is gone/archived — reading only generic session fields, never a pack store or a kind-specific branch. (3) Archiving/terminating the owner cascade-reaps the reviewer like any other `host.agents` child. See [docs/orchestration.md — Restart survival / Archive cascade-reap](orchestration.md#restart-survival) and [docs/design/pr-walkthrough-host-agents-migration.md](design/pr-walkthrough-host-agents-migration.md).
- **Pinning tests**: `tests/orchestration-core.test.ts` / `tests/host-agents-scope.test.ts` (generic `childTerminal`→reap, cascade-reap, terminated-child-reapable) and `tests/e2e/pr-walkthrough-host-agents.spec.ts` (reviewer spawn + cleanup).
