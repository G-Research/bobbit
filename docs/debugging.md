...
## `read_session` transcript errors

Cross-project reads are allowed for any session the authenticated caller can reach on the same gateway; the `x-bobbit-session-id` header no longer gates transcript access by matching `projectId`. Remaining structured error codes are `session_not_found`, `transcript_unavailable`, `invalid_regex`, and `invalid_params`. Files: `src/server/agent/transcript-reader.ts`, `defaults/tools/agent/read_session.yaml` + `extension.ts`.
