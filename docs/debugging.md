...
- **Cause**:
  ...
  - A component has distinct sibling maps: `commands` holds named shell commands, native `env` holds generic literal non-secret workflow-command environment, and opaque `config` holds skill settings. The seven `qa_*` keys no longer live at the top level; they belong in `components[<name>].config[<key>]`. Retired `qa_env` is not component `env`: QA authors inline it into `qa_start_command`. The wire-level rejection forwards a migration message pointing at the new location.
- **Fix**:
  ...
  - For QA keys: PUT a `components` array with the `qa_*` keys nested under the relevant component's `config:` map. Inline env vars (formerly `qa_env`) directly into `qa_start_command` itself, single-quoted with `'\''` escapes for embedded quotes. Do not put QA startup configuration or secrets in `components[].env`.
- **On-disk legacy form is still tolerated**: `ProjectConfigStore` parses legacy JSON-string and quoted-numeric values for `config_directories` / `sandbox_tokens` transparently via `getConfigDirectories()` / `getSandboxTokens()` and rewrites the file in native form on the next save. The first-boot migration in `state-migration/migrate-project-yaml.ts` moves any top-level `qa_*` keys it finds onto the relevant component's `config:` map (inlining `qa_env` into `qa_start_command`) and deletes the originals. Only the wire format is strict. See [docs/internals.md — Native-YAML project.yaml fields](internals.md#native-yaml-projectyaml-fields), [Multi-repo & components](internals.md#multi-repo--components), and [Project command environments](project-command-environments.md).

## Component command environment is missing or stale

- **Symptom**: a workflow command cannot see an expected component setting, a component setting appears to affect the wrong command, or a save seems to require a gateway restart.
- **Check the command shape first**: both `{ component, command }` and `{ component, run }` select `components[<name>].env`; only a pure `{ run }` has no component layer. A step's declared `env` overrides the selected component map. `commands` is only the named command map, while opaque `config` is for skill consumers such as QA—not command environment.
- **Check persistence and timing**: Components orders the maps as Commands, Command Environment, Config. Save the component card normally; values are literal plaintext (not shell expansion or secrets) and a later invocation reads the updated declaration without restart. A command already running retains its spawn-time snapshot.
- **Check scope**: component command environment does not configure QA startup, agent sessions, worktree setup, or the gateway. `qa_env` is retired; QA values remain in `config.qa_start_command` as literal command text.
- **Reference**: [Project command environments](project-command-environments.md).
