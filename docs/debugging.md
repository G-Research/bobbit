...
- **Symptom: sustained `[event-loop-lag] observed …ms delay after session-event:message_update:broadcast` with many live sessions, main thread pinned at 100%** — the lag label names the last *measured* operation, not necessarily the culprit; the broadcast itself is usually cheap. Take a CPU profile of the gateway without restarting it: `kill -USR1 <pid>` opens the inspector on `127.0.0.1:9229`, then drive `Profiler.start`/`stop` over the debugger WebSocket (or attach Chrome DevTools). Known offenders that were fixed this way: (1) per-event role resolution (`prepareVisibleAgentEvent` → `agentAuthorForSession` → `ConfigCascade.resolveRoles` → `RoleLoader.load` → `parseRolesDir`) re-reading and YAML-parsing every pack's role files for every streamed token — now cached by directory fingerprint in `builtin-config.ts` (pinned by `tests/unit/core/roles-dir-parse-cache.unit.test.ts`); (2) the worktree inventory spawning one `git show-ref` per candidate worktree (hundreds of synchronous `spawn`s per scan, and cleanup re-scans per item) — now one `git for-each-ref refs/heads/` per repository. If a profile shows a new synchronous hot loop under a streaming frame, wrap it in a cache keyed on the inputs that can actually change and drop it from `invalidateResolverCaches()`.
...
- **Restart worktree boundary** — graceful shutdown stops each `WorktreePool`, flushes its exact ownership record in `state/worktree-pools.json`, and retains unclaimed entries for the next start. Re-adoption requires that v1 record, the same project repository, exact Git path/branch agreement for every member, and no persisted or runtime live-session reference; any failed check drops authority and leaves the path untouched. Branch/path shape alone never authorizes adoption, repair, or deletion. Explicit project removal still drains the live pool locally. Maintenance cleanup remains separately authorized only by an exact archived-session repository/path/non-empty-branch record.
...
- Regression test: [`tests/browser/journeys/proposals.journey.spec.ts`](../tests/browser/journeys/proposals.journey.spec.ts) — stubs a 400 from `POST /api/goals` and asserts the assistant panel, chat, and `gateway.sessionId` survive.
...
- Regression test: [`tests/browser/journeys/goal-empty-workflows-banner.journey.spec.ts`](../tests/browser/journeys/goal-empty-workflows-banner.journey.spec.ts).
...
- Reports referencing screenshots via `<img src="file://...">` are inlined by the verifier-side extension when the agent submits `report_html_file`. The report must be a non-symlink regular file no larger than 10 MiB. Images must remain canonically inside the verifier cwd with stable file/root identities through the descriptor read; symlink escapes and ancestor symlink/junction swaps remain unchanged. Inlining has a 20 MiB source-image budget, and each rewrite must keep final UTF-8 HTML within 10 MiB. The gateway receives only `report_html` bytes and never reads the supplied path. See [qa-testing.md — Screenshots in QA reports](qa-testing.md#screenshots-in-qa-reports).
- If `verification_result` returns `403` with `VERIFIER_SESSION_SECRET_REQUIRED`, call it from the exact pending verifier session. Admin bearer credentials, browser cookies, copied public session IDs, missing secrets, and another session's secret do not authorize the callback; sandbox scope still applies.
- A direct API payload containing `report_html_file` is rejected intentionally. Use the verifier-side tool so it can perform bounded local reads. Direct `report_html` is accepted as opaque HTML, and its `file://` references are not dereferenced or rewritten by the gateway.
...
## Tools catalogue disagrees with guard, provider, or registered implementation

- **Symptoms:** `/api/tools` shows one policy or pack but the session asks, denies, omits, or runs a different tool; a renderer/action comes from another pack; or Pi reports a duplicate/unknown implementation. Do not copy the apparent winning override into every project. That hides the defect and breaks inherited server/global customization.
- **Confirm the same scope first:** request both `GET /api/tools?projectId=<id>` and `GET /api/tools/<name>?projectId=<id>`. Headquarters callers must pass `projectId=headquarters`; normal projects must pass their registered id. Compare canonical name, `grantPolicy`, group, `origin`, `overrides`, and `originPackId`/`originPackName`. A case-only duplicate should produce one row with the winner's authored spelling.
- **Follow provenance:** for ordinary tools, the raw `ConfigCascade.resolveToolsEntries(projectId)` / `ToolManager.getResolvedToolEntry(name)` record should identify the same scope, pack, exact source file, and ordered shadows. If a high market candidate is disabled, absent, or invalid, the record should show the revealed lower winner rather than the filtered candidate. Check `/api/tools` response `diagnostics` and gateway tool-extension preflight logs before assuming a missing override was ignored.
- **Check role policy before YAML:** inspect the effective role's exact per-tool and per-group `toolPolicies`. A role override intentionally wins over the resolved YAML policy; case-insensitive ordinary tool keys are accepted, while ambiguous case-only role keys warn and use a stable first match. Then check the existing persisted MCP hierarchy or tool-group policy as applicable. The shipped `session_prompt` YAML remains `never` unless an explicit higher-priority definition or role policy changes it.
- **Inspect the generated guard:** locate the active content-addressed `tool-guard/<hash>/guard.ts` under Headquarters state and compare its policy map with the scoped API row after applying the role/group layers. `ask` and `never` both require a guard; `never` is also omitted from the effective activation allowlist. If the guard reflects another project, trace the session's project-owned `ToolManager`, group-policy provider, and `McpManager` tuple through setup/restore.
- **Inspect winner-filtered activation:** Pi arguments should name `tool-extension-activation/<hash>/extension.ts`, not the raw shared group module. Its adjacent `manifest.json` must list the expected canonical tool in `allowedToolNames` and the expected target identity. `BOBBIT_TOOL_EXTENSION_TARGETS` must map that manifest's adapter id to the captured winner target. A missing/invalid mapping, corrupt artifact, target-capture failure, or target outside mounted roots should fail closed and emit a tool-activation or RPC-bridge diagnostic; it must never fall back to the raw extension.
- **Check lifecycle consistency:** setup, cold restore, role replacement, restart/respawn, and force-abort must all plan required `task_create`, `team_spawn`, or `propose_goal` names through the same manager-backed adapters. A fixed raw lifecycle module path is only a compatibility path when no `ToolManager` exists. If one lifecycle works only before or after restart, compare the manager/generation and adapter manifest from both activations.
- **Check activation and caches:** verify the winning pack and concrete tool are enabled in `GET /api/marketplace/pack-activation`. Whole-pack or per-tool disablement happens before merge and should reveal the same lower winner everywhere. Customization and marketplace mutations invalidate resolver/discovery state immediately; there is no TTL to wait out. Restarting may prove a stale process symptom, but is not the repair for a repeatable winner mismatch.
- **Check sandbox mounts:** inspect the container mounts for read-only adapter state at `/bobbit-state/tool-extension-activation`, ordinary tools at `/tools`, `/tools-global-user`, and `/tools-project`, and relevant builtin/market pack roots. Missing, duplicate, writable, or wrong-source ordinary-tool binds and missing/mismatched adapter state mark a container stale and should recreate it. If remapping says the target is outside mounted roots, repair the winner path or recreate the stale sandbox; never add a broad host mount as a workaround.
- **Regression suites:** `tests/unit/core/tool-resolution-parity.unit.test.ts` covers precedence, case folding, provenance, partial groups, validation fallback, disabled fallback, policy/provider/docs/surfaces, and adapter targets. The focused adapter publication/activation and guard/API integration suites pin atomic validation, registration filtering, lifecycle parity, and sandbox remapping.

See [Internals — Session tool runtime scope](internals.md#session-tool-runtime-scope), [Internals — Winner-filtered extension activation](internals.md#winner-filtered-extension-activation), and [Marketplace — Authoritative tool resolution](marketplace.md#authoritative-tool-resolution).
