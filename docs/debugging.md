...
For a Claude Agent SDK source, do not inspect a clone path: its history remains in the original official SDK conversation. Check the persisted runtime, UUID, and cwd, then confirm the SDK conversation remains accessible from that cwd. The opaque `503 SDK_SESSION_UNAVAILABLE` category means the source conversation is unavailable, not that a Pi clone failed; malformed UUID/model metadata instead returns `422 RUNTIME_CONTINUE_UNSUPPORTED`. For a Docker sandbox session, history is read from the same pooled container through bounded SDK calls, not from a host path or Pi JSONL. See [Claude Agent SDK sessions](claude-agent-sdk-sessions.md) and [Continue-Archived endpoint](rest-api.md#continue-archived-endpoint).

## Sandboxed Claude Agent SDK session will not start

- **Failure category**: `SDK_SESSION_UNAVAILABLE` is the public response. The safe diagnostic categories are `CLAUDE_AGENT_SDK_AUTH_UNAVAILABLE`, `CLAUDE_AGENT_SDK_SANDBOX_AUTH_UNAVAILABLE`, `CLAUDE_AGENT_SDK_SANDBOX_UNAVAILABLE`, and `CLAUDE_AGENT_SDK_RATE_LIMITED`; never expose provider bodies, paths, IDs, or credentials. Reconnect OAuth, repair the policy/image/workspace/callback prerequisite, then retry the existing session—do not add an API key, copy SDK state, start a replacement conversation, or fall back to Pi.
- **Image and workspace checks**: rebuild `bobbit-agent` with Agent SDK `0.3.222`, bundled Claude `2.1.222`, Pi `0.82.1`, runtime schema `2`, the image-owned dependency, and executable `bobbit-claude-agent-sdk` wrapper under `bobbit-sdk`. Bobbit also requires node-owned workspace roots, validates selected-workspace access, and translates the scoped callback to a container-reachable address. A host binary/dependency tree or rewritten callback is not a substitute.
- **Confirm placement**: a ready sandbox SDK session runs in the project's existing pooled container; it does not own a second container. On force-abort, restore, or container recovery, Bobbit rebuilds launch data and resumes the persisted SDK UUID.
- **History and secrecy**: history is a bounded, read-only SDK call in that container and receives neither OAuth nor gateway authority. The SDK child gets only allowlisted variables plus a short-lived OAuth token; never log, inspect, copy, persist, or dump it or a host/container auth environment.
- **Deterministic checks**: run `tests2/core/claude-agent-sdk-sandbox-spawn.test.ts`, `tests2/core/anthropic-sandbox-handoff-regression.test.ts`, and `tests2/integration/claude-agent-sdk-sandbox-runtime.test.ts` with Vitest before attempting a credentialed run.
- **Manual dogfood remains pending user signoff**: first use a fresh owner-only temporary `BOBBIT_AGENT_DIR`, authenticate through a separate loopback Bobbit gateway, and export it to Playwright as `MANUAL_CLAUDE_AGENT_SDK_AUTH_DIR` before directory reset or auth-sensitive imports. Never copy/paste auth data, mix this subscription OAuth with enterprise Anthropic OAuth in one normal instance, or delete the temporary directory before evidence review and signoff. Then run `BOBBIT_RUN_CLAUDE_AGENT_SDK_SANDBOX_SMOKE=1 MANUAL_CLAUDE_AGENT_SDK_MODEL=claude-sonnet-4-5 MANUAL_CLAUDE_AGENT_SDK_AUTH_DIR="$MANUAL_CLAUDE_AGENT_SDK_AUTH_DIR" npm run test:manual -- --grep "Docker sandbox lifecycle"`. The isolated Custom Provider/default model does not configure production. An idle session may be created before SDK auth/provider construction; the first accepted input is the canonical startup boundary. See [Pending credentialed Docker dogfood](claude-agent-sdk-sessions.md#pending-credentialed-docker-dogfood).

See [Claude Agent SDK sessions](claude-agent-sdk-sessions.md#docker-sandbox-sessions) for the operator contract and prerequisites.
