...
- Reports referencing screenshots via `<img src="file://...">` are inlined to base64 by the verifier-side tool when the agent submits via `report_html_file` (20 MB cumulative cap, canonical session-cwd scope). The gateway receives only report bytes and never reads the submitted path. See [qa-testing.md — Screenshots in QA reports](qa-testing.md#screenshots-in-qa-reports).
