...
- **Diagnostic chain**:
  ...
  3. Widget not pulsing despite `awaitingHuman: true` on the API? The sign-off cache bit (`state.gateStatusCache.<goalId>.awaitingHumanSignoff`) didn't reach the widget. Two pieces must line up: (a) the cache-refresh fetch uses `?view=summary` (only the summary endpoint includes `awaitingSignoffCount`); (b) `remote-agent.ts` has a `case "gate_verification_awaiting_human"` that calls `refreshGateStatusForGoal()`. Both were the regression fixed by commit `a75ca58` — if either is missing again, rule 2 of the notification policy is also dormant.
