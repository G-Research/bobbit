...
- **If it recurs**: grep server stdout for `[pause] forceAbort failed for session=` (sweep saw the session but `forceAbort` threw — inspect the per-session error). If the sweep didn't see a session at all, check that the session's `goalId` field is populated (orphan sessions with `goalId === undefined` are intentionally skipped). If a team-lead respawned, confirm `_bootRespawnSessionlessGoals` still has the `if (goal.paused) continue;` guard — the pinning test source-greps the predicate.
