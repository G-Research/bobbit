...
- **Fix**: after lifecycle admission and before cache reuse, the REST handler synchronously calls `fenceStaleVerificationsForGates(goalId, [gateId], cause)`. It durably fences cancellation before detached exact cleanup; if persistence fails, the route returns retryable `503 VERIFICATION_CANCELLATION_FENCE_FAILED` and admits neither a cached nor replacement signal. On success it follows fence → cache decision → `VerificationHarness.beginVerification(signal, gate)` → `recordSignal` → received → started → fire-and-forget `verifyGateSignal`. Synchronous enumeration writes the returned `GateSignalStep[]` atomically with the gate-store signal, and `verifyGateSignal` reuses the pre-seeded active entry instead of re-creating it. See [docs/gate-signal-step-enumeration.md](gate-signal-step-enumeration.md) for the full design.
- **If it recurs**: (1) confirm the `gate_signal` handler fences stale work after lifecycle admission and before cache reuse; a fence-persistence failure must return retryable `503 VERIFICATION_CANCELLATION_FENCE_FAILED` before either cached or replacement admission; (2) confirm the successful order is fence → cache decision → `beginVerification` → `recordSignal` → received → started → fire-and-forget `verifyGateSignal`; (3) confirm `verifyGateSignal` reads the pre-seeded `activeVerifications` entry rather than constructing a fresh one, and that seeded `GateSignalStep.status` values let the dashboard render `running`/`waiting` correctly.
