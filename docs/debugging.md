...
## Session hangs in `streaming` with no output (silent SSE)

- **Symptom** — session pinned at `status: streaming` for minutes with zero bytes received, no error in UI, status heartbeat re-broadcasting unchanged. Usually triggered by remote LLM SSE stream going silent (provider issue, dropped TCP keepalive, gateway buffering).
- **Why it can happen** — pi-coding-agent's `cli.js` disables undici's idle-gap timeout globally to protect buffered local-vLLM responses. Without bobbit's preload, that disabling applies to remote origins too.
- **Fix** — bobbit injects an undici dispatcher preload that re-enables idle timeouts for remote origins only. See [docs/internals.md — Idle-stream timeout preload](internals.md#idle-stream-timeout-preload-remote-llm-http) for env vars (`BOBBIT_REMOTE_BODY_TIMEOUT_MS`, `BOBBIT_REMOTE_HEADERS_TIMEOUT_MS`, `BOBBIT_TRUSTED_NO_TIMEOUT_ORIGINS`) and the origin classification rules.
- **Old containers** — a pre-`preload-1` Docker container won't have the bind mount; first session use should recreate it. If not, check `docker ps -a --filter label=bobbit-project-version` and bump / delete the stale container.
