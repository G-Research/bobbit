...
Fix: cost entries are stamped with `goalId` at record time (`SessionCost.goalId`, set write-once in `CostTracker.recordUsage`). `getGoalCost(goalId)` scans entries by stamped goalId instead of consulting `sessionStore`, decoupling cost from session lifetime. A boot-time backfill (`backfillLegacyCostGoalIds` in `src/server/agent/cost-backfill.ts`, wired in `src/server/server.ts`) runs once after session restore and tries two recovery paths for each unstamped entry: (1) live persisted session record (`getPersistedSession`), (2) sidecar scan — walks the agent sessions root for `*.bobbit.json` sidecars and indexes `bobbitSessionId → teamGoalId`. Entries that survive both passes contribute to the `unattributableLegacy` bucket on the `/tree-cost` response (an informational residual row, NOT added to `totalCostUsd`) rather than silently reading `$0.0000`.
...
Pinned by `tests/cost-tracker-goal-stamp.test.ts`, `tests/cost-tracker-backfill.test.ts`, `tests/cost-backfill.test.ts`, `tests/tree-cost-purge-survival.test.ts`, and `tests/e2e/cost-backfill-on-boot.spec.ts`. Full design in [docs/nested-goals.md — Cost rollup](nested-goals.md#cost-rollup).
