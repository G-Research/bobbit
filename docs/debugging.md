...
For a Claude Agent SDK source, do not inspect a clone path: its history remains in the original official SDK conversation. Check the persisted runtime, UUID, and cwd, then confirm the SDK conversation remains accessible from that cwd. `404 SDK_SESSION_UNAVAILABLE` means the source conversation is unavailable, not that a Pi clone failed; malformed UUID/model metadata instead returns `422 RUNTIME_CONTINUE_UNSUPPORTED`. For a Docker sandbox session, history is read from the same pooled container through bounded SDK calls, not from a host path or Pi JSONL. See [Claude Agent SDK sessions](claude-agent-sdk-sessions.md) and [Continue-Archived endpoint](rest-api.md#continue-archived-endpoint).

## Sandboxed Claude Agent SDK session will not start

- **`CLAUDE_AGENT_SDK_SANDBOX_AUTH_UNAVAILABLE`**: the Docker SDK path requires an explicitly enabled, empty `ANTHROPIC_OAUTH_TOKEN` project sandbox-token entry and a current local Anthropic OAuth subscription. It rejects project or host API-key credentials and does not fall back to `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, generic sandbox credentials, host settings, or a host SDK process. Enable that policy, sign in again, and retry; do not add an API key.
- **`CLAUDE_AGENT_SDK_SANDBOX_UNAVAILABLE`**: the SDK-only image capability, fixed launcher, container cwd, or scoped gateway authority was unavailable. Rebuild the `bobbit-agent` image after an SDK/server upgrade, then retry. A scoped-authority error can require a gateway restart. Do not substitute a globally installed `claude` binary.
- **Confirm placement**: a ready sandbox SDK session runs in the project's existing pooled container, with cwd `/workspace` or `/workspace-wt/...`; it does not own a second container. On force-abort, restore, or container recovery, Bobbit rebuilds the launch data for the current container and resumes the same persisted SDK UUID.
- **History and secrecy**: history is a bounded, read-only SDK call in that same container and receives neither OAuth nor gateway authority. The SDK child gets only allowlisted process variables plus a short-lived `CLAUDE_CODE_OAUTH_TOKEN`; its value must never appear in logs, errors, persistence, test output, or diagnostics. Do not inspect or copy host credential files or a container PID 1 environment.
...
- **Manual dogfood is pending, not evidence**: only with Docker, a rebuilt matching image, an active local OAuth subscription, and an unprefixed model id, run `BOBBIT_RUN_CLAUDE_AGENT_SDK_SANDBOX_SMOKE=1 MANUAL_CLAUDE_AGENT_SDK_MODEL=claude-sonnet-4-5 npm run test:manual -- --grep "Docker sandbox lifecycle"`. The smoke consumes `MANUAL_CLAUDE_AGENT_SDK_MODEL` only to configure its isolated Custom Provider and default session model; it does not select or configure a production runtime. The scenario provisions its explicit OAuth policy and checks readiness, prompt, steer, interrupt, stop, replacement, and restart resume; it must not be represented as executed unless its credentialed result is captured. See [Pending credentialed Docker dogfood](claude-agent-sdk-sessions.md#pending-credentialed-docker-dogfood).
