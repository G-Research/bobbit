...
## Sidebar nav feels slow

- **Enable the client trace.** `localStorage.setItem('bobbitPerf','1')` and reload, or load any URL with `?perf=1`. The trace primitive is `src/app/perf-trace.ts` and is a no-op when disabled.
- **Inspect live in DevTools.** `window.__bobbitPerf.entries()` returns the recent span ring buffer; `.summary()` aggregates p50/p95 per span. Canonical spans on the click → ready critical path: `nav.click` → `nav.session.ready` / `nav.goal.ready`, with `api.session.fetch`, `ws.attach`, `reducer.rehydrate`, `paint.first` as contributors.
- **Server side.** Restart the gateway with `BOBBIT_TIMING_LOG=1` and watch for `[timing] METHOD path Xms bytes=B io=N` lines on the five hot endpoints (`GET /api/sessions/:id`, `GET /api/goals/:id`, `/gates`, `/team/agents`, `/tool-content/...`). `bytes=` and `io=` distinguish payload-size from disk-IO cost.
- **Reproduce against a baseline.** Run the manual harness — `npx playwright test --config playwright-manual.config.ts --grep "perf-sidebar-nav"`. NOT in CI. Use `BOBBIT_PERF_FIXTURE_SIZE=large` to stress with a 200-msg-per-session transcript fixture. Output lands in `tests/manual-integration/.perf-out/` and a new `docs/perf/history/<sha>.json` entry.
- **Compare across commits.** Open `docs/perf/sidebar-nav-report.html` (regenerated by `scripts/perf-report.mjs` at the end of every harness run). The chart shows p50/p95 trend per span across every committed history file.
- **Ranked hotspots + decision rule + "tried, didn't pay off"** live in [docs/perf/sidebar-nav-baseline.md](perf/sidebar-nav-baseline.md). Orientation: [docs/perf/README.md](perf/README.md).
