...
const BUDGETS_PATH = join(REPO_ROOT, "tests", "support", "data", "quality", "budgets", "budgets.json");
...
export function resolveCaps(scope, pilot = false, underLoadOverride = undefined) {
	...
	if (!caps) throw new Error(`assert-budget: tests/support/data/quality/budgets/budgets.json has no tier "${tier}"`);
...
export function assertBudget({ scope, wallMs = null, cpuMs = null, rawCpuMs = null, cpuCeilingMs = null, cpuOverCount = false, pilot = false, underLoad = undefined, argv = null, reservations = null, rootPid = null, processCount = null, wallSource = null, cpuSource = null, perSpec = null }) {
	...
	// CPU-min is OBSERVABILITY ONLY — it NEVER gates (design §D7 + tests/support/data/quality/budgets/budgets.json:
