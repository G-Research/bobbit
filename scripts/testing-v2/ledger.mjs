 ...
 *           the granted count + a release() that removes the entry. Vitest 4's
 *           pool transport has no worker-RPC timeout, so standalone and lane
 *           orchestrator reservations share the same ledger cap.
 *
...
const BUDGET_CAPS_PATH = join(LEDGER_FILE_DIR, "..", "..", "tests", "support", "data", "shared", "budget-caps.json");
...
/** Read a committed per-pool cap from tests/support/data/shared/budget-caps.json (missing → null). */
 ...
 *
 * Priority: opts.cap (tests) > env override (tuning) > tests/support/data/shared/budget-caps.json
...
function cliSelftest() {
	...
	assert(solo.workerSlots <= VITEST_CAP, `standalone vitest capped at ${VITEST_CAP}, got ${solo.workerSlots}`);
