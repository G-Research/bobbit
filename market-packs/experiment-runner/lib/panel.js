var ze=["cost.totalUsd","cost.tokensTotal","cost.cacheHitRate","gates.passRate","gates.firstPassClean","tasks.completionRate","time.wallClockMs","objective.value","command.metric"],Ee={"cost.totalUsd":"lower-better","cost.tokensTotal":"lower-better","cost.cacheHitRate":"higher-better","gates.passRate":"higher-better","gates.firstPassClean":"higher-better","tasks.completionRate":"higher-better","time.wallClockMs":"lower-better","objective.value":"higher-better","command.metric":"neutral"},Be=new Set(["cost.totalUsd","time.wallClockMs","gates.passRate","objective.value"]),Z=[{id:"comparison-table",label:"Comparison table"},{id:"score-bars",label:"Score bars"},{id:"objective-curve",label:"Objective curve"},{id:"ledger-table",label:"Ledger"},{id:"summary-cards",label:"Summary cards"},{id:"raw-drilldown",label:"Raw runs"}],je=["median","mean","p90","min","max","count"],C={exp:n=>`exp/${n}`,state:n=>`exp/${n}/state`,runPrefix:n=>`exp/${n}/run/`,ledger:n=>`exp/${n}/ledger`,dashboard:n=>`exp/${n}/dashboard`,metrics:n=>`exp/${n}/metrics`,index:"index/experiments",draft:n=>`drafts/${n}`},Oe="bobbit:experiment-runner:draft:",f=n=>Array.isArray(n)?n:[],b=(n,l="")=>n==null?l:String(n),v=n=>{let l=Number(n);return Number.isFinite(l)?l:void 0},D=n=>b(n,"exp").replace(/[^a-zA-Z0-9_-]+/g,"-").replace(/^-+|-+$/g,"")||"exp";function De(n){let l=b(n).trim();if(l==="")return"";if(/^-?\d+(\.\d+)?$/.test(l))return Number(l);if(l==="true")return!0;if(l==="false")return!1;if(l.startsWith("{")&&l.endsWith("}")||l.startsWith("[")&&l.endsWith("]"))try{return JSON.parse(l)}catch{}return l}function H(n){let l={};for(let h of f(n)){let m=b(h&&h.key).trim();m&&(l[m]=De(h&&h.value))}return l}function V(n){let l=b(n).trim();if(l)try{let h=JSON.parse(l);return h&&typeof h=="object"?h:void 0}catch{return}}var Ne=n=>{let l=n.filter(m=>Number.isFinite(m)).slice().sort((m,p)=>m-p);if(!l.length)return;let h=Math.floor(l.length/2);return l.length%2?l[h]:(l[h-1]+l[h])/2},Q=(n,l)=>{let h=n.filter(m=>Number.isFinite(m));if(l==="count")return h.length;if(h.length)switch(l){case"mean":return h.reduce((m,p)=>m+p,0)/h.length;case"min":return Math.min(...h);case"max":return Math.max(...h);case"p90":{let m=h.slice().sort((p,w)=>p-w);return m[Math.min(m.length-1,Math.floor(.9*m.length))]}default:return Ne(h)}},j=n=>{if(n==null||!Number.isFinite(n))return"\u2014";let l=Math.abs(n);return l!==0&&l<.01?n.toExponential(2):Number.isInteger(n)?String(n):n.toFixed(l>=100?1:3)},O=n=>n==null||!Number.isFinite(n)?"\u2014":`$${n.toFixed(2)}`;function F(){return[{key:"",value:""}]}function ee(){return ze.map(n=>({metric:n,source:"built-in",collect:Be.has(n),aggregation:"median",direction:Ee[n]||"neutral",primary:n==="gates.passRate"}))}function G(){return{view:"mode-select",mode:null,experimentId:void 0,basics:{name:"",runnableUnit:"command",body:"",workflowId:""},ab:{variants:[{label:"baseline",metadata:F(),rolesJson:"",rolesOpen:!1},{label:"variant-b",metadata:F(),rolesJson:"",rolesOpen:!1}],repeats:3,sameCompletionBar:!0,concurrency:3},auto:{objectiveMetric:"objective.value",direction:"maximize",correctnessGateId:"",seed:F(),seedRolesJson:"",caps:{maxIterations:"",wallClockHours:"",costUsd:"",perIterBudget:""},stops:{plateauK:"",target:""},strategy:"greedy",batchSize:""},metrics:ee(),perRunBudget:"",confirmAck:!1}}var R=globalThis.__bobbitExperimentRunnerState||(globalThis.__bobbitExperimentRunnerState=new Map);function Ue(n){let l=[],h=n.basics||{};b(h.name).trim()||l.push("Name is required"),b(h.body).trim()||l.push("Spec / command body is required");let m=f(n.ab&&n.ab.variants);m.length<2&&l.push("A/B needs at least two variants");let p=new Set,w=[];m.forEach((k,A)=>{let L=b(k.label).trim();L?p.has(L)&&l.push(`Variant label "${L}" is duplicated`):l.push(`Variant ${A+1} needs a label`),p.add(L),w.push(JSON.stringify({m:H(k.metadata),r:V(k.rolesJson)||null}))});for(let k=0;k<w.length;k++)for(let A=k+1;A<w.length;A++)w[k]===w[A]&&l.push(`Variant "${b(m[A].label).trim()||A+1}" is identical to "${b(m[k].label).trim()||k+1}"`);let I=v(n.ab&&n.ab.repeats);(!I||I<1)&&l.push("Repeats must be \u2265 1");let z=v(n.perRunBudget);(!z||z<=0)&&l.push("Set a per-run budget");let E=v(n.ab&&n.ab.concurrency);E!=null&&(E<1||E>8)&&l.push("Concurrency must be 1\u20138"),f(n.metrics).some(k=>k.collect)||l.push("Select at least one metric");let B=m.length*(I||0),U=z?B*z:void 0;return{valid:l.length===0,errors:l,runCount:B,estCostMax:U}}function Le(n){let l=[],h=[],m=n.basics||{};b(m.name).trim()||l.push("Name is required"),b(m.body).trim()||l.push("Spec / command body is required");let p=n.auto||{};b(p.objectiveMetric).trim()||l.push("Choose an objective metric");let w=v(p.caps&&p.caps.perIterBudget);(!w||w<=0)&&l.push("Set a per-iteration budget");let I=p.caps||{},z=v(I.maxIterations)>0||v(I.wallClockHours)>0||v(I.costUsd)>0,E=p.stops||{},B=v(E.plateauK)>0||E.target!==""&&Number.isFinite(v(E.target));z||h.push("Set at least one hard cap (max-iterations, wall-clock, or cost)"),B||h.push("Set at least one stop condition (plateau-K or target)"),n.confirmAck||h.push("Acknowledge the autonomous-run warning");let U=v(I.maxIterations),k=v(I.costUsd),A;return w&&U&&(A=U*w),k!=null&&(A=A==null?k:Math.min(A,k)),{valid:l.length===0&&h.length===0,errors:l,checklist:h,estCostMax:A,hasCap:z,hasStop:B}}function K(n){return n.mode==="autoresearch"?Le(n):Ue(n)}function Pe(n){let l=n.basics||{},h=f(n.metrics).filter(p=>p.collect).map(p=>({metric:p.metric,aggregation:p.aggregation,direction:p.direction,primary:!!p.primary})),m={experimentId:n.experimentId,title:b(l.name).trim(),mode:n.mode==="autoresearch"?"autoresearch":"ab",runnable:{kind:l.runnableUnit==="goal"?"goal":"command",body:b(l.body)},workflowId:b(l.workflowId).trim()||void 0,metrics:h};if(m.mode==="ab"){let p=n.ab||{};m.variants=f(p.variants).map((w,I)=>({armId:D(b(w.label).trim()||`arm-${I}`),label:b(w.label).trim()||`arm-${I}`,metadata:H(w.metadata),inlineRoles:V(w.rolesJson)})),m.repeats=v(p.repeats)||1,m.sameCompletionBar=p.sameCompletionBar!==!1,m.maxConcurrency=v(p.concurrency)||3,m.perRunBudget=v(n.perRunBudget)}else{let p=n.auto||{};m.objective={metric:p.objectiveMetric,direction:p.direction==="minimize"?"minimize":"maximize"},m.correctnessGateId=b(p.correctnessGateId).trim()||void 0,m.seed={metadata:H(p.seed),inlineRoles:V(p.seedRolesJson)},m.caps={maxIterations:v(p.caps&&p.caps.maxIterations),wallClockMs:v(p.caps&&p.caps.wallClockHours)?v(p.caps.wallClockHours)*36e5:void 0,maxCostUsd:v(p.caps&&p.caps.costUsd),perRunBudget:v(p.caps&&p.caps.perIterBudget)},m.stop={plateauK:v(p.stops&&p.stops.plateauK),target:v(p.stops&&p.stops.target)},m.strategy=p.strategy==="best-of-batch"?"best-of-batch":"greedy",m.batchSize=v(p.batchSize),m.perRunBudget=v(p.caps&&p.caps.perIterBudget)}return m}function Fe({html:n,nothing:l,renderHeader:h}){let m=async(e,t,r)=>{try{return!e||!e.capabilities||!e.capabilities.callRoute||!e.callRoute?{ok:!1,error:"routes-unavailable"}:{ok:!0,data:await e.callRoute(t,r)}}catch(a){return{ok:!1,error:a&&a.message?String(a.message):String(a)}}},p=async(e,t)=>{try{return e&&e.store&&e.store.get?await e.store.get(t):null}catch{return null}},w=async(e,t,r)=>{try{e&&e.store&&e.store.put&&await e.store.put(t,r)}catch{}},I=async(e,t)=>{try{return e&&e.store&&e.store.list?await e.store.list(t)||[]:[]}catch{return[]}},z=e=>{try{e&&e.requestRender&&e.requestRender()}catch{}},E=(e,t)=>{try{e&&e.capabilities&&e.capabilities.ui&&e.ui&&e.ui.navigate&&e.ui.navigate({route:"experiment-runner",params:t})}catch{}},B=e=>`${Oe}${D(e)}`,U=e=>{try{let t=globalThis.localStorage&&globalThis.localStorage.getItem(B(e));return t?JSON.parse(t):void 0}catch{return}},k=(e,t)=>{try{globalThis.localStorage&&globalThis.localStorage.setItem(B(e),JSON.stringify(t))}catch{}},A=e=>R.get(e),L=(e,t,r)=>{R.set(t,r),z(e)},M=(e,t,r)=>{let s={...R.get(t)||{},...r};return R.set(t,s),z(e),s},T=(e,t,r)=>{let a=R.get(t)||{},s={...a.draft||G()};r(s);let i={...a,draft:s};R.set(t,i),k(t,s),w(a.host,C.draft(t),s),z(e)};function te(e,t,r,a){let s=R.get(t);return s&&s.hydrated?(s.host=e,r&&s.draft&&s.draft.experimentId!==r&&re(e,t,r,a),s):(s={hydrated:!1,host:e,draft:U(t)||G(),dashboard:null,experiments:[]},R.set(t,s),(async()=>{let i=await p(e,C.draft(t)),c=R.get(t)||s,d=i&&typeof i=="object"?i:c.draft;r&&(d={...d,experimentId:r,view:a||"dashboard"}),R.set(t,{...c,hydrated:!0,draft:d}),z(e),ae(e,t),d.experimentId&&d.view==="dashboard"&&N(e,t,d.experimentId)})(),R.get(t))}async function ae(e,t){let r=await m(e,"listExperiments",{method:"GET"}),a=[];if(r.ok&&r.data&&Array.isArray(r.data.experiments))a=r.data.experiments;else{let s=await p(e,C.index);s&&Array.isArray(s.experiments)&&(a=s.experiments)}M(e,t,{experiments:a})}async function re(e,t,r,a){T(e,t,s=>{s.experimentId=r,s.view=a||"dashboard"}),await N(e,t,r)}async function N(e,t,r){M(e,t,{dashboardLoading:!0});let a,s,i=[],c=[],d,o,u=await m(e,"getExperiment",{method:"GET",query:{experimentId:r}});if(u.ok&&u.data&&u.data.def&&(a=u.data.def,s=u.data.state,i=f(u.data.runs),c=f(u.data.ledger),d=u.data.dashboard,o=u.data.metrics),!a){a=await p(e,C.exp(r)),s=await p(e,C.state(r)),c=f(await p(e,C.ledger(r))),d=await p(e,C.dashboard(r)),o=await p(e,C.metrics(r));let x=await I(e,C.runPrefix(r));for(let y of x){let S=await p(e,y);S&&typeof S=="object"&&i.push(S)}}if(d==null&&(d=await p(e,C.dashboard(r))),!f(o).length){let x=await p(e,C.metrics(r));f(x).length&&(o=x)}if(s&&s.status==="running"){let x=await m(e,"poll",{method:"POST",body:{experimentId:r}});x.ok&&x.data&&Array.isArray(x.data.runs)&&(i=x.data.runs)}let g=await m(e,"report",{method:"POST",body:{experimentId:r}}),$=g.ok&&g.data?g.data:null;M(e,t,{dashboardLoading:!1,dashboard:{experimentId:r,def:a,state:s,runs:i,ledger:c,spec:d,metrics:f(o).length?o:a&&a.metrics||[],report:$}})}async function ne(e,t){let a=R.get(t).draft;M(e,t,{launching:!0,launchError:void 0});let s=Pe(a),i=await m(e,"defineExperiment",{method:"POST",body:{definition:s}}),c=a.experimentId;i.ok&&i.data&&i.data.experimentId&&(c=i.data.experimentId),c||(c=`${D(s.title)}-${Date.now().toString(36)}`),s.experimentId=c,await w(e,C.exp(c),s),await w(e,C.metrics(c),s.metrics),await se(e,t,{experimentId:c,title:s.title,mode:s.mode,status:"running"});let d=await m(e,"launch",{method:"POST",body:{experimentId:c}});if(!d.ok&&d.error!=="routes-unavailable"){M(e,t,{launching:!1,launchError:d.error});return}T(e,t,o=>{o.experimentId=c,o.view="dashboard"}),M(e,t,{launching:!1}),E(e,{experimentId:c,view:"dashboard"}),await N(e,t,c),s.mode==="autoresearch"&&await m(e,"iterate",{method:"POST",body:{experimentId:c}})}async function se(e,t,r){let a=await p(e,C.index)||{experiments:[]},s=f(a.experiments).filter(i=>i.experimentId!==r.experimentId);s.push(r),await w(e,C.index,{experiments:s}),M(e,t,{experiments:s})}async function ie(e,t,r){await m(e,"cancel",{method:"POST",body:{experimentId:r}}),await N(e,t,r)}async function oe(e,t,r,a){await m(e,"saveMetrics",{method:"POST",body:{experimentId:r,metrics:a}}),await w(e,C.metrics(r),a),await N(e,t,r)}async function ce(e,t,r,a){let s=Array.isArray(a)?{widgets:a}:a&&Array.isArray(a.widgets)?a:{widgets:[]};await m(e,"saveDashboard",{method:"POST",body:{experimentId:r,dashboard:s}}),await w(e,C.dashboard(r),s),M(e,t,{dashboardEditing:!1}),await N(e,t,r)}let _=(e,t,r)=>T(e,t,a=>{a.view=r});function de(e,t,r){let a=s=>T(e,t,i=>{i.mode=s,i.view="define"});return n`
		...
		`}function le(e,t,r){let a=r.basics||{},s=(i,c)=>T(e,t,d=>{d.basics={...d.basics,[i]:c}});return n`
			<section class="exp-card" data-testid="experiment-runner-basics">
				...
				<label class="exp-label">Experiment name
					<input class="exp-input" data-testid="experiment-runner-name" type="text" maxlength="80"
						placeholder="e.g. retry-temperature-sweep" .value=${b(a.name)}
				...
				<label class="exp-label">${a.runnableUnit==="goal"?"Goal spec":"Command"} body
					<textarea class="exp-input exp-mono" data-testid="experiment-runner-body" rows="4"
						...
						.value=${b(a.body)} @input=${i=>s("body",i.currentTarget.value)}></textarea>
				</label>
				<label class="exp-label">Workflow (optional)
					<input class="exp-input" data-testid="experiment-runner-workflow" type="text"
						placeholder="workflow id (optional)" .value=${b(a.workflowId)}
						@input=${i=>s("workflowId",i.currentTarget.value)} />
				</label>
			</section>
		`}function W(e,t,r,a,s){let i=c=>a(c.length?c:F());return n`
				...
				${f(r).map((c,d)=>n`
					<div class="exp-kv-row">
						<input class="exp-input exp-kv-key" type="text" placeholder="key" .value=${b(c.key)}
							@input=${o=>{let u=r.slice();u[d]={...c,key:o.currentTarget.value},i(u)}} />
						<input class="exp-input exp-kv-val" type="text" placeholder="value" .value=${b(c.value)}
							@input=${o=>{let u=r.slice();u[d]={...c,value:o.currentTarget.value},i(u)}} />
						<button class="exp-icon-btn" type="button" title="Remove" aria-label="Remove key"
							@click=${()=>{let o=r.slice();o.splice(d,1),i(o)}}>✕</button>
					</div>`)}
				<button class="exp-btn secondary tiny" type="button" @click=${()=>i([...f(r),{key:"",value:""}])}>+ Add key</button>
			</div>
		`}function pe(e,t,r){let a=f(r.metrics),s=(i,c)=>T(e,t,d=>{let o=f(d.metrics).slice();c.primary&&o.forEach((u,g)=>{o[g]={...u,primary:g===i}}),o[i]={...o[i],...c},d.metrics=o});return n`
		...
		`}function ue(e,t,r){let a=r.ab||{},s=f(a.variants),i=g=>T(e,t,$=>{$.ab={...$.ab,...g}}),c=(g,$)=>T(e,t,x=>{let y=f(x.ab.variants).slice();y[g]={...y[g],...$},x.ab={...x.ab,variants:y}}),d=g=>T(e,t,$=>{let x=f($.ab.variants).slice();x.splice(g,1),$.ab={...$.ab,variants:x}}),o=g=>T(e,t,$=>{let x=f($.ab.variants).slice(),y=g!=null?x[g]:null;x.push({label:`variant-${x.length+1}`,metadata:y?y.metadata.map(S=>({...S})):F(),rolesJson:y?y.rolesJson:"",rolesOpen:!1}),$.ab={...$.ab,variants:x}}),u=v(a.repeats);return n`
			<section class="exp-card" data-testid="experiment-runner-ab-form">
				...
				<div class="exp-grid2">
					<label class="exp-label">Repeats per variant
						<input class="exp-input" type="number" min="1" max="20" data-testid="experiment-runner-repeats"
							.value=${b(a.repeats)} @input=${g=>i({repeats:g.currentTarget.value})} />
					...
					<label class="exp-label">Concurrency cap
						<input class="exp-input" type="number" min="1" max="8" data-testid="experiment-runner-concurrency"
							.value=${b(a.concurrency)} @input=${g=>i({concurrency:g.currentTarget.value})} />
					</label>
				</div>
				...
				<label class="exp-label">Per-run budget (USD, the fixed comparable budget)
					<input class="exp-input" type="number" min="0" step="0.5" data-testid="experiment-runner-per-run-budget"
						placeholder="e.g. 0.80" .value=${b(r.perRunBudget)}
						@input=${g=>T(e,t,$=>{$.perRunBudget=g.currentTarget.value})} />
				</label>
			</section>
		`}function xe(e,t,r){let a=r.auto||{},s=o=>T(e,t,u=>{u.auto={...u.auto,...o}}),i=o=>T(e,t,u=>{u.auto={...u.auto,caps:{...u.auto.caps,...o}}}),c=o=>T(e,t,u=>{u.auto={...u.auto,stops:{...u.auto.stops,...o}}}),d=f(r.metrics).map(o=>o.metric);return n`
			...
			<section class="exp-card" data-testid="experiment-runner-auto-objective">
				...
				<label class="exp-label">Correctness gate (optional workflow gate)
					<input class="exp-input" type="text" data-testid="experiment-runner-correctness-gate" placeholder="review-findings gate id (optional)"
						.value=${b(a.correctnessGateId)} @input=${o=>s({correctnessGateId:o.currentTarget.value})} />
				...
				${W(e,t,a.seed,o=>s({seed:o}),"experiment-runner-seed-metadata")}
			</section>
				...
				<div class="exp-grid2">
					<label class="exp-label">Max iterations
						<input class="exp-input" type="number" min="1" data-testid="experiment-runner-cap-max-iterations"
							.value=${b(a.caps.maxIterations)} @input=${o=>i({maxIterations:o.currentTarget.value})} />
					</label>
					<label class="exp-label">Wall-clock cap (hours)
						<input class="exp-input" type="number" min="0" step="0.5" data-testid="experiment-runner-cap-wallclock"
							.value=${b(a.caps.wallClockHours)} @input=${o=>i({wallClockHours:o.currentTarget.value})} />
					</label>
					<label class="exp-label">Cost cap (USD)
						<input class="exp-input" type="number" min="0" step="1" data-testid="experiment-runner-cap-cost"
							.value=${b(a.caps.costUsd)} @input=${o=>i({costUsd:o.currentTarget.value})} />
					</label>
					<label class="exp-label">Per-iteration budget (USD, required)
						<input class="exp-input" type="number" min="0" step="0.5" data-testid="experiment-runner-per-iter-budget"
							.value=${b(a.caps.perIterBudget)} @input=${o=>i({perIterBudget:o.currentTarget.value})} />
					</label>
				</div>
			</section>
				...
				<div class="exp-grid2">
					<label class="exp-label">Plateau over K iterations
						<input class="exp-input" type="number" min="1" data-testid="experiment-runner-stop-plateau"
							.value=${b(a.stops.plateauK)} @input=${o=>c({plateauK:o.currentTarget.value})} />
					</label>
					<label class="exp-label">Target value
						<input class="exp-input" type="number" step="any" data-testid="experiment-runner-stop-target"
							.value=${b(a.stops.target)} @input=${o=>c({target:o.currentTarget.value})} />
					</label>
				</div>
				<details class="exp-details">
					...
					<div class="exp-grid2">
						...
						<label class="exp-label">Batch size
							<input class="exp-input" type="number" min="1" max="8" .value=${b(a.batchSize)}
								@input=${o=>s({batchSize:o.currentTarget.value})} />
						</label>
					</div>
				</details>
			</section>
		`}function be(e,t,r,a){if(r.mode==="autoresearch"){let s=f(a.checklist);return n`
			...
			<footer class="exp-projection" data-testid="experiment-runner-projection">
				<div class="exp-proj-stats">
					<span data-testid="experiment-runner-run-count">${f(r.ab&&r.ab.variants).length} variants × ${v(r.ab&&r.ab.repeats)||0} repeats = ${a.runCount} runs</span>
					<span data-testid="experiment-runner-cost">${a.estCostMax!=null?`est. \u2264 ${O(a.estCostMax)}`:"est. \u2014 set a per-run budget"}</span>
					<span>~${v(r.ab&&r.ab.concurrency)||1} concurrent</span>
				</div>
				${f(a.errors).length?n`<ul class="exp-checklist" data-testid="experiment-runner-error">
		...
		`}function me(e,t,r){let a=K(r);return n`
			<div class="exp-view exp-define" data-testid="experiment-runner-view-define" data-mode=${r.mode||"ab"}>
				...
				${le(e,t,r)}
				${r.mode==="autoresearch"?xe(e,t,r):ue(e,t,r)}
				${pe(e,t,r)}
				${be(e,t,r,a)}
			</div>
		`}function ge(e,t,r){let a=K(r),s=r.mode==="autoresearch",i=R.get(t)||{};return n`
			<div class="exp-view" data-testid="experiment-runner-view-confirm">
				...
				<section class="exp-card">
					...
					<div class="exp-confirm-row"><span>Name</span><strong>${b(r.basics&&r.basics.name)}</strong></div>
					${s?n`
							<div class="exp-confirm-row"><span>Objective</span><strong>${b(r.auto&&r.auto.objectiveMetric)} (${b(r.auto&&r.auto.direction)})</strong></div>
							<div class="exp-confirm-row"><span>Caps</span><strong>${b(v(r.auto.caps.maxIterations)?`\u2264 ${v(r.auto.caps.maxIterations)} iters`:"")} ${v(r.auto.caps.wallClockHours)?`\u2264 ${v(r.auto.caps.wallClockHours)}h`:""} ${v(r.auto.caps.costUsd)?`\u2264 ${O(v(r.auto.caps.costUsd))}`:""}</strong></div>
							...
							<div class="exp-confirm-row"><span>Fan-out</span><strong>${a.runCount} child goals (${f(r.ab.variants).length} variants × ${v(r.ab.repeats)} repeats)</strong></div>
				...
				<div class="exp-confirm-actions">
					...
					<button class="exp-btn primary" type="button" data-testid="experiment-runner-launch" ?disabled=${!a.valid||i.launching}
						@click=${()=>ne(e,t)}>${i.launching?"Launching\u2026":s?`Launch loop (\u2264 ${a.estCostMax!=null?O(a.estCostMax):"capped"})`:`Launch ${a.runCount} runs`}</button>
				</div>
			</div>
		`}function X(e){let t=e&&Array.isArray(e.spec)?e.spec:e&&e.spec&&Array.isArray(e.spec.widgets)?e.spec.widgets:null;return t&&t.length?t:e&&e.def&&e.def.mode==="autoresearch"?[{type:"summary-cards",title:"Summary"},{type:"objective-curve",title:"Best objective vs iteration"},{type:"ledger-table",title:"Ledger"},{type:"raw-drilldown",title:"Iterations"}]:[{type:"summary-cards",title:"Summary"},{type:"comparison-table",title:"Comparison"},{type:"score-bars",title:"Secondary metrics"},{type:"raw-drilldown",title:"Runs"}]}function J(e){let r=f(e&&e.metrics).filter(a=>a.collect!==!1).map(a=>a.metric);return r.length?r:["objective.value","cost.totalUsd","time.wallClockMs"]}function fe(e){let r=f(e&&e.metrics).find(a=>a.primary);return r?r.metric:e&&e.def&&e.def.objective?e.def.objective.metric:J(e)[0]}let P=(e,t)=>{let r=e&&e.metrics,a=r?r[t]:void 0;return Number.isFinite(Number(a))?Number(a):a&&Number.isFinite(Number(a.value))?Number(a.value):void 0};function Y(e){let t=new Map;for(let r of f(e&&e.runs)){let a=b(r.armId,"arm");t.has(a)||t.set(a,[]),t.get(a).push(r)}return t}function ve(e,t){let r=J(t),a=!(t.def&&t.def.sameCompletionBar===!1),s=Y(t),i=f(t.metrics),c=d=>(i.find(o=>o.metric===d)||{}).aggregation||"median";return n`<table class="exp-table" data-testid="experiment-runner-widget-comparison-table">
			<thead><tr><th>Variant</th>${r.map(d=>n`<th class="exp-mono">${d}</th>`)}<th>n</th></tr></thead>
			<tbody>
				${[...s.entries()].map(([d,o])=>{let u=a?o.filter($=>$.completionBar==="passed"):o,g=u.length?u:o;return n`<tr data-testid="experiment-runner-comparison-arm" data-arm=${d}>
						<td><strong>${d}</strong></td>
						${r.map($=>n`<td class="exp-mono">${j(Q(g.map(x=>P(x,$)),c($)))}</td>`)}
						<td>${g.length}</td>
					</tr>`})}
			</tbody>
		</table>`}function he(e,t){let r=J(t),a=Y(t);return n`<div class="exp-scorebars" data-testid="experiment-runner-widget-score-bars">
			${r.map((s,i)=>{let c=[...a.entries()].map(([o,u])=>({arm:o,v:Q(u.map(g=>P(g,s)),"median")})),d=Math.max(1,...c.map(o=>Number.isFinite(o.v)?Math.abs(o.v):0));return n`<div class="exp-scorebar-group"><div class="exp-field-label exp-mono">${s}</div>
		...
		</div>`}function $e(e,t){let r=fe(t),a=t.def&&t.def.objective&&t.def.objective.direction||"maximize",s=f(t.runs).filter(o=>o.iteration!=null).sort((o,u)=>o.iteration-u.iteration),i=null,c=s.map(o=>{let u=P(o,r);return Number.isFinite(u)&&(i=i==null?u:a==="minimize"?Math.min(i,u):Math.max(i,u)),{iteration:o.iteration,v:u,best:i,kept:o.verified!==!1&&o.completionBar!=="failed"}}),d=v(t.def&&t.def.stop&&t.def.stop.target);return n`<div data-testid="experiment-runner-widget-objective-curve">
			${d!=null?n`<div class="exp-hint">target ${a==="minimize"?"\u2264":"\u2265"} ${j(d)}</div>`:l}
		...
		</div>`}function ye(e,t){let r=f(t.ledger);return n`<table class="exp-table" data-testid="experiment-runner-widget-ledger-table">
			<thead><tr><th>Iter</th><th>verdict</th><th>objective</th><th>best</th></tr></thead>
			<tbody>${r.map(a=>{let s=b(a.verdict||a.decision,"\u2014"),i=/kept|accept/i.test(s)?"exp-pos":/verification|failed/i.test(s)?"exp-neg":"exp-muted";return n`<tr data-testid="experiment-runner-ledger-row"><td>${b(a.iteration)}</td><td class=${i}>${s}</td><td class="exp-mono">${j(v(a.objective))}</td><td class="exp-mono">${j(v(a.best))}</td></tr>`})}</tbody>
		</table>`}function we(e,t){let r=f(t.runs),a=r.filter(c=>["settled","collected","failed"].includes(c.status)).length,s=r.filter(c=>c.completionBar==="passed").length,i=r.reduce((c,d)=>c+(v(d.cost&&d.cost.totalUsd)||P(d,"cost.totalUsd")||0),0);return n`<div class="exp-cards" data-testid="experiment-runner-widget-summary-cards">
		...
		</div>`}function ke(e,t){let r=J(t),a=f(t.runs);return n`<table class="exp-table" data-testid="experiment-runner-widget-raw-drilldown">
			...
			<tbody>${a.map(s=>{let i=t.def&&t.def.sameCompletionBar!==!1&&s.completionBar&&s.completionBar!=="passed";return n`<tr class=${i?"exp-excluded":""} data-testid="experiment-runner-run-row" data-run=${b(s.runId)}>
					...
					${r.map(c=>n`<td class="exp-mono">${j(P(s,c))}</td>`)}
				</tr>`})}</tbody>
		</table>`}let Se={"comparison-table":ve,"score-bars":he,"objective-curve":$e,"ledger-table":ye,"summary-cards":we,"raw-drilldown":ke};function Ce(e){try{let t=document.createElement("div");return t.setAttribute("data-testid","experiment-runner-report-html"),t.innerHTML=String(e),t}catch{return l}}function Te(e,t,r){if(r.report&&typeof r.report.html=="string"&&r.report.html.trim())return n`<div class="exp-dashboard-body" data-testid="experiment-runner-dashboard-body">${Ce(r.report.html)}</div>`;let a=X(r);return n`<div class="exp-dashboard-body" data-testid="experiment-runner-dashboard-body">
			${a.map(s=>{let i=Se[s.type];return n`<section class="exp-widget exp-card" data-testid="experiment-runner-widget" data-widget-type=${s.type}>
					<h3 class="exp-widget-title">${b(s.title,s.type)}</h3>
					${i?i(e,r):n`<div class="exp-hint">Unknown widget: ${s.type}</div>`}
				</section>`})}
		</div>`}function Re(e,t,r){let a=X(r).slice(),s=x=>M(e,t,{dashboardDraftSpec:x}),i=R.get(t)||{},c=i.dashboardDraftSpec||a,d=(x,y)=>{let S=c.slice(),q=x+y;q<0||q>=S.length||([S[x],S[q]]=[S[q],S[x]],s(S))},o=x=>{let y=c.slice();y.splice(x,1),s(y)},u=x=>s([...c,{type:x,title:(Z.find(y=>y.id===x)||{}).label||x}]),g=(x,y)=>{let S=c.slice();S[x]={...S[x],title:y},s(S)},$=i.widgetTypes&&i.widgetTypes.length?i.widgetTypes:Z;return n`<div class="exp-card" data-testid="experiment-runner-dashboard-editor">
			...
			<div class="exp-confirm-actions">
				<button class="exp-btn secondary" type="button" @click=${()=>M(e,t,{dashboardEditing:!1,dashboardDraftSpec:void 0})}>Cancel</button>
				<button class="exp-btn primary" type="button" data-testid="experiment-runner-save-dashboard"
					@click=${()=>{M(e,t,{dashboardDraftSpec:void 0}),ce(e,t,r.experimentId,c)}}>Save dashboard</button>
			</div>
		</div>`}function Ie(e,t,r){let a=R.get(t)||{},s=a.dashboard,i=()=>M(e,t,{dashboard:null})&&T(e,t,y=>{Object.assign(y,G())});if(a.dashboardLoading&&!s)return n`<div class="exp-view" data-testid="experiment-runner-view-dashboard"><div class="exp-hint">Loading experiment…</div></div>`;if(!s)return n`<div class="exp-view" data-testid="experiment-runner-view-dashboard">
				...
				<header class="exp-dash-head">
					...
					<div class="exp-dash-actions">
						...
						<button class="exp-btn secondary" type="button" data-testid="experiment-runner-edit-dashboard"
							@click=${()=>M(e,t,{dashboardEditing:!a.dashboardEditing,dashboardDraftSpec:void 0})}>${a.dashboardEditing?"Close editor":"Edit dashboard"}</button>
						<button class="exp-btn link" type="button" data-testid="experiment-runner-new-experiment" @click=${i}>New experiment</button>
					</div>
				</header>
				${a.dashboardEditing?Re(e,t,s):l}
		...
		`}function Ae(e,t,r){let a=f(r.metrics).length?f(r.metrics):ee(),s=(i,c)=>{let d=a.map((o,u)=>u===i?{...o,collect:c}:o);M(e,t,{dashboard:{...r,metrics:d}}),oe(e,t,r.experimentId,d)};return n`<table class="exp-table">
			<thead><tr><th>Collect</th><th>Metric</th></tr></thead>
			<tbody>${a.map((i,c)=>n`<tr><td><input type="checkbox" data-testid="experiment-runner-dash-metric-collect" data-metric=${i.metric}
	...
	`;return{render(e,t){let r=e&&typeof e.__sessionId=="string"?e.__sessionId:"",a=e&&typeof e.experimentId=="string"?e.experimentId:"",s=e&&typeof e.view=="string"?e.view:void 0,i=r||"experiment-runner",c=te(t,i,a,s),d=c&&c.draft||G(),o=d.view||"mode-select",u;return o==="dashboard"?u=Ie(t,i,d):o==="confirm"?u=ge(t,i,d):o==="define"?u=me(t,i,d):u=de(t,i,d),n`
				<style>${Me}</style>
