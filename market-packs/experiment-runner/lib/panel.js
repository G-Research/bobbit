var je=["cost.totalUsd","cost.tokensTotal","cost.cacheHitRate","gates.passRate","gates.firstPassClean","tasks.completionRate","time.wallClockMs","objective.value","command.metric"],Be={"cost.totalUsd":"lower-better","cost.tokensTotal":"lower-better","cost.cacheHitRate":"higher-better","gates.passRate":"higher-better","gates.firstPassClean":"higher-better","tasks.completionRate":"higher-better","time.wallClockMs":"lower-better","objective.value":"higher-better","command.metric":"neutral"},Oe=new Set(["cost.totalUsd","time.wallClockMs","gates.passRate","objective.value"]),K=[{id:"comparison-table",label:"Comparison table"},{id:"score-bars",label:"Score bars"},{id:"objective-curve",label:"Objective curve"},{id:"ledger-table",label:"Ledger"},{id:"summary-cards",label:"Summary cards"},{id:"raw-drilldown",label:"Raw runs"}],Ne=["median","mean","p90","min","max","count"],A={exp:n=>`exp/${n}`,state:n=>`exp/${n}/state`,runPrefix:n=>`exp/${n}/run/`,ledger:n=>`exp/${n}/ledger`,dashboard:n=>`exp/${n}/dashboard`,metrics:n=>`exp/${n}/metrics`,index:"index/experiments",draft:n=>`drafts/${n}`},De="bobbit:experiment-runner:draft:",v=n=>Array.isArray(n)?n:[],u=(n,l="")=>n==null?l:String(n),$=n=>{let l=Number(n);return Number.isFinite(l)?l:void 0},D=n=>u(n,"exp").replace(/[^a-zA-Z0-9_-]+/g,"-").replace(/^-+|-+$/g,"")||"exp";function Ue(n){let l=u(n).trim();if(l==="")return"";if(/^-?\d+(\.\d+)?$/.test(l))return Number(l);if(l==="true")return!0;if(l==="false")return!1;if(l.startsWith("{")&&l.endsWith("}")||l.startsWith("[")&&l.endsWith("]"))try{return JSON.parse(l)}catch{}return l}function V(n){let l={};for(let y of v(n)){let h=u(y&&y.key).trim();h&&(l[h]=Ue(y&&y.value))}return l}function W(n){let l=u(n).trim();if(l)try{let y=JSON.parse(l);return y&&typeof y=="object"?y:void 0}catch{return}}var Le=n=>{let l=n.filter(h=>Number.isFinite(h)).slice().sort((h,k)=>h-k);if(!l.length)return;let y=Math.floor(l.length/2);return l.length%2?l[y]:(l[y-1]+l[y])/2},ee=(n,l)=>{let y=n.filter(h=>Number.isFinite(h));if(l==="count")return y.length;if(y.length)switch(l){case"mean":return y.reduce((h,k)=>h+k,0)/y.length;case"min":return Math.min(...y);case"max":return Math.max(...y);case"p90":{let h=y.slice().sort((k,S)=>k-S);return h[Math.min(h.length-1,Math.floor(.9*h.length))]}default:return Le(y)}},O=n=>{if(n==null||!Number.isFinite(n))return"\u2014";let l=Math.abs(n);return l!==0&&l<.01?n.toExponential(2):Number.isInteger(n)?String(n):n.toFixed(l>=100?1:3)},N=n=>n==null||!Number.isFinite(n)?"\u2014":`$${n.toFixed(2)}`;function J(){return[{key:"",value:""}]}function ae(){return je.map(n=>({metric:n,source:"built-in",collect:Oe.has(n),aggregation:"median",direction:Be[n]||"neutral",primary:n==="gates.passRate"}))}function H(){return{view:"mode-select",mode:null,experimentId:void 0,basics:{name:"",runnableUnit:"command",body:"",workflowId:""},ab:{variants:[{label:"baseline",metadata:J(),rolesJson:"",rolesOpen:!1},{label:"variant-b",metadata:J(),rolesJson:"",rolesOpen:!1}],repeats:3,sameCompletionBar:!0,concurrency:3},auto:{objectiveMetric:"objective.value",direction:"maximize",correctnessGateId:"",seed:J(),seedRolesJson:"",caps:{maxIterations:"",wallClockHours:"",costUsd:"",perIterBudget:""},stops:{plateauK:"",target:""},strategy:"greedy",batchSize:""},metrics:ae(),perRunBudget:"",confirmAck:!1}}var E=globalThis.__bobbitExperimentRunnerState||(globalThis.__bobbitExperimentRunnerState=new Map);function Pe(n){let l=[],y=n.basics||{};u(y.name).trim()||l.push("Name is required"),u(y.body).trim()||l.push("Spec / command body is required");let h=v(n.ab&&n.ab.variants);h.length<2&&l.push("A/B needs at least two variants");let k=new Set,S=[];h.forEach((T,j)=>{let P=u(T.label).trim();P?k.has(P)&&l.push(`Variant label "${P}" is duplicated`):l.push(`Variant ${j+1} needs a label`),k.add(P),S.push(JSON.stringify({m:V(T.metadata),r:W(T.rolesJson)||null}))});for(let T=0;T<S.length;T++)for(let j=T+1;j<S.length;j++)S[T]===S[j]&&l.push(`Variant "${u(h[j].label).trim()||j+1}" is identical to "${u(h[T].label).trim()||T+1}"`);let C=$(n.ab&&n.ab.repeats);(!C||C<1)&&l.push("Repeats must be \u2265 1");let f=$(n.perRunBudget);(!f||f<=0)&&l.push("Set a per-run budget");let I=$(n.ab&&n.ab.concurrency);I!=null&&(I<1||I>8)&&l.push("Concurrency must be 1\u20138"),v(n.metrics).some(T=>T.collect)||l.push("Select at least one metric");let M=h.length*(C||0),L=f?M*f:void 0;return{valid:l.length===0,errors:l,runCount:M,estCostMax:L}}function Fe(n){let l=[],y=[],h=n.basics||{};u(h.name).trim()||l.push("Name is required"),u(h.body).trim()||l.push("Spec / command body is required");let k=n.auto||{};u(k.objectiveMetric).trim()||l.push("Choose an objective metric");let S=$(k.caps&&k.caps.perIterBudget);(!S||S<=0)&&l.push("Set a per-iteration budget");let C=k.caps||{},f=$(C.maxIterations)>0||$(C.wallClockHours)>0||$(C.costUsd)>0,I=k.stops||{},M=$(I.plateauK)>0||I.target!==""&&Number.isFinite($(I.target));f||y.push("Set at least one hard cap (max-iterations, wall-clock, or cost)"),M||y.push("Set at least one stop condition (plateau-K or target)"),n.confirmAck||y.push("Acknowledge the autonomous-run warning");let L=$(C.maxIterations),T=$(C.costUsd),j;return S&&L&&(j=L*S),T!=null&&(j=j==null?T:Math.min(j,T)),{valid:l.length===0&&y.length===0,errors:l,checklist:y,estCostMax:j,hasCap:f,hasStop:M}}function te(n){return n.mode==="autoresearch"?Fe(n):Pe(n)}function _e(n){let l=n.basics||{},y={"higher-better":"max","lower-better":"min"},h=v(n.metrics).filter(f=>f.collect).map(f=>{let I={metricId:f.metric,aggregation:f.aggregation,primary:!!f.primary},M=y[f.direction];return M&&(I.directionOverride=M),I}),S=l.runnableUnit==="command"?{kind:"command",command:u(l.body)}:{kind:"agent",spec:u(l.body)},C={experimentId:n.experimentId,title:u(l.name).trim(),mode:n.mode==="autoresearch"?"autoresearch":"ab",runnable:S,workflowId:u(l.workflowId).trim()||void 0,metrics:h};if(C.mode==="ab"){let f=n.ab||{};C.variants=v(f.variants).map((I,M)=>({armId:D(u(I.label).trim()||`arm-${M}`),label:u(I.label).trim()||`arm-${M}`,metadata:V(I.metadata),inlineRoles:W(I.rolesJson)})),C.repeats=$(f.repeats)||1,C.sameCompletionBar=f.sameCompletionBar!==!1,C.maxConcurrency=$(f.concurrency)||3,C.perRunBudget=$(n.perRunBudget)}else{let f=n.auto||{};C.objective={metricId:f.objectiveMetric,direction:f.direction==="minimize"?"min":"max"},C.correctnessGateId=u(f.correctnessGateId).trim()||void 0,C.seed={metadata:V(f.seed),inlineRoles:W(f.seedRolesJson)};let I=$(f.caps&&f.caps.wallClockHours);C.caps={maxIterations:$(f.caps&&f.caps.maxIterations),maxWallClockMs:I?I*36e5:void 0,maxCostUsd:$(f.caps&&f.caps.costUsd)};let M=$(f.stops&&f.stops.target);C.stop={plateauK:$(f.stops&&f.stops.plateauK)},M!=null&&(C.stop.target=M),C.strategy=f.strategy==="best-of-batch"?"best-of-batch":"greedy",C.batchSize=$(f.batchSize),C.perRunBudget=$(f.caps&&f.caps.perIterBudget)}return C}function Je({html:n,nothing:l,renderHeader:y}){let h=async(e,t,r)=>{try{if(!e||!e.capabilities||!e.capabilities.callRoute||!e.callRoute)return{ok:!1,error:"routes-unavailable"};let a=await e.callRoute(t,r);return a&&typeof a=="object"&&a.error?{ok:!1,error:a.error}:{ok:!0,data:a}}catch(a){return{ok:!1,error:a&&a.message?String(a.message):String(a)}}},k=async(e,t)=>{try{return e&&e.store&&e.store.get?await e.store.get(t):null}catch{return null}},S=async(e,t,r)=>{try{e&&e.store&&e.store.put&&await e.store.put(t,r)}catch{}},C=async(e,t)=>{try{return e&&e.store&&e.store.list?await e.store.list(t)||[]:[]}catch{return[]}},f=e=>{try{e&&e.requestRender&&e.requestRender()}catch{}},I=(e,t)=>{try{e&&e.capabilities&&e.capabilities.ui&&e.ui&&e.ui.navigate&&e.ui.navigate({route:"experiment-runner",params:t})}catch{}},M=e=>`${De}${D(e)}`,L=e=>{try{let t=globalThis.localStorage&&globalThis.localStorage.getItem(M(e));return t?JSON.parse(t):void 0}catch{return}},T=(e,t)=>{try{globalThis.localStorage&&globalThis.localStorage.setItem(M(e),JSON.stringify(t))}catch{}},j=e=>E.get(e),P=(e,t,r)=>{E.set(t,r),f(e)},z=(e,t,r)=>{let s={...E.get(t)||{},...r};return E.set(t,s),f(e),s},R=(e,t,r)=>{let a=E.get(t)||{},s={...a.draft||H()};r(s);let i={...a,draft:s};E.set(t,i),T(t,s),S(a.host,A.draft(t),s),f(e)};function re(e,t,r,a){let s=E.get(t);return s&&s.hydrated?(s.host=e,r&&s.draft&&s.draft.experimentId!==r&&se(e,t,r,a),s):(s={hydrated:!1,host:e,draft:L(t)||H(),dashboard:null,experiments:[]},E.set(t,s),(async()=>{let i=await k(e,A.draft(t)),c=E.get(t)||s,d=i&&typeof i=="object"?i:c.draft;r&&(d={...d,experimentId:r,view:a||"dashboard"}),E.set(t,{...c,hydrated:!0,draft:d}),f(e),ne(e,t),d.experimentId&&d.view==="dashboard"&&U(e,t,d.experimentId)})(),E.get(t))}let X=e=>({experimentId:e.experimentId,title:u(e.title,e.experimentId),mode:e.mode==="autoresearch"?"autoresearch":"ab"});async function ne(e,t){let r=await h(e,"listExperiments",{method:"GET"}),a=[];if(r.ok&&Array.isArray(r.data))a=r.data.filter(s=>s&&typeof s=="object").map(X);else{let s=v(await k(e,A.index)).filter(c=>typeof c=="string");a=(await Promise.all(s.map(c=>k(e,A.exp(c))))).filter(c=>c&&typeof c=="object").map(X)}z(e,t,{experiments:a})}async function se(e,t,r,a){R(e,t,s=>{s.experimentId=r,s.view=a||"dashboard"}),await U(e,t,r)}async function U(e,t,r){z(e,t,{dashboardLoading:!0});let a,s,i=[],c=[],d,o,p=await h(e,"getExperiment",{method:"GET",query:{experimentId:r}});if(p.ok&&p.data&&p.data.def&&(a=p.data.def,s=p.data.state,i=v(p.data.runs),c=v(p.data.ledger),d=p.data.dashboard,o=p.data.metrics),!a){a=await k(e,A.exp(r)),s=await k(e,A.state(r)),c=v(await k(e,A.ledger(r))),d=await k(e,A.dashboard(r)),o=await k(e,A.metrics(r));let b=await C(e,A.runPrefix(r));for(let w of b){let B=await k(e,w);B&&typeof B=="object"&&i.push(B)}}if(d==null&&(d=await k(e,A.dashboard(r))),!v(o).length){let b=await k(e,A.metrics(r));v(b).length&&(o=b)}let g=b=>v(b).some(w=>w&&w.status==="settled");if(s&&s.status==="running"){let b=await h(e,"poll",{method:"POST",body:{experimentId:r}});if(b.ok&&b.data&&Array.isArray(b.data.runs)&&(i=b.data.runs),g(i)){let w=await h(e,"collect",{method:"POST",body:{experimentId:r}});w.ok&&w.data&&Array.isArray(w.data.runs)&&(i=w.data.runs)}}let m=await h(e,"report",{method:"POST",body:{experimentId:r}}),x=m.ok&&m.data?m.data:null;z(e,t,{dashboardLoading:!1,dashboard:{experimentId:r,def:a,state:s,runs:i,ledger:c,spec:d,metrics:v(o).length?o:a&&a.metrics||[],report:x}})}async function ie(e,t){let a=E.get(t).draft;z(e,t,{launching:!0,launchError:void 0});let s=_e(a),i=await h(e,"defineExperiment",{method:"POST",body:s});if(!i.ok&&i.error!=="routes-unavailable"){z(e,t,{launching:!1,launchError:i.error});return}let c=a.experimentId;i.ok&&i.data&&i.data.experimentId&&(c=i.data.experimentId),c||(c=`${D(s.title)}-${Date.now().toString(36)}`),s.experimentId=c,await S(e,A.exp(c),s),await S(e,A.metrics(c),s.metrics),await oe(e,t,{experimentId:c,title:s.title,mode:s.mode,status:"running"});let d=s.mode==="autoresearch"?"iterate":"launch",o=await h(e,d,{method:"POST",body:{experimentId:c}});if(!o.ok&&o.error!=="routes-unavailable"){z(e,t,{launching:!1,launchError:o.error});return}R(e,t,p=>{p.experimentId=c,p.view="dashboard"}),z(e,t,{launching:!1}),I(e,{experimentId:c,view:"dashboard"}),await U(e,t,c)}async function oe(e,t,r){let a=v(await k(e,A.index)).filter(c=>typeof c=="string"&&c!==r.experimentId);a.push(r.experimentId),await S(e,A.index,a);let s=E.get(t)||{},i=v(s.experiments).filter(c=>c.experimentId!==r.experimentId);i.push(r),z(e,t,{experiments:i})}async function ce(e,t,r){await h(e,"cancel",{method:"POST",body:{experimentId:r}}),await U(e,t,r)}async function de(e,t,r,a){await h(e,"saveMetrics",{method:"POST",body:{experimentId:r,metrics:a}}),await S(e,A.metrics(r),a),await U(e,t,r)}async function le(e,t,r,a){let s=Array.isArray(a)?{widgets:a}:a&&Array.isArray(a.widgets)?a:{widgets:[]};await h(e,"saveDashboard",{method:"POST",body:{experimentId:r,dashboard:s}}),await S(e,A.dashboard(r),s),z(e,t,{dashboardEditing:!1}),await U(e,t,r)}let q=(e,t,r)=>R(e,t,a=>{a.view=r});function pe(e,t,r){let a=s=>R(e,t,i=>{i.mode=s,i.view="define"});return n`
		...
		`}function Y(e,t,r,a,s){let i=p=>p.length?p:J(),c=(p,g,m)=>a(x=>{let b=x.slice();return b[p]={...b[p],[g]:m},b}),d=p=>a(g=>{let m=g.slice();return m.splice(p,1),i(m)}),o=()=>a(p=>[...p,{key:"",value:""}]);return n`
		...
		`}function be(e,t,r){let a=r.ab||{},s=v(a.variants),i=g=>R(e,t,m=>{m.ab={...m.ab,...g}}),c=(g,m)=>R(e,t,x=>{let b=v(x.ab.variants).slice();b[g]={...b[g],...m},x.ab={...x.ab,variants:b}}),d=g=>R(e,t,m=>{let x=v(m.ab.variants).slice();x.splice(g,1),m.ab={...m.ab,variants:x}}),o=g=>R(e,t,m=>{let x=v(m.ab.variants).slice(),b=g!=null?x[g]:null;x.push({label:`variant-${x.length+1}`,metadata:b?b.metadata.map(w=>({...w})):J(),rolesJson:b?b.rolesJson:"",rolesOpen:!1}),m.ab={...m.ab,variants:x}}),p=$(a.repeats);return n`
					...
					<div class="exp-variant" data-testid="experiment-runner-variant-row" data-variant-index=${m}>
						<div class="exp-variant-head">
							<input class="exp-input" type="text" data-testid="experiment-runner-variant-label" placeholder="variant label"
								.value=${u(g.label)} @input=${x=>c(m,{label:x.currentTarget.value})} />
						...
						${Y(e,t,g.metadata,x=>R(e,t,b=>{let w=v(b.ab&&b.ab.variants).slice(),B=v(w[m]&&w[m].metadata).slice();w[m]={...w[m],metadata:x(B)},b.ab={...b.ab,variants:w}}),"experiment-runner-variant-metadata")}
						<details class="exp-details" ?open=${g.rolesOpen}>
							...
							<textarea class="exp-input exp-mono" rows="3" placeholder='{"coder": {"model": "…"}}'
								.value=${u(g.rolesJson)} @input=${x=>c(m,{rolesJson:x.currentTarget.value})}></textarea>
						</details>
					</div>`)}
		...
		`}function ge(e,t,r,a){if(r.mode==="autoresearch"){let s=v(a.checklist);return n`
			...
			<footer class="exp-projection" data-testid="experiment-runner-projection">
				<div class="exp-proj-stats">
					<span data-testid="experiment-runner-run-count">${v(r.ab&&r.ab.variants).length} variants × ${$(r.ab&&r.ab.repeats)||0} repeats = ${a.runCount} runs</span>
					<span data-testid="experiment-runner-cost">${a.estCostMax!=null?`est. \u2264 ${N(a.estCostMax)}`:"est. \u2014 set a per-run budget"}</span>
					<span>~${$(r.ab&&r.ab.concurrency)||1} concurrent</span>
				</div>
				${v(a.errors).length?n`<ul class="exp-checklist" data-testid="experiment-runner-error">
			...
			<div class="exp-view" data-testid="experiment-runner-view-confirm">
				...
				<section class="exp-card">
					...
					${s?n`
							...
							<div class="exp-confirm-row"><span>Caps</span><strong>${u($(r.auto.caps.maxIterations)?`\u2264 ${$(r.auto.caps.maxIterations)} iters`:"")} ${$(r.auto.caps.wallClockHours)?`\u2264 ${$(r.auto.caps.wallClockHours)}h`:""} ${$(r.auto.caps.costUsd)?`\u2264 ${N($(r.auto.caps.costUsd))}`:""}</strong></div>
							...
							<div class="exp-confirm-row"><span>Fan-out</span><strong>${a.runCount} child goals (${v(r.ab.variants).length} variants × ${$(r.ab.repeats)} repeats)</strong></div>
		...
		`}function Z(e){let t=e&&Array.isArray(e.spec)?e.spec:e&&e.spec&&Array.isArray(e.spec.widgets)?e.spec.widgets:null;return t&&t.length?t:e&&e.def&&e.def.mode==="autoresearch"?[{type:"summary-cards",title:"Summary"},{type:"objective-curve",title:"Best objective vs iteration"},{type:"ledger-table",title:"Ledger"},{type:"raw-drilldown",title:"Iterations"}]:[{type:"summary-cards",title:"Summary"},{type:"comparison-table",title:"Comparison"},{type:"score-bars",title:"Secondary metrics"},{type:"raw-drilldown",title:"Runs"}]}let F=e=>e&&(e.metricId||e.metric)||void 0;function G(e){let r=v(e&&e.metrics).filter(a=>a.collect!==!1).map(F).filter(Boolean);return r.length?r:["objective.value","cost.totalUsd","time.wallClockMs"]}function he(e){let r=v(e&&e.metrics).find(a=>a.primary);return r?F(r):e&&e.def&&e.def.objective?e.def.objective.metricId||e.def.objective.metric:G(e)[0]}let _=(e,t)=>{let r=e&&e.metrics,a=r?r[t]:void 0;return Number.isFinite(Number(a))?Number(a):a&&Number.isFinite(Number(a.value))?Number(a.value):void 0};function Q(e){let t=new Map;for(let r of v(e&&e.runs)){let a=u(r.armId,"arm");t.has(a)||t.set(a,[]),t.get(a).push(r)}return t}function $e(e,t){let r=G(t),a=!(t.def&&t.def.sameCompletionBar===!1),s=Q(t),i=v(t.metrics),c=d=>(i.find(o=>F(o)===d)||{}).aggregation||"median";return n`<table class="exp-table" data-testid="experiment-runner-widget-comparison-table">
			...
			<tbody>
				${[...s.entries()].map(([d,o])=>{let p=a?o.filter(m=>m.completionBar==="passed"):o,g=p.length?p:o;return n`<tr data-testid="experiment-runner-comparison-arm" data-arm=${d}>
						...
						${r.map(m=>n`<td class="exp-mono">${O(ee(g.map(x=>_(x,m)),c(m)))}</td>`)}
						<td>${g.length}</td>
					</tr>`})}
			</tbody>
		...
		</div>`}function we(e,t){let r=he(t),a=t.def&&t.def.objective&&t.def.objective.direction||"maximize",s=v(t.runs).filter(o=>o.iteration!=null).sort((o,p)=>o.iteration-p.iteration),i=null,c=s.map(o=>{let p=_(o,r);return Number.isFinite(p)&&(i=i==null?p:a==="minimize"?Math.min(i,p):Math.max(i,p)),{iteration:o.iteration,v:p,best:i,kept:o.verified!==!1&&o.completionBar!=="failed"}}),d=$(t.def&&t.def.stop&&t.def.stop.target);return n`<div data-testid="experiment-runner-widget-objective-curve">
			...
			<tbody>${r.map(a=>{let s=u(a.verdict||a.decision,"\u2014"),i=/kept|accept/i.test(s)?"exp-pos":/verification|failed/i.test(s)?"exp-neg":"exp-muted";return n`<tr data-testid="experiment-runner-ledger-row"><td>${u(a.iteration)}</td><td class=${i}>${s}</td><td class="exp-mono">${O($(a.objective))}</td><td class="exp-mono">${O($(a.best))}</td></tr>`})}</tbody>
		</table>`}function Ce(e,t){let r=v(t.runs),a=r.filter(c=>["settled","collected","failed"].includes(c.status)).length,s=r.filter(c=>c.completionBar==="passed").length,i=r.reduce((c,d)=>c+($(d.cost&&d.cost.totalUsd)||_(d,"cost.totalUsd")||0),0);return n`<div class="exp-cards" data-testid="experiment-runner-widget-summary-cards">
		...
		</div>`}function Se(e,t){let r=G(t),a=v(t.runs);return n`<table class="exp-table" data-testid="experiment-runner-widget-raw-drilldown">
		...
		</div>`}function Ae(e,t,r){let a=Z(r).slice(),s=x=>z(e,t,{dashboardDraftSpec:x}),i=E.get(t)||{},c=i.dashboardDraftSpec||a,d=(x,b)=>{let w=c.slice(),B=x+b;B<0||B>=w.length||([w[x],w[B]]=[w[B],w[x]],s(w))},o=x=>{let b=c.slice();b.splice(x,1),s(b)},p=x=>s([...c,{type:x,title:(K.find(b=>b.id===x)||{}).label||x}]),g=(x,b)=>{let w=c.slice();w[x]={...w[x],title:b},s(w)},m=i.widgetTypes&&i.widgetTypes.length?i.widgetTypes:K;return n`<div class="exp-card" data-testid="experiment-runner-dashboard-editor">
			<h3 class="exp-h2">Edit dashboard</h3>
			${c.map((x,b)=>n`<div class="exp-editor-row" data-testid="experiment-runner-editor-widget" data-widget-type=${x.type}>
				<input class="exp-input" type="text" .value=${u(x.title)} @input=${w=>g(b,w.currentTarget.value)} />
				<span class="exp-badge exp-mono">${x.type}</span>
				<button class="exp-icon-btn" type="button" title="Move up" @click=${()=>d(b,-1)}>↑</button>
				<button class="exp-icon-btn" type="button" title="Move down" @click=${()=>d(b,1)}>↓</button>
				<button class="exp-icon-btn" type="button" title="Remove" @click=${()=>o(b)}>✕</button>
			</div>`)}
			<div class="exp-editor-add">
				<select class="exp-input" data-testid="experiment-runner-add-widget-type">
					${m.map(x=>n`<option value=${x.id}>${x.label||x.id}</option>`)}
				</select>
				<button class="exp-btn secondary" type="button" data-testid="experiment-runner-add-widget"
					@click=${x=>{let b=x.currentTarget.parentElement.querySelector("select");p(b.value)}}>+ Add widget</button>
			</div>
			<div class="exp-confirm-actions">
			...
			</div>`;let c=s.def||{},d=s.state||{},o=c.mode==="autoresearch",p=u(d.status,"running"),g=v(s.runs),m=g.filter(b=>["settled","collected","failed"].includes(b.status)).length,x=d.stopReason?`stopped: ${d.stopReason}`:p;return n`
			<div class="exp-view" data-testid="experiment-runner-view-dashboard" data-experiment-id=${s.experimentId}>
				<header class="exp-dash-head">
					...
					<div class="exp-dash-meta">
						<span class="exp-status" data-testid="experiment-runner-status" role="status">${p==="running"?`running ${m}/${g.length}`:x}</span>
					</div>
