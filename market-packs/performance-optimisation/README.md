...
The pack owns `<canonical-project-root>/.performance-optimisation/performance.sqlite` through Pack Local Data. On first open it creates a self-ignoring `.gitignore` in that directory, while preserving any operator-authored file already there. Model-facing tools and the server route use Bobbit's shared `better-sqlite3` runtime dependency. Adjacent `pack.build.json` metadata materializes the eight supported macOS, Linux/glibc, Linux/musl, and Windows x64/arm64 bindings into the pack as committed release artifacts, so standalone `build:server` flows can copy a complete pack without first running `build:packs`. Node bundles inline `bobbit:pack-native-assets`, deterministically select the current runtime target from the generated manifest, and never fall back to an ancestor package path. Browser code reads this versioned projection through `host.callRoute("performance-snapshot")`:
