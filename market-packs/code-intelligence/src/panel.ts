...
function records(value: unknown): Record<string, any>[] {
	...
	if (body.component || body.state || body.freshness) return [body];
	const collection = body.components ?? body.items ?? body.graphs;
	return Array.isArray(collection) ? records(collection) : [];
}
...
function componentName(status: any): string {
	return String(status?.component?.name ?? status?.component ?? "Component");
}
				...
				<section class="h-full overflow-auto p-4 space-y-4 text-sm" data-testid="code-intelligence-status-panel">
					<header class="flex items-start justify-between gap-3">
						...
						<div class="flex gap-2">
							...
							<button class="rounded bg-primary px-2 py-1 text-primary-foreground disabled:opacity-50" data-testid="code-intelligence-rebuild" ?disabled=${state.rebuilding} @click=${() => void invoke("rebuild")}>${state.rebuilding ? "Queued…" : "Rebuild"}</button>
						</div>
					</header>
					...
					`)}
					${state.loaded && statuses.length === 0 ? html`<p class="text-muted-foreground">No eligible component graph is published yet.</p>` : nothing}
