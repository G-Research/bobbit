...
**Add a tool**: Builtin tools live in `defaults/tools/<group>/` (tracked in git, shipped to all users). Project-specific tool overrides go in `.bobbit/config/tools/<group>/` (runtime only, gitignored). MCP tools are auto-discovered from `.mcp.json`. See [docs/internals.md — MCP servers](docs/internals.md#mcp-servers). Example groups: `proposals/` (non-blocking `propose_*` tools), `ask/` (non-blocking — `ask_user_choices` returns a synchronous stub and ends the turn; the user's answers arrive later as a tagged user message, see [docs/non-blocking-ask.md](docs/non-blocking-ask.md)), `tasks/` (blocking — `verification_result` waits for a reviewer verdict).

**Add a blocking tool (agent awaits user/UI input)**: The tool extension POSTs to an internal REST endpoint and blocks; the server parks a Promise in a harness keyed by `(sessionId, toolUseId)`; a later REST call (from a UI widget or another agent) resolves it, and the HTTP response carries the result back to the agent. Canonical example: `defaults/tools/tasks/` + `src/server/agent/verification-harness.ts` (`verification_result`). Session termination rejects all pending entries via `sessionManager.addTerminationListener`. See [docs/blocking-tools.md](docs/blocking-tools.md) for the full walkthrough. Note: `ask_user_choices` was previously blocking but is now non-blocking — see [docs/non-blocking-ask.md](docs/non-blocking-ask.md) for that pattern (tool returns a stub immediately; answers arrive as a later user message).
...
**Add/modify session creation**: Plan/execute pipeline in `session-setup.ts`, thin wrappers in `session-manager.ts`. Four modes: normal, worktree, delegate, continue-archived. Non-goal/non-assistant sessions in git repos auto-get worktree branches. See [docs/internals.md — Session worktrees](docs/internals.md#session-worktrees).

**Continue an archived session**: Archived, non-goal, non-delegate sessions render a "Continue in New Session" footer button (keyed by `[data-continue-archived-footer]` in `src/ui/components/AgentInterface.ts`). Clicking it opens `ContinueSessionChooser` (`src/ui/components/ContinueSessionChooser.ts`) which posts `POST /api/sessions/:archivedId/continue` with `{ mode: "summary" | "full" }`. The server (`src/server/agent/continue-archived.ts`) clones the archived session's settings — project, model, role, personality, worktree flag, sandbox flag — into a fresh session via the normal session-setup pipeline, then injects the archived transcript as `seedContext` on the plan. The seed is rendered into the system prompt under a `## Prior Session Transcript` section by `assembleSystemPrompt` in `src/server/agent/system-prompt.ts`. Runtime state (cwd, worktree path, branch, sandbox container identity, goal/team/delegate relationships) is **never** carried over — the new session gets fresh everything via the usual pipeline. See [docs/rest-api.md — Sessions](docs/rest-api.md#sessions) for the endpoint shape and [docs/internals.md — Continue-Archived sessions](docs/internals.md#continue-archived-sessions) for the full walkthrough.
...
**Work on the git-status widget**: Widget rendering and state live in `src/ui/components/GitStatusWidget.ts` (reactive `loading` / `partial` props, skeleton + refresh-dot + partial-dot states) and `src/ui/components/AgentInterface.ts` (tri-state `gitRepoKnown: 'yes' | 'no' | 'unknown'` gates visibility — widget hides **only** when server explicitly returns `Not a git repository`). Session-side refresh/poll orchestration lives in `src/app/session-manager.ts` (`refreshGitStatusForSession`, 4-attempt backoff at [0, 500, 2000, 5000]ms with a per-session `AbortController`, 30s safety poll gated on `visibilityState` + active session + `gitRepoKnown !== 'no'`, 10s event/poll coalesce via `gitStatusLastRefreshAt`). Goal dashboard uses the same tri-state + retry at its existing 60s cadence (`src/app/goal-dashboard.ts`). Helper shared between both sides: `src/app/git-status-refresh.ts`. API contract in `src/app/api.ts`: `fetchGitStatus` returns a discriminated `GitStatusResult` (`ok` / `not-a-repo` / `error`) — never `null`, never throws. Server side (`src/server/server.ts`): `batchGitStatus` is a 750ms-TTL single-flight cache keyed by `${containerId}::${cwd}::${summary|untracked}` wrapping the raw `runBatchGitStatus` spawn; `?untracked=1` swaps `-uno` → `-uall` for the dropdown-expanded view (dropdown-open fires a `git-status-dropdown-open` CustomEvent which triggers the full refetch); response carries `partial` and `untrackedIncluded` flags. Commit/pull/push endpoints call `invalidateGitStatusCache(cwd, containerId?)` to bust both keys. See [docs/design/git-status-widget-reliability.md](docs/design/git-status-widget-reliability.md) for the full design and [docs/internals.md — Git status cache & client resilience](docs/internals.md#git-status-cache--client-resilience) for the architectural overview.

**Large content truncation**: Tool content >32KB is truncated in WebSocket broadcasts and EventBuffer to prevent memory pressure. Applies to `write`/`edit` tool inputs and to `preview_open` snapshot blocks (marker-prefixed text blocks in `toolResult` messages). Key files: `truncate-large-content.ts` (threshold + truncation logic, including `truncateSnapshotBlock()`), `session-setup.ts`/`session-manager.ts` (applies truncation before broadcast), `WriteRenderer.ts` / `PreviewRenderer.ts` (show preview + lazy-load/Open button). Full content lazy-loaded via `GET /api/sessions/:id/tool-content/:messageIndex/:blockIndex` (falls back to `block.text` for text blocks). See [docs/internals.md — Large content truncation](docs/internals.md#large-content-truncation) and [docs/internals.md — Preview snapshots & reopening](docs/internals.md#preview-snapshots--reopening).
...
- **Gate verification baselines**: Pre-implementation gates (design-doc, issue-analysis — `content: true` with no `depends_on`) do NOT diff against the branch. Implementation and later gates diff against `origin/<primary>...HEAD`, never local `<primary>` (which can lag after another agent merges upstream). Primary branch is resolved dynamically via `detectPrimaryBranch()` in `src/server/skills/git.ts`. See [docs/goals-workflows-tasks.md — Gate verification baselines](docs/goals-workflows-tasks.md#gate-verification-baselines).
...
- **Dead/ghost search results**: `ProjectContextManager.searchAll()` post-filters hits whose owning project/goal/session/staff no longer exists and schedules fire-and-forget index cleanup (`searchService.removeGoal`/`removeSession`/`removeMessagesForSession`/`removeStaff`); `total` is recomputed from the filtered list so Load More stays consistent. `toSearchResult()` in `flex-store.ts` tags each hit with `matchedOn: "text" | "metadata"` based on whether the sanitized snippet contains `<b>` — `message` rows with `matchedOn === "metadata"` are dropped (phantom matches); goal/session/staff weak-matches are kept and rendered with a muted "matched on title" note. Client stale-click safety net: `connectToSession(..., { onMissing: "toast" })` and the 404 paths in `goal-dashboard.ts`/`staff-page.ts` dispatch a `search-result-stale` window event instead of the blocking modal; `search-page.ts` listens and shows an inline 5s toast. The search page groups hits via `buildGroups()` → `ResultGroup` cards (staff standalone, messages nested under sessions; auto-expand when `totalMatches === 1`). See [docs/internals.md — Orphan filtering & stale-click safety net](docs/internals.md#orphan-filtering--stale-click-safety-net).
...
- **Live-steer text lost after user clicks Stop**: A direct live-steer (WS `{type:"steer", text}`) sent while the agent is mid-tool-call must survive a subsequent abort and be redelivered as the next user turn. Invariant enforced by `SessionManager.deliverLiveSteer()` in `src/server/agent/session-manager.ts`: **every live steer is first persisted into `promptQueue` as `{ isSteered: true, dispatched: true }` and only then forwarded to `rpcClient.steer()`**. Without this, the SDK parks the text internally and silently discards it when `forceAbort` tears down the turn. Happy-path cleanup: when the SDK actually delivers the parked steer, the `message_end(user)` lifecycle hook in `handleAgentLifecycle` calls `promptQueue.removeDispatched()` to drop the row. Abort cleanup: `forceAbort` (and the `agent_end` branch gated on `status === "aborting"`) calls `promptQueue.resetDispatched()` + `drainQueue()`, which re-arms any non-flushed steered rows and redelivers them. Duplicate-delivery note: in the rare force-kill race where the SDK did deliver the steer but the `message_end(user)` hook was torn down before firing, the row stays flagged `dispatched` and gets redelivered once — at-least-once semantics are deliberate (seeing the message twice is strictly better than losing it). The same guarantee applies to team-steer callers because they route through `deliverLiveSteer` as well. Repro/regression: `PI-25b` in `tests/e2e/abort-status-e2e.spec.ts`; ordering guarantee with a followup sent during the aborting window is `PI-25c` in the same file. User stories: `userstories/prompt-interactions.md` PI-25 / PI-25b / PI-25c.
- **`bash_bg wait` not interrupted by steer**: A user steer or `team_steer` should abort any in-flight `bash_bg wait` within ~100ms (the bg process keeps running; only the wait call returns with `aborted: true`, and the extension emits `Process <hdr> wait interrupted by steer. Use 'logs' or 'wait' again to continue monitoring.`). If waits stay blocked, check: (1) the live-steer path goes through `SessionManager.deliverLiveSteer()` — callers must use that helper, not `rpcClient.steer()` directly (see `ws/handler.ts` `case "steer"` and `team-manager.ts`); (2) `BgProcessManager` holds a `waits: Map<sessionId, Set<AbortController>>` registry populated by `registerWait()` in the `/bg-processes/:pid/wait` handler and drained by `abortAllWaits(sessionId)`; (3) session termination also calls `abortAllWaits()` before `cleanup()` so hanging HTTP handlers resolve cleanly. See [docs/internals.md — Steer-interruptible bash_bg wait](docs/internals.md#steer-interruptible-bash_bg-wait).
...
- **Preview Open button broken / missing**: The `preview_open` widget's Open button replays a captured HTML snapshot into the single preview panel. Each successful `preview_open` emits a two-block `tool_result`: a human-readable status text block plus a snapshot block prefixed with the `__preview_snapshot_v1__\n` sentinel (see `defaults/tools/html/snapshot.ts`). `truncateSnapshotBlock()` in `src/server/agent/truncate-large-content.ts` walks `toolResult` messages and replaces snapshots >32KB with a `{ _truncated: true, preview, _originalLength }` stub (keeping the marker), so the full HTML never re-enters agent context. The renderer (`src/ui/tools/renderers/PreviewRenderer.ts`) either reads the snapshot inline or lazy-fetches it via `GET /api/sessions/:id/tool-content/:mi/:bi`, strips the marker, PATCHes `preview:true` on the session, and POSTs `/api/preview`. Common failures: (1) **button disabled on historical widgets** — the `tool_result` has only the legacy single status block (no marker), so the snapshot was never captured; expected for pre-feature sessions. (2) **lazy-load returns 404** — verify the tool-content endpoint returns `block.text` for `type:"text"` blocks (the text-block fallback in `src/server/server.ts`); without it, truncated snapshots can't be hydrated. (3) **panel doesn't flip** — check the PATCH to `/api/sessions/:id` with `{preview:true}` succeeded before the POST to `/api/preview`.
 - **Streaming dedup / reorder**: Every `{type:"event"}` broadcast carries a per-session monotonic `seq` + `ts`, assigned by `EventBuffer.push()` and emitted through `emitSessionEvent()` in `src/server/agent/session-manager.ts`. The client (`src/app/remote-agent.ts`) tracks `_highestSeq`, drops duplicates, buffers out-of-order frames in `_pendingEvents`, and on WS reopen sends `{type:"resume", fromSeq}` so the server replays only the missed tail. If the seq has been evicted from the 1000-entry ring the server replies `resume_gap` and the client falls back to the `get_messages` snapshot path. Repro test: `ST-DEDUP-01` in `tests/e2e/ui/stories-streaming.spec.ts`. Design doc: [docs/design/streaming-dedup-reorder.md](docs/design/streaming-dedup-reorder.md); architecture: [docs/internals.md — Event stream ordering & dedup](docs/internals.md#event-stream-ordering--dedup).
- **Continue-Archived button missing**: The footer button only renders when the session is archived **and** `goalId` is unset **and** `delegateOf` is unset **and** the source project is still registered. Check those four scope-gate fields on the session record in `AgentInterface.ts` — goal-linked, delegate, team, and assistant sessions are all excluded by design. If the project has been unregistered, continuing would 410; the button is hidden client-side to match.
