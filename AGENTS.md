...
- **Sandbox**: `GET /api/sandbox-status` for Docker state. Containers use `bobbit-sandbox-net` bridge network — check `docker network inspect bobbit-sandbox-net` for connectivity issues. For shared team repo issues, check `ls <poolDir>/team-*.git` on the host to verify the bare repo exists; inside a container, check `git remote -v` for the `team` remote pointing to `/team-repos/team-<goalId>.git`. Verification `command` steps run inside the team lead's container for sandboxed goals — if results look stale, check the container is still running. Sandboxed sessions survive server restarts — `restoreSession()` reconstructs `sandboxClaim` (branch + teamRepoPath) from persisted session fields. If a restored sandbox session lands on the wrong branch, check that `branch` and `teamGoalId` are persisted in `sessions.json`. Run sandbox tests: `npx playwright test --config playwright-e2e.config.ts --project=api sandbox-security sandbox-docker`.
