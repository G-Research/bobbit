...
- **Native-YAML project.yaml fields** → `config_directories`, `qa_env`, `sandbox_tokens`, `qa_max_duration_minutes`, `qa_max_scenarios` are stored as native YAML (not JSON-stringified). Read via typed accessors on `ProjectConfigStore` (`getConfigDirectories`, `getQaEnv`, `getSandboxTokens`, `getQaMaxDurationMinutes`, `getQaMaxScenarios`); legacy on-disk forms are parsed transparently and rewritten on next save. `PUT /api/projects/:id/config` rejects legacy JSON-string payloads for these keys with 400. See [docs/internals.md — Native-YAML project.yaml fields](docs/internals.md#native-yaml-projectyaml-fields).
...
- **Legacy JSON-string `project.yaml` field rejected (HTTP 400)** — `PUT /api/projects/:id/config` rejects JSON-string or quoted-numeric payloads for `config_directories`, `qa_env`, `sandbox_tokens`, `qa_max_duration_minutes`, `qa_max_scenarios`. Send structured types (arrays / mappings / numbers). The on-disk YAML loader still tolerates legacy form for back-compat — only the wire payload is strict. See [docs/debugging.md — Legacy JSON-string project.yaml field rejected](docs/debugging.md#legacy-json-string-projectyaml-field-rejected) and [docs/internals.md — Native-YAML project.yaml fields](docs/internals.md#native-yaml-projectyaml-fields).
