...
- **MCP meta-tool aggregation / one row per server** → server-side: helpers in `src/server/mcp/mcp-meta.ts` (`parseMcpToolName` is the single source of truth for splitting `mcp__<server>__<op>` and `mcp__<server>__<sub>__<op>`), extension generator `generateMcpMetaExtension` in `src/server/agent/tool-activation.ts` keyed by `(server, sub?)` so gateway-style servers emit one meta-tool per sub-namespace (`mcp_<server>__<sub>` / `mcp_<server>`). Dispatcher unchanged (`/api/internal/mcp-call` still receives `mcp__<server>__<sub>__<op>`). Discovery tool `mcp_describe` (`defaults/tools/mcp/`) routes through `POST /api/internal/mcp-describe`. Tools page renders one collapsible row per server in a dedicated "MCP" section; each server row has a group-policy `<select>` (key `mcp__<server>`, testid `mcp-server-policy`), expanding reveals one tool row per sub-namespace (testid `mcp-tool-row` + `mcp-tool-toggle`) with its own `<select>` (key `mcp__<server>__<sub>`, testid `mcp-tool-policy`); deepest level is the read-only operations list (`mcp-server-ops`). `/api/mcp-servers` carries `subNamespace` + `op` on each operation entry so the UI can group client-side. See [docs/mcp-meta-tools.md](docs/mcp-meta-tools.md) and [docs/design/mcp-meta-tool-aggregation.md](docs/design/mcp-meta-tool-aggregation.md).
...
- **MCP per-op `never` policy not enforced** — Layer A (model-facing) collapses N×M ops into per-`(server, sub)` meta-tools, so per-op grants flow through `mcpPolicyKeys(toolName)` in `src/server/agent/tool-activation.ts` (returns `{group, tool}` for all four name shapes — `mcp__<server>__<op>`, `mcp__<server>__<sub>__<op>`, `mcp_<server>`, `mcp_<server>__<sub>`). `mcpPolicyPrefix` is the legacy wrapper returning just `group`. Layer B enforcement happens server-side in `POST /api/internal/mcp-call`: `resolveGrantPolicy(tool, …)` runs before `mcpManager.callTool` and returns 403 on `never`. If a per-op policy isn't taking effect, check both layers.
...
- **MCP gateway server collapses sub-namespaces into one tool** — a gateway server emitting `mcp__gr__ai-adoption__list-articles` etc. should surface as ONE server row + one tool row per sub-namespace on the Tools page, and the agent should see one meta-tool per sub-namespace (`mcp_gr__ai-adoption`, `mcp_gr__jira`). Single source of truth is `parseMcpToolName()` in `src/server/mcp/mcp-meta.ts` — if a callsite parses the name with its own `indexOf("__", …)` instead of routing through this helper, sub-namespace grouping breaks at that callsite. Symptom: one giant `mcp_gr` meta-tool with a flat enum like `ai-adoption__list-articles, jira__get-queue, …`. Check the aggregation map in `tool-activation.ts::generateMcpMetaExtension` (keyed by `(server, sub?)`) and the two ad-hoc parse sites in `server.ts` (must call `parseMcpToolName`).
