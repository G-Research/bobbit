...
- **Never junction/symlink a worktree's `node_modules` into a shared or primary tree** (e.g. `mklink /J node_modules <primary>`, `fs.symlinkSync(primary, "node_modules", "junction")`). A later `npm ci` or recursive worktree delete descends the reparse point and wipes the shared tree — bricking every agent/verification spawn. Each worktree owns its `node_modules` via `worktree_setup_command` (`npm ci`); to build in a worktree, run `npm ci` there — never link the primary in. See [docs/testing-v2/node-modules-corruption-rca.md](docs/testing-v2/node-modules-corruption-rca.md).
