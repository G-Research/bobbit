...
- **Edit a goal's spec mid-flight (notification flow)** → `PUT /api/goals/:id` with a new `spec` field captures `prevSpec`, calls `goalManager.updateGoal(...)`, and — when `body.spec !== prevSpec` — broadcasts a typed `goal_spec_changed` WS event (`{goalId, prevSpecHash, newSpecHash, prevLen, newLen, ts}`; 16-char sha256 hex prefixes; full body NOT included) AND calls `teamManager.notifyTeamLeadOfSpecChange(goalId, prevLen, newLen)`. The nudge is throttled to one per `SPEC_NUDGE_THROTTLE_MS = 30_000` via `lastSpecNudgeTs: Map<goalId, ts>` and dispatches via the same shape as `notifyTeamLeadOfTaskCompletion` (streaming → `deliverLiveSteer`, idle → `enqueuePrompt({isSteered: true})`). Message body references the `view_goal_spec` tool by name so the agent has a canonical re-read path — the spec injected into the system prompt at session startup is NOT refreshed by the WS event. UI side: `src/app/goal-dashboard.ts::notifyGoalSpecEditedForDashboard(goalId, ts)` records the event in `recentSpecEdits` for `SPEC_PILL_WINDOW_MS = 60_000` and renders a `data-testid="spec-edited-pill"` in the dashboard header (click → spec tab). No-op skip when `body.spec` is absent OR identical to the previous value (mirrors `goal-store.update()` R-007 behaviour). Worker / reviewer / QA sessions are intentionally NOT notified — they read task specs, not goal specs. Tests: `tests/team-manager-spec-nudge.test.ts` (6 cases), `tests/e2e/api-goals-spec-edit-broadcast.spec.ts` (positive + barrier-tested no-ops). When adding new tools that mutate goal state out-of-band of the team-lead's awareness, follow the same nudge pattern.
...
- **Goal spec edit silently dropped / agent unaware of mid-flight spec changes** — should not happen post-fix. Diagnostic chain: (1) was a `goal_spec_changed` WS frame emitted? Check `broadcastToAll` call in the `PUT /api/goals/:id` handler (`src/server/server.ts`); the event is gated on `typeof body.spec === "string" && body.spec !== prevSpec`. (2) Did `teamManager.notifyTeamLeadOfSpecChange` fire? Look for `[team-manager] Notified team lead of spec change` in the log; missing means either no team-lead session, terminated session, or the throttle window suppressed the call (look for `[team-manager] Skipping spec-edit nudge for goal <id> (throttled, last <ms>ms ago)`). (3) Did the agent receive the queued prompt? `enqueuePrompt(..., { isSteered: true })` paths through the existing prompt queue — same dispatch shape as auto-nudges. (4) UI pill missing? Check the `case "goal_spec_changed"` branch in `src/app/remote-agent.ts` and `notifyGoalSpecEditedForDashboard` in `src/app/goal-dashboard.ts`; pill auto-clears after `SPEC_PILL_WINDOW_MS = 60_000`. The agent's canonical re-read tool is `view_goal_spec` (`defaults/tools/proposals/`).
