...
**Test types:**
- **Unit** (`tests/*.spec.ts`, `tests/*.test.ts`): Playwright `file://` fixtures + Node test runner. See `mobile-header.spec.ts` for Playwright fixtures, `verification-logic.test.ts` for pure Node test runner tests.
...
- **Path B — Directory has content but no `.bobbit/`**: Opens a Project Assistant session (detection mode, assistant type `"project"`). The server creates a **provisional project** (`provisional: true`) via `ProjectRegistry.registerProvisional()` and assigns the session to it — so the session has a real `projectId` from the start. The assistant explores the directory, detects the tech stack, and calls the `propose_project` tool with name, commands, system prompt, and `worktree_setup_command`. The session auto-sends an initial prompt containing the directory path (e.g., "Start the project registration session. The project directory is: /path/to/my-project"), matching the goal assistant auto-prompt pattern. The `connectToSession()` `projectDirPath` option carries the path from the dialog to the auto-prompt logic.
- **Path C — Directory is empty or doesn't exist**: Opens a Project Assistant session (scaffolding mode, assistant type `"project-scaffolding"`). Like Path B, a provisional project is created and the session is assigned to it. The assistant auto-sends a scaffolding prompt with the target directory path and guides through tech stack selection, scaffolding, and project configuration.

While the project assistant is active, the sidebar shows the session under its **provisional project folder** with a "(setting up)" badge. Action buttons (Add Goal, Add Staff, etc.) are suppressed for provisional projects. This is real server-side state — not transient client-side reconstruction — so it survives page refreshes. On session terminate without proposal acceptance, the provisional project is deleted via `DELETE /api/projects/:id`.

When the agent calls `propose_project`, the client populates `state.activeProjectProposal` and shows a **preview form** in the right panel (similar to goal proposals) with editable fields: project name, build/test/typecheck commands, and worktree setup command. The user reviews and clicks "Accept" — only then does the client promote the provisional project via `POST /api/projects/:id/promote` (which clears the `provisional` flag and updates the name), write config fields (`build_command`, `test_command`, `typecheck_command`, `test_unit_command`, `test_e2e_command`, `worktree_setup_command`) to `project.yaml` via `PUT /api/projects/:id/config`, and auto-initialize a worktree pool if the project's root is a git repo. The config write is atomic — all keys are validated before any are written. The client deduplicates proposal acceptance by tracking processed tool_use block IDs in `sessionStorage`. Projects can optionally have a `palette` (one of 10 built-in palettes) and accent colors (`colorLight`/`colorDark`) — see [docs/internals.md](docs/internals.md#per-project-palette) for details. See [docs/internals.md](docs/internals.md#multi-project-architecture) for the full architecture.

**Remove a project**: To remove a non-default project, go to its per-project settings page → General tab and click "Remove Project" in the Danger Zone section. This calls `DELETE /api/projects/:id`, which drains the project's worktree pool (if any) and unregisters the project from the server without deleting any files on disk. The button is hidden for the default project (the server's own CWD). After removal, the UI navigates back to system settings and refreshes the sidebar.
...
**Add a UI component**: Create in `src/ui/components/`, export from `src/ui/index.ts`. Notable built-in components: `BobbitLoadingAnimation` — an animated pixel-art bouncing mascot used as a loading indicator during session connection and dashboard loading. Review pane components live in `src/ui/components/review/`: `<review-pane>` (tabbed container), `<review-document>` (markdown renderer + text annotator), `<annotation-popover>` (comment input), and `AnnotationStore` (sessionStorage persistence). All use light DOM for annotator compatibility. Exported from `src/ui/index.ts`.

**Add a tool renderer**: Create in `src/ui/tools/renderers/`, register in `src/ui/tools/index.ts`. See `ProposalRenderer.ts` for an example of a renderer that handles multiple tool names via a `withToolName()` factory pattern. `ReviewRenderer.ts` handles `review_open` and `review_close` — registered as separate `ReviewOpenRenderer` and `ReviewCloseRenderer` classes in `src/ui/tools/index.ts`.

**Add a review tool**: `review_open` and `review_close` are registered in `.bobbit/config/tools/review/`. The extension (`extension.ts`) reads markdown files from disk or accepts inline content, returning JSON for client-side state updates. Tool group: `Review`. No gateway API calls — pure file I/O with JSON result parsing. Client-side detection in `remote-agent.ts` via `_checkReviewToolResult()` parses JSON from tool results to update `state.reviewDocuments`. The review pane integrates into the unified panel system (`render.ts`) alongside preview and goal tabs.

**Add a tool**: Create YAML in `.bobbit/config/tools/<group>/`. Define `name`, `description`, `summary`, `group`, `provider`. Add extension code in group's `extension.ts` if needed. MCP tools are auto-discovered from `.mcp.json` — no YAML needed. On connect, each MCP server's tools get auto-generated documentation: a one-line summary (first sentence of the description, ≤120 chars) and a full Markdown reference at `.bobbit/state/mcp-tool-docs/<serverName>.md` with descriptions and parameter tables. Summaries are cached with content hashing in `.bobbit/state/mcp-tool-docs/<serverName>.cache.json` and only regenerate when tool descriptions or schemas change.
...
**Modify a store constructor**: All stores accept `stateDir` or `configDir` as a constructor parameter (not module-level globals). State stores (GoalStore, SessionStore, etc.) take `stateDir: string`; config stores (RoleStore, WorkflowStore, etc.) take `configDir: string`. When adding a new store, follow this pattern — never call `bobbitStateDir()` or `bobbitConfigDir()` at module level. See `ProjectContext` in `project-context.ts` for how stores are wired together. Managers (`GoalManager`, `TaskManager`) accept store instances directly — they no longer create stores internally. `StaffManager` accepts `ProjectContextManager` and resolves the correct per-project `StaffStore` on each operation. **Store resolution never falls back to a default project** — all resolution goes through `ProjectContextManager` methods like `getContextForGoal()`, `getContextForSession()`, or explicit `projectId`. If the project can't be resolved, the operation throws or returns null. The `getDefault()` and `getDefaultProjectId()` methods on `ProjectContextManager` are deprecated — use them only for bootstrapping or API creation endpoints where defaulting to the server CWD project is the correct contract. Null-safe alternatives `getDefaultOrNull()` and `getDefaultProjectIdOrNull()` are available for code paths that must handle zero-project environments (e.g. fresh installs with no projects). See [docs/internals.md](docs/internals.md#store-resolution-pattern) for the resolution pattern.

**Add/modify session creation logic**: Session creation uses a plan/execute pipeline in `session-setup.ts`. The three creation modes (normal, worktree, delegate) share composable pipeline steps (`resolveBridgeOptions`, `resolveGoalExtensions`, `resolveTools`, `resolvePrompt`, `resolveToolActivation`) with different executors (`executePlan` for normal/delegate, `executeWorktreeAsync` for worktree). `session-manager.ts` contains thin wrappers (`createSession`, `createDelegateSession`) that build a `SessionSetupPlan` and delegate to the pipeline. Persistence uses put-once-then-update: `persistOnce()` does a single `store.put()` at creation, then `persistSessionMetadata()` only calls `store.update()` for the `agentSessionFile` field. Every non-goal, non-assistant session in a git repo automatically gets its own worktree branch (`session/new-session-{uuid8}` for host, `session/s-{uuid8}` for sandbox). Assistant sessions (`assistantType` is set) and goal sessions skip auto-worktree. Worktrees are cleaned up on session terminate/archive. Orphaned worktrees are **not** removed automatically on startup — use Settings → Maintenance tab to preview and clean them up manually. **Subdirectory project CWD offset**: When a project's `rootPath` is a subdirectory of a git repo, worktrees are created at the repo root (full checkout), but the session `cwd` is offset to the subdirectory within the worktree. `worktreePath` remains the worktree root (for cleanup). The offset is computed via `path.relative(repoRoot, project.rootPath)` and applied in goal creation (`goal-manager.ts`), `executeWorktreeAsync` (`session-setup.ts`), pool claims (`session-manager.ts`), and team member spawning (`team-manager.ts`). Non-subdirectory projects are unaffected (offset is empty). The worktree pool (`worktree-pool.ts`) pre-creates worktrees in the background for faster session startup. Pools are **per-project** — `SessionManager` maintains a `Map<string, WorktreePool>` keyed by project ID. On startup, a pool is initialized for every registered project whose `rootPath` is a git repo. Each pool uses that project's `worktree_pool_size` and `worktree_setup_command` config. On acquire, pooled worktrees are fetched from origin and hard-reset to the remote primary branch so they aren't stale. The primary branch is resolved dynamically via `git symbolic-ref refs/remotes/origin/HEAD` (falls back to `origin/master`). Pool claim in `createSession()` looks up the pool by the session's `projectId` — sessions only claim from their own project's pool. For sandboxed sessions, the persisted branch is reconciled after sandbox wiring — if `plan.sandboxBranch` differs from `plan.branch` (common for team-spawned sessions where the host auto-generates `session/new-session-<uuid8>` but the container uses `goal-<slug>-<role>-<id>`), the persisted branch is updated to match the actual container branch. This ensures PR status lookups, session restore, and orphaned worktree detection all use the correct branch. `createWorktree()` in `git.ts` is idempotent — if the branch already exists from a previous interrupted attempt, it recovers gracefully (repairs an existing worktree or re-creates without `-b`). This handles the retry-after-interrupt case (e.g. "Retry Setup" on the goal dashboard). See [docs/internals.md](docs/internals.md#session-worktrees) for the full lifecycle.

**Config cascade (roles, tools, personalities, workflows)**: All config types resolve through a three-layer cascade: **builtin → server → project**. Builtins are factory defaults compiled into `dist/server/defaults/` (read-only). Server-level overrides live in `<server-cwd>/.bobbit/config/`. Project-level overrides live in `<project-root>/.bobbit/config/`. First match by name wins, most-specific layer takes priority. Key modules: `BuiltinConfigProvider` (`builtin-config.ts`) reads builtins with lazy caching; `ConfigCascade` (`config-cascade.ts`) merges all three layers and tags each item with `origin` (`"builtin"`, `"server"`, or `"project"`) and optional `overrides` field. The session setup pipeline uses `ConfigCascade` for role/tool resolution so sessions see the full cascade. On startup, standalone stores are seeded with builtins that aren't already present (covers fresh scaffolds). Scaffolding creates empty `config/roles/`, `config/workflows/`, `config/personalities/` directories — these config types resolve at runtime via the cascade. Tools are still copied from defaults because they contain provider configs and extension code that `updateToolMetadata()` modifies in-place. See [docs/internals.md](docs/internals.md#config-cascade) for the full architecture.

**Add a config type to the cascade**: Add a loader method to `BuiltinConfigProvider` (mirrors the store's YAML parsing), add a `resolveX()` method to `ConfigCascade` using the generic `resolve()` helper, wire the REST API endpoint to call through `configCascade.resolveX(projectId)`, and add the scope row + origin badges in the UI page (import shared helpers from `config-scope.ts`).

**Config cascade REST API**: All config list endpoints accept `?projectId=` for project-scoped resolution. Items include `origin` and optional `overrides` fields. Override management: `POST /api/roles/:name/customize?scope=project&projectId=X` copies a resolved item to a target scope for editing; `DELETE /api/roles/:name/override?scope=project&projectId=X` removes an override, reverting to the inherited value. Same pattern for `/api/personalities/:name/` and `/api/workflows/:id/`. CRUD endpoints (`POST`, `PUT`, `DELETE`) accept optional `projectId` for scope-aware operations.

**Config cascade UI**: The five config pages (Roles, Tools, Skills, Personalities, Workflows) show a project scope row when multiple projects are registered. Each item displays an origin badge (grey=builtin, blue=server, green=project). Inherited items appear at 70% opacity. Customize/revert buttons let users create project- or server-level overrides. Shared UI code lives in `config-scope.ts` and `config-scope.css`.
...
- **Multi-project / per-project state**: Each project stores its own state in `<project-root>/.bobbit/state/` (goals, sessions, tasks, teams, gates, staff, search index, costs). The server aggregates via `ProjectContextManager`. Check `GET /api/projects` for registered projects — a fresh install returns an empty array (no default project is auto-created without `projects.json`). Sessions/goals/staff carry optional `projectId`; filter with `?projectId=` on list endpoints. Store resolution never falls back to a "default project" — all operations resolve the correct store via `ProjectContextManager` or throw/return null. Use `getDefaultProjectIdOrNull()` / `getDefaultOrNull()` for null-safe access in zero-project environments. API creation endpoints (sessions, goals, staff) return 400 "No projects configured" when no default project exists. Config directories (MCP servers, skills, AGENTS.md/agent files) are resolved per-project through each project's `config_directories` setting. Per-project config: `GET /api/projects/:id/config/resolved` shows resolved values with source. Sidebar collapse state (Sessions/Staff sections) is per-project — stored as collapsed-project-ID sets in localStorage (`bobbit-collapsed-ungrouped`, `bobbit-collapsed-staff`). See [docs/internals.md](docs/internals.md#multi-project-architecture) for architecture.
