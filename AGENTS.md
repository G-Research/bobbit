...
- **Use a custom workflow inline** → Advanced disclosure on the New Goal dialog (`src/app/dialogs.ts`) accepts inline YAML; parsed/validated on submit and snapshotted as `inlineWorkflow` on `PersistedGoal`. Resolver chain in `src/server/agent/workflow-resolution.ts` (and the matching `role-resolution.ts` for inline roles): goal's own inline → walk up `parentGoalId` chain → project config → server → builtin.
