...
- **Image generation failure** — symptoms: `generate_image` returns `503 image generation unavailable` (no provider credentials — check Settings → Models → Image), `400 unknown image model` (registry/canonicalisation mismatch — confirm both sides via `GET /api/image-models`), `400 openai-codex image driver supports n=1 only` (clamp — the Codex driver only supports single-image generation), `502 <status>: …` (upstream provider rejected — inspect `data?.error?.message`; never `[object Object]`), `502 remote image exceeds 25 MB cap` (URL ref crossed the streaming guard in `imageFromUrl`), `outputPath escapes worktree` (model-controlled `outputPath` failed containment check). For Codex driver model resolution, `getCodexImageDriverModel` walks `configured pref → gpt-5.5 → gpt-5 → gpt-4o`; if all four miss, the throw becomes a clean `503` instead of a confusing upstream 404. JWT-parse warnings from the OpenAI-Codex token helper are logged via the central logger — don't be swallowed silently. See [docs/internals.md — Image generation routing](docs/internals.md#image-generation-routing).
