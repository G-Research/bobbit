...
- **MCP per-op `never` policy not enforced** — two layers: `mcpPolicyPrefix` regex + server-side `resolveGrantPolicy`. See [debugging.md#mcp-per-op-never-policy-not-enforced](docs/debugging.md#mcp-per-op-never-policy-not-enforced).
