...
- **Add a mission feature** → `mission-store.ts` / `mission-manager.ts` / `mission-scheduler.ts`, REST in `server.ts`. Workflow: `defaults/workflows/mission.yaml`. Role: `defaults/roles/commander.yaml`. Tools: `defaults/tools/mission/`. See [docs/missions.md](docs/missions.md).
- **Mission integration branch hygiene** → `MissionGit.forwardMergeMaster` keeps the integration branch up-to-date with master; runs from the `mission-pr` gate verify and periodically from `MissionScheduler.tickMission`. See [docs/missions.md — Branching & integration](docs/missions.md#branching--integration).
...
- **Stuck mission scheduler** — check `gateStore.onStatusChange` listener wiring in `project-context.ts` (mission-aware tick); inspect `replanCount` cap (auto-pauses at `MAX_REPLANS = 3`); inspect `failedAttempts` per plan node (auto-pauses at `maxFailedAttempts = 2`). See [docs/debugging.md — Mission orchestration](docs/debugging.md#mission-orchestration).
- **Mission integration branch falls behind master** — `MissionGit.forwardMergeMaster` invoked by the `mission-pr` gate's first verify step and periodically by `MissionScheduler.tickMission`. If conflict, scheduler soft-fails and continues; `mission-pr` keeps failing until resolved manually. See [docs/missions.md — Recovery scenarios](docs/missions.md#integration-branch-falls-behind-master).
- **Child goal branched off master instead of integration branch** — `GoalManager.createGoal` accepts `baseBranch` and threads it to `_doSetupWorktree` → `createWorktree({ startPoint })`. `MissionManager.spawnChild` resolves `MissionGit.childStartPoint` (HEAD SHA pin). Verify with `git log <child-branch> --oneline | grep <integration-branch-tip>`. See [docs/debugging.md — Mission orchestration](docs/debugging.md#mission-orchestration).
- **Plan won't freeze after `goal-plan` passes** — verification harness post-pass hook on mission gates must call `MissionManager.freezePlan` when `gateId === "goal-plan"`. If broken, `mission_goal_spawn` returns 409 `"Plan not approved"`. Check `mission.planFrozenAt`; manual recovery via `POST /api/missions/:id/plan/freeze`. See [docs/missions.md — Recovery scenarios](docs/missions.md#plan-wont-freeze-after-goal-plan-passes).
