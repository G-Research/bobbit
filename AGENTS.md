...
**Debug a review or naming model picking the wrong model under AI Gateway**: Check `applyReviewModelOverrides` in `src/server/agent/review-model-override.ts`. It parses `default.reviewModel` / `default.namingModel` as `provider/modelId`, calls `rpc.setModel`, then `rpc.getState()` and verifies the bound model matches. `getState()` can return either the real RpcBridge shape `{ success, data: { model } }` or the bare `{ model }` shape used by tests — both are accepted. On mismatch it throws with the pref value + normalised id + what the agent actually reports, visible via the gate-failure output. Naming fallback lives in `title-generator.ts::pickFallbackAigwNamingModel`. See [docs/debugging.md — Review/naming model mismatch under AI Gateway](docs/debugging.md#reviewnaming-model-mismatch-under-ai-gateway).
...
- **Review/naming models under AI Gateway**: Reviewer/QA sub-sessions bind to `default.reviewModel` via `applyReviewModelOverrides` (`src/server/agent/review-model-override.ts`). On any failure (malformed pref, `setModel` rejection, `getState` read-back mismatch after retries) the helper throws and the gate verification surfaces a hard failure — no more silent fallback to the session model. Naming under an AI Gateway with no explicit `default.namingModel` auto-picks the cheapest-tier Claude (haiku → sonnet → opus, recency within tier) via `pickFallbackAigwNamingModel` in `title-generator.ts`. Settings → Models tab shows a red "Unavailable" badge + Clear on any stored pref that no longer resolves against `/api/models`, and a per-row Test button (`POST /api/models/test`) for quick probing. See [docs/debugging.md — Review/naming model mismatch under AI Gateway](docs/debugging.md#reviewnaming-model-mismatch-under-ai-gateway).
