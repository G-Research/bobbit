...
- **Bind an ephemeral role or workflow to one goal (no project-library pollution)** → mirror the existing `goal.workflow` snapshot pattern. Pass `inlineRoles: Record<roleName, Role>` on `POST /api/goals` (or `goal_spawn_child` / `propose_goal`) and the role is deep-cloned onto `PersistedGoal.inlineRoles`. The verification harness + `team-manager.spawnRole` resolve role names via the pure helper `resolveRole(goal, name, roleStore)` in `src/server/agent/resolve-role.ts` — inline first, project/server/builtin role-store second; `listAvailableRoles(goal, roleStore)` lists both for fail-loud error messages. Same pattern for ephemeral workflows: `body.workflow` on `POST /api/goals` and `goal_spawn_child` snapshots the full Workflow onto `goal.workflow`, bypassing the project workflow store. **Inheritance**: `goal_spawn_child` inherits BOTH the parent's `inlineRoles` (server-side merge `{...parent, ...body}`; child entries override) AND the parent's `goal.workflow` snapshot (deep-cloned; precedence is `body.workflow → parent.workflow → workflowId-from-store`). Critical companion: `spawn-child` also calls `gateStore.initGatesForGoal(child.id, child.workflow.gates.map(g => g.id))` after `createGoal` — without this, `gate_list` / `gate_signal` / `gate_status` / `gate_inspect` / the verification harness all see `[]` for the child even though `goal.workflow.gates` is populated. Tool definitions surface this trade-off explicitly: `propose_role` is for permanent roles; `inlineRoles` on `propose_goal` / `goal_spawn_child` is for one-off cases. Reviewer/QA session titles fall back to `step.role` / "Review" / "QA" when `step.name` is omitted (inline workflows commonly skip naming verify-steps) — see `verification-harness.ts::runLlmReviewStep` / `runAgentQaStep`. Tests: `tests/resolve-role.test.ts`, `tests/goal-manager-inline-roles.test.ts`, `tests/e2e/api-goals-spawn-child-route.spec.ts`.
...
- **Spawn a verification reviewer / QA session (sidebar attribution)** → use `buildVerificationReviewerMeta({kind, roleName, goalId, roleAccessory, teamLeadSessionId})` from `src/server/agent/verification-reviewer-meta.ts` for the `updateSessionMeta` payload. Both call sites in `verification-harness.ts` (`runLlmReviewStep`, `runAgentQaStep`) delegate to it. The helper OMITS the `teamLeadSessionId` key (not just sets it to `undefined`/`null`/`""`) when no team-lead is available so a re-stamp can't clear an existing link. Without the stamp the sidebar's archived render path can't attribute the session to its triggering team-lead; defensive backstop in `src/app/render-helpers.ts::renderTeamGroup` folds undefined-`teamLeadSessionId` archived members under the live team-lead. Tests: `tests/verification-reviewer-meta.test.ts`.
...
- **Child team-lead raises a PR** — Stanza B in `buildNestingContextSection` (`src/server/agent/system-prompt.ts`) requires `parts.nestingContext.parent` to be populated and `goal.parentGoalId` set on disk; otherwise the builder falls through to Stanza A's root mandate. Diagnostic: `GET /api/sessions/:id/prompt-sections`.
- **`PATCH /api/goals/:id/plan` returns 409/400** — (1) **400 `NO_EXECUTION_GATE`** workflow lacks an `execution` gate; (2) **409 `CRITERIA_DROP`** root acceptance criterion uncovered — quote it verbatim; (3) **409 `RESTRUCTURE_REQUIRES_PAUSE`** pause then re-PATCH; (4) malformed `proposedSteps[]`. See [docs/nested-goals.md — Mutation classifier](docs/nested-goals.md#mutation-classifier).
...
- **Archived reviewer/QA sessions don't nest under their team-lead in the sidebar** — confirm `teamLeadSessionId` was stamped at creation via `buildVerificationReviewerMeta` (single-source helper used by both reviewer/QA call sites in `verification-harness.ts`). Render-side fallback in `src/app/render-helpers.ts::renderTeamGroup` (`archivedForLiveLead`) catches legacy sessions where the field is undefined by folding them under the live lead. See `tests/verification-reviewer-meta.test.ts`.
