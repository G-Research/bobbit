...
- **MCP meta-tool kind tagging** → `computeEffectiveAllowedTools()` in `src/server/agent/tool-activation.ts` returns `EffectiveTool[]` = `{kind:"yaml"|"mcp", name:string}[]`. `kind:"yaml"` resolves through `ToolManager.getToolProviders()`; `kind:"mcp"` is satisfied by `mcpExtensionPaths` (proxy extensions written by `writeMcpProxyExtensions`). `computeToolActivationArgs` dispatches on `kind` — never re-introduce flat-`string[]` mixing of the two kinds. The `console.warn("has no provider")` branch is correct only for unknown `kind:"yaml"` names. Use `tagAllowedTool()` to lift legacy `string[]` at boundaries (session restoration, grant flows).
...
- **Spurious `[tool-activation] Tool "mcp_…" has no provider in .bobbit/config/tools/<group>/.yaml — skipping` warns on session start** — symptom of a regression that re-flattens `EffectiveTool[]` back to `string[]` somewhere between `computeEffectiveAllowedTools` and `computeToolActivationArgs`. The producer tags `kind:"mcp"` for MCP meta-tools at source; the consumer must skip them in the YAML provider-lookup loop. See `tests/tool-activation-mcp-warn.test.ts` for the regression pin and `EffectiveTool` JSDoc in `src/server/agent/tool-activation.ts`.
