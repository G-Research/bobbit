...
- **Gates**: Check `GET /api/goals/:id/gates` for dependency state. Check `GET /api/goals/:id/verifications/active` for in-flight verification with phase progression. Steps may show `"skipped"` status if an earlier phase failed. See [docs/goals-workflows-tasks.md](docs/goals-workflows-tasks.md#phased-verification) for phased execution details.
