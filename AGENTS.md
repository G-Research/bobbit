...
**Add an IndexSource (v2 file indexing or similar)**: Implement `IndexSource` from `src/server/search/types.ts`. Read `src/server/search/sources/files-source.stub.ts` as the reference shape. Wire into `SearchService` via the existing `sources` array. No other search module changes required. See [docs/design/semantic-search.md](docs/design/semantic-search.md) §2 and §8.
...
- **Search index**: LanceDB dataset at `<project-root>/.bobbit/state/search.lance/`. Delete the directory and restart to force a full rebuild. Meta mismatch (embedder id, schema version, content-policy version) auto-rebuilds. Shared model cache lives at `~/.bobbit/models/`. Status dot colors: green = idle, yellow = indexing (backlog > 50 or active rebuild), red = unavailable. See [docs/internals.md — Semantic search](docs/internals.md#semantic-search) and [docs/design/semantic-search.md](docs/design/semantic-search.md).
