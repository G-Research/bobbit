...
function artifactEntryRoute(sessionId: string): RegExp {
	return new RegExp(`/preview/${sessionId}/_artifact/[^/]+/${FILE_MODE_ENTRY_ROUTE}(?:\\?|$)`);
}
...
test.describe("Journey: Preview snapshot reopen", () => {
	...
	test("reopens a nested Unicode/dotted file-mode artifact with its declared asset", async ({ page }) => {
		...
		try {
			...
			const preview = page.frameLocator(".goal-preview-panel iframe").first();
			await expect(previewFrame, "the initial file-mode preview should target the immutable artifact's exact Unicode/dotted entry").toHaveAttribute("src", artifactEntryRoute(sessionId), { timeout: 20_000 });
			...
			await expect(previewFrame, "automatic rehydration should target the immutable artifact's exact Unicode/dotted entry").toHaveAttribute("src", artifactEntryRoute(sessionId), { timeout: 20_000 });
