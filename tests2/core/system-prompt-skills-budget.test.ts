...
// Source: tests/system-prompt-skills-budget.test.ts
...
// The module is imported under a scoped BOBBIT_DIR (restored afterwards) so its
// top-level evaluation never touches real state; the tested exports are pure.
let SKILLS_CATALOG_BUDGET: number;
let SKILLS_CATALOG_BUDGET_MIN: number;
let SKILLS_CATALOG_BUDGET_MAX: number;
let resolveSkillsCatalogBudget: (v: number | undefined) => number;
let buildSkillsCatalogSection: (skills: any[], budget?: number) => string | undefined;
...
beforeAll(async () => {
	await withEnv({ BOBBIT_DIR: tmpRoot }, async () => {
		...
		mod.initPromptDirs(path.join(tmpRoot, "state"));
	});
});
