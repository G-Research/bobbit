...
// Source: tests/session-store.test.ts
// Bucket: v2-core | Method: codemod | Classification: needs-withEnv
// Review: migrated to injected memfs; no process.env mutation remains.

/**
 * Unit tests for SessionStore — persistence for gateway session metadata.
 * Uses injected memfs to isolate from real state.
 */
...
function freshStore(): InstanceType<typeof SessionStore> {
	return new SessionStore(stateDir, memfs);
}
...
describe("SessionStore", () => {
	...
	describe("basic CRUD", () => {
		...
		it("normalizes legacy scheduled-advisor cadence and round-trips a valid count", async () => {
			memfs.mkdirSync(stateDir, { recursive: true });
			memfs.writeFileSync(STORE_FILE, JSON.stringify({
				version: 2,
				sessions: [makeSession({ scheduledAdvisorTurnCount: -3 }), makeSession({ id: "bad", scheduledAdvisorTurnCount: 1.5 })],
			}));
			const restored = freshStore();
			assert.equal(restored.get("sess-1")?.scheduledAdvisorTurnCount, 0);
			assert.equal(restored.get("bad")?.scheduledAdvisorTurnCount, 0);

			restored.update("sess-1", { scheduledAdvisorTurnCount: 4 });
	...
	describe("persistence", () => {
		...
		it("handles corrupt JSON gracefully", () => {
			memfs.writeFileSync(STORE_FILE, "not valid json{{{", "utf-8");
			const store = freshStore();
	...
	describe("empty store", () => {
		...
		it("starts with empty getAll from empty array file", () => {
			memfs.writeFileSync(STORE_FILE, "[]", "utf-8");
			const store = freshStore();
			assert.deepEqual(store.getAll(), []);
		});
	});
