...
// Source: tests/pr-walkthrough-hunk-id-roundtrip.test.ts
// Bucket: v2-core | Method: codemod | Classification: needs-manual
// Review: default `node:test` import mapped by hand to vitest; t.after() -> onTestFinished()
...
test("PR walkthrough hunk ID round-trip: compact manifest → submit chunk → finalize succeeds", async () => {
	const tmpDir = fs.mkdtempSync(join(os.tmpdir(), "prw-roundtrip-bundle-"));
	onTestFinished(() => { try { fs.rmSync(tmpDir, { recursive: true, force: true }); } catch { /* best effort */ } });
