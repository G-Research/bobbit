...
describe("affected repository reader inventory", () => {
	...
	it("pins every unresolved unit read to a live declaration or reviewed generated path", () => {
		...
		const packedConsumerOffline = "tests2/core/pi-packed-consumer-offline.test.ts";
		expect(audit.find((entry) => entry.consumer === packedConsumerOffline)).toEqual({
			consumer: packedConsumerOffline,
