...
// Source: tests/tool-contributions.test.ts
// Bucket: v2-core | Method: codemod | Classification: needs-manual
// Review: dist/* import rewritten to src/* — verify extension/paths resolve under vitest
...
describe("isMarketPackBaseDir / computeRendererKind (design §2.5)", () => {
	it("isMarketPackBaseDir matches only a real market-packs path segment", () => {
		...
		assert.equal(isMarketPackBaseDir("/opt/bobbit/src/server/defaults/tools"), false);
	...
	it("rendererKind is 'pack' ONLY for a market baseDir + a .js renderer", () => {
		...
		const builtin = "/opt/bobbit/src/server/defaults/tools";
