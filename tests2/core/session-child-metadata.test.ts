...
describe("child session metadata wiring", () => {
	...
	it("persists and restores explicit walkthrough allowedTools", () => {
		...
		assert.match(managerSrc, /tagAllowedTools\(persistedAllowedTools, restoredToolManager, restoreToolScope\)/, "restoreSession must prefer persisted allowedTools before role defaults and classify the allowlist with the project-scoped provider snapshot");
	});
});
