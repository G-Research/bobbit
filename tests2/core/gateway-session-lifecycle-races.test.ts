...
describe("gateway session lifecycle races", () => {
	...
	it("keeps a finish-run busy rejection queued without surfacing Agent is already processing", async () => {
		...
		vi.spyOn(console, "error").mockImplementation(() => {});
		const root = testRoot("gateway-finish-run-race");
		const manager = makeManager(root);
		managers.push(manager);
		const sessionId = "finish-run-busy-race";
		let piFinishingRun = true;
		const prompt = vi.fn(async () => piFinishingRun
			? { success: false, error: "Agent is already processing." }
			: { success: true });
		const session = {
			id: sessionId,
			title: "Finish run race",
			titleGenerated: true,
			cwd: root,
			status: "streaming",
			statusVersion: 1,
		...
		// Pi emits agent_end before its finally/finishRun path clears the busy guard.
		manager.handleAgentLifecycle(session, { type: "agent_end" });
		assert.equal(session.status, "idle");
		assert.equal(piFinishingRun, true);

		const ws = new FakeWebSocket();
		connect(ws, sessionId, manager);
		ws.emit("message", JSON.stringify({ type: "auth", token: "ignored" }));
		await waitFor(() => ws.sent.some((frame) => frame.type === "auth_ok"), "websocket authentication");
		ws.emit("message", JSON.stringify({ type: "prompt", text: "follow up at the turn boundary" }));
		await waitFor(() => commandErrors(ws).length > 0 || session.promptQueue.length === 1, "busy dispatch recovery");
		...
		assert.equal(
			errors.length,
			0,
			`COMMAND_ERROR: ${errors[0]?.message ?? "Agent is already processing."}`,
		);

		piFinishingRun = false;
		ws.close();
	});
});
