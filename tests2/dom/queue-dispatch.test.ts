...
// Migrated from tests/queue-dispatch.spec.ts (v2-dom tier).
// Pure logic — drives the REAL PromptQueue and a faithful copy of the
// SessionManager dispatch state machine. No DOM; the bridge import above is
...
class DispatchSimulator {
	 ...
	 * While streaming, promoted steers are removed from the queue and dispatched
	 * immediately through the live-steer path. Idle sessions drain normally;
	 * aborting sessions leave the row queued for abort/recovery handling.
	 */
...
describe("Queue Dispatch Integration", () => {
	...
	it("(2) idle + empty queue: new prompt dispatches directly, queue stays empty", () => {
	...
	it("(3) busy agent: new prompt IS enqueued, not dispatched", () => {
		const sim = new DispatchSimulator();
	...
	it("(4) idle + non-empty queue: enqueue triggers drain, queue becomes empty", () => {
		const sim = new DispatchSimulator();
	...
	it("(5) full drain: all items dequeue in correct order across multiple agent_end cycles", () => {
		const sim = new DispatchSimulator();
	...
	it("(6) optimistic status: status flips to streaming before dispatch completes", () => {
		const sim = new DispatchSimulator();
	...
	it("(7) queue persistence round-trip: serialize and restore identical state", () => {
	...
	it("(8) steer ordering: A,B,C enqueued, steer C then B, drain batches steered C+B then A", () => {
		const sim = new DispatchSimulator();
	...
	it("steer during busy: steered messages dispatch before non-steered on drain", () => {
		const sim = new DispatchSimulator();
	...
	it("remove from queue mid-drain: removed message is never dispatched", () => {
		const sim = new DispatchSimulator();
	...
	it("all dispatches use 'prompt' method (not steer) since agent is idle at drain time", () => {
		const sim = new DispatchSimulator();
	...
	it("empty drain on agent_end is a no-op", () => {
		const sim = new DispatchSimulator();
	...
	it("multiple rapid enqueues while idle all get queued and drain sequentially", () => {
		const sim = new DispatchSimulator();
	...
	it("(story 35) error gating: error turn with queued messages — queue stays intact, no drain", () => {
		const sim = new DispatchSimulator();
	...
	it("(story 36) error → retry → drain: queued messages dispatch after successful retry", () => {
		const sim = new DispatchSimulator();
	...
	it("(story 37, updated) error → new message: implicit unstick under cap dispatches prefixed", () => {
	...
	// ── Steered dispatch tests ─────────────────────────────────────────

	it("PI-10: steerQueued dispatches immediately while streaming", () => {
		const sim = new DispatchSimulator();
		...
		// Promotion dispatches immediately through the steer path.
		...
		// A later tool boundary has nothing left to dispatch.
		sim.toolBoundary();
		expect(sim.dispatched.length).toBe(2);
	});

	it("PI-10b: multiple steerQueued promotions dispatch immediately through steer", () => {
		const sim = new DispatchSimulator();
		...
		// Queue and promote two messages during the same in-flight turn.
		const msg1 = sim.enqueue("Steer A");
		sim.steerQueued(msg1!.id);
		const msg2 = sim.enqueue("Steer B");
		sim.steerQueued(msg2!.id);

		expect(sim.dispatched.length).toBe(3);
		expect(sim.dispatched[1].method).toBe("steer");
		expect(sim.dispatched[1].message.text).toBe("Steer A");
		expect(sim.dispatched[2].method).toBe("steer");
		expect(sim.dispatched[2].message.text).toBe("Steer B");
		expect(sim.queue.isEmpty).toBe(true);
	});

	it("toolBoundary still dispatches already accumulated steered rows as a batch", () => {
		const sim = new DispatchSimulator();

		// Make agent busy, then simulate steered rows that were already accumulated
		// before the current steerQueued promotion path could dispatch them.
		sim.enqueue("Running");
		sim.queue.enqueue("Steer A", { isSteered: true });
		sim.queue.enqueue("Steer B", { isSteered: true });
		sim.queue.enqueue("Normal");
		...
		expect(remaining.map(m => m.text)).toEqual(["Normal"]);
		expect(remaining[0].isSteered).toBe(false);
	});

	it("(story 11) steer 1 message while streaming → immediate steer dispatch, abort drains remaining", () => {
		const sim = new DispatchSimulator();
		...
		// Steer msgB — dispatches immediately.
		sim.steerQueued(msgB!.id);
		expect(sim.dispatched.length).toBe(2);
		expect(sim.dispatched[1].method).toBe("steer");
		expect(sim.dispatched[1].message.text).toBe("MsgB");

		// MsgA remains in queue (non-steered), and tool boundary is a no-op.
		sim.toolBoundary();
		expect(sim.dispatched.length).toBe(2);
		const remaining = sim.queue.toArray();
	...
	it("(story 12) steer C then B while streaming → immediate steers, abort drains A", () => {
		const sim = new DispatchSimulator();
		...
		// Steer C then B — each promotion dispatches immediately.
	...
	it("followUp direct dispatch: idle + empty queue + isFollowUp → dispatched via followUp method", () => {
		const sim = new DispatchSimulator();
	...
	it("dequeueAllSteered returns empty array when front is not steered", () => {
		const q = new PromptQueue();
		q.enqueue("Normal1");
		q.enqueue("Normal2");
	...
	it("(unstick 1) happy path: errored session, new enqueue dispatches prefixed", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("initial");
	...
	it("(unstick 2) cap triggers parking after 3 errored turns", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("first");
		sim.errorEnd("err 1");
		sim.errorEnd("err 2");
	...
	it("(unstick 3) success resets counter", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("first");
		sim.errorEnd("e1");
		sim.errorEnd("e2");
	...
	it("(unstick 4) explicit retry bypasses cap", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("first");
	...
	it("(unstick 5) steer path implicit unstick", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("first");
	...
	it("(unstick 5b) steer path parks at cap, persists to queue (PI-25 invariant)", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("first");
	...
	it("(unstick 6) queue drain ordering: new prefixed first, then parked FIFO unprefixed", () => {
		const sim = new DispatchSimulator();
	...
	it("(retry-overload) explicit retryLastPrompt clears pendingAutoRetryTimer", () => {
	...
	it("(retry-overload) explicit retryLastPrompt resets transientRetryAttempts; auto path preserves it", () => {
	...
	it("(retry-overload) provider-overload error can retry past the 3-attempt non-provider cap", () => {
	...
	it("(unstick 7) auto-retry timer cancelled by new input", () => {
		const sim = new DispatchSimulator();
		sim.enqueue("initial");
	...
	it("(retry-overload) pending auto-retry cancelled even when new prompt is parked at error cap", () => {
	...
	it("aborting status transition during force-abort (PI-21b)", () => {
		const sim = new DispatchSimulator();
	...
	it("(recover-spin) one-microtask busy race recovers on the first retry without deferring", () => {
	...
	it("(recover-spin) a persistently busy bridge bounds the redrains and defers, never spins", () => {
