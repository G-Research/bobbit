...
describe("Market extension capability grant API", () => {
	it("uses only the exact project, pack, hook, and capability tuple for grants and revokes", async () => {
		const tuple = { packId: "fixture-pack", hookId: "decision.hook", capability: "decide" as const };
		...
		expect(new URL(fetchRequests[1].url).pathname).toBe("/api/projects/project%20A/extension-grants/fixture-pack/decision.hook/decide");
