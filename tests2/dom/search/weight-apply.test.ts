...
// (src/server/search/flex-store.ts). No DOM — bridge import is for tier uniformity.
import { describe, expect, it } from "vitest";
import * as fs from "node:fs";
import * as os from "node:os";
import * as path from "node:path";
import { FlexSearchStore, type FlexDoc } from "../../../src/server/search/flex-store.js";
...
describe("weight multiplier", () => {
	it("weight 2.0 outranks weight 1.0 at identical text", async () => {
	...
	it("user-role (weight 2.0) outranks assistant-role (weight 1.0)", async () => {
		const dir = path.join(tmpDir(), "search.flex");
		const store = await FlexSearchStore.open({ dataDir: dir });
		try {
			await store.upsert([
	...
	it("weight ordering persists across three distinct weights", async () => {
		const dir = path.join(tmpDir(), "search.flex");
		const store = await FlexSearchStore.open({ dataDir: dir });
		try {
