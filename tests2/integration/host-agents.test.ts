...
import { createSession, deleteSession } from "./_e2e/e2e-setup.js";
...
async function buildHost(gateway: any, ownerId: string): Promise<any> {
	const { createServerHostApi } = await import("../../src/server/extension-host/server-host-api.js");
...
async function runFixture(gateway: any, ownerId: string, member: string, arg: unknown): Promise<any> {
	const { ModuleHost } = await import("../../src/server/extension-host/module-host-worker.js");
