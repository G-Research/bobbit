...
async function bootGateway(bobbitDir: string, opts: { freshDir: boolean }): Promise<StartedGateway> {
	...
	const bobbitDirMod = await import("../../src/server/bobbit-dir.js");
	...
	const { registerRpcBridgeFactory } = await import("../../src/server/agent/rpc-bridge.js");
	const { InProcessMockBridge, shouldUseInProcessMock } = await import("../../tests/e2e/in-process-mock-bridge.mjs");
	...
	if (opts.freshDir) {
		try {
			const { testWorkflows, TEST_DEFAULT_COMPONENT } = await import("../../tests/e2e/seed-workflows.js");
