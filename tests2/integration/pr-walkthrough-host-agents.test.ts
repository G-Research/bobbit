...
import { apiFetch, createSession, deleteSession, nonGitCwd } from "./_e2e/e2e-setup.js";
...
test.describe("PR walkthrough → host.agents reviewer (API E2E)", () => {
	...
	test.beforeAll(async () => {
		ModuleHostClass = (await import("../../src/server/extension-host/module-host-worker.js")).ModuleHost;
		createServerHostApi = (await import("../../src/server/extension-host/server-host-api.js")).createServerHostApi;
		getPackStore = (await import("../../src/server/extension-host/pack-store.js")).getPackStore;
	...
	test("a restored reviewer re-resolves the pack role (cascade + projectId) and keeps its walkthrough tools", async ({ gateway }) => {
		const { resolveGrantPolicy } = await import("../../src/server/agent/tool-activation.js");
		const fixture = makeGitFixture();
		const owner = await createSession({ cwd: fixture.cwd });
		createdSessionIds.push(owner);
		try {
