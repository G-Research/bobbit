// v2-native — shared fixture for genuine pre-review command-phase evidence lifecycle tests.
...
export function execOnly(runner: CommandRunner): CommandRunner {
...
export interface LifecycleScenario {
	...
	invoke: "success" | "conflict" | "mismatch" | "none";
...
export async function runLifecycle(commandRunner: CommandRunner, scenario: LifecycleScenario): Promise<LifecycleResult> {
	...
	const commandStepRunner = lifecycleCommandRunner(async spec => {
		...
		if (effect && scenario.invoke === "mismatch") {
			await harness.runWithSystemsReviewTargetCorrelation(token, async () => {
				await expect(runWithFinalMutationTargetAction(
					FINAL_MUTATION_TARGET_ACTIONS.mergeChildGoal,
					{ resolvedTarget: path.join(effect.parentRoot, "wrong-target"), resolvedScope: "branch:master" },
					() => mergeChildBranchLocal("master", "child", effect.parentRoot, commandRunner),
				)).rejects.toThrow(/target differs/i);
				await expect(runWithFinalMutationTargetAction(
					FINAL_MUTATION_TARGET_ACTIONS.mergeChildGoal,
					{ resolvedTarget: fs.realpathSync.native(effect.parentRoot), resolvedScope: "branch:wrong-scope" },
					() => mergeChildBranchLocal("master", "child", effect.parentRoot, commandRunner),
...
export async function assertRejectedLifecycle(commandRunner: CommandRunner, scenario: LifecycleScenario): Promise<void> {
	...
	const restartedExecution = new SystemsReviewExecutionStore(result.stateDir).get(result.executionId)!;
	expect(restartedExecution.targetAssertions, scenario.label).toEqual([]);
	expect(restartedExecution.status, scenario.label).toBe(result.execution.status);
