...
export interface UnreadPolicySession {
	id: string;
	status: string;
	lastActivity?: number;
	lastReadAt?: number;
...
export interface UnreadPolicyGoal {
	state?: string;
}

/** Subset of goal gate status consumed by the canonical policy. */
export interface UnreadPolicyGateStatus {
	verifying?: boolean;
...
function isDelegate(s: UnreadPolicySession): boolean {
	return !!s.delegateOf;
}

/** A team-member session — has a team lead, or a non-lead role inside a team goal. */
function isTeamMember(s: UnreadPolicySession): boolean {
...
function isTeamLead(s: UnreadPolicySession): boolean {
	return s.role === "team-lead";
}

/** Resolve the goal id this session belongs to (team goal or regular goal). */
function resolveGoalId(s: UnreadPolicySession): string | undefined {
...
function hasLiveSiblings(
	leadId: string,
	goalId: string,
	allSessions: readonly UnreadPolicySession[],
): boolean {
...
export function hasLiveDownstreamWork(
	...
	goalId: string,
	allSessions: readonly UnreadPolicySession[],
	gateStatusCache: ReadonlyMap<string, { verifying: boolean }>,
): boolean {
...
export function needsImmediateHumanAttention(
	session: UnreadPolicySession,
	gateStatusCache: ReadonlyMap<string, UnreadPolicyGateStatus>,
): boolean {
...
function needsHumanAttentionInternal(
	session: UnreadPolicySession,
	goal: UnreadPolicyGoal | undefined,
	allSessions: readonly UnreadPolicySession[],
	gateStatusCache: ReadonlyMap<string, UnreadPolicyGateStatus>,
...
export interface SessionUnreadContext {
	goal?: UnreadPolicyGoal;
	allSessions: readonly UnreadPolicySession[];
	gateStatusCache: ReadonlyMap<string, UnreadPolicyGateStatus>;
	/** The currently viewed session is read by definition. */
	activeSessionId?: string;
	/** Optional client-side optimistic read timestamp. */
	optimisticLastReadAt?: number;
}

/**
 * Canonical persistent unread treatment used by the sidebar and server tag
 * projection. Immediate human-action states intentionally bypass timestamps.
 */
export function isSessionUnread(
	session: UnreadPolicySession,
	context: SessionUnreadContext,
): boolean {
	// Preserve the production unread dot's exact active-work treatment.
	if (session.status === "streaming" || session.status === "busy") return false;
	if (context.activeSessionId === session.id) return false;

	if (needsImmediateHumanAttention(session, context.gateStatusCache)) return true;
	if (!needsHumanAttention(session, context.goal, context.allSessions, context.gateStatusCache)) return false;

	const lastReadAt = Math.max(session.lastReadAt ?? 0, context.optimisticLastReadAt ?? 0);
	return (session.lastActivity ?? 0) > lastReadAt;
}
