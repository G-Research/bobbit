...
export class RemoteAgent {
	...
	private _replaceDeliveryProjection(rows: any[]): void {
		...
		for (const row of normalized) this._deliveryProjection.set(row.id, row);
		this.onQueueUpdate?.(this.getQueue());
	}
	...
	private _updateDeliveryProjection(raw: any): void {
		...
		if (normalized) this._deliveryProjection.set(id, normalized);
		this.onQueueUpdate?.(this.getQueue());
	}
	...
	private async handleServerMessage(msg: any) {
		...
		switch (msg.type) {
			...
			case "queue_update": {
				...
				this._serverQueue = rows;
				// Modern queue rows are also delivery projections. Keep them through
				// dispatch even if a later legacy queue-only frame omits the row.
				for (const row of rows) this._deliveryProjection.set(row.id, row);
				this.onQueueUpdate?.(this.getQueue());
				break;
			}
