...
function snapshotWorkEntries(value: unknown): Array<[string, unknown]> {
	if (value instanceof Map) return [...value.entries()].filter(([key]) => typeof key === "string" && key.length > 0);
	...
	return Object.entries(value).filter(([key]) => key.length > 0);
}
...
export function projectClaudeSdkSubagentSnapshot(
	...
	for (const [parent, work] of snapshotWorkEntries(subagentWork)) {
		next.set(parent, normalizeWork(parent, work));
	}
