...
export async function deleteGoal(id: string): Promise<void> {
	...
	if (descendants === 0) {
		...
		try {
			...
			eagerMarkArchived([id]);  // eager UI update — refresh races otherwise
	...
	if (result.archived > 0) {
		...
		// still shows the (now-server-archived) goals as live — exactly
		// the symptom the user reported when they expected the goal gone
		// and saw it still in the menu.
