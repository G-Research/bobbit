...
export class ProjectImportProposalRenderer {
	render(proposal: ProjectImportProposalProjection): TemplateResult {
		let pending = proposal.status === "applying";
		let error = "";
		...
		return html`
			<section class="rounded-md border border-border p-3 space-y-3" data-testid="project-import-proposal" data-project-import-proposal-id=${proposal.requestId}>
				<div class="flex items-center justify-between gap-3">
					...
					<span class="text-xs text-muted-foreground">${proposal.status === "applying" ? "Applying (locked)" : `Revision ${proposal.rev}`}</span>
				</div>
				...
				<div class="flex justify-end gap-2">
					...
					${Button({ variant: "default", disabled: pending, onClick: () => void decide("accept"), children: pending ? "Applying…" : "Apply proposal" })}
				</div>
			</section>
		`;
	}
