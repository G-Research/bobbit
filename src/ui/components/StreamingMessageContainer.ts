...
export class StreamingMessageContainer extends LitElement {
	...
	override updated(changed: Map<string, unknown>) {
		// Defensive cleanup: when the agent is not streaming and we still have a
		// stale in-flight `_message` cached, drop it. The authoritative copy
		// lives in the message-list; leaving the partial rendered here on top
		// produces the "duplicate Thinking bubble at the end of an idle chat"
		// bug (or, when the session is archived mid-turn, a stranded assistant
		// card on top of the now read-only transcript — UX-05). AgentInterface's
		// `agent_end` / `message_end` handlers already call
		// `setMessage(null, true)` on the happy path, but they can be bypassed
		// by:
		...
		//   • a session getting archived/terminated while backgrounded (no
		//     active view re-rendering it), so `isStreaming` and `archived`
		//     both land in the SAME update once the user re-selects it — this
		//     MUST run before the archived early-return below, otherwise the
		//     stale message is never dropped once the session is read-only.
		...
		// unconditionally on the transition, regardless of `archived`.
		if ((changed.has("isStreaming") || changed.has("archived")) && !this.isStreaming && this._message !== null) {
