...
/**
 * Sidebar search input with debounced queries and clear/escape behavior.
 *
 * The Ctrl/Cmd+K focus shortcut is registered globally in `main.ts` as
 * `focus-search` (so it shows up in the Ctrl+, settings page and can be
 * rebound); this component just renders the input.
 *
 * Events:
				...
				<div class="relative pb-1">
					<div class="relative flex items-center">
						...
						${this.query
							...
							: (() => {
								const hint = shortcutHint("focus-search", { prefix: "", suffix: "" });
								return hint && !this._focused
									? html`<kbd class="absolute right-1.5 top-1/2 -translate-y-1/2 px-1 py-px text-[10px] leading-none rounded border border-border text-muted-foreground/70 bg-muted/40 pointer-events-none select-none">${hint}</kbd>`
									: "";
							})()}
					</div>
				</div>
