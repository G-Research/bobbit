...
export const sessionsBgRoutes: Route[] = [
	{
		...
		handler: async ({ deps, params, readBody, json, jsonError }) => {
			...
			if (!body?.command) { jsonError(400, new Error("command is required")); return; }
			try {
				...
				if (err?.message?.includes("Sandboxed session without containerId")) {
					jsonError(403, new Error("Sandboxed session cannot run host processes"));
				} else {
					throw err;
				}
			}
	...
	},
	{
		...
		handler: ({ deps, params, url, json, jsonError }) => {
			...
			if (!logs) { jsonError(404, new Error("Process not found")); return; }
	...
	},
	{
		...
		handler: ({ deps, params, url, json, jsonError }) => {
			...
			if (!pattern) { jsonError(400, new Error("pattern is required")); return; }
			...
			if (!result) { jsonError(404, new Error("Process not found")); return; }
			json(result);
		},
	},
	{
	...
	},
	{
		...
		handler: ({ deps, params, json, jsonError }) => {
			...
			if (!killed) {
				...
				if (!removed) { jsonError(404, new Error("Process not found")); return; }
			}
			json({ ok: true });
		},
	},
