...
import { SessionPromptPlumbing, type SessionPromptPlumbingDeps } from "./session-prompt-plumbing.js";
...
import { SessionLiveControl, type SessionLiveControlDeps } from "./session-live-control.js";
...
export function canResumeClaudeCodeSession(ps: Pick<PersistedSession, "runtime" | "modelProvider" | "claudeCodeSessionId"> | undefined): boolean {
...
export class SessionManager {
	...
	private sessionLiveControl!: SessionLiveControl;
	...
	constructor(options?: SessionManagerOptions) {
		...
		this.sessionPromptPlumbing = new SessionPromptPlumbing({
			...
			resolveRolePromptTemplate: (roleName, projectId) => this.resolveRolePromptTemplate(roleName, projectId),
			getIsSubgoalsEnabled: () => this.isSubgoalsEnabled,
			resolveMarketplacePiExtensionArgs: (projectId, cwd) => this.resolveMarketplacePiExtensionArgs(projectId, cwd),
			getTestGoalManager: () => this._testGoalManager,
		} satisfies SessionPromptPlumbingDeps);
		this.retainSessionPromptPlumbingHostSurface();

		this.sessionToolPermissions = new SessionToolPermissions({
			getSessions: () => this.sessions,
			getMcpManager: () => this.mcpManager,
			getToolManager: () => this.toolManager,
			getRoleManager: () => this.roleManager,
			getLifecycleHub: () => this.lifecycleHub,
			getToolPermissionAuditLog: () => this.toolPermissionAuditLog,
			getSession: (id) => this.getSession(id),
			mergeToolNames: (existing, additions) => this.mergeToolNames(existing, additions),
			resolveSessionRole: (roleName, assistantType, projectId) => this.resolveSessionRole(roleName, assistantType, projectId),
			resolveEffectiveAllowedTools: (role) => this.resolveEffectiveAllowedTools(role),
			restartSessionWithUpdatedRole: (session) => this._restartSessionWithUpdatedRole(session),
		...
		this.sessionLiveControl = new SessionLiveControl({
			getSessions: () => this.sessions,
			...
			resolveStoreForId: (id) => this.resolveStoreForId(id),
			resolveStoreForSession: (id) => this.resolveStoreForSession(id),
			trackConnectedSession: (session) => this._trackConnectedSession(session),
			restoreSessionCoalesced: (ps) => this._restoreSessionCoalesced(ps),
			cancelPendingAutoRetry: (session, reason) => this.cancelPendingAutoRetry(session, reason),
			reconcileAfterAbort: (session) => this._reconcileAfterAbort(session),
			coalesceRestore: (id, fn) => this._coalesceRestore(id, fn),
			applySandboxWiring: (bridgeOptions, id, sandboxOpts) => this.applySandboxWiring(bridgeOptions, id, sandboxOpts),
			applyScopedGatewayCredentials: (bridgeOptions, sessionId, projectId, goalId) => this.applyScopedGatewayCredentials(bridgeOptions, sessionId, projectId, goalId),
			getTeamLeadExtensionPath: () => this.getTeamLeadExtensionPath(),
			getGoalToolsExtensionPath: () => this.getGoalToolsExtensionPath(),
			getProposalToolsExtensionPath: () => this.getProposalToolsExtensionPath(),
			resolveSessionRole: (roleName, assistantType, projectId) => this.resolveSessionRole(roleName, assistantType, projectId),
			resolveEffectiveAllowedTools: (role) => this.resolveEffectiveAllowedTools(role),
			ensureMcpManagerForContext: (projectId, cwd) => this.ensureMcpManagerForContext(projectId, cwd),
			buildToolActivationArgs: (sessionId, allowedTools, role, cwd, projectId, effectiveGoalId, grantedTools) => this.buildToolActivationArgs(sessionId, allowedTools, role, cwd, projectId, effectiveGoalId, grantedTools),
			...
			applyDirectProviderEnv: (bridgeOptions, sandboxed, provider) => this.applyDirectProviderEnv(bridgeOptions, sandboxed, provider),
			readClaudeCodeConfigForProject: (projectId) => this.readClaudeCodeConfigForProject(projectId),
			...
			recordPiExtensionDiagnostic: (session, diagnostic, extension) => this.recordPiExtensionDiagnostic(session, diagnostic as RuntimePiExtensionDiagnostic, extension),
			tryAutoSelectModel: (session) => this.tryAutoSelectModel(session),
			drainQueue: (session) => this.drainQueue(session),
		} satisfies SessionLiveControlDeps);
		this.retainSessionLiveControlHostSurface();
	...
	private retainSessionPromptPlumbingHostSurface(): void {
		// SessionPromptPlumbing owns prompt/tool activation behavior after
		// cohort 12, while SessionManager keeps same-named wrappers for callers
		// and source-shape pins.
		void this._buildWorkflowList;
		void this.resolveEffectiveAllowedTools;
		void this.mergeToolNames;
		void this.resolveEffectiveGoalMetadataForSession;
		void this.dispatchGoalProvisionedForWorktree;
		void this.disabledToolsForGoal;
		void this.promptSectionOrderForGoal;
		void this.buildToolActivationArgs;
		void this.resolveSessionRole;
		void this.assemblePrompt;
		void this._assemblePrompt;
		void this.projectConfigStoreForPrompt;
		void this.computeSkillsCatalog;
		void this.buildDelegateTaskSpec;
		void this.buildDelegatePromptParts;
		void this.getPromptParts;
	}

	private retainSessionToolPermissionsHostSurface(): void {
		// SessionToolPermissions owns tool grant/permission behavior after
		// cohort 13, while SessionManager keeps same-named wrappers for callers
		// and source-shape pins.
		void this.grantToolPermission;
		void this.consultToolApproveHub;
		void this.appendToolPermissionAudit;
		void this.requestToolGrant;
	...
	private retainSessionLiveControlHostSurface(): void {
		// SessionLiveControl owns client attachment and abort/force-abort
		// behavior after cohort 15, while SessionManager keeps same-named
		// wrappers for callers and source-shape pins.
		void this.addClient;
		void this.removeClient;
		void this.abortSessionTurn;
		void this.forceAbort;
	}
	...
	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private _buildWorkflowList(projectId?: string): string {
		return this.sessionPromptPlumbing._buildWorkflowList(projectId);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private resolveEffectiveAllowedTools(role: import("./role-store.js").Role | undefined): EffectiveTool[] {
		return this.sessionPromptPlumbing.resolveEffectiveAllowedTools(role);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private mergeToolNames(existing: string[] | undefined, additions: string[] | undefined): string[] | undefined {
		return this.sessionPromptPlumbing.mergeToolNames(existing, additions);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private resolveEffectiveGoalMetadataForSession(goalId: string | undefined, projectId?: string): Record<string, unknown> {
		return this.sessionPromptPlumbing.resolveEffectiveGoalMetadataForSession(goalId, projectId);
	}
	...
	async dispatchGoalProvisionedForWorktree(opts: {
		...
		return this.sessionPromptPlumbing.dispatchGoalProvisionedForWorktree(opts);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private disabledToolsForGoal(goalId: string | undefined, projectId?: string): ReadonlySet<string> | undefined {
		return this.sessionPromptPlumbing.disabledToolsForGoal(goalId, projectId);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private promptSectionOrderForGoal(goalId: string | undefined, projectId?: string): string[] | undefined {
		return this.sessionPromptPlumbing.promptSectionOrderForGoal(goalId, projectId);
	}
	...
	private buildToolActivationArgs(
		...
		return this.sessionPromptPlumbing.buildToolActivationArgs(sessionId, allowedTools, role, cwd, projectId, effectiveGoalId, grantedTools);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private resolveSessionRole(roleName?: string, assistantType?: string, projectId?: string): import("./role-store.js").Role | undefined {
		return this.sessionPromptPlumbing.resolveSessionRole(roleName, assistantType, projectId);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private assemblePrompt(sessionId: string, parts: PromptParts): string | undefined {
		return this.sessionPromptPlumbing.assemblePrompt(sessionId, parts);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private _assemblePrompt(sessionId: string, parts: PromptParts): string | undefined {
		return this.sessionPromptPlumbing._assemblePrompt(sessionId, parts);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private projectConfigStoreForPrompt(projectId?: string): import("./project-config-store.js").ProjectConfigStore | undefined {
		return this.sessionPromptPlumbing.projectConfigStoreForPrompt(projectId);
	}
	...
	private computeSkillsCatalog(
		...
		return this.sessionPromptPlumbing.computeSkillsCatalog(allowedTools, discoveryRoot, projectConfigStore, projectId);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	private buildDelegateTaskSpec(instructions: string, context?: Record<string, string>): string {
		return this.sessionPromptPlumbing.buildDelegateTaskSpec(instructions, context);
	}
	...
	private buildDelegatePromptParts(opts: {
		...
		return this.sessionPromptPlumbing.buildDelegatePromptParts(opts);
	}

	/** Delegates to SessionPromptPlumbing (SessionManager decomposition cohort 12). */
	getPromptParts(sessionId: string): PromptParts | undefined {
		return this.sessionPromptPlumbing.getPromptParts(sessionId);
	}
	...
	async grantToolPermission(sessionId: string, toolName: string, scope: "tool" | "group", group?: string, mode?: ToolGrantMode): Promise<string[]> {
		return this.sessionToolPermissions.grantToolPermission(sessionId, toolName, scope, group, mode);
	}
	...
	private async consultToolApproveHub(session: SessionInfo, toolName: string, toolGroup: string): Promise<Decision<ToolApproveVerdict> | undefined> {
		return this.sessionToolPermissions.consultToolApproveHub(session, toolName, toolGroup);
	}
	...
	private appendToolPermissionAudit(
		...
		return this.sessionToolPermissions.appendToolPermissionAudit(session, ask, decision, source);
	}
	...
	async requestToolGrant(sessionId: string, toolName: string, toolGroup: string): Promise<ToolGrantResolution> {
		return this.sessionToolPermissions.requestToolGrant(sessionId, toolName, toolGroup);
	}
	...
	denyToolPermission(sessionId: string, _toolName: string): void {
		return this.sessionToolPermissions.denyToolPermission(sessionId, _toolName);
	}

	private recomputeAllowedToolsForRestart(session: SessionInfo, ps: PersistedSession): string[] | undefined {
		return this.sessionToolPermissions.recomputeAllowedToolsForRestart(session, ps);
	}
	...
	getPendingToolPermission(id: string): /* includes replayed seq: number; ts: number */ PendingToolPermissionSnapshot | undefined {
		return this.sessionToolPermissions.getPendingToolPermission(id);
	}
	...
	/** Delegates to SessionLiveControl (SessionManager decomposition cohort 15). */
	addClient(sessionId: string, ws: WebSocket): boolean {
		return this.sessionLiveControl.addClient(sessionId, ws);
	}

	/** Delegates to SessionLiveControl (SessionManager decomposition cohort 15). */
	removeClient(sessionId: string, ws: WebSocket): void {
		this.sessionLiveControl.removeClient(sessionId, ws);
	}

	/** Delegates to SessionLiveControl (SessionManager decomposition cohort 15). */
	async abortSessionTurn(id: string): Promise<void> {
		return this.sessionLiveControl.abortSessionTurn(id);
	}

	/** Delegates to SessionLiveControl (SessionManager decomposition cohort 15). */
	async forceAbort(id: string, gracePeriodMs = 3000): Promise<void> {
		return this.sessionLiveControl.forceAbort(id, gracePeriodMs);
	}
