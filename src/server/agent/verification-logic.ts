...
/** Command-template variables whose values are generated by Bobbit or Git. */
export const SAFE_COMMAND_TEMPLATE_VARIABLES = new Set(["branch", "baseBranch", "master", "commit"]);

/**
 * Substitute the only template variables allowed in shell command text.
 *
 * Signal metadata, upstream metadata, goal specs, and arbitrary project input
 * are deliberately unavailable here. They remain available to prompt/review
 * substitution through `substituteVars()` above, but must never reach a shell.
 */
export function substituteCommandVars(template: string, builtinVars: Record<string, string>): string {
	return template.replace(/\{\{([^}]+)\}\}/g, (match, key: string) => {
		const name = key.trim();
		return SAFE_COMMAND_TEMPLATE_VARIABLES.has(name) && typeof builtinVars[name] === "string"
			? builtinVars[name]
			: match;
	});
}

/** Return a clear failure for a command template that uses untrusted input. */
export function unsafeCommandTemplateFailure(template: string): string | null {
	for (const match of template.matchAll(/\{\{([^}]+)\}\}/g)) {
		const name = match[1].trim();
		if (!SAFE_COMMAND_TEMPLATE_VARIABLES.has(name)) {
			return `Failed — unsafe command template variable {{${name}}}. Command steps may only interpolate {{branch}}, {{baseBranch}}, {{master}}, or {{commit}}.`;
		}
	}
	return null;
}
 ...
 * - The resolved run string is empty or whitespace-only.
 *
 * Unresolved templates are not skippable: command execution fails closed
 * before this helper can run so legacy unsafe workflows cannot pass silently.
 */
export function isCommandStepSkippable(resolvedCmd: string): string | null {
	return resolvedCmd.trim() === "" ? "Skipped — command is empty (no value configured)" : null;
}
