...
// Case-insensitive (mirrors PANEL_ID_RE/PROVIDER_ID_RE): real first-party packs
// (e.g. market-packs/experiment-runner) declare camelCase route names such as
// `defineExperiment` / `projectCost` / `saveDashboard`. A case-sensitive regex
// here silently drops every one of those routes (see
// tests/pack-contributions.test.ts "accepts camelCase route names").
