...
export class VerificationHarness {
	...
	private async _finalizeCancelledVerification(active: ActiveVerification): Promise<void> {
		...
		if (!active.terminalVerdictPublished) {
			// Session termination for cancelled reviewers is the caller's
			// responsibility (see _terminateCancelledReviewers) — started
			// concurrently with cleanup rather than sequentially in here.
	...
	async cancelStaleVerificationsForGates(
		...
		// Published pass/fail rows remain private cleanup obligations. Stale
		// invalidation may accelerate their release, but never rewrites verdicts.
		const terminalResourceCleanups: ActiveVerification[] = [];
