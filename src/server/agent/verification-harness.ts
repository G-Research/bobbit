			...
			const runOneStep = async (
				...
				if (step.type === "command") {
					...
					if (process.env.BOBBIT_LLM_REVIEW_SKIP) {
						...
						// evaluateDocGateSkip / _maybeSkipDocGateReview. Ported into
						// runOneStep at the w6 merge (VER-07 extracted this runner
						// from the inline body that carried the original wiring).
		...
		try {
			...
			if (piRoleModel_r) {
				try {
					...
					console.log(`[verification] Applied role model policy for reviewer ${sessionId} (selected="${piRoleModel_r}", role=${roleName})`);
				} catch (err) {
					console.error(`[verification] Role model "${sanitizeModelErrorForLog(piRoleModel_r, 500)}" failed for reviewer ${sessionId}: ${sanitizeModelErrorForLog(err)}`);
					throw err;
				}
				...
				try {
					await applyReviewModelOverrides(session.rpcClient, {
						prefs: { get: (k) => k === "default.reviewModel" ? reviewModelPref : this.preferencesStore!.get(k) },
						sessionManager: this.sessionManager ?? null,
						sessionId,
						role: "reviewer",
		...
		try {
			...
			if (piRoleModel_q) {
				try {
					...
					console.log(`[verification] Applied role model policy for QA ${qaSessionId} (selected="${piRoleModel_q}", role=${qaRoleName})`);
				} catch (err) {
					console.error(`[verification] Role model "${sanitizeModelErrorForLog(piRoleModel_q, 500)}" failed for QA ${qaSessionId}: ${sanitizeModelErrorForLog(err)}`);
					throw err;
				}
			} else if (this.preferencesStore) {
				const reviewModelPref = filterPiBackedReviewModelForSetModel(this.preferencesStore.get("default.reviewModel") as string | undefined);
		...
		try {
			...
			if (piRoleModel_s) {
				try {
					...
					console.log(`[verification] Applied role model policy for sub-session ${subSessionId} (selected="${piRoleModel_s}", role=${roleName})`);
				} catch (err) {
					console.error(`[verification] Role model "${sanitizeModelErrorForLog(piRoleModel_s, 500)}" failed for sub-session ${subSessionId}: ${sanitizeModelErrorForLog(err)}`);
					throw err;
				}
			} else if (this.preferencesStore) {
				const reviewModelPref = filterPiBackedReviewModelForSetModel(this.preferencesStore.get("default.reviewModel") as string | undefined);
