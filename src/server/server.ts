...
async function handleApiRoute(
	...
	// family (src/server/routes/projects-routes.ts); cohort 2, the
	// /api/marketplace/* + GET /api/packs/conflicts family
	// (src/server/routes/marketplace-routes.ts).
	{
		...
		if (coreMatch) {
			const coreCtx: CoreRouteCtx = {
				...
				// Cohort 2 (marketplace) additions — append-only, see core-route-ctx.ts.
